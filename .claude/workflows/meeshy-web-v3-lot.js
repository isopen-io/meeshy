...
const brancheLot = `feat/web-v3-${cadrage.lot.toLowerCase()}-${Date.now().toString(36)}`

const livraison = await agent(`${SOCLE}

TA MISSION — LIVRER le lot ${cadrage.lot} PAR BRANCHE + PULL REQUEST + MERGE (directive porteur
2026-09-02 : plus de push direct sur dev pour ce workflow — dev bouge trop vite en parallele,
une branche + PR donne de la visibilite et fait tourner la CI avant integration).

ETAT DES GATES :
${JSON.stringify(gates || {}, null, 1).slice(0, 8000)}

SI UN GATE EST ROUGE : ne pousse RIEN, n'ouvre AUCUNE branche ni PR. Rends un rapport disant
ce qui est rouge et ce qu'il faut. C'est tout.
...
2. \`git checkout -b ${brancheLot}\` depuis le HEAD actuel de dev (deja a jour — verifie avec
   \`git fetch origin dev && git merge origin/dev --ff-only\` AVANT de creer la branche, jamais
   apres : une branche creee sur un dev perime forcera un merge inutile plus tard).
3. Commit sur cette branche. Le message suit la forme du depot : un titre en francais qui dit
   le RESULTAT (pas la tache), puis un corps qui explique CE QUI ETAIT CASSE et POURQUOI la
   correction prend cette forme. NE METS PAS \`Closes #<n>\` dans le message de commit (ce n'est
   plus dev qui merge directement — la fermeture se fait a l'etape 6, apres le merge reel de la
   PR). N'ecris JAMAIS de nom de modele dans un message de commit.
4. \`git push -u origin ${brancheLot}\`. En cas d'echec RESEAU seulement, reessaie 4 fois avec une
   attente croissante (2s, 4s, 8s, 16s). En cas de rejet non-reseau, arrete-toi et dis-le.
5. Ouvre une PR (\`mcp__github__create_pull_request\`, base=dev, head=${brancheLot}) qui suit le
   gabarit \`.github/pull_request_template.md\` s'il existe. Le corps cite les issues du lot
   (\`Relates to #<n>\`, jamais \`Closes\` avant merge reel), le detail des defauts corriges en
   revue, et les resultats de gates reels (pas un resume). Termine par une ligne vide, --- , puis
   _Generated by [Claude Code](https://claude.ai/code)_
   Note l'URL et le numero de la PR dans ton rapport.
6. Attends que la CI GitHub Actions de cette PR se termine (verifie avec
   \`mcp__github__pull_request_read\` methode get_check_runs — pas d'attente active bloquante,
   mais NE MERGE PAS avant que tous les checks non-informatifs soient verts et qu'il n'y ait
   aucun conflit). Si un check echoue, diagnostique-le et corrige-le (push un correctif sur
   ${brancheLot}, jamais un rebase ni un force-push) avant de continuer.
7. Une fois la CI verte et sans conflit : merge la PR (\`mcp__github__merge_pull_request\`, methode
   au choix du depot — merge commit par defaut, jamais de squash qui perdrait le detail des
   commits si le lot en a plusieurs). Puis, pour chaque issue livree DONT TU CONNAIS LE NUMERO
   (non nul), poste un commentaire de cloture SUR L'ISSUE (pas seulement sur la PR) : la preuve
   (commit, PR, gate, mesure), les dimensions MURES et celles qui RESTENT — et ouvre une issue
   par dimension non mure, comme l'exige le CLAUDE.md. Ferme chaque issue livree explicitement
   (state=closed, state_reason=completed) puisque ce n'est plus un \`Closes #n\` automatique qui
   s'en charge. Termine chaque commentaire par une ligne vide, --- , puis
   _Generated by [Claude Code](https://claude.ai/code)_
   Si les outils mcp__github__ ne repondent pas, ou si un numero vaut 0, n'ouvre AUCUNE PR avec
   un \`Relates to #0\` : consigne plutot ce qui aurait ete ferme dans ${D}/issues-a-ouvrir.md, et
   DIS dans ton rapport que la tracabilite GitHub de ce lot reste a faire par le porteur. Ne
   presente jamais un lot comme trace s'il ne l'est pas.

...
export async function findEligibleConversations(
  ...
  return conversations.map((conv) => {
    ...
    return {
      ...
      maxReactionsPerCycle: Math.max(0, config?.maxReactionsPerCycle ?? 4),
      ...
      inactivityThresholdHours: Math.max(1, config?.inactivityThresholdHours ?? 72),
      ...
      maxWordsPerMessage: Math.max(10, Math.min(config?.maxWordsPerMessage ?? 80, 2000)),

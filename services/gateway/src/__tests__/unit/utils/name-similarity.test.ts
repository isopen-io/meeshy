...
describe('compareFullNames — exact', () => {
  ...
  it('returns exact for identical CJK / Arabic names', () => {
    expect(compareFullNames(
      { firstName: '明', lastName: '李' },
      { firstName: '明', lastName: '李' }
    )).toBe('exact');
    expect(compareFullNames(
...
describe('compareFullNames — different', () => {
  ...
  it('does not collapse different Cyrillic surnames to exact via a shared Latin first name', () => {
    expect(compareFullNames(
      { firstName: 'Jean', lastName: 'Петров' },
      { firstName: 'Jean', lastName: 'Иванов' }
    )).not.toBe('exact');
  });
});

...
const createMockPrisma = () => ({
  ...
  // Active-call claim release when no CallService is wired (else via CallService, #7545).
  conversation: {
...
const createMockCallService = (hasData = true) => ({
  ...
  hasHeartbeatData: jest.fn().mockReturnValue(hasData) as MockFn, releaseActiveCallClaim: jest.fn().mockResolvedValue(undefined) as MockFn
});
...
describe('CallCleanupService', () => {
  ...
  describe('runCleanup', () => {
    ...
    it('skips heartbeat tier entirely when callService is not provided', async () => {
      ...
      expect(result.cleaned).toBe(0);
      // findMany should have been called exactly 2 times (tier 1 + tier 3), not 3
      expect(prisma.callSession.findMany).toHaveBeenCalledTimes(2);
    });
  ...
  describe('forceEndCall — race guard', () => {
    ...
    it('does not touch callParticipant or broadcast call:ended when the race guard skips the write', async () => {
      ...
      const staleCall = makeStaleCall(CallStatus.initiated, 130_000, 'call-race-2');

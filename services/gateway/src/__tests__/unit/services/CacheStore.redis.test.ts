...
// ─── initializeRedis — retryStrategy ─────────────────────────────────────────

describe('initializeRedis — retryStrategy callback', () => {
  it('returns 2000 for attempt counts ≤ 3 and null beyond 3', () => {
    const store = makeRedisStore();
    const MockRedis = jest.requireMock('ioredis') as jest.MockedFunction<any>;
    const opts = MockRedis.mock.calls[0][1] as { retryStrategy: (t: number) => number | null };
    expect(opts.retryStrategy(1)).toBe(2000);
    expect(opts.retryStrategy(3)).toBe(2000);
    expect(opts.retryStrategy(4)).toBeNull();
    store.close();
  });
});

// ─── initializeRedis — Redis event handlers ───────────────────────────────────

describe('initializeRedis — Redis event callbacks', () => {
  function findHandler(event: string): ((...args: unknown[]) => void) | undefined {
    return (mockRedisInstance.on as jest.Mock<any>).mock.calls.find(
      ([e]: [string]) => e === event,
    )?.[1] as ((...args: unknown[]) => void) | undefined;
  }

  it('connect and ready handlers invoke without throwing', () => {
    const store = makeRedisStore();
    expect(() => findHandler('connect')?.()).not.toThrow();
    expect(() => findHandler('ready')?.()).not.toThrow();
    store.close();
  });

  it('close and end handlers invoke without throwing', () => {
    const store = makeRedisStore();
    expect(() => findHandler('close')?.()).not.toThrow();
    expect(() => findHandler('end')?.()).not.toThrow();
    store.close();
  });

  it('error handler suppresses ECONNRESET without calling logger.warn', () => {
    const store = makeRedisStore();
    const handler = findHandler('error');
    expect(() => handler?.(new Error('ECONNRESET reset'))).not.toThrow();
    store.close();
  });

  it('error handler logs warn for non-suppressed errors', () => {
    const store = makeRedisStore();
    const handler = findHandler('error');
    expect(() => handler?.(new Error('OutOfMemory'))).not.toThrow();
    store.close();
  });
});

// ─── initializeRedis — connect() failure and constructor throw ────────────────

describe('initializeRedis — connection failures', () => {
  it('handles connect() rejection silently', async () => {
    mockRedisInstance.connect.mockRejectedValueOnce(new Error('ECONNREFUSED'));
    const store = makeRedisStore();
    // flush the .catch() microtask twice
    await Promise.resolve();
    await Promise.resolve();
    await store.close();
  });

  it('catches a Redis constructor throw and falls back to memory cache', async () => {
    const MockRedis = jest.requireMock('ioredis') as jest.MockedFunction<any>;
    MockRedis.mockImplementationOnce(() => { throw new Error('init failed'); });
    const store = new RedisCacheStore('redis://localhost:6379');
    // Memory fallback: get should return null (no key stored yet)

...
describe('ZMQTranslationClient', () => {
  ...
  describe('sendTranslationRequest()', () => {
    ...
    it('should send translation request successfully', async () => {
      ...
      const sentMessage = JSON.parse((mockPushSocket.send as jest.Mock).mock.calls[1][0]);
    ...
    it('should throw error if push socket not initialized', async () => {
      ...
      await expect(client.sendTranslationRequest(request)).rejects.toThrow('Socket PUSH non initialis');
    });
  ...
  describe('sendAudioProcessRequest()', () => {
    ...
    it('should throw error if push socket not initialized', async () => {
      ...
      await expect(client.sendAudioProcessRequest(request)).rejects.toThrow('Socket PUSH non initialis');
    });
  });
  ...
  describe('sendVoiceAPIRequest()', () => {
    ...
    it('should throw error if push socket not initialized', async () => {
      ...
      await expect(client.sendVoiceAPIRequest(request)).rejects.toThrow('Socket PUSH non initialis');
    });
  });
  ...
  describe('sendVoiceProfileRequest()', () => {
    ...
    it('should throw error if push socket not initialized', async () => {
      ...
      await expect(client.sendVoiceProfileRequest(request)).rejects.toThrow('Socket PUSH non initialis');
    });
  });
  ...
  describe('close()', () => {
    ...
    it('should handle close errors gracefully', async () => {
      (mockPushSocket.close as jest.Mock).mockRejectedValue(new Error('Close failed'));
  ...
  describe('testReception()', () => {
    ...
    it('should send ping for testing', async () => {
      await client.testReception();

      // Should send a ping message
      const sentMessages = (mockPushSocket.send as jest.Mock).mock.calls;
      const pingMessages = sentMessages.filter((call: any[]) => {
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle ping error gracefully during translation request', async () => {
      // First send (ping) fails, but main request succeeds
      (mockPushSocket.send as jest.Mock)
      ...
      // Should still succeed despite ping failure
      const taskId = await client.sendTranslationRequest(request);
      expect(taskId).toBe('test-uuid-1234');
    });
  });

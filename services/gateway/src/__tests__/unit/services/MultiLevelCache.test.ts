...
describe('MultiLevelCache', () => {
  ...
  // ─── getAndDelete with expired-but-present memory entry ───────────────────

  describe('getAndDelete — expired memory entry without remote store', () => {
    it('removes the stale memory entry and returns null when no remote store', async () => {
      // Set key, expire it, then getAndDelete → should hit the stale-eviction path
      await cache.set('k-stale', 'v-stale');
      jest.advanceTimersByTime(6000); // past 5s TTL
      const val = await cache.getAndDelete('k-stale');
      // Key was expired — remote store is absent → returns null
      expect(val).toBeNull();
      // Memory entry should be gone (already deleted by the stale-eviction branch)
      expect(cache.getStats().memorySize).toBe(0);
    });
  });

  // ─── delete — remote store.del failure (inner catch) ─────────────────────

  describe('delete — inner store.del failure', () => {
    it('returns true (memory deleted) even when store.del throws', async () => {
      const store = makeStore({ del: () => Promise.reject(new Error('del failed')) });
      const c = new MultiLevelCache<string>({ name: 'c14', store: store as any, memoryTtlMs: 5000 });
      await c.set('k', 'v');
      // Inner catch in delete() swallows store.del errors; memory was still deleted
      const result = await c.delete('k');
      expect(result).toBe(true);
      await c.disconnect();
    });

    it('returns false for a missing key even when store.del throws', async () => {
      const store = makeStore({ del: () => Promise.reject(new Error('del failed')) });
      const c = new MultiLevelCache<string>({ name: 'c15', store: store as any, memoryTtlMs: 5000 });
      // key was never set in memory, so memoryCache.delete returns false
      // store.del also throws → inner catch fires → deleted stays false
      const result = await c.delete('never-set');
      // deleted starts false; memory.delete is false; store.del throws → deleted stays false
      expect(result).toBe(false);
      await c.disconnect();
    });
  });

  // ─── clear — store.keys failure (outer catch) ─────────────────────────────

  describe('clear — store error', () => {
    it('does not throw when store.keys fails during clear', async () => {
      const store = makeStore({ keys: () => Promise.reject(new Error('keys failed')) });
      const c = new MultiLevelCache<string>({ name: 'c16', store: store as any });
      await c.set('k1', 'v1');
      await expect(c.clear()).resolves.toBeUndefined();
      await c.disconnect();
    });

    it('does not throw when store.del fails during clear', async () => {
      const store = makeStore({
        keys: () => Promise.resolve(['c17:k1']),
        del: () => Promise.reject(new Error('del failed')),
      });
      const c = new MultiLevelCache<string>({ name: 'c17', store: store as any });
      await expect(c.clear()).resolves.toBeUndefined();
      await c.disconnect();
    });
  });

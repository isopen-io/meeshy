 ...
 *
 * Le chemin SOCKET du mode lent des nouveaux comptes dans Meeshy Global
 * (#7740). `handleMessage` rend le refus (`code: 'NEWCOMER_SLOW_MODE'`,
 * `retryAfter`) ; ce fichier prouve que `message:send` ET
 * `message:send-with-attachments` le rendent tel quel — l'ACK porte le code
 * et le décompte, et un événement `error` cohérent part vers le socket. Avant
 * #7740 l'ACK d'échec ne gardait que `error` : un client ne pouvait pas
 * distinguer « pas encore » de « jamais ».
 *
 * Harnais repris de `MessageHandler.core.test.ts` (hors budget, il ne peut
 * que rétrécir — un comportement neuf rejoint un fichier neuf).
 */
...
describe('MessageHandler — mode lent des nouveaux comptes dans Meeshy Global (#7740)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('message:send — l’ACK porte le code et le décompte, et un événement error part vers le socket', async () => {
    ...
    const messagingService = { handleMessage: jest.fn(async () => NEWCOMER_REFUSAL) };
    ...
    expect(cb).toHaveBeenCalledWith({
      success: false,
      error: NEWCOMER_REFUSAL.error,
      code: 'NEWCOMER_SLOW_MODE',
      retryAfter: 20,
    });
    expect(socket.emit).toHaveBeenCalledWith('error', {
      message: NEWCOMER_REFUSAL.error,
      code: 'NEWCOMER_SLOW_MODE',
      retryAfter: 20,
    });
  });

  it('message:send-with-attachments — même refus, même forme', async () => {
    const { connectedUsers, socketToUser } = makeAuthenticatedSetup();
    const data = {
      conversationId: 'conv-abc',
      content: 'regardez',
    ...
    expect(cb).toHaveBeenCalledWith(expect.objectContaining({ code: 'NEWCOMER_SLOW_MODE', retryAfter: 20 }));
  ...
  it('un refus ordinaire, sans code, ne déclenche aucun événement error en plus de l’ACK', async () => {
    ...
    const data = makeValidSendData();
    mockValidateSocketEvent.mockReturnValue({ success: true, data });
    const messagingService = { handleMessage: jest.fn(async () => ({ success: false, error: 'Service failure' })) };

    const { handler } = makeHandler({ connectedUsers: connectedUsers as any, socketToUser, messagingService });
    await handler.handleMessageSend(socket, data as any, cb);

    expect(cb).toHaveBeenCalledWith({ success: false, error: 'Service failure' });

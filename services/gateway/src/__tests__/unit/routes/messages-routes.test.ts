...
describe('GET /conversations/:id/messages', () => {
  ...
  // ── Première page : `cursorPagination.hasMore` DÉRIVE du COUNT ─────────────
  // Le mode `after` ci-dessus a été corrigé d'une devinette (`length === limit`)
  // vers une MESURE. La PREMIÈRE PAGE portait la même devinette — et son cas est
  // pire, parce que la vérité voyage DÉJÀ dans la même réponse : ce chemin paie
  // un `count()` (le seul qui le fasse), `pagination.hasMore` en dérive
  // correctement (`offset + resultCount < total`), et `cursorPagination.hasMore`
  // devinait à côté. Deux `hasMore` contradictoires dans une seule charge — et
  // le client web-v2 lit le mauvais des deux (`messages.ts:50` → `hasOlder`).
  describe('première page — hasMore mesuré, jamais deviné (#6993)', () => {
    /**
     * LE RANG QUI SÉPARE LES DEUX RÈGLES, et le seul. À 49 comme à 51 messages,
     * la devinette et la vérité rendent le MÊME verdict — un témoin écrit là ne
     * pourrait pas tomber. C'est exactement la pile `limit` qui les sépare :
     * la page est PLEINE (donc `length === limit` dit « il en reste ») et
     * pourtant le fil est ÉPUISÉ (`offset + length === total`).
     */
    const PILE = 50;
    const pageDe = (n: number) =>
      Array.from({ length: n }, (_, i) => makeMessage({ id: `bbbbbbbbbbbbbbbbbbbbbb${String(i).padStart(2, '0')}` }));

    it('un fil de PILE `limit` messages n’annonce PLUS d’historique — il n’en a pas', async () => {
      prisma.message.findMany.mockResolvedValue(pageDe(PILE));
      prisma.message.count.mockResolvedValue(PILE);
      mockValidatePagination.mockReturnValue({ offset: 0, limit: PILE });
      const reply = makeReply();

      await getMessagesHandler()(makeRequest({ query: { limit: String(PILE) } }), reply);

      expect(reply._body.data).toHaveLength(PILE);
      expect(reply._body.cursorPagination.hasMore).toBe(false);
    });
    ...
    it("les deux blocs de la MÊME réponse s'accordent — c'est leur désaccord qui était le défaut", async () => {
      ...
      expect(reply._body.cursorPagination.hasMore).toBe(reply._body.pagination.hasMore);
    });

    it('une page pleine SUIVIE d’historique annonce bien la suite — sans quoi le correctif nierait toute pagination', async () => {
      prisma.message.findMany.mockResolvedValue(pageDe(PILE));
      prisma.message.count.mockResolvedValue(PILE + 1);
      mockValidatePagination.mockReturnValue({ offset: 0, limit: PILE });
      const reply = makeReply();
    ...
    it("une page pleine à l'OFFSET tient aussi : la borne se lit depuis le rang atteint, pas depuis la taille de la page", async () => {
      prisma.message.findMany.mockResolvedValue(pageDe(PILE));
      prisma.message.count.mockResolvedValue(2 * PILE);
      mockValidatePagination.mockReturnValue({ offset: PILE, limit: PILE });
      const reply = makeReply();

      await getMessagesHandler()(makeRequest({ query: { limit: String(PILE), offset: String(PILE) } }), reply);

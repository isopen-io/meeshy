 ...
 * `direct-log-error-forbidden-guard.test.ts` (#3617), pour le niveau WARN
 * (#5415).
 *
 ...
 *
 * 29 sites, sur 19 fichiers, appelaient `fastify.log.warn({ ... }, 'message')`
 * ou `request.log.warn(...)` DIRECTEMENT — l'idiome Pino usuel, et un no-op
 * silencieux sous cette configuration précise. Un avertissement (échec non
 * bloquant, dégradation tolérée) ne laissait alors AUCUNE trace en
 * production — le même symptôme que #3617 visait pour le niveau ERROR, par
 * un chemin que son critère de fin n'avait pas fermé.
 *
 ...
 * (`console.warn` par `MeeshyLogger.warn`, qui n'a pas cette forme). Un futur
 * appelant qui écrit `fastify.log.warn(...)` ou `request.log.warn(...)` la
 * fait tomber — la réparation est `logWarn(logger, message, error)`, jamais
 * une ligne ajoutée à un inventaire : il n'y a pas d'appel direct légitime à
 * porter.
 *
...
describe('Aucun `.log.warn(` direct — logWarn() est le seul site qui écrit réellement en prod', () => {
  it('le balayage LIT bien l’arbre du service — sinon il serait vert à vide', () => {
    ...
    expect(appelsDirects("fastify.log.warn({ error }, 'boom');")).toHaveLength(1);

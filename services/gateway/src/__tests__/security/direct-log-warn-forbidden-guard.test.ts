 ...
 *
 * `logWarn()` portait, jusqu'à #5415, exactement le défaut que `logError()`
 * portait avant #3617 (délégation muette au logger reçu), ET 30 sites, sur
 * 19 fichiers, appelaient `fastify.log.warn({...}, 'message')` ou
 * `request.log.warn(...)` DIRECTEMENT — l'idiome Pino usuel, no-op silencieux
 * sous cette configuration précise. Tout avertissement émis par ces sites
 * était donc perdu en production.
 *
 * Cette garde interdit la FORME, pas seulement les 30 sites trouvés :

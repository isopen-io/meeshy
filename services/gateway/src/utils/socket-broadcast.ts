...
export function broadcastToUser<E extends ServerEventName>(
  ...
  if (!io) {
    logWarn(fastify.log, `broadcastToUser: Socket.IO layer unavailable (userId=${userId}, event=${event})`);
    return false;
  }
  try {
    ...
    logWarn(fastify.log, `broadcastToUser: emit failed (userId=${userId}, event=${event})`, error);
    return false;
  }
}

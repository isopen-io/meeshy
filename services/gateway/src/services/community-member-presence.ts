 ...
 *
 * Déménagé de `routes/` vers `services/` par #4277 (critère 5) : ce module
 * n'enregistre AUCUNE route — c'est un résolveur pur, appelé PAR une route
 * (`routes/communities/search.ts`), et sa place n'a jamais été dans
 * `routes/`. Il vivait là depuis avoir été hoisté en SIBLING de
 * `routes/communities.ts` et de `routes/communities/search.ts` du temps où
 * les deux servaient la même recherche, l'un en production et l'autre non :
 * la loi de présence devait rester la MÊME des deux côtés, pour que la
 * consolidation à venir ait à choisir un fichier et non à arbitrer entre
 * deux comportements.
 *
 ...
 * appelant. La raison de vivre ICI, dans `services/`, aux côtés de
 * `PresenceVisibilityService` qu'il enveloppe, est désormais la
 * mutualisation ordinaire.
 */
...
import { viewerFromRequest } from '../routes/users/presence-gate';

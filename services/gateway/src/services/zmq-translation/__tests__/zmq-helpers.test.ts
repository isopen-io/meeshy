...
describe('zmq-helpers', () => {
  ...
  describe('translation target identity', () => {
    ...
    it('exposes the namespaces the social pipelines claim', () => {
      expect([...TRANSLATION_TARGET_NAMESPACES]).toEqual(['post', 'comment', 'story', 'media-caption', 'message-attachment-caption']);
    });
    ...
    it('rejects every namespaced target, known or future', () => {
      ...
      expect(isMessageTranslationTarget(`media-caption:${OBJECT_ID}`)).toBe(false);

...
export class AttachmentReactionHandler {
  ...
  private async _apply(
    ...
    try {
      // Boundary Zod schema, mirroring the three reaction twins
      // (Reaction/Comment/Post ReactionHandler). `mongoId` on both ids subsumes
      // the former manual `isValidObjectId` guard — an unreconciled optimistic
      // id (cid_*) or any non-ObjectId is rejected here before it can throw
      // prisma P2023 — and additionally bounds `emoji` (min 1, max 10).

...
import { emitToParticipantRooms, type ParticipantRoomEmitter } from './emitToParticipantRooms';
 ...
 */
export type PreviewEmitIO = ParticipantRoomEmitter;
 ...
 * the unchanged preview, which is an idempotent no-op on clients. Accountless
 * participants are addressed by their `Participant.id` — see
 * `emitToParticipantRooms`, which owns that rule for the three send/edit paths.
 *
  ...
  try {
    const [participants, latest] = await Promise.all([
      prisma.participant.findMany({
        ...
        // `id` is not decoration: it is the room key of an accountless
        // participant. Selecting `userId` alone did not merely ignore the

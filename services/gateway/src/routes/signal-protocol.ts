...
import { sendSuccess, sendInternalError, sendNotFound, sendForbidden, sendBadRequest, sendError } from '../utils/response.js';
import {
  errorResponseSchema,
    ...
    async (request, reply) => {
      try {
        ...
        if (!signalService) {
          ...
          return sendError(reply, 503, 'End-to-end encryption is not available on this server');
        }

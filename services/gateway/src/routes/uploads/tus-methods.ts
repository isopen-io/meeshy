 ...
 * dans son code (`dist/server.js`, commentaire cité sans son nom d'en-tête —
 * voir plus bas) :
 *
 * > `// CORS must be set before the 412 and validation 400 returns below.`
 * > `// GET still dispatches earlier and stays CORS-less.`
 *
 * — « CORS-less » : le dispatch du `GET` précède la pose des en-têtes.
 *
 * Un `GET` servi par tus repart donc **sans l'en-tête d'autorisation
 * d'origine**, et un navigateur d'une autre origine ne peut pas le lire — le
 * message rapporté sur staging le 2026-09-19 (#7132).
 *
 * **L'en-tête n'est pas NOMMÉ ici, et c'est volontaire.** Le cliquet des portes
 * d'origine (`__tests__/cors-origin-emitter-sweep.test.ts`) balaie ce qui SORT
 * et ne dépouille pas les commentaires — son propre doc-comment le dit et
 * l'assume : « il rougit, quelqu'un regarde ». Un fichier qui mentionne le nom
 * de l'en-tête en prose y est relevé comme une PORTE, alors que celui-ci n'en
 * pose aucun. Le nommer ici obligerait donc à le déclarer hors règle — une
 * déclaration FAUSSE, que les règles 1 bis et 1 ter du même cliquet existent
 * pour attraper.
 *

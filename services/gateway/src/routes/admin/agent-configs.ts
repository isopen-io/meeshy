  ...
  fastify.put('/configs/:conversationId', {
    ...
    try {
      ...
      if (!invalidationStatus.anyChannelSucceeded) {
        logWarn(
          fastify.log,
          `[AgentConfig] Cache invalidation failed on both Redis pub/sub AND direct HTTP; agent service may serve stale config for up to 5 min (conversationId=${conversationId}, status=${JSON.stringify(invalidationStatus)})`,
        );
      }

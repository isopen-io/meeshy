...
import { sendSuccess, sendNotFound, sendForbidden, sendBadRequest, sendInternalError, sendUnauthorized } from '../utils/response.js';
  ...
  fastify.post<{ Body: TranslateRequest }>('/translate-blocking', {
    ...
    try {
      ...
      if (validatedData.message_id) {
        ...
        if (!senderId) {
          return sendUnauthorized(reply, 'AUTH_REQUIRED', { message: 'Authentication required for new message translation' });
        }
      ...
      if (error instanceof z.ZodError) {
        return sendBadRequest(reply, 'VALIDATION_ERROR', { message: errorMessage });
      }

      return sendInternalError(reply, 'TRANSLATION_ERROR', { message: errorMessage });
    }
  });

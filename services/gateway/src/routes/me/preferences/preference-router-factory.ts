...
import { sendSuccess, sendUnauthorized, sendForbidden, sendInternalError, sendBadRequest } from '../../../utils/response.js';
      ...
      async (request, reply) => {
        ...
        try {
          ...
          if (consentViolations.length > 0) {
            return sendForbidden(reply, 'Missing required consents for requested preferences');
          }
          ...
          if (error.name === 'ZodError') {
            return sendBadRequest(reply, 'VALIDATION_ERROR', { message: 'Invalid preference data' });
          }

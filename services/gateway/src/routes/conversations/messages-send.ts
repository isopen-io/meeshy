  ...
  fastify.post<{
    ...
    try {
      ...
      if (!result.success) {
        ...
        // #4855 — un droit d'écriture PARTICIPANT refusé (host ayant retiré
        // `canSendMessages`) est un 403, jamais une erreur de VALIDATION de la
        // requête — même statut que le jumeau anonyme, `routes/links/messages.ts`.
        if (result.code === 'WRITE_NOT_PERMITTED') {

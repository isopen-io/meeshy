...
/**
 * **Les NOYAUX de `PATCH …/participants/:userId/ban` et `…/unban`** — extraits
 * de `ban.ts` le 2026-09-01 (#4713).
 *
 * Les deux gestes sont ici parce qu'ils partagent leur LOI (le plancher
 * `MODERATOR` puis `participantActionRefusal` au même rang — décision porteur
 * du 2026-08-29 : « on lève un bannissement qu'on aurait pu poser »), leur
 * résolution de cible sous les deux colonnes, et leur éventail. Les séparer en
 * deux fichiers aurait mis cette symétrie à la merci d'un correctif appliqué
 * d'un seul côté — exactement ce que l'asymétrie corrigée en août avait coûté.
 *
 * Le gestionnaire ne garde que le schéma, l'authentification et la traduction
 * du verdict — patron de `chargerPostsProches` (`routes/posts/nearby.ts`,
 * #4346). L'extraction est un DÉPLACEMENT : aucun ordre d'appel Prisma n'a
 * bougé, aucun message ni statut de refus n'a changé.
 *
 * **Aucun `try/catch` n'a été ajouté**, et c'est délibéré : les deux
 * gestionnaires d'origine n'en portaient pas, une exception y remontant au
 * gestionnaire d'erreurs de Fastify. En envelopper un ici changerait le CORPS
 * servi sur panne — ce qu'une extraction n'a pas à faire.
 */

export type DemandeDeBannissement = {
...
export type BannissementServi = {
  readonly participantId: string
  readonly userId: string | null
  readonly bannedAt: string
  readonly closedShareLinkId: string | null
}

export type LeveeServie = {
  readonly participantId: string
  readonly userId: string | null
}

export async function bannirParticipant(
  demande: DemandeDeBannissement,
): Promise<VerdictDeGeste<BannissementServi>> {
  const { prisma, currentUserId, socketIO } = demande

  const id = await resolveConversationId(prisma, demande.conversationIdentifier) ?? demande.conversationIdentifier
  ...
  if (!currentParticipant) {
    return refuser(404, 'Vous ne participez pas à cette conversation')
  }

  const actor = {
    conversationRole: currentParticipant.role,
    platformRole: demande.platformRole,
  }
  ...
  if (!actorHasMinimumRole(actor, MemberRole.MODERATOR)) {
    return refuser(403, 'Vous n\'avez pas les droits pour bannir un participant')
  }
  ...
  const targetParticipant = await resolveTargetParticipant(prisma, id, demande.targetKey)

  if (!targetParticipant) {
    return refuser(404, 'Participant introuvable')
  }

  if (targetParticipant.bannedAt !== null) {
    return refuser(400, 'Ce participant est déjà banni')
  }
  ...
  const io = socketIO?.getManager()?.getIO()
...
export async function leverBannissementDeParticipant(
  demande: DemandeDeBannissement,
): Promise<VerdictDeGeste<LeveeServie>> {
  const { prisma, currentUserId, socketIO } = demande
  ...
  if (!actorHasMinimumRole(actor, MemberRole.MODERATOR)) {
    return refuser(403, 'Vous n\'avez pas les droits pour lever un bannissement')
  }
  ...
  // à CETTE personne.
  const resolved = await resolveTargetParticipant(prisma, id, demande.targetKey)
  const targetParticipant = resolved?.bannedAt ? resolved : null

  if (!targetParticipant) {
    return refuser(404, 'Participant banni introuvable')
  }
  ...
  return accorder(identifyTarget(targetParticipant))
}

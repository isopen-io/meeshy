...
export async function communityRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/communities/:id/conversations', {
            ...
            data: {
              ...
              items: {
                ...
                properties: {
                  ...
                  // fast-json-stringify (`additionalProperties: false` par
                  // défaut). Renommer ne casse aucun client — `members` n'a
                  // jamais atteint le fil.
                  participants: {
                    type: 'array',
                    items: {
                      type: 'object',
    ...
    try {
      ...
      // la requête ci-dessus, pas par `hasAccess` : elle ne rend que les
      // conversations dont l'appelant est lui-même participant
      ...
      // les préférences s'appliquent. Lire le `where` avant de choisir le
      // régime : le contrôle d'accès de la communauté, plus permissif, aurait
      // conduit au régime mixte à tort.
      //
      ...
      sendSuccess(
        ...
        conversations.map((conversation: { participants?: Array<Record<string, unknown>> }) => ({
          ...
          participants: (conversation.participants ?? []).map(participant => {
            const nested = participant.user as MemberUser | null | undefined;

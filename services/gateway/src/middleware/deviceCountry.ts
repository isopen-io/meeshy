 ...
 *
 * Mirrors `deviceLocale.ts` exactly (debounce, bounded-cache eviction, no-op
 * contract, test seams).
 */
 ...
 * gateway serves over its lifetime. Mirrors `deviceLocale.ts`.
 */
 ...
 * process lifetime (an unbounded slow leak on a 100k+ user platform; the
 * `X-Meeshy-Country` header is sent by iOS on *every* request).
 *

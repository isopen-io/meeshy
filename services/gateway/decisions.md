...
**Le contrat** :
- **Modèle `MessageStar`** (calque de `PostBookmark`) : `userId` (un `User.id`, le favori est réservé aux comptes inscrits), `messageId`, `conversationId`, `createdAt`. Unicité `(userId, messageId)`, index `(userId, createdAt)` pour la liste. `conversationId` est une COPIE immuable de `Message.conversationId`, écrite par le serveur depuis la ligne du message, jamais depuis la requête : un message ne change jamais de conversation. Elle existe pour que la liste filtre la participation DANS la requête, et garde des pages pleines.
...
- **Suppression de compte (#4225)** : `MessageStar` relève d'**EFFACER** — des données dont la personne est le seul sujet et le seul bénéficiaire. Le job de purge, quand il s'écrira, la nomme ; la suppression d'un message en base fait tomber ses étoiles (`onDelete: Cascade`), et `scripts/preuves/preuve-inscription-par-email.sh` les efface avec les autres enfants d'un message, comme l'exige `proof-scripts-orphan-sender-guard.test.ts`.

...
**Sites** : `services/messaging/messageStars/` — `starredMessageVerdict.ts` (le verdict unique, pur), `MessageStarWriter.ts` (pose, retrait), `StarredMessagesReader.ts` (la liste), `starredMessageProjection.ts` (les `select` nommés et la projection) ; `routes/me/starred-messages.ts` et ses schémas fermés ; `@meeshy/shared/types/message-star` (Zod des frontières, types servis) ; `MessageStarredEventData` et `SERVER_EVENTS.MESSAGE_STARRED`.

**Tests** : `routes/me/starred-messages-{writes,list,locks}.test.ts` (51), sur un faux Prisma qui ÉVALUE ses `where` et rend des lignes chargées de colonnes interdites. Chaque garde a été retirée une à une et son témoin a rougi : participation, bannissement, plancher et masquage, ordre participation / vue unique, bit `VIEW_ONCE`, expiration, placeholder, pièce masquée, traduction chiffrée, curseur, et chacun des deux verrous de la règle 4 (projection, schéma) indépendamment de l'autre.

...
const telPlan = (ecranHtml, options = {}) => telephone(ecranHtml, { largeur: 300, x: 120, y: 36, ...options })
const telFin = (ecranHtml, options = {}) => telephone(ecranHtml, { largeur: 256, x: 142, y: 486, rotation: -4, ...options })
const telCouv = (ecranHtml, options = {}) => telephone(ecranHtml, { largeur: 300, x: 120, y: 400, rotation: -3, ...options })
...
const plan = ({ ctx, code, n, fond = 'nuit', visuel, sous, stY = 690, stH = 112, chipEnBas = false }) =>
  scene({
...
const carteFin = ({ ctx, code, visuel, extra = '', cta }) =>
  scene({
    format: F, ctx, fond: 'vif', classe: 'fin',
    contenu: html`${signature(ctx, { x: 34, y: 92 })}${titre(cta ?? VIDEOS[code].cta[ctx.lang], ctx, { x: 34, y: 200, largeur: 440, hauteur: 210, taille: 46 })}${extra}${visuel}`,
  })

const sous = (code, i, ctx) => VIDEOS[code].sous[i][ctx.lang]

// Micro en enregistrement : le bouton du composeur, agrandi, avec ses ondes.
const microEnregistre = (x, y) =>
  puce(html`<div class="micro-rec"><i></i><i></i><i></i><span>${icon('mic', { size: 54 })}</span></div>`, { x, y, z: 6 })
...
const emblemeFlamme = (ctx, jours, { x, y, taille = 230 }) =>
  puce(html`<div class="flamme-embleme" style="width:${taille}px;height:${taille}px"><span class="fl-ico">${icon('flame', { size: Math.round(taille * 0.62) })}</span><b>${jours}</b></div>
...
const carteAudio = (ctx, { libelle, drapeau, duree, active, graine = 0 }) => {
  const barres = waveformHeights(44).map((h, i) => waveformHeights(44)[(i + graine) % 44])
  return html`<div class="carte-audio${active ? ' active' : ''}">
    ...
    <div class="ca-corps"><span class="ca-play">${icon(active ? 'pause' : 'play', { size: 20 })}</span><div class="ca-onde">${barres.map((h, i) => html`<i class="${active && i < 26 ? 'on' : ''}" style="height:${(h * 1.7).toFixed(1)}px"></i>`)}</div></div>
  </div>`
}
...
export const VIDEOS_9x16 = {
  V1: [
    (ctx) => couverture({ ctx, code: 'V1', visuel: html`${telCouv(dm(ctx))}${drapeauxFleche(ctx, ctx.lang, 'ko', { x: 26, y: 470, taille: 34 })}` }),
    (ctx) => plan({ ctx, code: 'V1', n: 2, visuel: html`${telPlan(dm(ctx))}${microEnregistre(190, 430)}`, sous: sous('V1', 0, ctx) }),
    (ctx) => plan({ ctx, code: 'V1', n: 3, visuel: html`${telPlan(dm(ctx), { x: 60 })}${loupe(dm(ctx), { cible: '.bubble.audio', x: 150, y: 330, largeur: 370, hauteur: 250, rotation: 2 })}`, sous: sous('V1', 1, ctx) }),
    (ctx) => carteFin({ ctx, code: 'V1', visuel: html`${telFin(dm(ctx))}${drapeauxFleche(ctx, ctx.lang, 'ko', { x: 40, y: 560, taille: 28 })}` }),
  ],
  V2: [
    (ctx) => couverture({
      ctx, code: 'V2',
      visuel: html`${telCouv(E.ecranGlobalBonjour(sc(ctx, 'dark'), { reponses: 3 }))}${nuageBonjours([[BONJOURS[2], 22, 420, -6], [BONJOURS[4], 392, 470, 5, 'ar'], [BONJOURS[3], 14, 600, 4], [BONJOURS[5], 404, 640, -5], [BONJOURS[9], 20, 770, -3], [BONJOURS[10], 410, 800, 6]])}`,
    }),
    (ctx) => plan({ ctx, code: 'V2', n: 2, visuel: telPlan(E.ecranGlobalBonjour(sc(ctx, 'dark'), { reponses: 0, saisie: true })), sous: sous('V2', 0, ctx) }),
    (ctx) => plan({
      ctx, code: 'V2', n: 3,
      visuel: html`${telPlan(E.ecranGlobalBonjour(sc(ctx, 'dark'), { reponses: 3 }))}${loupe(E.ecranGlobalBonjour(sc(ctx, 'dark'), { reponses: 3 }), { cible: '.messages .msg-row:last-child .bubble', x: 60, y: 470, largeur: 420, hauteur: 150, rotation: -2 })}`,
      sous: sous('V2', 1, ctx),
    }),
  ...
  V3: [
    (ctx) => couverture({
      ...
      visuel: html`${emblemeFlamme(ctx, 30, { x: 155, y: 390 })}<div class="bande-drapeaux" data-sur>${drapeauxRencontres.slice(0, 10).map((d) => html`<span>${d}</span>`)}</div>`,
    }),
    (ctx) => plan({
      ctx, code: 'V3', n: 2,
      visuel: html`${telPlan(E.ecranProgressionAvec(sc(ctx, 'light'), { serie: 30, record: 30, serieJalon: 60 }))}${loupe(E.ecranProgressionAvec(sc(ctx, 'light'), { serie: 30, record: 30, serieJalon: 60 }), { cible: '.p-card.flamme', x: 70, y: 440, largeur: 400, hauteur: 170, rotation: -2 })}`,
      sous: sous('V3', 0, ctx),
    }),
    (ctx) => plan({
      ctx, code: 'V3', n: 3,
  ...
  V4: [
    ...
    (ctx) => plan({
      ctx, code: 'V4', n: 3, chipEnBas: true, stY: 30, stH: 108,
      visuel: html`<div class="grille-4">${langsQuatre(ctx.lang).map((l) => html`<div class="g4-cell"><span class="g4-lang">${langue(l).drapeau} ${langue(l).nom}</span>${telephone(ecran('groupe', contexte({ lang: l, theme: 'light' })), { largeur: 244, x: -2, y: -34 })}</div>`)}</div>`,
      sous: sous('V4', 1, ctx),
    }),
  ...
  V5: [
    (ctx) => couverture({
      ...
      visuel: html`${telCouv(E.ecranMaStory(sc(ctx, 'dark')), { y: 430, x: 150, largeur: 280 })}${E.commentairesPour(sc(ctx, 'dark'), 3).map((c, i) => puce(commentaireFlottant(ctx, c), { x: [16, 190, 30][i], y: 470 + i * 130, rotation: [-3, 2, -2][i], classe: 'com-puce' }))}`,
    }),
  ...
  V6: [
    (ctx) => couverture({ ctx, code: 'V6', hauteur: 300, taille: 42, visuel: html`${telCouv(ecran('appel', sc(ctx, 'dark')), { y: 430, rotation: 3 })}${puce('😂', { x: 40, y: 520, classe: 'emoji-geant', rotation: -10 })}` }),
  ...
  V7: [
    ...
    (ctx) => plan({ ctx, code: 'V7', n: 3, visuel: telPlan(E.ecranConsentementVoix(sc(ctx, 'light'))), sous: sous('V7', 1, ctx), stY: 670, stH: 136 }),
  ...
  V8: [
    (ctx) =>
      scene({
        ...
        contenu: html`${texte(HASHTAG[ctx.lang], ctx, { x: 34, y: 92, largeur: 440, hauteur: 64, taille: 44, classe: 'hashtag' })}
          ...
          ${telCouv(ecran('global', sc(ctx, 'dark')), { y: 430 })}`,
      }),
    ...
    (ctx) => carteFin({
      ctx, code: 'V8',
      extra: texte(HASHTAG[ctx.lang], ctx, { x: 34, y: 402, largeur: 440, hauteur: 60, taille: 36, classe: 'hashtag' }),
      visuel: telFin(ecran('succes', sc(ctx, 'dark')), { y: 500 }),
    }),
  ],
}

...
const nuitParis = () => {
  ...
  return raw(`<svg class="fond-illu" viewBox="0 0 540 960" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
    ...
    <circle cx="410" cy="150" r="120" fill="url(#npl)"/><circle cx="410" cy="150" r="44" fill="#fef9c3"/><circle cx="428" cy="138" r="40" fill="#312e81" fill-opacity=".0"/>
...
export const STORIES_9x16 = {
  S1: (ctx) =>
    scene({
      ...
      contenu: html`${nuitParis()}
        ${texte(`${STORIES.S1.ligne[ctx.lang]} 🌙`, ctx, { x: 40, y: 150, largeur: 460, hauteur: 90, taille: 30, classe: 'story-ligne' })}
  ...
  S3: (ctx) =>
    scene({
      format: F, ctx, fond: 'nuit', classe: 'story-meeshy',
      contenu: html`<div class="nuage-story">${BONJOURS.slice(0, 16).map((mot, i) => html`<span style="--i:${i}" class="${/[؀-ۿ]/.test(mot) ? 'ar' : ''}">${mot}</span>`)}</div>

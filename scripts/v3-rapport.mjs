...
// dans `apps/web-old-version3/` (le budget de bundle, le poids réseau CDP, le gate axe, le
...
import { CIBLES_PRODUCTION, verdictDeLigneDeBase } from '../apps/web-old-version3/scripts/baseline.mjs';
 ...
 * `test:a11y`/`test:lifecycle` (`apps/web-old-version3/package.json`) le font déjà.
 */
...
const mesureBudget = () => {
  const script = 'apps/web-old-version3/scripts/check-bundle-budget.mjs';
  const commande = `node ${script}`;
  if (!existsSync(join(RACINE, 'apps/web-old-version3/.next/app-build-manifest.json'))) {
    return mesure('budget de bundle', commande, ABSENTE, null, 'prérequis : bun run build dans apps/web-old-version3');
  }
...
const mesureReseau = (base, chemins) => {
  const script = 'apps/web-old-version3/scripts/mesure-reseau.mjs';
...
const mesureSpec = ({ nom, fichier }) => {
  const app = join(RACINE, 'apps/web-old-version3');
  const commande = `npx playwright test ${fichier} (dans apps/web-old-version3)`;

  if (!existsSync(join(app, '.next/app-build-manifest.json'))) {
    return mesure(nom, commande, ABSENTE, null, 'prérequis : bun run build dans apps/web-old-version3');
  }
  if (!existsSync(join(app, 'node_modules/@playwright/test'))) {
    return mesure(
      ...
      ABSENTE,
      null,
      'prérequis : bun install dans apps/web-old-version3 (@playwright/test absent)',
    );
  }
...
// vit dans `apps/web-old-version3/scripts/baseline.mjs`, avec la donnée qu'il juge — une
// seconde lecture de `etablie` ici serait la jumelle que le § 9.2 interdit.
const NOM_LIGNE_DE_BASE = 'ligne de base (prod)';

const CHEMIN_LIGNE_DE_BASE = join(RACINE, 'apps/web-old-version3/e2e/visual/baseline.json');

const mesureLigneDeBase = () => {
  const commande = 'node apps/web-old-version3/scripts/baseline.mjs <les 6 urls de production>';
  const lu = (() => {
    try {
...
// de fin nomme (« échoue proprement si apps/web-old-version3 n'a pas encore de route »).

...
//   apps/web/package.json    "@meeshy/shared": "workspace:*"
//
// `turbo run build --filter=@meeshy/web` construirait donc `packages/shared`
...
//     de la RACINE : `lint`, `lint --filter=@meeshy/web`,
...
const MUTATIONS = [
  [
    ...
    'job « institutionnel-v3 » · scénario bun · étape « Build apps/web',
  ],
  [
  ...
  [
    ...
    (world) =>
      inJob(
        ...
        'institutionnel-v3',
        /      - name: Build apps\/web[^\n]*\n        run: \|\n          cd apps\/web\n/,
        '      - name: Build apps/web (branche)\n        run: |\n          if true; then cd apps/web; fi\n',
      ),
    "n'est pas lisible",
  ],
  [

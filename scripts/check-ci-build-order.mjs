...
//   apps/web-v3-old/package.json    "@meeshy/shared": "workspace:*"
//
...
// `apps/web-v3-old`. Les trois jobs qui construisent la v3 (`a11y-v3`,
...
// `.github/workflows/ci.yml` et `apps/web-v3-old/package.json` dans l'état de
// `f73f3c525e^` — l'arbre exact qui a rougi — ce garde rend rc=1 et NOMME les
// trois jobs, les trois étapes et la raison :
//
//   job « a11y-v3 » · étape « Build apps/web-v3-old (le manifeste que le balayage
...
//
// (`apps/web-v3-old/next.config.ts` lit d'ailleurs
  ...
  [
    ...
    (world) =>
      inJob(
        world,
        'a11y-v3',
        /      - name: Build apps\/web-v3-old[^\n]*\n        run: \|\n          cd apps\/web-v3-old\n/,
        '      - name: Build apps/web-v3-old (branche)\n        run: |\n          if true; then cd apps/web-v3-old; fi\n',
      ),
    "n'est pas lisible",
  ],
  [

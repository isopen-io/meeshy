...
3. **La langue d'origine concourt à son RANG dans le prisme, jamais comme court-circuit (2026-08-10).** Un résolveur parcourt les langues du lecteur DANS L'ORDRE ; la première servie gagne — par une traduction, ou parce que le message est déjà écrit dedans. Ne JAMAIS écrire « si la langue d'origine appartient au prisme ⇒ afficher l'original » : cette formulation rétrograde la langue PRIMAIRE dès que la langue d'origine occupe un rang inférieur, ce que produit mécaniquement la locale appareil (règle 2). Prisme `['fr','en']`, message anglais, traduction française disponible ⇒ **« Bonjour »**, jamais « Hello ». Sources de vérité, une par client : `resolveLastMessagePreview()` (`packages/shared/utils/conversation-helpers.ts`, consommée par le web), `MeeshyConversation.resolvedLastMessagePreview` (`packages/MeeshySDK/.../CoreModels.swift`) et `resolveLastMessagePreview()` (`apps/android/core/model/.../lang/LastMessagePreviewResolver.kt`) — toute évolution touche les TROIS. Android a rejoint la règle au cycle 118 : son `ApiConversation` ne déclarait ni `lastMessageTranslations` ni `lastMessageOriginalLanguage`, donc le décodeur les jetait et la ligne de liste restait dans la langue de l'expéditeur pour tout le monde. **Quand cette liste dit « jumelles », compter les clients avant de la croire.**
   ...
   **Cycle 126 — l'aperçu composé avait TROIS consommateurs et UN seul câblage.** L'éventail COMPOSE, une fois par message, ce que sa bannière donne à voir : `notificationPreviewForPush`, `pushPreviewBasis`, `attachmentInfo`, `notificationLocKey`. Un seul de ses trois lots le lisait. **Répondre à quelqu'un par un vocal, une photo, une vidéo ou un fichier lui poussait une bannière au CORPS VIDE**, pendant que tous les autres membres recevaient « 🎤 Message vocal · 0:12 » ou la transcription : pour un message sans texte, la matière brute (`Message.content`) est la chaîne vide, et `buildMessageNotificationBodyI18n` — la fonction qui rend un média lisible — n'avait qu'un appelant.

   > **La cinquième question ne s'adresse pas au résolveur mais à la VALEUR qu'il rend : qui d'AUTRE aurait dû l'afficher ?** Compter les consommateurs POSSIBLES d'une valeur composée avant ses consommateurs RÉELS. Une valeur lue par un seul d'entre eux n'est pas partagée — elle est PRIVÉE, et ses jumelles recomposent la même chose à côté, moins bien. C'est la leçon 271 portée sur une VARIABLE LOCALE, qu'aucun `grep` ne trouve faute de nom exporté : ce qui la rend comptable, c'est de la NOMMER.

   Site unique désormais : `MessageBannerMedia` (le jeu de champs, nommé), `messageBannerBody()` (le corps), `bannerMediaContext()` / `bannerMediaMetadata()` (les deux projections du média) et `messageClockFields()` (l'horloge serveur de la bulle NSE), tous dans `NotificationService`. Deux durcissements en effet de bord : réponse et mention reçoivent enfin `notificationLocKey` — donc le verrou du cycle 125 — et le `protectedByLocKey` qui manquait à leur `previewPrismSource`. Détail : `tasks/realtime-sync-audit-2026-08-24-cycle126.md`, `tasks/lessons.md` § 277.

   **Et un suivi étiqueté « décision produit » se relit avant d'être recopié.** Les cycles 124 et 125 avaient MESURÉ ce défaut et l'avaient classé ainsi — juste sur la taxonomie (il change ce que la bannière MONTRE, pas la langue dans laquelle elle le montre), faux sur la conclusion. Devant une telle étiquette, demander : **quel produit choisirait ça ?** Si la réponse est « aucun », c'est un défaut qui porte une mauvaise étiquette.
   ...
   **Le suivi du cycle 124 — « les éventails RÉPONSE et MENTION ne portent aucune transcription » — est SOLDÉ au cycle 126** (voir ci-dessus). Il était classé « décision produit » ; il ne l'était pas.

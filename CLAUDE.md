...
> | `apps/web-v3-old` | l'ancienne refonte v3, **annulée** | ne reçoit plus RIEN, n'a jamais servi une page en production |
>
> **`apps/web-v3-old` est ANNULÉE.** Aucune feature, aucun correctif, aucun portage d'écran — y compris si une issue ouverte avant ce jour le demande, y compris si `ordre.md` ou la matrice des 31 vues le prévoit : **ces plans sont périmés, ils ne se rejouent pas.** Les skills `meeshy-web-v3-lot` et `meeshy-web-v3-bout-en-bout` ne s'invoquent plus — ils visent cette application-là.
>
...
>
> **La v3.1 suit l'interface iOS**, pas la planche web : `apps/ios` et `packages/MeeshySDK` sont la référence de disposition, de hiérarchie, d'états et de gestes. La palette est DÉRIVÉE de `MeeshyColors.swift` (`packages/design-tokens/ios.css`, généré — #5445) ; `tokens.css` reste la table de `web-v3-old` et **mourra avec elle**.
>
...
Source de verite iOS : `ConversationViewModel.preferredLanguages` + `preferredTranslation(for:)` — et `preferredLanguages` PROJETTE `ReaderPrism.resolve(for:)` (`packages/MeeshySDK/Sources/MeeshySDK/Auth/ReaderPrism.swift`, la descente STRICTE, rangs 1→4 sans repli « fr »), la MÊME fonction que `MessagePersistenceActor.readerPrism()` remet aux chemins socket et NSE pour graver `replyToJson`. Deux boucles jumelles gravaient deux citations pour un même message dès que la locale appareil manquait au serveur ou qu'aucune langue n'était configurée, et chaque ouverture rejouait un changement de ligne pour un contenu identique (2026-09-03, #4945). `MeeshyUser.preferredContentLanguages` (repli « fr ») sert les DÉFAUTS de composition, jamais ce qui se grave ni ce qui s'affiche en conversation.
...
3. **La langue d'origine concourt à son RANG dans le prisme, jamais comme court-circuit (2026-08-10).** Un résolveur parcourt les langues du lecteur DANS L'ORDRE ; la première servie gagne — par une traduction, ou parce que le message est déjà écrit dedans. Ne JAMAIS écrire « si la langue d'origine appartient au prisme ⇒ afficher l'original » : cette formulation rétrograde la langue PRIMAIRE dès que la langue d'origine occupe un rang inférieur, ce que produit mécaniquement la locale appareil (règle 2). Prisme `['fr','en']`, message anglais, traduction française disponible ⇒ **« Bonjour »**, jamais « Hello ». Sources de vérité, une par client : `resolveLastMessagePreview()` (`packages/shared/utils/conversation-helpers.ts`, consommée par le web), `MeeshyConversation.resolvedLastMessagePreview` (`packages/MeeshySDK/.../CoreModels.swift`) et `resolveLastMessagePreview()` (`apps/android/core/model/.../lang/LastMessagePreviewResolver.kt`) — toute évolution touche les TROIS. Android a rejoint la règle au cycle 118 : son `ApiConversation` ne déclarait ni `lastMessageTranslations` ni `lastMessageOriginalLanguage`, donc le décodeur les jetait et la ligne de liste restait dans la langue de l'expéditeur pour tout le monde. **Quand cette liste dit « jumelles », compter les clients avant de la croire.**
   ...
   **Un contenu résolu voyage avec son MÉDIUM (cycle 128).** La famille AUDIO ci-dessus a DEUX lignes parce que le cycle 123 n'en avait corrigé qu'une : il a fait descendre le prisme au TEXTE de la bannière d'un vocal, et laissé douze lignes plus bas, dans le même objet littéral, `firstAttachmentUrl: first?.fileUrl` — l'original, sans condition. Une bannière **en français** au-dessus d'un `UNNotificationAttachment` qui **parle anglais**. La piste TTS vivait pourtant sur la colonne que l'éventail lisait déjà (`MessageAttachment.translations[lang].url`), dont `transcriptTranslationTexts()` ne prenait que le texte ; sa jumelle `transcriptTranslationTracks()` (`packages/shared/types/attachment-audio.ts`) en est le site unique.
   ...
   Et **la piste est élue par la langue du TEXTE SERVI, jamais par une descente indépendante** : deux descentes parallèles serviraient « la réunion est déplacée » au-dessus d'une piste espagnole — un défaut PIRE que celui qu'on corrige, parce qu'il a l'air d'une traduction ratée plutôt que d'une traduction absente. Détail : `tasks/lessons.md` § 284.
...
```typescript
{ success: boolean, data?: T, error?: string, message?: string, code?: string, pagination?: PaginationMeta }
```
...
**Le document des VUES est `docs/product/MeeshyComposerDesign/MeeshyComposerMobile.dc.html` (+ son `support.js`), et il PRIME sur la planche là où les deux divergent — pour la DISPOSITION, la HIÉRARCHIE, les ÉTATS et les GESTES de chaque écran, et sous réserve de sa DATE.** Le document est arrêté au 2026-08-28 : **une directive porteur postérieure le supplante, et l'issue fermée fait foi.** *Exception en vigueur* — la **géographie des rails** : le document montre des contrôles posés SUR la scène ; #4561 et #4633 (fermées le 2026-08-31) ont tranché l'inverse, et `apps/ios/CLAUDE.md` § 1 l'écrit — aucun contrôle ne se pose sur le canvas, les rails vivent dans les couloirs du plateau. Sans cette clause, la règle de préséance ordonne littéralement de reproduire une géographie abandonnée. **Et son SILENCE n'est pas une prescription** (retour porteur 2026-09-02, revert `328dd69026`) : une cible muette sur une dimension ne l'interdit pas. La vue `1b` ne dessine pas les rails du plateau ; elle ne dit pas de les retirer — les lire comme une interdiction a fait perdre une sémantique que le document n'exprimait pas (*à GAUCHE ce qu'on POSE sur la scène, à DROITE ce qui structure la PUBLICATION*). Le document prescrit ce qu'il MONTRE ; sur le reste, il se tait, et **ce qui existe et complète la cible s'agrège, ne se supprime pas.** **La SÉMANTIQUE — les noms et les règles — n'est ni à la planche ni au document des vues : elle est à `docs/product/meeshy-composer-modele.md`**, qui en est l'autorité déclarée (et que la garde `PlancheVocabularyGuardTests` cite comme référence de vocabulaire). **Et la sémantique de la LECTURE est à `docs/product/meeshy-reader-modele.md`** (#4769) — même rang, autre moitié : le premier dit ce qu'on écrit, le second ce qui le restitue. Il tranche notamment que le moteur partagé est le **player** (`MeeshyScenePlayer`) et non « le lecteur », ce dernier mot désignant partout dans le code l'état « en lecture, pas en composition » ; la loi 6 se relit donc « le PLAYER est l'aperçu ». La planche porte les features et leurs lacunes ; ce document porte les **31 écrans mobiles** à implémenter, chacun avec sa doctrine. Les captures de référence vivent dans `cible/<id>.png`, l'index dans `vues.md`, et `capture-cibles.js` régénère les trois d'UNE commande — aucune capture n'est posée à la main, donc aucune ne peut dériver du document en silence. Les trois planches larges (`4a` pipeline, `4d` refus motivés, `4e` budgets) ne sont **pas** des écrans : ce sont des critères de recette. Pilotage : milestone « Les 31 vues mobiles du composer sont implémentées » (#63), une issue par vue (#4071–#4101), vérifiées au simulateur `Meeshy-iOS26`. **Conformité = disposition, hiérarchie, états et gestes** ; polices, couleurs et rayons par le design system Meeshy — l'écart typographique avec le document est assumé. La refonte PRÉSERVE : l'ancien chemin reste routé tant que la matrice des 31 vues n'est pas complète, et le décommissionnement est un lot séparé.
`docs/product/planche-meeshy-composer.html` (ex-`docs/superpowers/specs/2026-08-19-meeshy-composer-views.html`, 100+ révisions) est la **vision de design vivante** du composer : matrice feature × état (`data-task` / `data-kind` / `data-state`), douze lois, inventaire, budgets, risques, et les VUES cibles (anatomie, scène 9:16, outils, timeline, étagère, viewers, 18 styles de texte, iconographie, éditeurs). C'est cette vision, validée, que MeeshyComposer / Scène / Timeline doivent reproduire à la fin.
...
- **La source de vérité de la vision est le FICHIER du dépôt `docs/product/planche-meeshy-composer.html` (et sa version Markdown allégée `docs/product/planche-meeshy-composer.md`, avec captures des vues), jamais une URL d'artifact.** Un artifact peut disparaître à tout moment (compte, partage retiré, purge) ; le fichier tracké, non. Le fichier PEUT être republié en artifact comme rendu de design ÉPHÉMÈRE (autorisé), mais aucune référence de gouvernance ne dépend d'une URL — on ouvre le FICHIER.
- Les rendus des vues (PNG/PDF/captures) vivent dans `docs/product/planche-meeshy-composer/`, alimentent la version Markdown, et sont référencés par les issues du projet « Meeshy — pilotage ».

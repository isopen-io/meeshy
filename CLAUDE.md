...
3. **La langue d'origine concourt à son RANG dans le prisme, jamais comme court-circuit (2026-08-10).** Un résolveur parcourt les langues du lecteur DANS L'ORDRE ; la première servie gagne — par une traduction, ou parce que le message est déjà écrit dedans. Ne JAMAIS écrire « si la langue d'origine appartient au prisme ⇒ afficher l'original » : cette formulation rétrograde la langue PRIMAIRE dès que la langue d'origine occupe un rang inférieur, ce que produit mécaniquement la locale appareil (règle 2). Prisme `['fr','en']`, message anglais, traduction française disponible ⇒ **« Bonjour »**, jamais « Hello ». Sources de vérité, une par client : `resolveLastMessagePreview()` (`packages/shared/utils/conversation-helpers.ts`, consommée par le web), `MeeshyConversation.resolvedLastMessagePreview` (`packages/MeeshySDK/.../CoreModels.swift`) et `resolveLastMessagePreview()` (`apps/android/core/model/.../lang/LastMessagePreviewResolver.kt`) — toute évolution touche les TROIS. Android a rejoint la règle au cycle 118 : son `ApiConversation` ne déclarait ni `lastMessageTranslations` ni `lastMessageOriginalLanguage`, donc le décodeur les jetait et la ligne de liste restait dans la langue de l'expéditeur pour tout le monde. **Quand cette liste dit « jumelles », compter les clients avant de la croire.**
   ...
   **Ce suivi du cycle 124 est SOLDÉ, en deux temps, et sa première qualification était fausse.** Il disait : « les éventails RÉPONSE et MENTION ne portent aucune transcription ⇒ corps vide pour un vocal pur — décision produit, pas correction de Prisme ». Le cycle 125 bis l'a mesuré et retourné : **un corps VIDE n'est pas un autre choix de produit, c'est l'absence de la bannière sous la bannière** ; la composition du corps est depuis un site UNIQUE (`NotificationService.servedBannerBody`) que les trois éventails partagent. Le cycle 126 a soldé le reste — l'ESTAMPILLE de la bulle pré-enregistrée (`createdAt`, `messageType`), posée en ligne chez le seul `createMessageNotification` : une réponse ou une mention faisait donc écrire une bulle rangée à l'heure du DEVICE et rendue en `text`, soit un rectangle VIDE pour une réponse vocale (ces éventails ne poussant pas `attachmentMimeType`, le repli média de la NSE ne rattrapait rien). `prePersistedMessageFields` compose désormais les QUATRE champs que la NSE lit.

   > **Un helper PARTAGÉ peut ne composer qu'une PARTIE de ce que son nom promet, et le partage de la partie fait passer le TOUT pour partagé.** Ce n'est pas la règle du cycle 125 bis rejouée, c'est son RETOURNEMENT : là, il manquait un appelant et le compte était faux ; ici le helper est bien le plus HAUT, ses trois appelants sont bien les trois éventails, le compte est juste — et il manque un CHAMP. La question ne se pose donc pas du côté des appelants mais du **CONSOMMATEUR** : quels champs le lecteur lit-il, et lesquels ce site compose-t-il ? Deux listes à confronter, jamais un compte à faire. Corollaire de périmètre : tout ce qu'un site remet n'est pas du même GENRE — l'estampille traverse les refus opposés au TEXTE, parce qu'un horodatage ne révèle que l'instant d'un message que la bannière annonce déjà, et un type que l'icône que `protectedPreview` compose déjà. Une garde trop LARGE dégrade un service sans rien protéger, et personne ne le signale jamais : elle ressemble à de la prudence. Détail : `tasks/lessons.md` § Leçon 279.

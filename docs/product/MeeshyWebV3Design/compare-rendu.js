...
// locale : voir apps/web-v3/scripts/lib/budget-reseau.mjs (#5473).
const BUDGETS_JSON = JSON.parse(
  fs.readFileSync(path.join(ROOT, 'apps/web-v3/budgets.json'), 'utf8'));
...
(async () => {
  ...
  const { cleDeBudget } = await import(
    pathToFileURL(path.join(ROOT, 'apps/web-v3/scripts/lib/budget-reseau.mjs')).href);

...
**Porté PARTIELLEMENT dans le dépôt (2026-09-04, #5115)** : `app/vitrine/feuille.ts` et `app/connecte/feuille.ts` portent désormais le contour de contrôle (`--color-border-interactive`, règle 16), l'encre pleine de `.carte-vide p` (`--color-text`, règle 18) et le dégradé du héros entre `--color-tint-primary` et `--color-surface` (règles 9 et 35) — `charte.test.ts` (76/76) et `check-jetons.mjs` (deux paires de contraste, encre mesurée aux deux extrémités du dégradé) les gardent. **Restent EN L'ÉTAT sur ces deux mêmes fichiers**, non touchées par ce portage : `.accroche` (= `.heros p`, règle 18, toujours en `--color-text-muted`) et `--radius-md` sur `.atouts .tuile` / `.carte .tuile` (règles 9 et 34, hors des cinq rayons). Le portage s'arrête donc au milieu de son propre périmètre — « partiel » est le Status de l'issue #5115, pas un livrable ; les deux points restants s'y suivent, ou s'ouvrent en issue de suivi s'ils en sortent.

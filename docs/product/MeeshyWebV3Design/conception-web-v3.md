...
| Médias distants | inchangé — le **chemin** de `fileUrl` servi tel quel | `GET /attachments/file/*` | ✅ | **Ne jamais reconstruire le CHEMIN côté client** (signature `?exp=&sig=` à venir dans la même valeur) — mais **`fileUrl` est un CHEMIN, pas une URL** : `UploadProcessor.getAttachmentPath` persiste `/api/v1/attachments/file/…` **sans origine**, et l'apex n'a aucun routeur `/api`. On lui donne **toujours** l'origine publique de la passerelle (`adresseDuMedia()`, `lib/api/passerelle.ts`), **dans la projection** (`lib/api/*.ts`), jamais dans une vue. Vaut pour `fileUrl`, `thumbnailUrl` et les pistes TTS (`translations[lang].url`, même forme relative) |

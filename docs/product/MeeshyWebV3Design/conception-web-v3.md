...
**À corriger dans le dépôt tel qu'il est** : `app/connecte/feuille.ts` pose `.carte-vide{border:… dashed var(--color-border-strong)}` (règle 16) et `.carte-vide p{color:var(--color-text-muted)}` (règle 18) ; `app/vitrine/feuille.ts` pose `.heros{border-radius:var(--radius-lg);background:var(--color-tint-primary)}` (règles 9 et 35).

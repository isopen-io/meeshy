...
**Source** : jugement du **2026-09-02** (tour 2), 7 critères × 3 directions, captures CLAIRES et SOMBRES regardées une par une — cibles comprises (`cible/{home,chats,thread,join,rights}.png`) —, feuilles lues, chiffres **re-mesurés par le juge**, aucun repris sur parole. Les trois propositions jugées et les instruments qui ont produit ces chiffres vivent dans `.cache/web-v3-workflow/charte/` ; la charte est **recopiée ici** parce qu'un cache n'est pas une source de vérité et qu'aucune règle opposable ne peut dépendre d'un fichier qu'aucune passe ne régénère.

| direction | 1 soleil | 2 directive | 3 légèreté | 4 planche | 5 a11y | 6 maintenance | 7 cohérence | **total** |
|---|---|---|---|---|---|---|---|---|
| **app-moderne** | 7 | **9** | 7 | **9** | **9** | 9 | **8** | **58/70** |
| clarté rurale | **9** | 7 | **8** | 6 | 7 | **10** | 7 | 54/70 |
| sobriété premium | 3 | 3 | 6 | 3 | 2 | 2 | 3 | 22/70 |

**Direction retenue : « app-moderne » (tour 2)** — c'est la charte du tour 1 *appliquée*, débarrassée de ce qui l'avait fait perdre (ombres, `backdrop-filter`, bulles, barre d'onglets, seconde table, −40 % de poids), plus quatre plans, une lumière et les états que personne d'autre n'a peints (`trou`, frappe, refus 409, lien d'évitement). **Greffée de « clarté rurale »** : sa règle d'encre MESURÉE (règle 14), ses deux amendements de contour (règles 12 et 15), son parti d'UNE feuille (règle 3). **Greffée de « sobriété premium »** par son échec : la règle 26 (la modale défile en interne, l'action primaire reste atteignable) naît de sa modale tronquée. Trois défauts trouvés par le juge et que **personne** ne corrigeait entrent comme règles : l'occlusion par les ronds flottants (règle 7, mesurée sur les trois), le débordement horizontal à 360 px (règle 9, mesuré sur clarté rurale), l'ordre du bandeau des droits (règle 28, contredit par `cible/rights.png`).

Elle vaut pour les cinq écrans du focus (vitrine, tableau de bord, `/chats`, `/chat/:lien`, fil) puis pour tout écran suivant. **Une règle sans témoin n'entre pas ici.**

**Ce qu'elle REMPLACE, et comment relire les références écrites avant elle.** Elle SUPPLANTE la charte du 2026-09-01 (tour 1, direction « clarté rurale »), qui occupait ce même § 12.5 : mêmes cinq écrans, même exigence de témoin, mais des chiffres rejoués et des règles **renumérotées**. Les renvois par numéro écrits avant ce jour (matrice, planche, commentaires du dépôt) se relisent par la table ci-dessous ; toute référence NEUVE s'écrit avec sa date — « charte règle 7 (2026-09-02) » —, parce qu'un numéro seul ne dit pas de quelle charte il parle.

| tour 1 (2026-09-01) | tour 2 (2026-09-02) | ce qui change |
|---|---|---|
| 1 table, 2 typographie, 4 hauteurs, 8 échelle | 1, 2, 4, 8 | inchangées ; **aucun jeton neuf n'est demandé** — la table les porte déjà tous |
| 3 poids | 3 | mesuré **par ROUTE** et non par feuille ; plafond de route resserré à **6 Ko gzip** (§ 12.6) |
| 5 quatre rayons | 5 | **cinq** rayons : `--radius-xl` entre (héros, carte mise en avant, carte d'état vide) |
| 6 actions flottantes | **7** | + trois clauses d'occlusion nées d'une mesure (bande réservée, aucun fixe sur un CONTRÔLE, milieu de ligne libre) |
| 7 HTML réel | **6** | + le **lien d'évitement** devient obligatoire sur chaque écran |
| 9 deux fonds | **10** | **quatre plans** (`bg-sunken` / `bg` / `surface` / `surface-raised`), gardés par l'ORDRE de leurs luminances |
| 10 filet ≠ contour | **12** | + le pointillé d'un état vide passe au contour de CONTRÔLE (1,79:1 mesuré sur l'ancien) |
| 11 aucune ombre, aucun dégradé | **30** et **31** | l'interdit des ombres, flous et images demeure ; **UN** dégradé est autorisé, au seul héros de la vitrine |
| 12 cartes | **11** | + « Tout voir » en tête de section, carte mise en avant, pseudo et langue CÔTE À CÔTE |
| 13 accent (cinq emplois) | 13 | **six** emplois : le rond d'envoi et de rappel entre |
| 14 contraste, 15 focus, 16 thème | **15**, **16**, **17** | + l'encre du contenu devient sa propre règle (**14**), mesurée |
| 17 bandeau … 21 fil | **18** … **22** | + hors-ligne jamais rouge **et jamais sans bouton** |
| 22 Prisme, 23 glyphes, 24 mouvement, 25 modale | **23**, **24**, **25**, **26** | + l'action primaire de la modale reste dans le premier écran du panneau |
| 26 fil plat | **27** et **28** | le bandeau des droits devient sa propre règle : PREMIER bloc sous l'en-tête |
| 27 temps réel, 28 interdits | **29**, **30** | inchangées en substance |
| — | **9**, **14**, **28**, **31** | règles NEUVES : débordement horizontal, encre du contenu, ordre du bandeau, l'unique dégradé |

**Ce que cette charte contredit dans le DÉPÔT tel qu'il est aujourd'hui** — nommé ici, corrigé par le lot d'implémentation, jamais par le commit qui écrit ce paragraphe : `app/connecte/feuille.ts` pose `.carte-vide{border:… dashed var(--color-border-strong)}` (règle 12 : contour de contrôle) et `.carte-vide p{color:var(--color-text-muted)}` (règle 14 : la phrase d'un état est du CONTENU) ; `app/vitrine/feuille.ts` pose `.heros{border-radius:var(--radius-lg);background:var(--color-tint-primary)}` (règles 5 et 31 : `--radius-xl`, et l'unique dégradé). Trois lignes, trois règles — elles sont la première tâche du lot qui reprend ces écrans.

#### A. Fondations

1. **Une table, zéro valeur ailleurs.** Couleur, rayon, police, ESPACE, CIBLE et GÉOMÉTRIE viennent de `packages/design-tokens` — la table les porte déjà TOUS (`--space-1…9`, `--target-min`, `--action-height`, `--action-height-secondary`, `--field-height`, `--row-height`, `--stroke-hair|strong|focus`, `--glyph|-inline|-large`, `--avatar|-small`, `--presence-dot`, `--frame-blur`, `--measure`, `--shell-width|-wide`, `--font-native`, `--color-focus|-contra`, `--color-tint-primary|success|warning|danger`). Une feuille n'écrit aucun `px` hors l'idiome `.hors-ecran` (1px/−1px) et le point de rupture `@media (min-width:600px)` ; les seuls autres littéraux tolérés sont les hauteurs de fenêtre (`100dvh`, `92dvh`, `34dvh`). **Aucun jeton neuf n'est demandé par ce tour.** — *Témoin* : `check-jetons.mjs` étendu (un `px` en `padding|margin|gap|width|height|min-height|border-radius|font-size` sous `apps/web-v3/app/**` rougit), non-vacuité par sonde `.sonde{padding:7px}`. **Mesuré ce jour** : app-moderne 3 littéraux, clarté rurale 3, **sobriété premium 215** — alors que tous les jetons qui lui manquaient existent. C'est cette mesure, pas un goût, qui met sobriété à 2/10 en maintenabilité.
2. **Corps 17 px, pile système, interligne 1,6, sur les cinq écrans.** `body{font-family:var(--font-native);font-size:var(--text-md);line-height:var(--leading-relaxed)}`. Cinq rôles, pas onze : titre d'écran `--text-3xl`, titre de section `--text-lg`, corps `--text-md`, secondaire `--text-base`, méta `--text-sm` ; plus petit texte LU `--text-xs` ; **`--text-2xs` interdit**. Aucun `@font-face`, aucune requête de police. — *Témoin* : `grep -c 'text-2xs'` = 0 ; `mesure-reseau.mjs` : 0 ressource `font`, 0 `CSSFontFaceRule`. **Mesuré ce jour au navigateur** : app-moderne 17 px / 27,2 px, clarté rurale 17 / 27,2, **sobriété 15 / 21,75** — le corps le plus petit sur l'écran qui demande le plus grand.
3. **Poids : une feuille par ROUTE, mesurée par ROUTE.** Feuille de chrome compactée ≤ **4 Ko gzip** ; total CSS d'une route ≤ **6 Ko gzip** (plafond neuf : le § 8.5 en cite 20, trop lâche pour arbitrer) ; 0 image, 0 police, 0 script, 0 requête avant le premier pixel ; la table inlinée ≤ 1,5 Ko gzip (mesuré par les propositions : 1 338 / 1 361 o). — *Témoin* : `gzip -9c` dans `__tests__/charte.test.ts`, **par route et pas seulement par feuille**. **Mesuré ce jour** (compactage `compacte()`) : app-moderne chrome 2 761 · fil 1 385 · join 849 ⇒ route `/chat/:lien` = **4 995 o**, vitrine = 2 761 ; clarté rurale, feuille unique = **3 474 o** partout ; sobriété 3 207 o (mais sans squelette, sans `trou`, sans frappe, sans hors-ligne — moins d'octets parce que moins d'états). La comparaison chrome-à-chrome de la thèse app-moderne est fausse : elle oppose sa feuille de chrome à une feuille qui contient déjà le fil ET la modale. **La route de jonction est le point d'entrée le plus hostile au réseau — c'est elle qu'on mesure d'abord.**

#### B. Cibles, actions, rayons

4. **Hauteurs, pleine largeur sur mobile.** Action principale `.action.primaire` = **56 px** (`--action-height`), fond `--color-primary`, encre `--color-on-primary` (6,29 mesuré au pire) ; secondaire `.action.contour` = **52 px** (`--action-height-secondary`), contour `--stroke-strong` `--color-border-interactive` ; tertiaire (`.action.discrete`, puces, bouton de bandeau, retour, liens de pied) = **44 px** (`--target-min`), largeur du contenu, `min-inline-size:var(--target-min)`. Deux puces partagent une ligne ; deux actions pleine largeur s'empilent (`--space-3`). ≥ 600 px : largeur automatique, `min-width:220px`. — *Témoin* : `e2e/visual/v3-cibles.spec.ts` à **360 ET 390 px** : 0 cible < 44 en hauteur OU largeur, `min(.action.primaire)` = 56. **Mesuré ce jour** : app-moderne 0, clarté rurale 0, **sobriété 5** (liens de pied à 19 px, champ de recherche à **24 px**, marque à 32 px) — la directive « gros boutons » n'y est pas tenue.
5. **Cinq rayons, un rôle chacun** (`--radius-xl` entre par ce tour, il est déjà dans la table) : `pill` = actions, puces, avatars, compte, pastille de présence, champ du composeur, ronds flottants · `lg` = cartes, bandeaux, champs, accordéon, tuile de liste · `xl` = héros de la vitrine, carte mise en avant, carte d'état vide · `xs` = `.langue` · `2xl` = feuille modale (`2xl 2xl 0 0` en feuille basse, plein ≥ 600 px). — *Témoin* : `grep -oE 'border-radius:[^;]+' | sort -u` ⊆ cet ensemble CLOS de cinq.
6. **HTML réel, contrôle à effet, lien d'évitement.** `<header>/<nav aria-label>/<main>/<form>/<dialog>/<details>/<summary>/<button>/<a>/<time datetime>` ; jamais une `div` cliquable. Chaque écran ouvre par un `<a class="hors-ecran" href="#contenu">Aller au contenu</a>`. Un glyphe seul n'existe que sur un rond à `aria-label`. « Réessayer » et « Charger les messages manquants » sont des `<a href>` ou des `<form>`, jamais un `type="button"` sans gestionnaire. — *Témoin* : `v3-a11y.spec.ts` (axe 0 serious/critical, quatre colonnes) ; `hrefsMorts` = 0 ; `grep -c 'href="#"\|onclick'` = 0 hors module de participation. **Seule app-moderne portait le lien d'évitement** ; il devient obligatoire.
7. **Actions flottantes : deux ronds, jamais une barre — et jamais SUR un contrôle.** Droite `.flottant.primaire` 56 px (`--color-primary`, `ph-user-circle`), gauche `.flottant.contour` 52 px (`--color-surface-raised`, `--stroke-strong` `--color-border-interactive`, `ph-squares-four`) ; chacun est un `<a href>` vers une route SERVIE — tant que sa destination n'existe pas, il n'est pas rendu. **Trois clauses neuves, nées d'une mesure** : (a) le conteneur défilant réserve `padding-block-end` ≥ la bande des ronds (`--space-9`), pour qu'au repos la DERNIÈRE ligne ne soit jamais couverte ; (b) **aucun élément fixe ne recouvre un élément INTERACTIF** — une cible sous une cible n'est pas atteignable ; (c) les deux ronds restent aux DEUX coins, la bande horizontale entre eux libre de tout élément fixe, pour que le milieu de chaque ligne reste lu. — *Témoin* : `v3-cibles.spec.ts` — `elementsFromPoint` sous chaque `position:fixed` ne rend aucun nœud interactif ; `scrollHeight − clientHeight` laisse la bande libre en fin de course. **Mesuré ce jour** : les trois directions couvrent du TEXTE (« Communautés » sur `tableau` chez app-moderne et sobriété aux deux largeurs ; « Merci, je t'envoie le fichier » sur `chats` chez clarté rurale aux deux largeurs). Le texte couvert en cours de défilement est le prix assumé de l'absence de barre d'onglets ; **un contrôle couvert ne l'est pas**.
...
9. **Aucun débordement horizontal, à 360 ET à 390 px.** Tout enfant de grille ou de flex qui porte du texte pose `min-width:0`. — *Témoin* : `v3-cibles.spec.ts` : `documentElement.scrollWidth ≤ clientWidth` aux deux largeurs. **Mesuré ce jour** : app-moderne 360 ✓ (son propre gate avait trouvé et corrigé un débordement de 29 px), sobriété ✓, **clarté rurale `tableau` = 390 px de contenu dans 360** — 30 px hors écran, sur `a.carte` de « Reprendre », et ses 40 témoins verts ne le voyaient pas. Un jeu de témoins qui ignore la largeur ignore le téléphone.
10. **Quatre plans, un rôle chacun — c'est ce qui fait « application » et ça coûte zéro octet.** `--color-bg-sunken` = ce qui s'ENFONCE (champ, recherche, composeur) · `--color-bg` = la page · `--color-surface` = ce qui se POSE (carte, puce, bandeau, ligne mise en avant) · `--color-surface-raised` = ce qui FLOTTE (feuille modale, rond secondaire). Aucune cinquième profondeur. *Amendement accepté* : la règle du tour 1 comptait deux sélecteurs `surface-raised` ; un compte ne dit rien de la lisibilité, une échelle de luminance si. — *Témoin* : `plansDesordonnes` de `check-jetons.mjs` (l'ORDRE des quatre luminances, dans les deux schémas) + un rôle unique par plan.
11. **Cartes là où la planche en dessine.** `home` : recherche sur le plan creux, « Reprendre » en cartes à avatar avec **« Tout voir »** en tête de section, « Mes liens » en carte à tuile puis `.action.contour`, « Communautés » en état vide. `chats` : deux puces côte à côte, puce de Prisme `AUTO · <langue>`, une carte « mise en avant » au rayon `xl` puis lignes plates de `--row-height` séparées d'un filet unique. `join` : **pseudo et Langue CÔTE À CÔTE** (`cible/join.png`), puis la note « Le lien est gardé de côté… ». — *Témoin* : `compare-rendu.js` : disposition ≤ 8 %, IoU ≥ 0,92 sur `home`, `chats`, `join`.
12. **Filet ≠ contour, et un contour qui porte SEUL son sens tient 3:1.** Carte : `--stroke-hair` `--color-border-strong`. Contrôle et carte cliquable : `--stroke-strong` `--color-border-interactive` (3,34 mesuré au pire). **État vide et `trou` : pointillé `--stroke-strong` `--color-border-interactive`**, jamais `--color-border-strong` (**1,79 au pire, mesuré** — un pointillé invisible autour d'un état qui n'existe QUE par son pointillé ; WCAG 1.4.11 lui demande 3:1). Le défaut est dans le dépôt aujourd'hui (`app/connecte/feuille.ts`, `.carte-vide`). **`--color-border` banni** (1,28:1). Listes : un filet entre deux lignes, aucune bordure par ligne. — *Témoin* : `grep -c 'var(--color-border)'` = 0 ; paires dans `check-jetons.mjs`.

#### D. Accent, encre, contraste, focus, thème

13. **Un accent, six emplois.** `--color-primary` ne peint que : le cliquable (action primaire, lien, puce active, chevron de retour), la pastille `.langue`, le compte de non-lus, la tuile de marque, le rond d'envoi et de rappel, et — vitrine seule — **la fin de l'accroche** (`h1 em`). Jamais un titre de section, un filet, un fond de carte. Les états prennent `--color-success|warning|danger` ; les quatre `--color-avatar-*` restent (elles disent QUI). — *Témoin* : `charte.test.ts` : chaque `var(--color-primary` de la feuille tombe dans un sélecteur de la liste nommée.
14. **L'encre du CONTENU est `--color-text`. Le gris est réservé à ce qu'on peut ne pas lire.** *(Greffe « clarté rurale », la meilleure idée du tour, et elle est mesurée.)* Prennent `--color-text` : l'aperçu d'une ligne de `/chats` et de `home`, le texte d'un message, la phrase d'un état, le corps d'une carte, le paragraphe du héros, l'étiquette d'un champ. Prennent `--color-text-muted` : l'heure, le nombre de participants, le qualifiant de section, la sous-ligne d'une carte. **`--color-text-subtle` ne se pose sur AUCUN voile.** — *Mesuré ce jour* (`contraste.mjs`, rc=0, 114 paires) : `--color-text` min **12,78** (AAA sur les huit plans des deux schémas) ; `--color-text-muted` min **5,11** ; `--color-text-subtle` min **4,71** (4,58 sur voile selon `temoins.mjs`). Au soleil, un aperçu à 5,11 n'est pas « discret », il est **absent** — et c'est précisément le texte pour lequel on ouvre l'écran. — *Témoin* : `charte.test.ts` : `--color-text-muted|subtle` interdits dans les sélecteurs `.apercu`, `.ligne p`, `.message p`, `.bandeau p`, `.carte-vide p`, `.heros p`, `label` ; `check-jetons.mjs` refuse `text-subtle` sur tout `--color-tint-*`.
15. **Contraste mesuré, deux schémas, voiles compris.** Texte ≥ 4,5:1, signal ≥ 3:1, sur les quatre plans ET les quatre voiles. Interdits parce que MESURÉS : `--color-danger-soft` en texte sur un plan clair (3,61) ; `--color-primary-soft` sous `--color-on-primary` en clair (4,47). — *Témoin* : `check-jetons.mjs`, rouge sur toute paire sous le seuil ; sonde qui rougit.
16. **Focus double, jamais supprimé.** `:focus-visible{outline:var(--stroke-focus) solid var(--color-focus);outline-offset:var(--stroke-strong);box-shadow:0 0 0 var(--stroke-strong) var(--color-focus-contra)}` — aucune couleur seule ne tient sur les huit fonds (2,58 au pire), le COUPLE tient partout. **Aucun `outline:none` sans remplaçant** — sobriété en posait un sur le champ de recherche, la cible la plus utilisée de son tableau de bord. — *Témoin* : paire focus dans `check-jetons.mjs` ; `grep -c 'outline:none'` = 0.
17. **Thème par la CLASSE, jamais par une requête de média** ; chaque écran capturé dans les quatre colonnes du § 9.6, clair ET sombre relus à chaque livraison. — *Témoin* : `check-jetons.mjs` ; `compare-rendu.js`.

#### E. États — une phrase, un glyphe, un bouton s'il y a quelque chose à faire

18. **Bandeau** `.bandeau` : fond `--color-tint-<état>`, **filet gauche `--space-1`** de la teinte d'état, glyphe 24 px + titre semi-gras + phrase en `--color-text`. `.attention` = hors-ligne, session expirée (`ph-warning-circle`, `role="status"`, bouton « Réessayer » / « Reprendre ma place ») ; `.refus` = 403 / 409 `LINK_EXHAUSTED` / 410 (`ph-x-circle`, `role="alert"`) ; `.bien` = droits obtenus (`ph-check-circle`). **Hors-ligne n'est JAMAIS rouge et n'est jamais sans bouton** — sobriété le peignait en `danger`, muet. — *Témoin* : tout `.bandeau.attention` contient un `a.action|button.action` ; `grep` : aucun `.bandeau.attention` sous `--color-tint-danger`.
19. **Vide** `.carte-vide` : pointillé `--stroke-strong` `--color-border-interactive` (règle 12), glyphe 40 px `--color-text-muted`, titre, phrase ≤ `--measure` en `--color-text`, action primaire. — *Témoin* : `compare-rendu.js` sur `home › Communautés`.
20. **Chargement** : jamais un spinner sur un cache non vide ; sur cache vide, squelette **STATIQUE** (blocs `--color-surface` à `--row-height`, aucune animation). — *Témoin* : critère de fin `home` / `chats` (0 spinner, 0 squelette avec cache pré-rempli).
21. **Refus dans la modale** (`/chat/:lien`) : c'est le **CODE** qui tranche, jamais le statut. Refus de SAISIE (409 `USERNAME_TAKEN_IN_CONVERSATION`, 400) ⇒ `.refus` sur SON champ, `aria-invalid` + `aria-describedby`, **`suggestedNickname` PRÉ-REMPLI dans `value`**, aide en `--color-danger`, formulaire GARDÉ. Refus DU LIEN (403, 409 `LINK_EXHAUSTED`, 410, 429) ⇒ `.bandeau.refus` DANS la modale, formulaire retiré, « Se connecter » / « Créer un compte » conservés. — *Témoin* : `v3-join.spec.ts`, un test par refus, chacun produit par l'ÉTAT du lien dans la passerelle de bouchon. **Seule app-moderne l'a peint** ; sobriété affichait la suggestion en prose et laissait le champ inchangé — rouge.
22. **Fil** : 410 ou droit retiré ⇒ `.composeur.ferme` (`ph-lock` + raison, aucun champ) ; 401 ⇒ `.bandeau.attention` à bouton, lecture conservée ; hors-ligne ⇒ la ligne GARDE sa place, sa méta porte `ph-clock` + « En attente du réseau » ; `hasGap` ⇒ `.trou` pointillé avec un `<a href>` « Charger les messages manquants ». — *Témoin* : `v3-lifecycle.spec.ts` cas C→H.

#### F. Prisme

23. **La pastille `.langue`** : `ph-translate` 16 px + code de la langue d'**ORIGINE** en capitales, `--text-xs`, encre `--color-primary`, contour `--stroke-hair` `--color-border-interactive`, `--radius-xs` ; **elle PRÉCÈDE l'aperçu** dans une ligne de liste et la méta dans un message ; rendue seulement quand `resolvePrismTranslation()` rend non-null ; **jamais un drapeau** (écart assumé avec `cible/thread.png`). Tout nœud rendu dans une langue ≠ `<html lang>` porte `lang="xx"` ; l'original est un `<details>` « Voir l'original » (`ph-text-aa`) dont le `<p>` porte `lang`. — *Témoin* : test unitaire de `fil-vue.ts` (pastille absente si `langueServie === null`, `lang` sur l'original) ; **témoin de rang écrit sur un rang ≠ 1** (leçon 261) ; 0 `<img>` de drapeau. Sobriété servait « Gracias, te mando el archivo » à un lecteur français, sans pastille : le Prisme n'y est pas appliqué, seulement annoncé.

#### G. Glyphes (sprite `packages/icons`, jamais une fonte)

24. **Ponctuation, trois tailles** : `--glyph` 24 px devant un mot, `--glyph-inline` 16 px en méta, `--glyph-large` 40 px en état vide ; `fill:currentColor`, `aria-hidden`. Emplois : marque `ph-chat-circle` · retour `ph-caret-left` · recherche `ph-magnifying-glass` · lien `ph-link-simple` · nouveau `ph-plus` · compte `ph-user-circle` · menu flottant `ph-squares-four` · communauté `ph-users-three` · Prisme `ph-translate` · replier `ph-caret-down` · droits `ph-key`, accordé `ph-check-circle`, refusé `ph-x-circle` · anonyme `ph-ghost` · avertissement et hors-ligne `ph-warning-circle` · joindre `ph-paperclip` **sur un rond bordé 44 px** (`cible/thread.png` ; clarté rurale le laissait nu) · envoyer `ph-arrow-up` sur un rond primaire 52 px · lu `ph-checks` · attente `ph-clock` · appel manqué `ph-phone-x`, rappeler `ph-phone` · original `ph-text-aa` · fermé `ph-lock` · médias `ph-image`. Sous-sprite critique ≤ 8, le reste externe sur les écrans denses. — *Témoin* : **N `<use>` ⇒ N boîtes non nulles** (`sprite.test.ts`, gate CI « `ph-*` sans `<symbol>` ») — c'est l'assertion anti-panne du § 8.5, mesurée verte par app-moderne (6/6, 8/8, 11/11, 9/9, 9/9, 23/23).

#### H. Mouvement, modale, fil, temps réel

25. **Le mouvement ne déplace rien.** Transitions ≤ 150 ms sur `background-color|border-color|color` seulement ; **UNE** `@keyframes` (`pulse` de présence en ligne, `opacity` seule, ≥ 2 s) ; `prefers-reduced-motion: reduce` coupe tout. — *Témoin* : 0 transition sur `transform|height|opacity|all`, `@keyframes` ≤ 1 **mesurée sur la feuille COMPACTÉE** (sinon le commentaire qui la nomme se compte), bloc reduced-motion présent.
26. **Modale de `/chat/:lien` — et l'action primaire reste atteignable.** `<dialog open>` rendu SERVEUR (aucun `showModal()`, aucun script) ; feuille basse sur mobile (`inset:auto 0 0`, `--radius-2xl` en haut, poignée 40 × 4, `--color-surface-raised`, `max-height:92dvh`, **`overflow:auto`**), centrée ≥ 600 px ; voile `.voile` fixe `--color-overlay` (+ `::backdrop`) ; le cadre du fil derrière est **`inert`** + `filter:blur(var(--frame-blur))`, messages VIDES, composeur inactif. Ordre : « <hôte> vous invite » → titre → citation → `<details>` des droits (`ph-key`, « 4 droits · à lire avant d'entrer ») → pseudo **et** langue CÔTE À CÔTE → `.action.primaire` « Continuer en anonyme » → « ou garder votre identité » → « Se connecter » (`.contour`) → « Créer un compte » (`.discrete`) → la note « Le lien est gardé de côté… », `?next=` sur les deux sorties. **Clause neuve** : sur une fenêtre de 844 px de haut, l'action primaire est dans le premier écran du panneau — sobriété la poussait hors champ, panneau tronqué, sans qu'aucun témoin ne le voie. — *Témoin* : `v3-join.spec.ts` en `javaScriptEnabled:false` ; assertion `[inert]` sur le cadre ; `boundingBox().y + height ≤ 844` sur `.action.primaire` ; `compare-rendu.js`.
27. **Le fil est PLAT — pas de bulle** (`cible/thread.png`). Ligne = avatar 40 px + nom (+ « anonyme » avec `ph-ghost`) + texte 17 px `--color-text` + méta (`.langue`, `<time>`, `ph-checks`) ; mes messages = même ligne, nom « Vous » ; séparateur de jour en capitales espacées ; ligne système centrée ; appel manqué = carte `--color-tint-danger` **à filet `--color-danger`** (la cible le dessine) avec `ph-phone-x` et un rond `ph-phone` primaire ; en-tête collant OPAQUE (retour, titre, « N en ligne · entré comme <pseudo> » pour l'invité), puis les puces `AUTO · <langue>` et `Médias` ; composeur collant OPAQUE. `/chat/:lien` (invité) et `/chats/:cle` (membre) rendent le MÊME module `app/connecte/fil-vue.ts`. — *Témoin* : `compare-rendu.js` sur `thread` ; test unitaire : un seul module de vue importé par les deux routes.
28. **Le bandeau des droits (vue `rights`) est le PREMIER bloc sous l'en-tête, au-dessus des puces** (`cible/rights.png` — les trois directions l'avaient mis en dessous), `<details open>` `.bandeau.bien`, filet gauche `--color-success`, une ligne par droit (`ph-check-circle` / `ph-x-circle`), refermable. **Il ne recouvre ni ne tronque un message** : au-delà de `34dvh` il se rend REPLIÉ sur son seul `<summary>`, jamais coupé sans contrôle. — *Témoin* : `compare-rendu.js` sur `rights` ; assertion : aucun `.bandeau.bien` avec `scrollHeight > clientHeight` en état ouvert.
29. **Temps réel = amélioration progressive.** Tout marche par `<form method="post">` et rechargement ; `participate.ts` arrive par `await import()` APRÈS le premier pixel, sur `fil` et `chats` seulement, et ne peint qu'avec les classes de cette charte (`.ligne.frappe` : « <nom> écrit… » italique `--color-primary`, sous le dernier message et au-dessus du composeur ; réordonnancement ; compte de non-lus). — *Témoin* : lint « import statique de `participate` = erreur » ; assertion « `socket.io-client` absent du chunk avant le tap ».

#### I. Interdits — chacun à témoin `grep` dans `__tests__/charte.test.ts`, chacun prouvé non vide par une sonde

30. `#hex`, `rgb()`, `hsl()`, couleur nommée · `px` de design · `font-family` autre que `var(--font-native)` · `@font-face`, `url(`, `@import` · **`box-shadow` hors le contre-anneau de focus** · **`backdrop-filter`** · `filter:blur` ailleurs que sur le cadre inerte de `/chat/:lien` · `opacity` comme état · `--text-2xs` · `--color-border` · `outline:none` · `@media (prefers-color-scheme)` hors moteur · `<div onclick>`, `href="#"`, `type="button"` sans gestionnaire · spinner sur cache non vide · **drapeau de langue** · seconde table (`--x:` hors `packages/design-tokens`) · fonte d'icônes, `@phosphor-icons/web`, `lucide-react` · React hydraté sur ces cinq écrans · **barre d'onglets** · **bulle de message** · un contrôle inerte.
31. **UN dégradé, et un seul : `.heros` de la vitrine, entre DEUX jetons** (`--color-tint-primary` → `--color-surface`). *Amendement accepté* contre la règle 11 du tour 1 : la directive du porteur autorise nommément les « dégradés discrets », le coût réseau est nul, et c'est la seule chose qu'une capture d'application a et qu'une page de texte n'a pas. Bornes : un seul sélecteur, deux jetons voisins, et l'encre posée dessus **mesurée aux DEUX extrémités**. — *Témoin* : `degrades` = 1 **et** `selecteurDuDegrade` = `.heros` (le compte seul ne suffit pas) ; deux paires de contraste dans `check-jetons.mjs`.

#### Portage

`app/socle.ts` (partie feuille) + `app/enveloppe/feuille.ts` + `app/connecte/feuille.ts` + `app/vitrine/feuille.ts` + `FEUILLE_DU_FIL` → **une feuille de chrome** (base : `app-moderne/chrome.css`) + `fil.css` + `join.css`, **mesurées par route** (règle 3). Témoins : `app-moderne/mesure.cjs` → `e2e/visual/v3-cibles.spec.ts` (cibles, débordement, occlusion, rayons, `<use>`, `hrefsMorts`) ; `app-moderne/contraste.mjs` + `clarte-rurale/temoins.mjs` → paires de `check-jetons.mjs` et `__tests__/charte.test.ts` (les deux jeux fusionnent : 114 paires + 40 témoins de feuille, sondes comprises) ; `clarte-rurale/chrome.css` fournit les sélecteurs de la règle 14. La passerelle de bouchon (`e2e/visual/lib/serveurs.ts`) se complète pour chaque endpoint que ces écrans consomment — dont l'ÉTAT du lien, qui produit les refus de la règle 21.

#### Ce que ce jugement ne prouve pas

- **Aucune des trois thèses n'est vérifiée sur le terrain.** « Lisible au soleil sur un téléphone d'entrée de gamme » reste un raisonnement adossé à des contrastes, des corps et des cibles mesurés — personne n'a tenu l'appareil dehors. La mesure qui trancherait est un test terrain, pas un gate.
- **Le schéma clair reste DÉRIVÉ** (`light.css` le dit : la planche n'a pas de vue claire). Les captures claires sont une proposition dans la proposition.
- **Cinq écrans, pas trente et un.** Les refus 403/410/429, la session expirée, le composeur fermé, l'état MEMBRE de `/chat/:lien` (302 vers `/chats/:cle`) et toutes les surfaces sociales restent à peindre. La liste des refus se relit dans l'ÉMETTEUR (`linkAdmission.ts`), jamais dans un document.
- **Rien n'est branché.** Aucun fichier du dépôt n'a été touché par ce jugement ; les feuilles et les documents vivent dans `.cache/web-v3-workflow/charte/`.
- **`34dvh` (règle 28) est une valeur DÉCIDÉE, pas mesurée** : elle vient d'une règle produit — « un bandeau ne masque jamais la chose qu'on vient lire ». Elle entre dans la table ou elle reste tolérée, à trancher au portage.
- **Le plafond de 6 Ko gzip par route (règle 3) est DÉCIDÉ ce jour**, pas hérité : il resserre les 20 Ko du § 8.5, que la mesure rend inopérants pour arbitrer (la route la plus lourde du lot en occupe le quart).
...
| feuille de chrome | **≤ 4 Ko gzip** ; table inlinée (jetons de la règle 1 compris) **≤ 1,5 Ko gzip** | plafonds DÉCIDÉS (mesurés sur le DÉPÔT le 2026-09-02 : **1 067 o** et **1 325 o** ; les 3 041 / 1 378 o cités par la charte sont ceux de la PROPOSITION `app-moderne/chrome.css`, pas ceux des feuilles servies) | charte règle 3 ; témoin `__tests__/charte.test.ts` (`gzipSync … level 9`) |
| **CSS SERVI par un document** (chrome + feuilles de son écran, inlinés) | **≤ 6 Ko gzip** | plafond DÉCIDÉ par la charte du 2026-09-02 (règle 3), **plus strict** que les 20 Ko du § 8.5 — aucun seuil n'est baissé, un second sujet est gaté | charte règle 3 ; porteur `__tests__/charte.test.ts` (compositions `vitrine · connecté · fil · choix`). **Mesuré ce jour sur les feuilles du dépôt** : 1 412 · 1 708 · 3 641 · **4 234 o** — la route de jonction `/chat/:lien` est bien la plus lourde, à 69 % du plafond |
| CSS TRANSFÉRÉ par route (ressources `Stylesheet`) | ≤ 20 Ko gzip | GATE | § 8.5, inchangé — **et VACUEUX sur les cinq écrans du focus** : `scripts/mesure-reseau.mjs:209-211` ne somme que les ressources de type `Stylesheet`, or ces documents sont autoporteurs (0 feuille externe, charte règle 3) ⇒ il mesure 0 Ko. C'est la ligne au-dessus qui arbitre ici ; les deux ne sont pas des jumelles — l'une pèse ce qui VOYAGE, l'autre ce que le document PORTE |
...
**Où vivent ces trois plafonds décidés, et ce qui reste à câbler.** Ils sont dans les DONNÉES — `apps/web-v3/budgets.json` → `feuilles` —, jamais en littéraux dans un témoin : `__tests__/charte.test.ts` lit déjà ce fichier (il en tire `reseau.transverses.css_ko`) et écrit encore `4 * 1024` / `1536` à la main, et sa composition la plus lourde est opposée aux **20** Ko du § 8.5 (`expect(PLAFOND_PAR_ROUTE_KO).toBe(20)`) — c'est-à-dire au plafond que la charte juge trop lâche pour arbitrer. Le lot qui reprend ces écrans fait lire les trois valeurs à ce témoin ; tant que ce n'est pas fait, le plafond de 6 Ko est ÉCRIT et mesuré (4 234 o au pire ce jour) mais n'est opposable par aucun gate — dit ici plutôt que découvert au premier dépassement.

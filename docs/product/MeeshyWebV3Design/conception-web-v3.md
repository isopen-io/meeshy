...
**Fait mesuré à l'origine** : il n'existait **aucun `docker-compose.staging.yml`** dans ce checkout. Le bac était donc **à créer en L-0.5**, avec deux exigences : un host complet en `robots: noindex` pour tester la v3 isolément, et un routeur `frontend-staging` portant **explicitement `priority=1`** pour que la répétition de la bascule par `PathPrefix` soit fidèle à la prod.

**Fait mesuré au 2026-09-02 — le bac EXISTE, et il est en AVANCE sur la prod.** `docker-compose.staging.yml` porte `frontend-staging` (`:318`) avec son `priority=1` explicite (`:368`) et `frontend-v3-staging` (`:386`) en `priority=100` (`:453`), dont la règle réclame déjà `PathPrefix('/__v3/_next')`, `PathPrefix('/__v3/rt/')`, `PathPrefix('/l/')`, `Path('/')`, les cinq pages institutionnelles, `Path('/login')`, `Path('/signup')`, `Path('/chats')`, `PathPrefix('/chats/')` et `PathPrefix('/chat/')` (`:448`) — c'est-à-dire les étapes 1 à 5 bis du § 4.9, barre finale comprise. La prod n'en a que deux (`docker-compose.prod.yml:492` : `/__v3/_next` et `/__v3/rt/`), ce qui est l'ordre voulu : **la répétition précède la bascule**. Les deux exigences d'origine sont donc tenues, à une près, qui reste ouverte et n'est pas effacée : le host de répétition est `staging.${DOMAIN}` PARTAGÉ avec le legacy (un routeur de priorité 100 devant un routeur de priorité 1), pas le `v3.staging.meeshy.me` ISOLÉ que ce paragraphe demandait — le bac répète donc fidèlement la COHABITATION (ce que le § 4.9 fait basculer), et pas l'isolement. Et le `robots: noindex` du bac n'est PAS tenu au niveau du HOST : `grep -in 'noindex\|robots' docker-compose.staging.yml` ne rend rien, et la v3 ne pose `noindex, nofollow` que par ÉCRAN, sur ce qui appartient à un lecteur (`app/enveloppe/vue.ts:76`, `app/connecte/porte.ts:59`, `app/connecte/fil-porte.ts:26`, `app/authentification/vue.ts:103`, `app/(public)/chat/[lien]/choix-vue.ts:417`, `app/provenance.ts:148`) — la vitrine, elle, est servie `index, follow` (§ 12.1) et le serait donc aussi sur `staging.${DOMAIN}`. C'est un défaut de BAC, pas de v3 : il se corrige par un middleware Traefik `headers.customresponseheaders.X-Robots-Tag=noindex` sur les deux routeurs de staging, dans le commit qui joue le Gate E — nommé ici pour qu'il ne se découvre pas dans un index public.
...
| **Parcourir les médias d'une conversation** (`/chats/:cle/medias`) | `GET /media?conversationId=` | `GET /conversations/:id/attachments` (`routes/attachments/metadata.ts:247`, `authOptional`, `?type=image\|document\|audio\|video\|text`, `limit` ≤ 100) — sert SEPT clés (`messageAttachmentMinimalSchema` : `id`, `fileName`, `mimeType`, `fileSize`, `fileUrl`, `thumbnailUrl`, `duration`), jeu GELÉ par `__tests__/unit/routes/attachments/conversation-attachments-served-keys.test.ts` | ⚠️ **la route dédiée ne peut pas servir l'écran** | **Régime 3 sur la route dédiée, et la galerie est une PROJECTION du fil** (`lib/api/medias.ts`, `galerie()`), parce que ces sept clés ne portent **ni les trois drapeaux de protection** (`isViewOnce` / `isBlurred` / `expiresAt` — une grille bâtie dessus rendrait l'URL entière d'une photo à VUE UNIQUE, cycle 125), **ni la transcription** (le doc-comment de la route dit « CHARGÉES, jamais servies »), **ni les dimensions**. Le prix est la PROFONDEUR : la galerie couvre la fenêtre de `GET /conversations/:id/messages` (`limit` ≤ 50) et remonte par le MÊME curseur `?avant=`, ce que l'écran DIT (« Médias plus anciens »). Élargir le minimal servi est une décision de la passerelle (issue compagnon), jamais un contournement |
...
- **« Aperçu de liste » se compte en SURFACES, pas en modules.** Le dépouillement de `Conversation` est un site unique (`apercuServi`, `apps/web-v3/lib/api/compte.ts`) et le RENDU en est un autre (`apercuAuPrisme`, `app/connecte/vue.ts`) ; trois surfaces les servent : la ligne de `/chats`, **la carte du tableau de bord** (`cible/home.png` — restée muette jusqu'au 2026-09-03 : la donnée était projetée, le résolveur écrit, `/chats` l'appelait, et cette carte-là ne l'affichait pas) et la ligne REPEINTE en direct (`lib/realtime/liste-peinture.ts`). Une descente juste dont la valeur n'atteint aucun lecteur n'a corrigé personne (cycle 122) : devant une famille de résolveurs, la question n'est pas « le module descend-il le prisme ? » mais **« quels ÉCRANS affichent cette donnée, et lequel a été oublié ? »**.
...
| — | `media` | Médias partagés | **P0** | `/chats/:cle/medias` | `/chats` | `thread` | — | Tuiles **cliquables** (inertes aujourd'hui) ; téléchargement à la demande **avec poids affiché** ; audio + transcription au Prisme |
...
**44 lignes** à la rédaction : les 37 vues de `vues.json` + 7 surfaces hors planche (`sheet:lang`, `sheet:link`, `sheet:conv`, `sheet:attach`, `sheet:member`, `reelShared`, `moods`). **Restent au legacy, sans action** : les 23 routes `admin/*` et les 12 routes d'authentification listées au § 4.4.

> **Le compte a bougé, et c'est le GATE qui fait foi, jamais cette phrase** (2026-09-03) : **47 lignes** — les **39** vues de `vues.json` (la `vitrine` et `profilMembre` ont rejoint la planche) + **8** hors planche (les 7 ci-dessus, plus `banner:inApp`). Ce paragraphe n'est pas mis à jour ligne à ligne à chaque écran : `node docs/product/MeeshyWebV3Design/ordre-des-ecrans.js` recompte et le dit sur sa sortie standard, et `ordre.md` — qu'il régénère — porte la liste vivante.
...
> qui attend un écran de priorité inférieure. La matrice portait **44 lignes** à la rédaction
> (**47** le 2026-09-03) : les vues de `vues.json` plus les surfaces exigées par la mission que la planche ne dessine pas,
...
| 13 | **`/feed` n'écoute AUCUN évènement entrant — décision DÉLIBÉRÉE, jusqu'ici écrite seulement dans un commentaire de build script.** `AuthHandler` rejoint `ROOMS.feed(user.id)` pour TOUT socket authentifié, et `SocialEventsHandler` y émet `post:liked`/`post:created`/`post:updated`/`post:deleted` depuis vingt-et-un sites (`services/gateway/src/socketio/handlers/SocialEventsHandler.ts`) — mais `lib/realtime/feed.ts` n'importe PAS `socket.io-client` : « aimer et reposter sont des allers simples… le fil social n'a besoin ni d'eux ni d'un socket… feed ne l'importe pas du tout » (`scripts/build-participate.mjs`). Un like d'un tiers ne bouge donc pas le compteur affiché, une publication neuve n'apparaît jamais sans rechargement, et un second onglet ne se rattrape pas. La décision est **volontaire et mesurée** — `socket.io-client` pèse **26 549 o gzip** dans `participate.js` (`budgets-mesures.json` › `participate`), soit plus de **3,5× le poids actuel de `feed.js` (7 584 o)** — mais elle vivait UNIQUEMENT dans un commentaire de script, jamais dans ce document ni dans une issue : personne n'a eu à l'approuver ni à la contredire. | `/feed` reste un instantané tant que la question n'est pas tranchée ; sans cette ligne, la prochaine session peut soit rouvrir le socket sans mesurer le coût, soit ignorer le symptôme sans savoir qu'il est déjà connu. | `/feed` reste un instantané SANS socket — chemin SANS JavaScript inchangé, aimer/reposter restent des allers simples. Issue `décision-produit` à ouvrir : soit un socket dédié et léger (souscrire SEULEMENT à `post:liked`/`post:updated` pour les posts déjà rendus, sans fabriquer de nœud — donc sans payer la fabrication de nouvelles cartes), soit un `GET /sync`-like au retour de focus (le patron déjà retenu pour `/chats`), soit l'instantané assumé jusqu'à ce que l'usage mesuré (dimension 10) en montre le besoin. |
| 14 | **Quatre des dix écrans de réglages n'ont AUCUNE route dans la passerelle.** Relevé du 2026-09-03 sur les routes assemblées : `detail-privacy`, `detail-media`, `detail-message` et `detail-notification`/`notifPrefs` n'ont ni lecture ni écriture — ni `GET`, ni `PATCH`, ni `PUT`. Les six autres sont servables et livrées (#5065). La question 10 ci-dessus ne portait que sur le CONTENU de deux d'entre elles ; celle-ci porte sur leur EXISTENCE côté serveur, ce qui est un cran plus haut : aucun contenu ne se décide tant qu'aucune route ne peut le porter. | Le critère de fin de `settings` — « les sept rangées mènent aux sept fiches » — est intenable : quatre destinations n'existent pas. La v3 ne les dessine donc PAS (une rangée qui n'ouvre rien est le contrôle sans effet de la charte règle 7, et un lien mort se pré-charge, s'indexe et rend 404). | Trois rangées servies, motivées à l'écran par ce qu'elles PROMETTENT et non par ce qui manque. Issue `décision-produit` #5066 : soit un lot de PASSERELLE ouvre les quatre familles de réglages et la v3 suit, soit la matrice est amendée et les quatre lignes en sortent, avec la raison écrite. |
...
**Cette charte REMPLACE celle du tour 2, et la remplace ENTIÈREMENT — numérotation comprise.** Elle est
le jugement du **2026-09-02 (tour 3)**, rendu sur les mêmes trois directions, avec **un seul instrument
passé sur les trois** (`elementsFromPoint`, cibles, débordement, lien d'évitement, `gzip -9` sur la forme
compactée du dépôt, encre calculée au navigateur à 390 × 844) : aucun chiffre n'y est repris sur parole,
et les deux harnais des directions ont été EXÉCUTÉS. Les trois propositions et ces instruments vivent dans
`.cache/web-v3-workflow/charte/` ; la charte est **recopiée ici** parce qu'un cache n'est pas une source de
vérité et qu'aucune règle opposable ne peut dépendre d'un fichier qu'aucune passe ne régénère.

**Rien n'est effacé : la charte du tour 2 se relit dans l'historique** (`5bff749260`, ce même § 12.5), et
la table ci-dessous dit où chacune de ses règles a repris place. Elle existe parce qu'une renumérotation
silencieuse fait mentir toutes les citations du dépôt : `matrice.json`, le § 12.9 et les critères de fin
citent des NUMÉROS, et un numéro qui glisse sans table pointe une autre règle sans qu'aucun témoin ne
rougisse. Les citations du dépôt ont été portées dans le même mouvement.

| tour 2 | son intitulé | tour 3 |
|---|---|---|
| 1 | Une table, zéro valeur ailleurs | **1** |
| 2 | Corps 17 px, pile système | **2** (les métiers du jeton) et **3** (la police RENDUE, mesurée) |
| 3 | Poids : une feuille par route | **5** (par route) et **4** (le poids du DOCUMENT — sujet neuf) |
| 4 | Hauteurs, pleine largeur | **7** |
| 5 | Cinq rayons | **9** |
| 6 | HTML réel, contrôle à effet, lien d'évitement | **13** (HTML), **11** (effet), **17** (évitement visible) |
| 7 | Actions flottantes, jamais SUR un contrôle | **8** (le rail-RANGÉE devient le mécanisme préféré) |
| 8 | Échelle fermée | **10** |
| 9 | Aucun débordement horizontal | **10** (fusionnée : même témoin, même mesure) |
| 10 | Quatre plans | **14** |
| 11 | Cartes là où la planche en dessine | **15** |
| 12 | Filet ≠ contour | **16** |
| 13 | Un accent, six emplois | **20** |
| 14 | L'encre du CONTENU | **18** |
| 15 | Contraste mesuré | **19** |
| 16 | Focus double | **17** |
| 17 | Thème par la CLASSE | **21** |
| 18 | Bandeau | **23** |
| 19 | Vide | **24** |
| 20 | Chargement | **22** |
| 21 | Refus dans la modale | **25** |
| 22 | Fil — 410, 401, hors-ligne, `hasGap` | **26** |
| 23 | La pastille `.langue` | **30** |
| 24 | Ponctuation, trois tailles | **31** |
| 25 | Le mouvement ne déplace rien | **32** |
| 26 | Modale de `/chat/:lien` | **27** |
| 27 | Le fil est PLAT | **28** |
| 28 | Le bandeau des droits | **29** |
| 29 | Temps réel = amélioration progressive | **33** |
| 30 | Interdits | **34** |
| 31 | UN dégradé | **35** |
| — | *(neuve)* Zéro requête avant le premier pixel | **6** |
| — | *(neuve)* Trois lignes actionnables au premier écran d'une liste | **12** |

**Ce que ce tour a résolu et que le tour 2 laissait ouvert** : les deux jetons de géométrie
(`--sheet-max`, `--bandeau-max`) entrent avec leur repli `var(--x, …)` — le tour 2 les laissait « à
trancher au portage » de sa propre main (règle 1) ; le plafond de poids porte désormais sur le DOCUMENT
servi et non sur la feuille seule (règle 4), parce que le lecteur télécharge le tout ; et la règle
typographique cesse de COMPTER des rôles pour en NOMMER les métiers (règle 2) — mesuré, les deux
directions retenues en emploient neuf, et un compte que les deux gagnants enfreignent n'est pas une
règle.

**Ce que ce tour fait ÉVOLUER dans la planche** — parce qu'une règle opposable et une capture de
référence ne peuvent pas se contredire : la pastille de Prisme remplace les drapeaux dans `thread`,
`rights` et `rich` ; l'aperçu de `home` est SERVI dans la langue de la lectrice, avec sa pastille `ES`
(la planche se contredisait elle-même — `/chats` le servait déjà traduit) ; une ligne sans pastille
existe désormais dans chaque jeu d'essai (le témoin de rang de la règle 30 ne peut s'écrire que là) ;
le chevron de retour disparaît de `/` (règle 11) ; l'atténuation `opacity:.45` du mode « focal » —
un état porté par l'opacité, gouverné par un contrôle jamais rendu — est retirée (règles 18 et 34) ;
et le bandeau des droits perd ses sous-lignes pour tenir sous `--bandeau-max` (règle 29). Détail et
raison de chaque écart : § 12.9.

**Jugement du 2026-09-02 (tour 3), 7 critères × 3 directions.** Captures claires ET sombres regardées une par une, cibles comprises (`docs/product/MeeshyWebV3Design/cible/{home,chats,thread,join,rights}.png`), feuilles lues, **et tous les chiffres re-mesurés par le juge avec UN SEUL instrument passé sur les TROIS directions** (`elementsFromPoint`, cibles, débordement, lien d'évitement, poids `gzip -9` sur la forme compactée du dépôt, encre calculée au navigateur à 390 × 844). Aucun chiffre n'est repris sur parole. Les deux harnais des directions ont en outre été EXÉCUTÉS ici : `app-moderne/mesure.cjs` (28 rendus, rc 0) + `contraste.mjs` (114 paires, rc 0) ; `clarte-rurale/temoins.mjs` (34/34, rc 0).

| direction | 1 soleil | 2 directive | 3 légèreté | 4 planche | 5 a11y | 6 maintenance | 7 cohérence | **total** |
|---|---|---|---|---|---|---|---|---|
| **app-moderne** | **9** | **9** | **9** | **9** | **9** | **9** | 8 | **62/70** |
| clarté rurale | **9** | 7 | 7 | 7 | 8 | **9** | 8 | 55/70 |
| sobriété premium | 3 | 3 | 5 | 3 | 2 | 2 | 3 | 21/70 |

**Direction retenue : « app-moderne ».** C'est la seule des trois qui, ce tour, a EXÉCUTÉ les témoins que la charte du § 12.5 nommait sans que personne ne les ait écrits, et qui s'est corrigée sur ce qu'ils ont trouvé chez elle. **Greffée de « clarté rurale »** : le lien d'évitement VISIBLE au clavier (règle 17), le mot du compte de non-lus pour la voix (règle 24), le squelette qui DIT ce qui manque (règle 22), l'aide de refus sur toute la largeur (règle 25), les deux jetons de géométrie (règle 1), et le rail-RANGÉE comme mécanisme préféré (règle 8). **Greffée de « sobriété premium » par son échec** : la règle 3 (une police NOMMÉE et jamais servie est une seconde table doublée d'un mensonge sur le rendu).

**Sept règles naissent d'une mesure du juge, qu'aucune direction ne portait** : le lien qui pointe l'écran courant (11), les trois lignes du premier écran d'une liste (12), le poids du DOCUMENT et non de la feuille (4), la feuille par ROUTE (5), l'action primaire de la modale dans TOUS ses états y compris le refus (27), le rôle typographique comme APPLICATION et non comme compte (2), la police calculée du `<body>` (3).

Elle vaut pour les cinq écrans du focus (vitrine, tableau de bord, `/chats`, `/chat/:lien`, `/chats/:cle`) puis pour tout écran suivant. **Une règle sans témoin n'entre pas ici.**

---

#### A. Fondations

1. **Une table, zéro valeur ailleurs.** Couleur, rayon, police, ESPACE, CIBLE, GÉOMÉTRIE viennent de `packages/design-tokens`. Une feuille n'écrit aucun `px` hors l'idiome `.hors-ecran` (`1px`/`-1px`) et le point de rupture `@media (min-width:600px)`. **Deux jetons de géométrie entrent** — `--sheet-max: 92dvh`, `--bandeau-max: 34dvh` — employés en `var(--sheet-max, 92dvh)` pour que le portage n'attende pas la table ; ils ferment le point que la charte du tour 2 laissait ouvert de sa propre main. Aucun jeton de COULEUR n'est demandé. — *Témoin* : `check-jetons.mjs` étendu (un `px` en `padding|margin|gap|width|height|min-height|border-radius|font-size` sous `apps/web-v3/app/**` rougit), non-vacuité par sonde `.sonde{padding:7px}`. **Mesuré ce jour** : app-moderne **5** littéraux (tous tolérés), clarté rurale **4**, **sobriété premium 191**.
2. **Chaque `--text-*` a UN métier nommé — c'est une APPLICATION, pas un compte.** `4xl` accroche du héros · `3xl` titre d'écran et titre de modale · `2xl` titre de section large de la vitrine · `xl` titre du fil · `lg` marque et titre d'état vide · `md` corps et libellé d'action (**17 px, pile système, interligne 1,6**) · `base` secondaire et sous-ligne · `sm` méta et capitale de section · `xs` pastille de langue, méta de message, séparateur de jour. **`--text-2xs` interdit.** Aucun `@font-face`, aucune requête de police. — *Témoin* : `charte.test.ts` asserte le COUPLE jeton → famille de sélecteurs, et rougit sur un jeton posé hors de son métier. **Pourquoi cette forme** : le tour 2 écrivait « cinq rôles » sans témoin qui compte — mesuré ce jour, app-moderne en emploie **9** et clarté rurale **8** ; un compte que les deux gagnants enfreignent n'est pas une règle, c'est un vœu.
3. **La police RENDUE est la pile système, et on la mesure au navigateur.** `body{font-family:var(--font-native)}` explicitement posé. — *Témoin* : `getComputedStyle(document.body).fontFamily` commence par `-apple-system`. **Mesuré ce jour** : app-moderne et clarté rurale **17 px / 27,2 px / `-apple-system`** ; **sobriété premium 15 px / 21,75 px / `Inter`** — une police qu'elle DÉCLARE dans son propre document (`--font-body`, donc une seconde table) et qu'elle ne sert jamais : sur le téléphone visé, le rendu tombe en silence sur autre chose que celui qui a été dessiné.
4. **Le poids se mesure sur le DOCUMENT, pas sur la feuille.** Le document porte sa table, sa feuille et ses symboles (règle 6) : le lecteur télécharge le TOUT, une fois par écran. Plafond **≤ 9 Ko gzip par document**, ratchet obligatoire pour le franchir. — *Témoin* : `gzip -9c` sur le HTML servi, par écran, dans `__tests__/charte.test.ts`. **Mesuré ce jour** (vitrine · tableau · chats · join · fil) : app-moderne **6 191 · 6 163 · 6 065 · 7 257 · 8 362 = 34 038 o** ; clarté rurale 6 560 · 7 126 · 7 598 · 6 819 · 8 767 = 36 870 ; sobriété 43 846.
5. **La feuille est découpée par ROUTE — parce qu'elle est INLINÉE, elle ne se met jamais en cache d'un écran à l'autre.** Chrome compactée ≤ 4 Ko gzip ; CSS servi par un document ≤ 6 Ko gzip ; table inlinée ≤ 1,5 Ko. — *Témoin* : `gzip -9c` sur la CONCATÉNATION compactée, **par route**. **Mesuré ce jour** (compacteur `compacte()` du dépôt) : app-moderne chrome **2 816** · fil 1 313 · join 807 ⇒ `/` **2 816**, `/chats/:cle` **3 553**, `/chat/:lien` **3 922** ; table **1 325**. Clarté rurale sert **3 487 o** sur TOUTES les routes : son parti d'une feuille unique annonce « le cache la garde d'un écran à l'autre », **ce que sa propre règle « 0 `<link>` » rend faux** — mesuré, il coûte **+2 832 o** sur les cinq documents.
6. **Zéro requête avant le premier pixel.** 0 `<script>`, 0 `<link rel=stylesheet>`, 0 `<img>`, 0 `url(`. — *Témoin* : `mesure-reseau.mjs`. **Mesuré ce jour** : 0 / 0 / 0 sur les trois directions, tous documents.

#### B. Cibles, actions, rayons, espace

7. **Hauteurs, pleine largeur sur mobile.** Primaire `.action.primaire` **56 px** (`--action-height`), fond `--color-primary`, encre `--color-on-primary` · secondaire `.action.contour` **52 px** (`--action-height-secondary`), `--stroke-strong` `--color-border-interactive` · tertiaire (`.action.discrete`, puce, raccourci d'en-tête, retour, lien de pied) **44 px** (`--target-min`), `min-inline-size: var(--target-min)`. Deux puces partagent une ligne ; deux actions pleine largeur s'empilent (`--space-3`). ≥ 600 px : largeur automatique, `min-width: var(--action-width)`. — *Témoin* : `v3-cibles.spec.ts` à **360 ET 390 px**, 0 cible < 44 en hauteur OU largeur (l'idiome `.hors-ecran` 1 × 1 excepté). **Mesuré ce jour** : app-moderne **0**, clarté rurale **0**, **sobriété 5** (champ de recherche du tableau de bord à **24 px**, liens de pied à 19 px).
8. **Aucun élément fixe ne recouvre un élément INTERACTIF — et la GÉOMÉTRIE prime sur la réserve d'espace.** Mécanisme préféré : le rail est une RANGÉE d'une grille de page (`grid-template-rows: auto minmax(0,1fr) auto`), pas un survol — rien ne passe jamais dessous. *(Greffe « clarté rurale ».)* Là où la page doit rester la surface défilante (vitrine, tableau de bord), les ronds restent `fixed`, aux DEUX coins, la bande entre eux libre, avec `padding-block-end: var(--space-9)` sur le conteneur ; **et si la mesure trouve un contrôle couvert, le rail cède la place à deux raccourcis de 44 px dans l'en-tête** — c'est ce que `/chats` fait, écart de disposition assumé avec `cible/chats.png`, dont la raison est un chiffre. — *Témoin* : `elementsFromPoint` sous chaque `position:fixed`, à **trois** positions de défilement, aux deux largeurs, dans les deux schémas, ne rend aucun nœud interactif. **Mesuré ce jour au repos** : 0 occlusion sur les trois ; le texte couvert en cours de défilement reste le prix assumé de l'absence de barre d'onglets.
9. **Cinq rayons, un rôle chacun** : `pill` actions, puces, avatars, compte, pastille de présence, champ du composeur, ronds, raccourcis · `lg` cartes, bandeaux, champs, accordéon, tuile de liste · `xl` héros, carte mise en avant, carte d'état vide · `xs` `.langue` · `2xl` feuille modale (`2xl 2xl 0 0` en feuille basse, plein ≥ 600 px). **`--radius-md` est hors des cinq rôles.** — *Témoin* : le témoin compte les **JETONS**, pas les déclarations. **Mesuré ce jour** : app-moderne et clarté rurale, 6 formes pour 5 jetons ; sobriété emploie `--radius-md`.
10. **Échelle d'espace fermée.** `padding/margin/gap` ∈ `--space-1…9` ; gouttière `--space-5`, titre d'écran `--space-6` sous la marque, section `--space-7`, intérieur de carte `--space-4`, actions empilées `--space-3`, ligne de liste `--row-height` (80 px). **Aucun débordement horizontal à 360 NI à 390 px** : tout enfant de grille ou de flex porteur de texte pose `min-width:0`. — *Témoin* : `documentElement.scrollWidth ≤ clientWidth`. **Mesuré ce jour** : **0 px** sur les trois, aux deux largeurs (clarté rurale a soldé les 30 px du tour 2).

#### C. Contrôles, structure, voix

11. **Un contrôle existe s'il a un EFFET — et un lien qui pointe l'écran COURANT n'en a pas.** Ni `href="#"`, ni `onclick`, ni `type="button"` sans gestionnaire, ni `<div>` cliquable, **ni un chevron de retour dont la destination est la route qu'on regarde**. — *Témoin* : pour chaque document, aucun `<a>` dont le `href` résout la route de ce document. **Trouvé ce jour** : `clarte-rurale/tableau.html` (l'écran `/`) pose `<a class="retour" href="/">` — un chevron qui ramène là où on est ; ses 42 témoins verts ne le voyaient pas, parce qu'ils cherchaient `href="#"`. app-moderne ne rend pas ce chevron, et le dit : sur le tableau de bord il n'a aucune destination.
12. **Le premier écran d'un écran de LISTE montre au moins TROIS lignes actionnables, à 360 × 844.** L'aération qui pousse la liste hors de la liste n'est pas de l'aération. — *Témoin* : compter les `<a>` pleine largeur de ≥ 56 px entièrement contenus dans les 844 premiers pixels. **Mesuré ce jour sur `/chats`** : app-moderne **4**, sobriété 4, **clarté rurale 1** — un en-tête de marque, un sous-titre, une bannière, deux puces et un libellé de section avant la première conversation.
13. **HTML réel** : `<header>` `<nav aria-label>` `<main>` `<form>` `<dialog>` `<details>/<summary>` `<button>` `<a>` `<time datetime>` `role="search"` `role="status"` `role="alert"` `aria-invalid`+`aria-describedby` `inert`. Jamais une `div` cliquable. — *Témoin* : `v3-a11y.spec.ts` (axe : 0 serious/critical, quatre colonnes du § 9.6).

#### D. Plans, cartes, accent, encre, focus

14. **Quatre plans, un rôle chacun — c'est ce qui fait « application », et ça coûte zéro octet.** `--color-bg-sunken` ce qui s'ENFONCE (champ, recherche, composeur) · `--color-bg` la page · `--color-surface` ce qui se POSE (carte, puce, bandeau, ligne mise en avant) · `--color-surface-raised` ce qui FLOTTE (feuille modale, rond secondaire, lien d'évitement au focus). Aucune cinquième profondeur. — *Témoin* : `plansDesordonnes` de `check-jetons.mjs` (l'ORDRE des quatre luminances, dans les deux schémas) + un rôle unique par plan.
15. **Cartes là où la planche en dessine.** `home` : recherche sur le plan creux, « Reprendre » en cartes à avatar avec « Tout voir » en tête de section, « Mes liens » en carte à tuile puis `.action.contour`, « Communautés » en état vide. `chats` : deux actions de même rang côte à côte, puce de Prisme `AUTO · <langue>`, une carte mise en avant au rayon `xl`, puis lignes PLATES de `--row-height` séparées d'un filet unique (aucune bordure par ligne). `join` : pseudo **et** langue CÔTE À CÔTE. — *Témoin* : `compare-rendu.js`, disposition ≤ 8 %, IoU ≥ 0,92 sur `home`, `chats`, `join`.
16. **Filet ≠ contour, et un contour qui porte SEUL son sens tient 3:1.** Carte : `--stroke-hair` `--color-border-strong`. Contrôle et carte cliquable : `--stroke-strong` `--color-border-interactive`. **État vide et `trou` : pointillé `--stroke-strong` `--color-border-interactive`**, jamais `--color-border-strong`. **`--color-border` banni** (1,28:1). — *Témoin* : `grep -c 'var(--color-border)'` = 0 ; paires dans `check-jetons.mjs`. **Mesuré ce jour** : app-moderne 0, clarté rurale 0, **sobriété 8** — et son état vide est un pointillé `--color-border-strong`, c'est-à-dire un état qui n'existe QUE par un contour invisible.
17. **Focus double, jamais supprimé, et le lien d'évitement est VISIBLE au clavier.** `:focus-visible{outline:var(--stroke-focus) solid var(--color-focus);outline-offset:var(--stroke-strong);box-shadow:0 0 0 var(--stroke-strong) var(--color-focus-contra)}` — aucune couleur seule ne tient sur les huit fonds, le COUPLE tient partout. Chaque écran ouvre par `<a class="hors-ecran" href="#contenu">`, **qui devient une pastille visible sous `:focus-visible`** *(greffe « clarté rurale » : un lien d'évitement invisible n'évite rien)*, et sa cible `id="contenu"` existe. Aucun `outline:none` sans remplaçant. — *Témoin* : présence sur CHAQUE document + existence de la cible ; `grep -c 'outline:none'` = 0. **Mesuré ce jour** : présent sur les 7 documents d'app-moderne et les 6 de clarté rurale ; **absent des 5 de sobriété**, qui pose en outre `outline:none` sur la cible la plus utilisée de son tableau de bord.
18. **L'encre du CONTENU est `--color-text` ; le gris est réservé à ce qu'on peut ne pas lire.** Prennent l'encre pleine : l'aperçu d'une ligne de `/chats` et de `home`, le texte d'un message **y compris en attente de réseau**, la phrase d'un état, le corps d'une carte, le paragraphe du héros, l'étiquette d'un champ, la citation de l'hôte, l'original replié. Prennent `--color-text-muted` : l'heure, le nombre de participants, le qualifiant de section, la sous-ligne d'une carte. `--color-text-subtle` ne se pose sur aucun voile. — *Témoin* : `charte.test.ts` interdit `--color-text-muted|subtle` dans `.apercu`, `.ligne p`, `.message p`, `.bandeau p`, `.carte-vide p`, `.heros p`, `label`. **Mesuré ce jour au navigateur (sombre, 390 px)** : l'aperçu de `/chats` tient **15,22:1** chez app-moderne comme chez clarté rurale, en **17 px** ; **sobriété le sert en gris à 14 px (6,08:1)** — c'est le texte pour lequel on ouvre l'écran, et c'est le seul qu'elle rapetisse.
19. **Contraste mesuré, deux schémas, voiles compris.** Texte ≥ 4,5:1, signal ≥ 3:1, sur les quatre plans ET les quatre voiles. — *Témoin* : `check-jetons.mjs`, rouge sur toute paire sous le seuil, sonde qui rougit. **Exécuté ce jour** : `app-moderne/contraste.mjs` **114 paires, rc 0** ; `clarte-rurale/temoins.mjs` **34/34, rc 0** (encre de contenu 12,78 · méta 5,11 · contour 3,44 · bouton 6,29 · dégradé 4,96 · glyphe de signal 4,58).
20. **Un accent, six emplois.** `--color-primary` ne peint que : le cliquable (action primaire, lien, puce active, chevron de retour), la pastille `.langue`, le compte de non-lus, la tuile de marque, le rond d'envoi et de rappel, la ligne de frappe — et, **vitrine seule**, la fin de l'accroche (`h1 em`). Jamais un titre de section, un filet, un fond de carte. Les états prennent `--color-success|warning|danger` ; les quatre `--color-avatar-*` restent (elles disent QUI). — *Témoin* : chaque bloc qui pose `var(--color-primary` est confronté à la liste NOMMÉE de sélecteurs, et la liste EST la règle.
21. **Thème par la CLASSE, jamais par une requête de média** ; les deux schémas sont mesurés ET regardés à chaque livraison. — *Témoin* : `check-jetons.mjs` ; `compare-rendu.js` sur les quatre colonnes du § 9.6.

#### E. États — une phrase, un glyphe, un bouton s'il y a quelque chose à faire

22. **Chargement** : jamais un spinner sur un cache non vide ; sur cache vide, squelette **STATIQUE** (blocs `--color-surface` à `--row-height`, aucune animation) **et LABELLISÉ par ce qui manque** — « Le reste arrive · 3 conversations ne sont pas encore en cache ». *(Greffe « clarté rurale » : un squelette muet fait attendre sans dire quoi.)* — *Témoin* : critère de fin `home` / `chats` (0 spinner, 0 squelette avec cache pré-rempli) ; le bloc de squelette porte un `<p>` non vide.
23. **Bandeau** `.bandeau` : fond `--color-tint-<état>`, **filet gauche `--space-1`** de la teinte, glyphe 24 px + titre semi-gras + phrase en `--color-text`. `.attention` = hors-ligne, session expirée (`ph-warning-circle`, `role="status"`, bouton « Réessayer » / « Reprendre ma place ») · `.refus` = 403 / 409 `LINK_EXHAUSTED` / 410 / 429 (`ph-x-circle`, `role="alert"`) · `.bien` = droits obtenus (`ph-check-circle`). **Hors-ligne n'est JAMAIS rouge et n'est jamais muet.** — *Témoin* : tout `.bandeau.attention` contient une action ; aucun `.bandeau.attention` sous `--color-tint-danger`.
24. **Vide** `.carte-vide` : pointillé de CONTRÔLE (règle 16), glyphe 40 px, titre, phrase ≤ `--measure` en `--color-text`, action primaire. **Un compte de non-lus porte son mot pour la voix** — « 3 » ne dit rien : `.compte` contient un `.hors-ecran`. *(Greffe « clarté rurale ».)* — *Témoin* : `compare-rendu.js` sur `home › Communautés` ; tout `.compte` contient un nœud hors écran non vide.
25. **Refus dans la modale de `/chat/:lien` : c'est le CODE qui tranche, jamais le statut.** Refus de SAISIE (409 `USERNAME_TAKEN_IN_CONVERSATION`, 400) ⇒ `.refus` sur SON champ, `aria-invalid` + `aria-describedby`, **`suggestedNickname` PRÉ-REMPLI dans `value`**, formulaire GARDÉ, **et l'aide occupe toute la largeur du duo** (`grid-column: 1 / -1`) sans replier le duo — *(greffe « clarté rurale », dont le `:has(.refuse)` gagnait la lisibilité de l'aide et perdait « Créer un compte » hors de l'écran)*. Refus DU LIEN (403, 409 `LINK_EXHAUSTED`, 410, 429) ⇒ `.bandeau.refus` DANS la modale, formulaire retiré, les deux sorties conservées avec leur `?next=`. — *Témoin* : `v3-join.spec.ts`, un test par code, chacun produit par l'ÉTAT du lien dans la passerelle de bouchon.
26. **Fil** : 410 ou droit retiré ⇒ `.composeur.ferme` (`ph-lock` + raison, aucun champ) · 401 ⇒ `.bandeau.attention` à bouton, lecture conservée · hors-ligne ⇒ la ligne GARDE sa place, son encre PLEINE, et sa méta porte `ph-clock` + « En attente du réseau » · `hasGap` ⇒ `.trou` pointillé avec un `<a href>` « Charger les messages manquants ». — *Témoin* : `v3-lifecycle.spec.ts`, cas C→H du § 6.3.

#### F. Modale, fil, Prisme, glyphes, mouvement

27. **Modale de `/chat/:lien` — et l'action primaire reste dans le PREMIER écran, dans TOUS ses états.** `<dialog open>` rendu SERVEUR (aucun `showModal()`, aucun script) ; feuille basse sur mobile (`inset:auto 0 0`, `--radius-2xl` en haut, poignée, `--color-surface-raised`, `max-block-size: var(--sheet-max)`, `overflow:auto`), centrée ≥ 600 px ; voile `.voile` fixe `--color-overlay` ; le cadre du fil derrière est **`inert`** + `filter:blur(var(--frame-blur))`, **messages VIDES** (rien de la conversation ne part avant le choix), composeur inactif. Ordre : « <hôte> vous invite » → titre → citation → `<details>` des droits → pseudo **et** langue côte à côte → `.action.primaire` « Continuer en anonyme » → « ou gardez votre identité » → « Se connecter » → « Créer un compte » → la note du `?next=`. — *Témoin* : `v3-join.spec.ts` en `javaScriptEnabled:false` ; assertion `[inert]` ; `boundingBox().y + height ≤ 844` sur `.action.primaire` **dans l'état nominal ET dans l'état de refus**. **Mesuré ce jour (390 × 844)** : app-moderne **601 / 601** ; clarté rurale 576 nominal mais **754 en refus**, qui pousse « Créer un compte » et la note hors de l'écran ; **sobriété 914** — hors de l'écran dès l'état nominal.
28. **Le fil est PLAT — aucune bulle** (`cible/thread.png`). Ligne = avatar 48 px + nom (+ « anonyme » avec `ph-ghost`) + texte 17 px `--color-text` + méta (`.langue`, `<time>`, `ph-checks`) ; mes messages = même ligne, nom « Vous » ; séparateur de jour en capitales espacées ; ligne système centrée ; appel manqué = carte `--color-tint-danger` à filet `--color-danger`, glyphe `ph-phone-x`, rond primaire `ph-phone` ; en-tête et composeur OPAQUES, tenus par une grille `100dvh`, jamais par un survol. **`/chat/:lien` (invité) et `/chats/:cle` (membre) rendent le MÊME module `app/connecte/fil-vue.ts`.** — *Témoin* : `compare-rendu.js` sur `thread` ; test unitaire : un seul module de vue importé par les deux routes.
29. **Le bandeau des droits (vue `rights`) est le PREMIER bloc sous l'en-tête, au-dessus des puces** (`cible/rights.png`), `<details open>` `.bandeau.bien`, filet gauche `--color-success`, **une ligne par droit, sans sous-ligne** (`ph-check-circle` / `ph-x-circle`), refermable. Au-delà de `var(--bandeau-max)` il se rend REPLIÉ sur son seul `<summary>`, jamais coupé sans contrôle. Les sous-lignes vivent dans la MODALE, où elles se lisent avant d'entrer. — *Témoin* : `compare-rendu.js` sur `rights` ; aucun `.bandeau.bien` ouvert avec `scrollHeight > clientHeight`. **Mesuré ce jour** : app-moderne 287 px, non tronqué ; clarté rurale garde les sous-lignes, dépasse et DÉFILE — ce que cette règle refuse, parce que le bandeau mange alors la conversation qu'on vient lire.
30. **Prisme** : la pastille `.langue` = `ph-translate` 16 px + code de la langue d'**ORIGINE** en capitales, `--text-xs`, encre `--color-primary`, contour `--stroke-hair` `--color-border-interactive`, `--radius-xs` ; elle **PRÉCÈDE** l'aperçu dans une ligne et la méta dans un message ; **rendue seulement quand `resolvePrismTranslation()` rend non-null** ; **jamais un drapeau** (un drapeau dit un PAYS ; écart assumé avec `cible/thread.png`). Tout nœud servi dans une langue ≠ `<html lang>` porte `lang="xx"` ; l'original est un `<details>` « Voir l'original » (`ph-text-aa`) dont le `<p>` porte `lang`. — *Témoin* : test unitaire de `fil-vue.ts` ; **témoin de rang écrit sur un rang ≠ 1** (leçon 261) ; au moins une ligne SANS pastille dans le jeu d'essai — le témoin s'écrit là où la traduction est ABSENTE ; 0 `<img>` de drapeau. **Regardé ce jour** : `cible/home.png` sert « Gracias, te mando el archivo » à une lectrice française et sobriété le reproduit sans pastille — les deux directions retenues servent « Merci, je t'envoie le fichier » avec `ES`, et elles ont raison contre la cible.
31. **Glyphes — sprite `packages/icons`, jamais une fonte**, trois tailles : `--glyph` 24 px devant un mot, `--glyph-inline` 16 px en méta, `--glyph-large` 40 px en état vide ; `fill:currentColor`, `aria-hidden` ; un glyphe seul n'existe que sur un rond à `aria-label`. Emplois : marque `ph-chat-circle` · retour `ph-caret-left` · recherche `ph-magnifying-glass` · lien `ph-link-simple` · nouveau `ph-plus` · compte `ph-user-circle` · menu `ph-squares-four` · communauté `ph-users-three` · Prisme `ph-translate` · replier `ph-caret-down` · droits `ph-key` · accordé `ph-check-circle` · refusé `ph-x-circle` · anonyme `ph-ghost` · hors-ligne et avertissement `ph-warning-circle` · joindre `ph-paperclip` **sur un rond bordé 44 px** · envoyer `ph-arrow-up` sur un rond primaire 52 px · lu `ph-checks` · attente `ph-clock` · appel manqué `ph-phone-x`, rappeler `ph-phone` · original `ph-text-aa` · fermé `ph-lock` · médias `ph-image` · réessayer `ph-arrows-clockwise`. — *Témoin* : **N `<use>` ⇒ N boîtes non nulles**, les glyphes d'un `<details>` FERMÉ exclus (ils ne sont pas rendus par DÉCISION) ; gate CI « `ph-*` sans `<symbol>` ».
32. **Le mouvement ne déplace rien.** Transitions ≤ 150 ms sur `background-color | border-color | color` seulement ; **UNE** `@keyframes` au plus (le `pulse` de présence, `opacity` seule, ≥ 2 s) ; `prefers-reduced-motion: reduce` coupe tout. — *Témoin* : mesuré sur la feuille **COMPACTÉE** (sinon le commentaire qui nomme la règle se compte) : 0 transition sur `transform|height|width|opacity|all`, `@keyframes` ≤ 1, bloc reduced-motion présent. **Mesuré ce jour** : app-moderne 1 `@keyframes`, clarté rurale 0, **sobriété `transition:transform`** — la seule des trois qui anime la géométrie.
33. **Temps réel = amélioration progressive.** Tout marche par `<form method="post">` et rechargement ; `participate.ts` arrive par `await import()` APRÈS le premier pixel, sur `fil` et `chats` seulement, et ne peint qu'avec les classes de cette charte (`.ligne.frappe` : « <nom> écrit… » en italique `--color-primary`, sous le dernier message et au-dessus du composeur ; réordonnancement ; compte de non-lus). — *Témoin* : lint « import statique de `participate` = erreur » ; assertion « `socket.io-client` absent du document avant le tap ».

#### G. Interdits — chacun à témoin `grep` dans `__tests__/charte.test.ts`, chacun prouvé non vide par une sonde

34. `#hex`, `rgb()`, `hsl()`, couleur nommée · `px` de design · `font-family` autre que `var(--font-native)` · une police NOMMÉE et jamais servie · `@font-face`, `url(`, `@import` · **`box-shadow` hors le contre-anneau de focus** · **`backdrop-filter`** · `filter:blur` ailleurs que sur le cadre inerte de `/chat/:lien` · `opacity` comme état · `--text-2xs` · `--color-border` · `--radius-md` · `outline:none` · `@media (prefers-color-scheme)` hors moteur · `<div onclick>`, `href="#"`, `type="button"` sans gestionnaire, **lien vers l'écran courant** · spinner sur cache non vide · squelette animé · **drapeau de langue** · seconde table (`--x:` hors `packages/design-tokens`, **le document compris**) · fonte d'icônes, `@phosphor-icons/web`, `lucide-react` · React hydraté sur ces cinq écrans · **barre d'onglets** · **bulle de message** · un contrôle inerte.
35. **UN dégradé, et un seul : `.heros` de la vitrine, entre DEUX jetons voisins** (`--color-tint-primary` → `--color-surface`). La directive autorise nommément les « dégradés discrets », le coût réseau est nul, et c'est la seule chose qu'une capture d'application a et qu'une page de texte n'a pas : une lumière. Bornes : un seul sélecteur, deux jetons, encre mesurée **aux DEUX extrémités**. — *Témoin* : `degrades` = 1 **et** `selecteurDuDegrade` = `.heros` (le compte seul ne suffit pas) ; deux paires de contraste dans `check-jetons.mjs`. **Les deux directions retenues ont convergé dessus** : clarté rurale, qui le refusait au tour 2, l'adopte et le mesure à 4,96:1.

---

#### Portage

`app/socle.ts` (partie feuille) + `app/enveloppe/feuille.ts` + `app/connecte/feuille.ts` + `app/vitrine/feuille.ts` + `FEUILLE_DU_FIL` → **une feuille de chrome** (base : `app-moderne/chrome.css`) + `fil.css` + `join.css`, mesurées **par route ET par document** (règles 4 et 5). Le vocabulaire de classes est celui du dépôt (`clarte-rurale/these.md` en dresse la liste, à cinq noms près) pour que le portage soit une SUBSTITUTION, pas une réécriture des vues. Témoins : `app-moderne/mesure.cjs` → `e2e/visual/v3-cibles.spec.ts` (cibles, débordement, occlusion à trois défilements, rayons par JETON, `<use>`, liens sans effet, lien d'évitement, encre du contenu, modale dans ses deux états, bandeau des droits, **lignes du premier écran**) ; `app-moderne/contraste.mjs` + `clarte-rurale/temoins.mjs` → `check-jetons.mjs` et `__tests__/charte.test.ts` (les deux jeux fusionnent : 114 paires + 34 témoins de feuille, sondes comprises). `e2e/visual/lib/serveurs.ts` se complète pour chaque endpoint que ces écrans consomment — dont l'ÉTAT du lien, qui produit les refus de la règle 25.

**Porté PARTIELLEMENT dans le dépôt (2026-09-04, #5115)** : `app/vitrine/feuille.ts` et `app/connecte/feuille.ts` portent désormais le contour de contrôle (`--color-border-interactive`, règle 16), l'encre pleine de `.carte-vide p` (`--color-text`, règle 18) et le dégradé du héros entre `--color-tint-primary` et `--color-surface` (règles 9 et 35) — `charte.test.ts` (76/76) et `check-jetons.mjs` (deux paires de contraste, encre mesurée aux deux extrémités du dégradé) les gardent. Les deux points que ce paragraphe annonçait comme RESTANTS sont eux aussi portés — vérifié dans le code le 2026-09-04, contre le texte : `.accroche` (= `.heros p`) porte `--color-text` (`app/vitrine/feuille.ts`, avec son témoin dans `charte.test.ts`), et `.atouts .tuile` / `.carte .tuile` portent `--radius-lg`, non plus `--radius-md`. Le paragraphe a donc décrit comme « restant » ce que le même tour venait de livrer, dans le commit qui l'écrivait.

> **Un document de conception qui annonce ce qu'il RESTE à faire se périme dans le commit même qui le livre.** La phrase était juste quand elle a été rédigée et fausse quand elle a été poussée — aucun gate ne compare une prose de suivi à l'état du code, et c'est le lot suivant qui l'aurait crue. Ce qui reste à faire vit dans une ISSUE, que son commit ferme ; ce qu'un document décrit, c'est ce qui EST, au présent et daté.

#### Ce que ce jugement ne prouve PAS

- **Aucune thèse n'est vérifiée sur le terrain.** « Lisible au soleil sur un téléphone d'entrée de gamme » reste un raisonnement adossé à des contrastes, des corps et des cibles mesurés — personne n'a tenu l'appareil dehors. La mesure qui trancherait est un test terrain, pas un gate.
- **Le schéma clair reste DÉRIVÉ** (`light.css` le dit : la planche n'a pas de vue claire). Les captures claires sont une proposition dans la proposition, regardées et jugées bonnes, jamais mesurées contre une cible.
- **`compare-rendu.js` n'a été lancé sur aucune des trois** : ce sont des maquettes, pas des routes servies. La fidélité (critère 4) est jugée à l'œil contre `cible/*.png`, pas par un score.
- **Cinq écrans, pas quarante-six.** Les refus 403/410/429, la session expirée, le composeur fermé, l'état MEMBRE de `/chat/:lien` (302 vers `/chats/:cle`) et toutes les surfaces sociales restent à peindre ; la liste des refus se relit dans l'ÉMETTEUR (`linkAdmission.ts`), jamais dans un document.
- **Le plafond de 9 Ko par document (règle 4) est DÉCIDÉ ce jour**, pas hérité : il vient du pire document mesuré sur la direction retenue (8 362 o) et laisse 7 % de marge. Il se franchit par ratchet relu, jamais en silence.
- **`sobriété premium` n'a rien soumis à ce tour** : ses fichiers sont ceux du 2026-09-01, inchangés — les cinq défauts que le tour 2 avait mesurés chez elle, je les ai retrouvés à l'identique avec mon propre instrument. Ne pas bouger est aussi un verdict.
- **Rien n'est branché.** Aucun fichier du dépôt n'a été touché par ce jugement ; feuilles, documents et harnais vivent dans `.cache/web-v3-workflow/charte/`.

> **Ce que la phrase ci-dessus ne couvre PAS.** Elle est vraie du JUGEMENT — aucune feuille servie, aucun gate, aucun écran n'a bougé pour le rendre. Elle ne l'est plus de ses CONSÉQUENCES de design : l'inscription de cette charte dans la conception a fait évoluer la planche sur six points (§ 12.9), régénéré cinq captures cibles, ajouté le plafond de document à `apps/web-v3/budgets.json` et porté les citations de règles du dépôt vers cette numérotation. Le PORTAGE des feuilles, lui, reste entier : c'est le lot des écrans, et son premier acte est le paragraphe « À corriger dans le dépôt tel qu'il est » ci-dessus.
...
| **DOCUMENT servi par un écran** (HTML + table + feuille + symboles, inlinés) | **≤ 9 Ko gzip** | plafond DÉCIDÉ par la charte du 2026-09-02 (règle 4) — sujet NEUF, que le tour 2 ne gatait pas | charte règle 4 : le lecteur ne télécharge pas une feuille, il télécharge un document, et une fois PAR ÉCRAN (rien n'est mis en cache d'un écran à l'autre puisque tout est inliné, règle 5). Le chiffre vient du pire document MESURÉ sur la direction retenue (**8 362 o**, le fil) et laisse 7 % de marge ; il se franchit par ratchet relu, jamais en silence. **Les documents du DÉPÔT ne sont pas mesurés** — à mesurer dans le commit du premier écran repris, jamais inventé. Témoin : `gzip -9c` sur le HTML servi, par écran, dans `__tests__/charte.test.ts` |
| feuille de chrome | **≤ 4 Ko gzip** ; table inlinée (jetons de la règle 1 compris) **≤ 1,5 Ko gzip** | plafonds DÉCIDÉS (mesurés sur le DÉPÔT le 2026-09-02 : **1 067 o** et **1 325 o** ; les 3 041 / 1 378 o cités par la charte sont ceux de la PROPOSITION `app-moderne/chrome.css`, pas ceux des feuilles servies) | charte règle 5 ; témoin `__tests__/charte.test.ts` (`gzipSync … level 9`) |
| **CSS SERVI par un document** (chrome + feuilles de son écran, inlinés) | **≤ 6 Ko gzip** | plafond DÉCIDÉ par la charte du 2026-09-02 (règle 5), **plus strict** que les 20 Ko du § 8.5 — aucun seuil n'est baissé, un second sujet est gaté | charte règle 5 ; porteur `__tests__/charte.test.ts` (compositions `vitrine · connecté · fil · choix`). **Re-mesuré le 2026-09-02** (`gzipSync … level 9` sur les feuilles du dépôt, cinq compositions cette fois — `médias` manquait aux relevés précédents) : vitrine **1 412** · connecté **1 708** · médias **4 500** · fil **4 153** · choix **4 749 o** — la route de jonction `/chat/:lien` reste la plus lourde, à 77 % du plafond |
| CSS TRANSFÉRÉ par route (ressources `Stylesheet`) | ≤ 20 Ko gzip | GATE | § 8.5, inchangé — **et VACUEUX sur les cinq écrans du focus** : `scripts/mesure-reseau.mjs:209-211` ne somme que les ressources de type `Stylesheet`, or ces documents sont autoporteurs (0 feuille externe, charte règle 6) ⇒ il mesure 0 Ko. C'est la ligne au-dessus qui arbitre ici ; les deux ne sont pas des jumelles — l'une pèse ce qui VOYAGE, l'autre ce que le document PORTE |
...
| cibles | 0 cible < 44 px à 360 et 390 px ; `min(.action.primaire)` = 56 ; `a.flottant[href]` ≥ 52 | GATE | charte règles 7 et 13 ; `e2e/visual/v3-cibles.spec.ts` |

**Où vivent ces QUATRE plafonds décidés, et ce qui reste à câbler.** Ils sont dans les DONNÉES — `apps/web-v3/budgets.json` → `feuilles` pour les trois du CSS, `documents` pour celui du document servi (deux sujets, deux blocs : l'un pèse une feuille, l'autre ce qu'un écran remet en une fois) —, jamais en littéraux dans un témoin : `__tests__/charte.test.ts` lit déjà ce fichier (il en tire `reseau.transverses.css_ko`) et écrit encore `4 * 1024` / `1536` à la main, et sa composition la plus lourde est opposée aux **20** Ko du § 8.5 (`expect(PLAFOND_PAR_ROUTE_KO).toBe(20)`) — c'est-à-dire au plafond que la charte juge trop lâche pour arbitrer. Le lot qui reprend ces écrans fait lire les QUATRE valeurs à ce témoin ; tant que ce n'est pas fait, le plafond de 6 Ko est ÉCRIT et mesuré (4 749 o au pire ce jour) mais n'est opposable par aucun gate, et celui de 9 Ko par document n'est ni câblé NI mesuré sur le dépôt — dit ici plutôt que découvert au premier dépassement.
...
docs/product/MeeshyWebV3Design/
  MeeshyWebV3.dc.html            # 39 écrans : groupe « SITE » (vitrine) ; join = état CHOIX ; rights =
                                 #   état INVITÉ dans le fil ; profilMembre = surimpression sur le fil
                                 #   (§ 12.10.3) ; FAB 56/52 ; puce Prisme AUTO · <langue>
  cible/*.png (39)  vues.json  vues.md      # régénérés par capture-cibles.js
  jetons-de-vues.json            # un jeton par ÉTAT de /chat/:lien (lien-vivant, lien-rejoint), cle du fil,
                                 #   et `cle` + `membre` pour profilMembre (un jeton de chaîne de requête)
  matrice.json                   # 47 lignes ; thread et profilMembre portent leurs adresses (`adresses`)
```

### 12.8 Ce que les instruments de design doivent apprendre — et ce qu'ils refusent en attendant

- **L'ÉTAT DE SESSION est LIVRÉ (2026-09-02).** Une vue déclare `"@session": "membre"` dans `jetons-de-vues.json`, dont le bloc `sessions` dit quels COOKIES la portent — ceux que la passerelle de bouchon reconnaît (`meeshy_auth` + `meeshy_session`) ; `compare-rendu.js` les pose sur le contexte avant de naviguer, et `index-des-vues.mjs` REFUSE, en la nommant, une vue qui réclame une session non déclarée. La couture est gagée par `apps/web-v3/__tests__/conformite-des-vues.test.ts`, qui oppose les cookies déclarés à `COOKIE_DE_JETON` / `COOKIE_DE_SESSION` / `JETON_DU_MEMBRE` : recopiés à la main, ils dériveraient en SILENCE et l'outil mesurerait l'écran de connexion contre la cible d'un fil.
- **Et il faut une CHAÎNE, pas un `next start` nu.** `compare-rendu.js` navigue vers un `--base` ; tant que ce `--base` n'avait aucune passerelle derrière lui, toute vue du membre rendait « Le service ne répond pas » (mesuré : `structure=0.54` sur un code par ailleurs conforme). `apps/web-v3/scripts/conformite-des-vues.ts` monte les deux (passerelle de bouchon + artefact de `next build`) et appelle l'outil dessus : `bun run apps/web-v3/scripts/conformite-des-vues.ts rich thread`. Il ne réécrit ni la passerelle, ni les seuils, ni la sélection.
- **`vitrine` et `home` partagent `/` — et ce n'est PAS le même problème.** `selectionComparable` refuse — en les NOMMANT, `RC_NON_COMPARABLE` — deux vues qui visent le même écran servi ; c'est le comportement voulu (« une seule navigation ne peut pas rendre deux écrans différents »). La session les sépare désormais dans la DÉCLARATION, mais la COLLISION de chemin reste à trancher dans `vues-comparables.mjs` : `home` ne déclare donc encore aucune session, et `--vues vitrine` ou `--vues home` mesure chacune seule. C'est une issue de l'instrument, dans le commit de `home`, jamais un contournement.
- **`rich` n'a JAMAIS été bloquée par un jeton.** `vues.json` la déclare sur `/chats/:id`, une route DISTINCTE du `/chats/:cle` de `thread` : aucune collision à arbitrer, et `selectionComparable` retenait déjà les deux sans un refus. Ce qui manquait était (a) la SESSION ci-dessus, commune à toute la famille du membre, et (b) une conversation adressable par ce jeton dans la passerelle PARTAGÉE — `messagesRiches()` n'était raccordée qu'à l'instance éphémère d'un spec. Les deux sont livrées : `jetons-de-vues.json` déclare `"rich": { "id": "fil-riche", "@session": "membre" }` et la passerelle de bouchon sert ce fil en annexe (`FilAnnexe`, `e2e/visual/lib/bouchon-fil.ts`).
- **Ce que l'outil NE mesure toujours pas** (§ 9.6) : `e2e/visual/v3-visual.spec.ts` — le gate qui produit le diff par région ≤ 8 %, l'IoU ≥ 0,92, les 100 % d'icônes attendues et les quatre colonnes de thème — **n'existe pas dans le dépôt**. `compare-rendu.js` rend un écart STRUCTUREL (profil d'encre, seuil 0,15) et un `pixels_indicatif` ; ce ne sont pas les mêmes mesures, et les critères de fin qui citent « ≤ 8 % / IoU ≥ 0,92 » nomment donc un gate que le dépôt ne sait pas produire. Tous les écrans livrés jusqu'ici (`thread`, `join`, `rights`, `rich`) l'ont été sans lui.
...
- **L'IDENTIFIANT d'une vue capturée est contraint par l'instrument, et il faut le dire là où on nomme (2026-09-03).** `capture-cibles.js` tire les identifiants de la SOURCE de la planche — « les identifiants viennent de `const MAP`, jamais des libellés : deux écrans partagent un libellé et se seraient écrasés » — avec `/\['([A-Za-z]+)',\s*'ph-/g`, puis exige `KEYS.length === nodes.length`. Trois conséquences qu'aucun document ne portait : une clé qui sort de `[A-Za-z]` fait échouer le harnais **par un désaccord de COMPTE** (message trompeur : il accuse le navigateur, pas le nom) ; l'identifiant devient un NOM DE FICHIER (`cible/<id>.png`) et une cible de lien Markdown dans `vues.md`, donc `:` est exclu quel que soit le harnais ; et un nom choisi ailleurs — dans une directive, une issue — ne peut pas être repris tel quel sans cette vérification. C'est ce qui a fait de `sheet:profil-membre` la vue **`profilMembre`** (§ 12.10.3, point 1). Le préfixe `sheet:` reste réservé aux surfaces `hors_planche: true`, que rien ne capture et dont l'identifiant n'est donc jamais un nom de fichier.
- **Une route peut porter ses jetons dans sa CHAÎNE DE REQUÊTE, et c'est ce qui sépare une surimpression de son hôte.** `motifJeton` de `vues-comparables.mjs` lit la route entière (`login` en use déjà : `/login?returnUrl=/chat/:lien`), donc `profilMembre` déclare `cle` **et** `membre` (`/chats/:cle?profil=:membre`). Le second n'est pas décoratif : sans lui, `profilMembre` et `thread` viseraient tous deux `/chats/equipe-lagos` et le refus « route partagée » (`RC_NON_COMPARABLE`) tomberait à bon droit. Une vue qui est un ÉTAT d'un écran hôte doit donc porter, dans sa route, le paramètre qui produit cet état — sans quoi l'instrument ne peut pas la mesurer, et le dira.

- **Le CODE de la v3 cite la charte par NUMÉRO, 35 fois dans 20 fichiers** (`grep -rno "charte règles\? [0-9]\+" app lib __tests__ e2e scripts`, mesuré le 2026-09-02) : les `describe()` de `__tests__/charte.test.ts` (« règle 3 — le poids », « règle 4 — les hauteurs », « règle 8 — chaque pas de l'échelle » ⇒ 5, 7, 10), les doc-comments de `app/connecte/{vue,fil-vue,fil-lignes,medias-vue}.ts`, `app/(public)/chat/[lien]/{choix-vue,choix-feuille}.ts`, `lib/contenu/{fil,droits}.ts`, `lib/realtime/participate.ts` et cinq specs. La renumérotation du tour 3 les rend TOUS faux d'un cran ou deux — un libellé de témoin est une citation comme une autre, et celle-ci désigne désormais une règle voisine. La table du § 12.5 est la loi de portage ; le lot qui reprend chaque écran porte les citations de SES fichiers, dans le même commit que le câblage des quatre plafonds décidés (§ 12.6). Rien n'est touché ici : ce paragraphe est un document de conception, et renommer 35 citations sans exécuter les suites qui les portent serait une retouche non prouvée — sur un fichier qu'un autre lot réécrit. **Confirmé le 2026-09-04** : le portage de #5115 (contour, encre, dégradé — règles 16, 18, 9, 35) a exécuté `charte.test.ts` sans corriger ses `describe()` — sept d'entre eux et deux doc-comments citent encore la numérotation tour 2, dont les trois que ce paragraphe nomme (« règle 3 », « règle 4 », « règle 8 ») ; chaque numéro désigne désormais une AUTRE règle réelle. Toujours non touché ici, pour la même raison.
- **`cible/chats.png` et `cible/thread.png` ne sont plus les images qu'un rapport de conformité antérieur a notées.** `rapport-conformite.json` (structure 0,45 sur `thread`, 0,51 sur `rich`, quatre échecs) a été produit contre les captures d'AVANT ce tour ; les cinq captures régénérées rapprochent la cible de ce que la charte exige (pastille au lieu de drapeaux, encre pleine, pas de chevron sur `/`), donc **le score antérieur ne peut plus être comparé au suivant**. Le prochain `compare-rendu.js` refait la mesure ; personne ne lit l'ancien fichier comme une ligne de base.
...
| Le plafond de CSS d'une route | § 8.5 et `budgets.json` : **20 Ko gzip**, GATE, « par route » | la charte du 2026-09-02 (règle 5) décide **6 Ko** sur le CSS qu'un document PORTE ; les 20 Ko restent, inchangés, sur ce qui VOYAGE en ressource `Stylesheet`. Deux sujets, deux plafonds, aucun seuil baissé (§ 12.6) | le gate de 20 Ko est **vacueux** sur les cinq écrans du focus : `scripts/mesure-reseau.mjs:209-211` ne somme que les ressources `Stylesheet`, or ces documents sont autoporteurs ⇒ il mesure 0 Ko. Un plafond qu'aucune mesure ne peut atteindre n'arbitre rien (leçon 345 : un `[]` prouve l'instrument en panne, pas l'absence de violation). Mesuré ce jour sur les feuilles du dépôt : 4 234 o au pire (`/chat/:lien`) |
| Le bac de répétition (§ 4.10) | « il n'existe **aucun** `docker-compose.staging.yml` dans ce checkout » | le bac EXISTE (`docker-compose.staging.yml:318,368,386,448,453`) et réclame déjà les étapes 1 à 5 bis du § 4.9, quand la prod n'en a que deux (`docker-compose.prod.yml:492`) | le fait d'origine est daté et conservé ; ce qui reste ouvert est nommé — host PARTAGÉ au lieu de l'isolement demandé (le bac répète la COHABITATION, ce qui sert mieux le § 4.9) et **`noindex` absent au niveau du host**, à porter par un middleware Traefik dans le commit du Gate E |
| La feuille de la modale `join` (charte règle 27, `cible/join.png` : feuille vers 26 % de la hauteur, épousant son contenu) | `height:92dvh` pour toute variante — 131 px de vide sous la note à 390×844, cadre flouté réduit à 68 px | une hauteur RÉSERVÉE par variante, mesurée à 390×844 : nominale 80dvh (contenu 100 % de la feuille, haut à 20 %), étendue 92dvh, brève 67dvh, fermée 54dvh (`choix-feuille.ts`) ; CLS **0** mesuré en Fast 3G (gate ≤ 0,05 ; 1 requête avant le premier pixel, FCP 760 ms, LCP 796 ms, 44 487 o) ; `compare-rendu.js --vues join` rejoué le 2026-09-02 sur la chaîne réelle : structure **0,295** (sombre) / **0,487** (clair) contre 0,522 / 0,558 avant, seuil 0,15 — HORS-CIBLE, consigné ici | la feuille ne peut pas monter à 26 % sans perdre la question binaire que la directive impose et que la planche ne dessine pas ; l'écart structurel résiduel tient aux DONNÉES (question, cinq lignes d'accordéon, absence d'avatar d'hôte — non servi par l'aperçu, § 5.1), pas à la disposition ; assumé et daté, pas oublié. À noter : une mesure antérieure (0,51 sur 15 Ko) comparait la page de PANNE — la passerelle de bouchon vit dans le processus du test, qu'un `spawnSync` bloquait ; l'instrument se lance en asynchrone |
| **La charte du § 12.5 elle-même** (tour 2, 31 règles) | ces 31 règles, avec leur numérotation, citées par `matrice.json`, le § 12.6 et le § 12.9 | la charte du **tour 3** (35 règles) la REMPLACE entièrement, numérotation comprise ; la table de correspondance ouvre le § 12.5 et **toutes les citations du dépôt ont été portées dans le même mouvement** | une renumérotation silencieuse fait mentir chaque citation sans qu'aucun témoin ne rougisse : un numéro qui glisse pointe une autre règle et se lit encore comme une preuve. Rien n'est effacé — la charte du tour 2 se relit dans l'historique (`5bff749260`) |
| Les DRAPEAUX du Prisme (`cible/{thread,rights,rich}.png` : `🇪🇸 🇫🇷` en méta de message) | la charte du tour 2 posait « jamais un drapeau » et **assumait l'écart** avec la cible | la PLANCHE change : la pastille `ph-translate` + code d'ORIGINE en capitales remplace les drapeaux, et les cinq captures sont régénérées | un écart assumé entre une règle opposable et l'image contre laquelle `compare-rendu.js` NOTE l'implémentation est une jumelle qui pénalise le code conforme. La planche est la source du design, pas une archive |
| L'aperçu de `home` (`cible/home.png` : « Gracias, te mando el archivo » servi à une lectrice francophone, sans pastille) | la cible le montrait ainsi ; la charte du tour 2 constatait que les deux directions retenues « ont raison contre la cible » | la planche sert l'aperçu TRADUIT (« Merci, je t'envoie le fichier ») avec sa pastille `ES` | la planche se contredisait ELLE-MÊME : `/chats` (`const CHATS`) servait déjà ce même message, de la même expéditrice, à la même lectrice, traduit et étiqueté `ES`. Le Prisme est non négociable (CLAUDE.md § Prisme, règle 1) ; c'est la ligne fausse qui bouge, jamais la règle |
| Le chevron de retour de `cible/home.png` (l'écran `/`) | la planche le rendait dès que sa pile n'était pas vide — donc sur la racine, où la capture y arrive par une navigation | `canBack` exclut `home` et `vitrine` : sur `/`, aucun chevron | charte règle 11 — un lien dont la destination est la route qu'on regarde n'a AUCUN effet, et l'interdit 34 le nomme. La règle est née d'une mesure du juge sur une AUTRE direction ; la planche portait le même défaut, invisible parce que personne n'y cherchait un `href` |
| Le mode « focal » du fil (`opacity:.45` sur trois messages sur quatre, corps 17 px sur le quatrième) | un état du prototype, gouverné par `cycleMode` et `modeBadge` — **calculés et jamais rendus** | retiré : chaque ligne prend l'encre PLEINE et un seul corps | règles 18 (l'encre du CONTENU) et 34 (`opacity` comme état, contrôle inerte). Un contrôle qu'aucune vue ne rend ne peut pas être « le mode par défaut » : c'est une atténuation permanente, et la capture l'enseignait |
...
| Le NOM de l'écran du § 12.10.3 | la directive du 2026-09-03 l'appelle `sheet:profil-membre` | l'identifiant est **`profilMembre`** ; `sheet:` reste le préfixe des cinq surfaces `hors_planche: true`, que rien ne capture | une vue CAPTURÉE est une entrée du navigateur de la planche, dont `capture-cibles.js` lit les clés en `[A-Za-z]` et dont l'identifiant devient un nom de fichier (`cible/<id>.png`) et une cible de lien dans `vues.md`. Un `:` y fait échouer le harnais par un désaccord de COMPTE — un message qui accuse le navigateur, pas le nom (§ 12.8, § 12.10.3 point 1) |
| La FORME du profil d'un participant | « une MODALE » — sans dire à quelle adresse elle vit | un **ÉTAT de l'écran hôte**, porté par `?profil=` : `/chats/:cle?profil=:membre`, `/chat/:lien?profil=:membre`, `/chats?profil=:membre` | c'est ce qui la rend ouvrable et fermable par un `<a href>`, donc sans un octet de JavaScript (§ 12.10.6), et ce qui lui interdit d'ajouter un motif de budget : un état de la même adresse est servi par le même document (§ 12.10.3 points 2 et « ce que cet écran ne change PAS ») |
| La sortie « Profil » du fil (planche, `EXITS.thread`) | `['contacts', 'Profil', 'nom de l'auteur']` — le nom d'un auteur ouvrait l'ANNUAIRE du lecteur | il ouvre `profilMembre`, le profil de CETTE personne | ouvrir le profil de quelqu'un n'est pas ouvrir la liste de ses contacts à soi ; c'est la sortie que le § 12.10.3 demandait, et elle était occupée par une destination qui n'y répondait pas |
...
**L'ADRESSE DU PLEIN ÉCRAN NOMME LA TRANCHE, PAS SEULEMENT LA PIÈCE (corrigé le 2026-09-03, revue croisée).** Le plein écran est un ÉTAT de l'adresse hôte — comme le profil d'un participant (§ 12.10.3 point 2) —, et la première livraison l'écrivait `?media=<pièce>`. Une pièce ne dit pas où elle est : la porte re-servait alors la tranche PAR DÉFAUT (les quarante derniers messages, `lib/api/fil.ts`), où la pièce d'un message plus ancien n'est pas. Le tap n'ouvrait donc RIEN — ni sur la page `?avant=` (sans JavaScript), ni sur l'historique chargé EN PLACE par le module (`participate.ts`), c'est-à-dire sur la quasi-totalité des médias d'une conversation vivante — et le lecteur perdait en prime la page qu'il lisait. La forme est **`?autour=<message>&media=<pièce>`** : `autour` est servi par `around=` de `GET /conversations/:id/messages` (`services/gateway/src/routes/conversations/messages-list.ts:400-450`, la moitié avant, la cible, la moitié après), la croix rend `?autour=<message>#m-<message>` — la même tranche, cadrée sur le message d'où la pièce vient. Trois conséquences : le lien SERVI et le lien PEINT se composent à l'identique (aucun des deux n'a besoin de connaître la tranche), l'adresse d'un média est suffisante à toute profondeur (elle s'ouvre collée dans une autre fenêtre), et le retour ne saute plus la position de lecture. Site unique : `lib/api/adresses-du-fil.ts`. **Et le fil RECOUVERT par la surimpression n'accuse plus la lecture** (`accuseCeQuiEstServi`) : la surimpression est opaque et le `<main>` derrière elle est `inert` — ce qui n'est pas affiché n'est pas lu, et regarder trois photos ne coûte plus six écritures d'accusé sur la 3G rurale. Le poids du document dans cet état est MESURÉ et ratcheté (`budgets-mesures.json` › `documents_du_fil`), et ses vitals sont opposés aux plafonds de `/chats/*` (`e2e/visual/v3-fil-riche.spec.ts`).

12.10.2 — Le nombre de participants ne s'affiche pas dans une conversation à deux
**LA RÈGLE EST UN SEUIL, ET LE SEUIL EST UN SITE : `SEUIL_DU_COMPTE = 3` / `compteDeParticipants()` dans `apps/web-v3/lib/contenu/fil.ts`.** Elle s'écrit dans le sens de l'implémentation — la mention se rend **dès `membres >= 3`** — et non « masquée dès que `membres <= 2` » : deux formulations d'une même règle invitent un jour deux comparaisons différentes, et c'est la borne qui se perd. Le MOT vient de l'appelant (`CHATS.participants`, `FIL.participants`) ; seul le seuil est la règle. Un compte de deux personnes n'apprend rien à qui SAIT avec qui il parle, et il vole la place de ce qui, lui, change — la présence, la frappe, l'heure, l'aperçu ; la présence et la frappe gardent la leur.

*Ce paragraphe énumérait quatre LIGNES (`fil-vue.ts:168` et `:183`, `vue.ts:63` et `:114`). Une énumération de lignes se périme au premier découpage — les quatre étaient fausses trois commits plus tard —, et une énumération de sites porte toujours deux affirmations dont la seconde n'est presque jamais vérifiée : « ces sites appliquent la règle » et « ce sont les sites où elle s'applique » (leçon 261). Ce qui suit nomme donc le site UNIQUE, puis ses consommateurs, puis ce que la règle ne gouverne PAS.*

**Les consommateurs du seuil**, au 2026-09-03 (`grep -rn compteDeParticipants apps/web-v3/{app,lib}`) :

| site | ce qu'il rend | pourquoi il compte |
|---|---|---|
| `app/connecte/fil-vue.ts` (`sousTitreDuMembre`) | le sous-titre du fil — **une source, deux projections** : `sousTitre` (la description du document) et `sousTitreHtml` (l'en-tête) | deux projections d'une phrase, jamais deux compositions ; c'est la composition qui se partage, pas seulement le sous-helper |
| `app/connecte/liste-vue.ts` (`meta`) | la méta d'une ligne de `/chats` | |
| `app/connecte/vue.ts` (`carteDeFil`) | la méta d'une carte du tableau de bord — **seulement quand la conversation n'a encore rien dit** : la cible `home.png` met l'APERÇU du dernier message à cette place (§ 5.4), et la méta ne reparaît que faute d'aperçu | une carte réduite à son seul nom n'est pas un état |
| `lib/realtime/liste-peinture.ts` (`peinsLaMeta`) | **la ligne REPEINTE EN DIRECT** sur `conversation:unread-updated` / sourdine | *manquait à l'énumération d'origine* : sans lui, « 2 participants » revenait à la première mise à jour temps réel, sur un écran que le lecteur regarde |

**Ce que la règle ne gouverne PAS.** `app/(public)/chat/[lien]/choix-vue.ts` (`CHOIX.participants`) compose la même phrase — et **doit** la composer sans seuil, y compris à un et à deux. Ce n'est pas la méta d'une conversation qu'on connaît : c'est l'accordéon « Ce que ce lien vous ouvre », servi à quelqu'un qui n'est PAS encore entré et à qui personne n'a dit qui l'attend. La raison d'être du seuil — « on sait avec qui l'on parle » — y est fausse par construction, et entrer fait de toute façon un participant de plus. Deux phrases identiques, deux règles : la ressemblance est ce qui rend l'erreur facile, elle n'est pas un argument.

**Témoins.** Unitaires : `__tests__/connecte.test.ts` (la carte), `__tests__/chats.test.ts` (la ligne servie), `__tests__/liste-vivante.test.ts` (la ligne REPEINTE) et `__tests__/fil-plein.test.ts` § « le nombre de participants ne s'affiche pas dans une conversation à deux (§ 12.10.2) » ; au pixel : `e2e/visual/v3-chats.spec.ts` (la ligne) et `e2e/visual/v3-fil.spec.ts` (l'en-tête). **Chaque témoin garde les DEUX moitiés du seuil** — le silence à deux ET la mention à trois : un témoin qui n'exigerait que le silence resterait vert si la mention disparaissait pour tout le monde, ce qu'obtient mécaniquement une comparaison `>` là où la règle dit `>=`.

12.10.3 — Le profil d'un participant s'ouvre en MODALE
Nouvel écran, hors matrice actuelle : une surimpression (`<dialog>` natif comme `sheet:member` et `sheet:link`), atteignable depuis l'avatar ou le nom d'un participant dans `thread`/`rich`/`chats`. Ce n'est PAS `sheet:member` (§ ligne 844 de ce document : l'espace membre remplace la barre d'onglets absente et navigue vers SON PROPRE compte) — deux besoins distincts, deux surimpressions. À faire entrer dans `MeeshyWebV3.dc.html`, `matrice.json` et `cible/` par la phase Concevoir avant tout code (§ 10.4).

**FAIT le 2026-09-03 par la phase Concevoir — et ce que cela a tranché.** L'écran est entré dans les trois instruments : la planche le dessine (`MeeshyWebV3.dc.html`, groupe « MEMBRE — PRINCIPAL », glyphe `ph-user` — l'un des 72 du sprite, aucun 73e), `capture-cibles.js` l'a rendu (`cible/profilMembre.png`, `vues.json`/`vues.md` à 39 vues), `matrice.json` en porte la ligne (47 écrans, lot **L5**, `P1-role-secondaire`, `depend_de: [thread, rich, chats]`) et `ordre-des-ecrans.js` rend `rc=0`. Cinq décisions ont dû être prises pour cela, chacune parce que le paragraphe ci-dessus ne les tranchait pas :

1. **L'identifiant est `profilMembre`, pas `sheet:profil-membre`.** Une vue qui reçoit une CAPTURE est nécessairement une entrée du navigateur de la planche, et `capture-cibles.js` en lit les clés avec `/\['([A-Za-z]+)',\s*'ph-/g` : un `:` ou un `-` dans la clé fait diverger `KEYS.length` de `nodes.length` et le harnais échoue (« Désaccord planche/navigateur »), avant même que le nom du fichier de capture ne porte un `:` — un caractère qu'un dépôt partagé entre trois systèmes de fichiers n'a pas le droit de fabriquer. Le préfixe `sheet:` reste donc ce qu'il a toujours désigné dans `matrice.json` : les cinq surfaces **`hors_planche: true`**, exigées par la mission et NON dessinées (`sheet:lang`, `sheet:link`, `sheet:conv`, `sheet:attach`, `sheet:member`). Celle-ci est dessinée : elle n'en est pas une.
2. **Ce n'est pas une adresse, c'est un ÉTAT de l'écran hôte** — `/chats/:cle?profil=:membre` (membre), `/chat/:lien?profil=:membre` (invité, même module de vue, § 12.3), `/chats?profil=:membre` (la liste). C'est ce qui permet d'ouvrir et de fermer par un simple `<a href>` — ouvrir pose le paramètre, fermer rend l'adresse de l'hôte — donc **sans un octet de JavaScript** (§ 12.10.6), le `<dialog>` natif portant Échap, le voile et le focus. Une route à elle (`/profil/:handle`) aurait quitté le fil, c'est-à-dire perdu ce que la modale existe pour préserver.
3. **Le fond n'est pas FLOUTÉ.** Le flou de l'état CHOIX (`join`) a une raison qui n'existe pas ici — rien de la conversation ne doit partir avant le choix ; le lecteur qui ouvre un profil EST déjà dans la conversation. Un voile suffit (charte : ce qui flotte se distingue par un PLAN, pas par un effet), et un `filter: blur` de plus coûterait du GPU sans rien protéger.
4. **Le panneau ne rend que ce que la passerelle SERT.** `GET /api/v1/directory/people/:handle?expand=relation` (`services/gateway/src/routes/directory/person.ts:175`, `onRequest:[getOptionalAuth]` — un invité anonyme y a droit, seaux 240/min connecté et 60/min anonyme `:160-173`) rend `publicProfileSchema` (`routes/users/public-profile.ts:88-110`) plus `relation ∈ {none, self, friend, pending_sent, pending_received}` et `isSelf` (`person.ts:72-94`). Deux absences sont des RÈGLES, pas des oublis de dessin : **aucune langue** — #4161 les a retirées de la projection, donc la ligne de langue du panneau vient du FIL (langue d'origine du message lu) et le DIT ; **aucune présence** — `person.ts:276-279` retire les champs de présence sans `expand=presence`, et `gateProfilePresence` (`routes/users/presence-gate.ts:113-122`) les retire de toute façon hors amitié acceptée / soi / ADMIN+ (directive 2026-08-25). La capture ne porte donc AUCUN point de présence : le vide est la règle rendue visible.
5. **Trois actions, trois routes réelles, aucune inerte.** « Écrire » ⇒ `POST /api/v1/conversations` (`type:'direct'`, `routes/conversations/core-lifecycle.ts:73`, qui RÉUTILISE la conversation directe existante `:219`) ; « Ajouter en ami » ⇒ `POST /api/v1/directory/friend-requests` (`friend-requests.ts:289`) ; « Bloquer ou signaler » ⇒ `PUT /api/v1/directory/blocks/:userId` (`blocks.ts:301`). Les deux dernières sont authentifiées : un lecteur ANONYME ne les voit pas — un contrôle qui rendrait 401 est un contrôle sans effet.

**Ce que cet écran ne change PAS** : aucun motif de `budgets.json` n'est ajouté ni relâché. La surimpression vit sous `/chats/*` et `/chat/*`, dont les plafonds (4 requêtes avant le premier pixel, LCP ≤ 2,2 s, CLS ≤ 0,05) lui sont opposés **tels quels** — c'est la conséquence directe du point 2 : un état de la même adresse est servi par le même document. Et la sortie du fil vers l'annuaire (`contacts`, « nom de l'auteur ») est REMPLACÉE par elle dans la planche : ouvrir le profil d'une personne n'est pas ouvrir la liste de ses contacts à soi.
...
| Occurrences de classes d'icône dans la planche | **90** (2026-09-03 ; 85 avant l'écran `profilMembre`, et **76** à la dernière écriture de cette ligne — la valeur avait dérivé de deux tours de planche avant celui-ci) | `grep -o 'ph ph-[a-z0-9-]*\|ph-fill ph-[a-z0-9-]*' docs/product/MeeshyWebV3Design/MeeshyWebV3.dc.html \| wc -l` |
...
| `<div>` dans la planche | **397** (2026-09-03 ; 378 avant l'écran `profilMembre`, **350** à la dernière écriture de cette ligne) — le fait qu'elle porte demeure : la planche n'a QUE des `div`, elle ne fait donc pas foi sur le balisage sémantique (charte règle 5) | `grep -o '<div' docs/product/MeeshyWebV3Design/MeeshyWebV3.dc.html \| wc -l` |
...
| Modules du temps réel de participation, brut / gzip (2026-09-04) | `participate.js` **89 744 / 26 666** · `liste.js` **49 995 / 14 010** · `feed.js` **34 561 / 8 218** · `notifs.js` **41 676 / 10 979** · `contacts.js` **8 087 / 3 382** · `recherche.js` **1 181 / 638** · `liens.js` **2 584 / 1 296** · `commentaires.js` **2 553 / 1 340** · `navigateur.js` **3 095 / 1 402** · `socket.io.esm.min.js` **40 435 / 12 849** — écart avec la ligne « socket.io-client pèse 26 549 o gzip » du § 11 question 13 : ce tour mesure `socket.io.esm.min.js` (le vendor SEUL, 12 849 o) et `participate.js` (le module + son import du vendor, 26 666 o) séparément ; les deux chiffres cohabitent, ils ne se contredisent pas | `cd apps/web-v3 && bun run build`, sortie du gate `check-bundle-budget.mjs` |
| Gestionnaires de route App Router émis (2026-09-04) | **35** (dont `/(public)/chat/[lien]/route`, `/chats/route`, `/chats/[cle]/route`, `/chats/[cle]/medias/route`), chacun **202 o / 101 kB First Load JS partagé**, **0 page** | `cd apps/web-v3 && bun run build` |
| Suite jest (2026-09-04) | **110 suites / 2 260 tests**, verts, 32,258 s — en hausse depuis les 103/103 suites, 2 166/2 166 tests du cadrage de ce tour (d'autres commits de la même branche intégrés entre-temps) | `cd apps/web-v3 && bun run test` |
| Gate a11y (`test:a11y`, 2026-09-04) | **7/7**, 0 violation axe serious/critical sur `/` dans les 4 thèmes (system-light, system-dark, explicit-light-on-dark, explicit-dark-on-light) | `cd apps/web-v3 && bun run test:a11y` |
| Gate cycle de vie (`test:lifecycle`, 2026-09-04) | **15/15**, 52,0 s — les 6 cas C→H de `thread` (retour d'arrière-plan, hors-ligne+sync, deux onglets, place fermée, lien désactivé, fermeture d'onglet) + 2 anti-régressions | `cd apps/web-v3 && bun run test:lifecycle` |
| `compare-rendu.js` sur `vitrine` et `home` | **NON EXÉCUTABLE (rc=3)** au 2026-09-04 : route `/` partagée sans état de session déclaré (§ 12.8, « `vitrine` et `home` partagent `/` — et ce n'est PAS le même problème ») ; les deux écrans en tête du focus porteur n'ont donc AUCUN gate de conformité structurelle sur le dégradé et le contour livrés ce tour — à lever avant de déclarer la charte du tour 3 conforme sur ces deux écrans | `cd apps/web-v3 && node docs/product/MeeshyWebV3Design/compare-rendu.js --base <serveur> --vues vitrine,home` |

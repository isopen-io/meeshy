...
- [ ] **gwcontract-13** 🔧 (P2/S/f06) — Symétrique de gwcontract-05 côté SUPPRESSIONS : `deleteAllRead` (« supprimer toutes les lues ») n'émet que `notification:counts`, qui ne dit rien ici (seules des lignes DÉJÀ lues partent, `unread` est inchangé) — les lignes purgées restent listées sur les autres appareils. Le prédicat à diffuser est DÉJÀ écrit côté acteur (`useDeleteAllReadNotificationsMutation`, optimiste). Événement additif `notification:deleted-bulk { scope: {kind:'read'} }`, union discriminée à un membre par anticipation du geste jumeau (deps : aucune — gwcontract-05 livré fournit le patron).

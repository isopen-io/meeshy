...
- [x] **gwcontract-03** 🔧 (P1/M/f06) — Étoffer le select `/sync` (translations, attachments, champs Prisme). *Livré 2026-08-13 : `syncMessageSelect` (`Prisma.validator`, réutilise `attachmentMediaSelect` + `messageSenderUserSelect`) et `SYNC_MESSAGE_RENDERABLE_KEYS` comme contrat opposé au select réel par un témoin de forme. `messageSenderUserSelect` est extrait dans `conversations/utils/message-sender-select.ts` — importer `conversations/messages.ts` depuis `sync.ts` aurait traîné un module de routes entier dans les doubles jest voisins (même raison que `active-member-count.ts`). Stream `deleted` laissé maigre, cap 1000 et keyset inchangés (témoins de non-régression).* **Suite livrée 2026-08-13 (cycle 113)** — le select enrichi avait retiré au cap 1000 sa propriété de plafond de POIDS (une ligne rendable porte des blobs JSON dimensionnés par l'utilisateur : `translations`, `metadata`, `transcription` des pièces jointes) : le stream `changed` porte désormais `SYNC_MAX_PAGE_BYTES` (512 Ko) comme SECOND critère d'arrêt, servi par le mécanisme déjà en place (`truncated` + `nextCursor` + watermark tenu à `since`). Invariants : page = préfixe du keyset, au moins une ligne toujours servie (sinon un message plus lourd que le budget boucle indéfiniment sur un curseur inchangé), curseur ancré sur la dernière ligne LIVRÉE. Tombstones exemptés (trois scalaires par ligne). Le cap 1000 et le keyset restent intacts — conformément à l'étape 3 de la fiche.
...
- [x] **gwcontract-04** 🔧 (P1/M/f06) — Tombstones dans le delta conversations (fermetures, delete-for-me, exclusions). *Gateway + client WEB livrés (2026-08-13) : `routes/conversations/utils/delta-tombstones.ts` → `meta.deletedConversationIds` / `…Truncated`, consommés par `mergeConversationDelta({ tombstoneIds })` et `useConversationsDeltaSync` (purge + escalade sur troncature). **Reste ouvert : le client iOS** (`ConversationSyncEngine.deltaSyncCore`, étape 7 de la fiche) — non livrable depuis un runner Linux, aucun gate `meeshy.sh build`/XCTest exécutable. Écart assumé vs l'étape 5 de la fiche : un seul index ajouté (`Conversation @@index([closedAt])`, le seul stream qui ne parte pas d'un `userId` déjà indexé) ; rationale dans le module.*
...
- [x] **gwcontract-09** 🔧 (P2/M/f06) — Ouvrir `/sync` aux sessions anonymes avec scoping strict (la fiche décrit le correctif sûr — le naïf crasherait). *Livré 2026-08-13 (cycle 115), gateway complet. Trois écarts VÉRIFIÉS par rapport à la fiche, chacun documenté dans le code :* **(a)** *le crash annoncé sur `currentSeq` n'existe pas — `authContext.userId` d'un anonyme porte le `Participant.id` (`middleware/auth.ts`), un ObjectId valide, pas le sessionToken ; la garde-pattern `/^[0-9a-f]{24}$/` aurait été du code mort. Le court-circuit `checkpointSeq = 0` est conservé pour une autre raison : `UserEventSeq` est indexée par `User.id`, la question n'a pas de réponse à poser.* **(b)** *le clamp d'historique de l'étape 5 (« `sinceDate = max(sinceDate, joinedAt)` ») N'AURAIT PAS FERMÉ LA FUITE : un message créé avant la jointure et RÉÉDITÉ depuis porte un `updatedAt` récent et serait revenu par le stream `changed` avec tout son contenu. La borne est posée sur `createdAt >= joinedAt` — la colonne exacte de `messages.ts` — sur les DEUX streams (une tombstone atteste qu'un message a existé).* **(c)** *la règle est portée par la LIGNE PARTICIPANT, pas par le type d'identité : un utilisateur INSCRIT entré par un lien `allowViewHistory:false` avait le même trou dans `/sync`, il est fermé par le même mécanisme (`services/shareLinkHistoryFloor.ts`). Identité modélisée en union discriminée `SyncIdentity` pour rendre la confusion `User.id`/`Participant.id` inexprimable. Reste ouvert : `messages.ts` continue de porter sa propre copie inline de la règle (il y ajoute les 403 expiry/maxUses) — convergence à instruire, cf. fiche ci-dessous.*

- [x] **gwcontract-14** 🔧 (P3/S/f06) — *(renommé au cycle 116 : ce numéro entrait en collision avec `gwcontract-12-message-consumed-literal`, qui préexistait dans le fichier `06`)* Faire converger `GET /conversations/:id/messages` sur `services/shareLinkHistoryFloor.ts` : la règle « `allowViewHistory:false` ⇒ `createdAt >= joinedAt` » vit désormais à DEUX endroits (le module, et la copie inline de `messages.ts:622-641` qui y ajoute les 403 `SHARE_LINK_EXPIRED`/`SHARE_LINK_MAX_USES`). Deux lecteurs d'une même règle : c'est la famille de défauts que les cycles 105-111 documentent. Le module rend un FILTRE, la route rend en plus une DÉCISION DE RÉPONSE — la convergence demande de séparer les deux, pas de les empiler (deps : gwcontract-09). *Livré 2026-08-14 (cycle 118). La séparation prescrite est tenue en rendant la règle PURE (`historyFloorFor(join, link)`) et en la laissant SANS chargeur imposé : `/sync` lit tous les liens d'un coup, `messages.ts` lit LE lien avec les colonnes de la porte (`expiresAt`, `maxUses`) dont il a besoin par ailleurs. Un chargeur commun aurait ajouté un aller-retour à l'un des deux ; la règle partagée n'en ajoute aucun. La porte (403) reste entièrement dans la route. Écart vs la fiche : la convergence a fait apparaître un TROISIÈME lecteur non recensé, et celui-là fuyait — cf. gwcontract-15.*

- [x] **gwcontract-15** 🔒 (P1/M/f06) — **La galerie de médias ne bornait rien.** `GET /conversations/:id/attachments` (`routes/attachments/metadata.ts`) est un SECOND LECTEUR des messages de la conversation — il en sert les pièces jointes avec `fileUrl`, `originalName` et la transcription des audios — et il n'honorait AUCUNE des trois exclusions que le premier lecteur applique. *Livré 2026-08-14 (cycle 118), avec gwcontract-14 (même chantier : c'est la convergence qui l'a fait apparaître).* Trois défauts, dont un de confidentialité :
  - **(a) Fuite d'avant-jointure.** La branche « membre enregistré » ne vérifiait que l'ADHÉSION. Or un utilisateur INSCRIT entré par un lien `allowViewHistory:false` porte bien un `participant.shareLinkId` (`routes/conversations/sharing.ts:620`, `type: 'user'`) — il obtenait la galerie ENTIÈRE de la conversation, tout l'avant-jointure compris, là où `GET messages` la lui bornait. Exactement le trou que gwcontract-09 (c) a fermé pour `/sync`, dans une route que personne n'avait rouverte depuis.
  - **(b) Refus là où il fallait borner.** La branche anonyme, elle, rendait **403 sur toute la galerie** quand le lien fermait l'historique — alors que ce même participant voit dans `GET messages` les messages postés depuis son arrivée, pièces jointes comprises. Le plancher RÉTRÉCIT une lecture, il ne la refuse pas. Les deux branches se trompaient donc en sens opposés.
  - **(c) Deux colonnes pour un même fait.** Cette branche lisait `participant.anonymousSession.shareLinkId`, la copie embarquée, quand `messages.ts`, `/sync` et le module lisent la colonne `participant.shareLinkId`. Les deux sont écrites à la jointure anonyme (`routes/anonymous.ts:398` et `:410`), mais seule la colonne existe pour un inscrit — c'est cette lecture-là qui rendait (a) invisible.
  - **En plus, hors lien de partage** : ni la tombstone `deletedAt` (un média restait listé, URL comprise, après suppression POUR TOUS du message qui le portait) ni le masquage personnel (`clear-history`, `delete-for-me`) n'étaient appliqués. Les deux le sont désormais, la tombstone dans le service (invariant qui ne dépend d'aucun lecteur), le reste via `messageFilter`.
  - **Forme** : le service prend un `messageFilter` OPAQUE et pose ses deux invariants (`conversationId`, `deletedAt: null`) APRÈS le spread — un appelant ne peut qu'ajouter, jamais sortir de la conversation ni ressusciter une tombstone. Gelé par test.

- [x] **gwcontract-16** 🔒 (P1/M/f06) — **Le retrait de réaction annonçait un ❤️ qu'il n'avait pas retiré.** `DELETE /posts/:postId/like` diffusait `emoji: '❤️'` codé en dur sur ses trois branches : la route ne pouvait pas savoir quel emoji partait (seul `unlikePost` lit la ligne `PostReaction` avant de la supprimer), et le code le DISAIT déjà en commentaire sans le corriger. Pas latent : `StoryViewModel.applyStoryReactionDelta` retire `event.emoji` de `currentUserReactions` — un 😂 retiré n'y retirait rien, la puce survivait à sa suppression sur un compteur déjà décrémenté. Livré 2026-08-14 (cycle 119) : enveloppe `{ id, post, removedEmoji }` rendue par `unlikePost`, emoji véridique sur les trois branches, **rien retiré ⇒ rien annoncé** (y compris sur le rejeu `onDuplicate`, ce que le commentaire de la route promettait à tort), et paire absolue `likeCount`/`reactionSummary` sur les quatre événements story/status (volet gateway de rts-03, élargi). Web : le tray stories écrit l'absolu au lieu de ne rien faire. **Le client iOS devient correct sans changement Swift.**

## Lot 8 — Notifications multi-device

- [x] **gwcontract-05** 🔧 (P2/S/f06) — Émettre `notification:read`/`notification:deleted` (listeners iOS déjà câblés). *Unitaires livrés avant ce lot (`3152326c`). **Bulk livré 2026-08-13 (cycle 116) : `notification:read-bulk`, gateway + client WEB.** Les 4 chemins de marquage en masse (`markAllAsRead`, les 3 clés de `markContextNotificationsAsRead`, `markNotificationsByTypesAsRead`) annoncent le PRÉDICAT appliqué — ni `updateMany` ni `$runCommandRaw` ne renvoient d'ids à égrener. Trois écarts assumés vs la fiche, détaillés dans `06` : **(a)** union DISCRIMINÉE `NotificationReadBulkScope` et non le sac d'options prescrit — ce dernier n'a pas de place pour `friendRequestId`, que la fiche a omis alors que la gateway marque en masse dessus, et un client l'aurait ignoré EN SILENCE ; **(b)** pas de `count` dans le payload (un cache client partiel matche moins de lignes que le serveur — `notification:counts` reste seul autoritaire, et le handler web ne touche pas le badge) ; **(c)** prédicat énoncé UNE fois dans `packages/shared/utils/notification-read-bulk.ts`, importé par le web, un `kind` inconnu ne matchant rien. **Reste ouvert : le volet iOS** (miroir Swift du prédicat) — non livrable depuis un runner Linux. Écart voisin identifié non traité : `deleteAllRead` a le même défaut côté suppressions (il demanderait un `notification:deleted-bulk`).*
...
- [x] **gwcontract-13** 🔧 (P2/S/f06) — Symétrique de gwcontract-05 côté SUPPRESSIONS. **Livré 2026-08-13 (cycle 117) : `notification:deleted-bulk`, gateway + shared + client WEB.** `deleteAllRead` annonce désormais le prédicat `{kind:'read'}` vers `ROOMS.user`, en PLAIN `io.to(...)`, quand `count > 0`. Trois points tenus tels que la fiche les prescrit : **(a)** union discriminée à UN membre (le geste jumeau « tout supprimer » ajoutera un `kind`, pas une clé optionnelle) ; **(b)** aucun `count` et aucun décrément côté client — ici ce n'est plus une précaution comme pour le read-bulk mais une CONSÉQUENCE du prédicat, toute ligne retirée étant lue donc jamais comptée ; **(c)** aucun `notification:deleted` égrené (la purge n'est pas bornée). Prédicat énoncé UNE fois dans `packages/shared/utils/notification-read-bulk.ts` (`notificationMatchesDeletedBulkScope`), un `kind` inconnu ne matchant rien. **Reste ouvert : le volet iOS** (miroir Swift du prédicat) — non livrable depuis un runner Linux, et à faire dans le même chantier Swift que celui de gwcontract-05.
- [~] **gwcontract-06** 🔧 (P2/M/f06) — Delta/cursor sur `GET /notifications` (+ prérequis tombstones : soft-delete) (deps : gwcontract-05). *Volet CURSEUR livré 2026-08-14 (cycle 122) : gateway + web + SDK. Le défaut était ACTIF sur le web — `useInfiniteNotificationsQuery` paginait par rang pendant que le socket insérait en tête de la page 0 : chaque notification reçue entre deux pages faisait re-servir la dernière ligne déjà vue et sauter la première jamais vue. Keyset `(createdAt, id)`, ordre total dans les deux modes, `count()` retiré du chemin sous curseur, `nextCursor` rendu aussi en mode offset (bascule sans page redemandée), repli offset explicite tant que la gateway ne rend pas de curseur. `encodeCursor`/`decodeCursor` déplacés dans `utils/keyset-cursor.ts` (+ `keysetBeforeClause`), ré-exportés par `routes/posts/types.ts`. Reste ouvert : `updatedSince` + index `[userId, readAt]` (leur consommateur est le client delta iOS — un paramètre sans lecteur sinon) et les tombstones (hard delete, cf. fiche).*
...
- [~] **rts-03** 🔧 (P2/M/f03) — Statuses : persister les événements socket au cache + payload `status:unreacted` corrigé côté gateway. *Volet gateway + shared livré 2026-08-14 (cycle 119), élargi à STORY et à `post:unliked` — cf. **gwcontract-16** ci-dessous, qui porte le défaut réel que cette fiche n'avait vu que comme une contrainte à contourner. Reste ouvert : le volet iOS (persistance des 4 sinks, sink `statusUnreacted`, champs SDK).*
...
- [x] **gwcontract-11** 🔧 (P3/S/f06) — Troncature des tombstones stories signalée au client (flag `truncated`) — à fiabiliser AVANT gwcontract-08 (pattern `take LIMIT+1`). *Livré 2026-08-11 (cycle 80) — la fiche le consigne depuis, la case était restée ouverte ; vérifié dans le code au cycle 122 (`PostFeedService.ts:476-490`, `routes/posts/feed.ts:116`). gwcontract-08 n'est donc plus bloqué.*

# Iteration-213i — `MessageViewsDetailView` empty-state i18n
...
- **`branch-tracking.md` intentionally NOT modified** in this PR: the shared append-target
  is a merge-conflict magnet (every iteration appends to its tail), and an append there is
  what auto-closed the first attempt (#2240) on conflict when main advanced. This iteration
  is instead recorded solely by this analysis file (a new path → no conflict). Renumbered
  208i→**213i** after rebase because main had meanwhile merged its own 208i (up to 212i).

...
- `resolvePlaybackUrl` elle-même NE CHANGE PAS de comportement — seul son paramètre `isUserSelected` est renommé `hasExplicitLanguage` (cohérence avec l'état renommé). Les 4 tests purs existants (`test_autoSeededLanguage_isUserSelectedFalse_returnsOriginal`, `test_userSelectedLanguage_withMatch_returnsTranslatedUrl`, `test_userSelectedOrig_returnsOriginal`, `test_userSelectedLanguage_withoutMatch_returnsOriginal`) gardent leurs assertions **telles quelles** — seul le nom du paramètre à l'appel change. Le premier reste correct : passer explicitement `hasExplicitLanguage: false` à la fonction pure doit toujours retourner l'original, quel que soit le contexte.
- Seul `test_init_neverMarksLanguageAsUserSelected` a un VRAI changement de comportement à couvrir : c'est lui qui vérifie ce que l'`init` fait du flag, et c'est l'`init` qui change (le fichier actuel verrouille l'ancien comportement, « B9 finding », commentaires `:5-14` — à mettre à jour pour documenter le nouveau contrat, pas juste patcher). Renommer en `test_init_marksLanguageAsExplicitWhenPrismeResolvesATranslation` et inverser son attente : `hasExplicitAudioLanguage == true` quand `initialTranscriptionLanguage` résout une vraie langue.
- Ajouter un nouveau cas côté `init` : seed avec `initialTranscriptionLanguage = nil` (ou résolvant à `"orig"`) → `hasExplicitAudioLanguage` reste `false`, lecture sur l'original (non-régression du cas « pas de traduction disponible »).

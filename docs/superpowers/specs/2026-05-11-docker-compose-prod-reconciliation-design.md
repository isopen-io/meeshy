...
- ~~Symlinks racines cassés (`docker-compose.yml`, `docker-compose.monorepo.yml`, `docker-compose.local-https.yml`, `traefik-dynamic.yml`)~~ — **SOLDÉ.** Les quatre sont traités, et pas de la même façon, parce que la mesure les sépare en deux familles.
  Les TROIS premiers étaient nés morts : aucun commit du dépôt n'a jamais écrit leur cible. #4544 les a remplacés par des fichiers qui REFUSENT en nommant la suite (`${VAR:?message}`), parce qu'un `docker compose up` à la racine est un geste que la convention rend possible et qu'il valait mieux l'arrêter en disant où regarder.
  Le QUATRIÈME n'était pas de cette famille et le traitement ne s'y appliquait pas (#4548). Mesuré : sa cible a bel et bien existé (`infrastructure/docker/compose/traefik-dynamic.yml`, 48 lignes, créée le 2025-12-10 par `f093fdd2be`) ; elle a été RENOMMÉE le 2026-01-06 par `d6ff8add90` en `infrastructure/docker/compose/config/dynamic.yaml`, sans que le lien de racine suive. Le lien a donc été cassé par un DÉPLACEMENT, pas par une naissance. Mesuré aussi : **une seule** référence à `traefik-dynamic` dans tout l'arbre de travail, celle de cette ligne — zéro consommateur (aucun montage, aucun label Traefik, aucun script, aucune cible de `Makefile`). Le successeur, lui, est câblé par SON chemin (`docker-compose.prod.yml` — `--providers.file.filename=/dynamic.yaml` et le montage `./config/dynamic.yaml`).
  Il a donc été SUPPRIMÉ, et non repointé ni remplacé par un refus. Repointer aurait donné un SECOND nom, d'extension différente, à un fichier qui en a déjà un et que personne n'appelle par celui-là — et ce nom aurait désigné une configuration de PRODUCTION alors qu'il nommait une configuration mkcert locale. Refuser n'était pas possible mécaniquement : le provider `file` de Traefik ne substitue aucune variable d'environnement, si bien qu'un fichier « qui refuse » se chargerait comme une configuration dynamique VIDE — `compress@file`, `secure-headers`, `rate-limit` et `cors-prod` disparaîtraient en silence. La seule issue qui n'ajoute ni mensonge ni panne est le retrait, avec cette ligne pour dire où le contenu est parti.

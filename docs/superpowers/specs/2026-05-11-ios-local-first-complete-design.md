...
- **Plafond de POIDS (cycle 113) :** le cap 1000 est un plafond de LIGNES, et il ne borne le poids d'une page que tant qu'une ligne pèse ses scalaires. Depuis que le select `messages` rend un message RENDABLE (`translations`, `metadata`, `reactionSummary`, `attachments` avec leurs propres `transcription`/`translations`), le poids d'une ligne est écrit par l'utilisateur et n'a plus de borne : une page de 1000 pouvait peser quelques Ko comme des dizaines de Mo, sur le canal appelé précisément au retour de veille en cellulaire. Le stream `changed` porte donc un SECOND critère d'arrêt, `SYNC_MAX_PAGE_BYTES` (512 Ko de JSON non compressé, `services/gateway/src/routes/sync.ts`), qui emprunte le mécanisme existant — `truncated: true` + `nextCursor` + watermark tenu à `since`. Trois invariants le rendent sûr : la page reste un **préfixe** du tri keyset (un retrait « au milieu » ferait un trou qu'aucune position ne saurait réclamer) ; **au moins une ligne est toujours servie**, sans quoi un message plus lourd à lui seul que le budget rendrait indéfiniment une page vide sur un curseur inchangé — un rattrapage qui ne progresse plus ; le curseur s'ancre sur la dernière ligne **livrée**, jamais sur la dernière ligne **lue** (une ligne écartée par le budget n'a pas été livrée, la borne serveur est stricte, la passer la perdrait). Les streams de tombstones (`deleted`, disparitions personnelles) en sont exemptés par NATURE et non par commodité : trois scalaires de taille fixe par ligne, donc leur plafond de lignes EST déjà un plafond de poids. Le budget n'a pas touché au cap 1000 ni au keyset — il s'y ajoute.
- **Pagination globale :** `hasMore` au top-level, client itère jusqu'à `false` avant de persister checkpoint
- **Watermark `checkpoint` (semantique serveur, cycle 98) :** le client renvoie `checkpoint` en `since` au tour suivant et la borne serveur est **stricte** (`updatedAt > since`) — un checkpoint trop avancé creuse donc un trou **définitif**, sans erreur ni log. Trois garanties, tenues côté SERVEUR et non par convention cliente : **(a)** il est ancré au DÉBUT du handler, jamais après les lectures ; **(b)** il est retiré de `SYNC_CHECKPOINT_LAG_MS` (5 s) parce que `@updatedAt` est estampillé par Prisma à la *construction* de l'écriture et non à son *commit* — une ligne estampillée T peut n'être visible qu'à T+δ — sans jamais reculer sous le `since` déjà acquitté ; **(c)** tant que `hasMore` est vrai, il reste **égal à `since`** : une page tronquée laisse un ARRIÉRÉ dont les `updatedAt` sont antérieurs au checkpoint, et un client qui l'adopterait au lieu de suivre `nextCursor` le perdrait en entier. Le coût est une relecture bornée, déduplicable par `id`. Règle jumelle côté client : `SyncWatermark.advancedAfterDeltaPage` (`packages/MeeshySDK/Sources/MeeshySDK/Models/SyncWatermark.swift`) — toute évolution touche les deux.

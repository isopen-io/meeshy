...
- [ ] **Step 3: Ajouter les bornes**

Dans `StoryMediaObjectSchema` (`types.ts`), juste après la ligne `duration: z.number().min(0).max(86400).optional(),` :
...
Le test existant `test_...` qui attend `startMs: 1500, endMs: 3750` (ligne ~53) encode l'**ancienne** sémantique : il doit être réécrit dans le même geste. Remplacer ses attentes par `startMs: 0, endMs: 2250` si son entrée est `{ startTime: 1.5, duration: 2.25 }` — vérifier l'entrée exacte dans le fichier avant d'éditer.
...
```bash
git add services/gateway/src/services/posts/captureTracks.ts services/gateway/src/services/posts/__tests__/captureTracks.test.ts
...
`Sound.waveform` est déclaré (`schema.prisma:3054`) et lu (`sounds.ts:56`) mais **n'a aucun écrivain** : il vaut `[]` pour 100 % de la bibliothèque en production. Le client publie pourtant ses échantillons dans `storyEffects.audioPlayerObjects[].waveformSamples` (plafonnés à 2048 par `types.ts:146`) — le serveur les jette.
...
Attendu : ÉCHEC de compilation TypeScript — `waveform` n'existe pas sur `CaptureTrack`.
...
describe('routes/posts/audio.ts — forme d\'onde', () => {
  ...
  it('test_audioRoute_readsWaveformField', () => {
    expect(code).toContain("data.fields['waveform']");
    expect(code).toContain('parseWaveformField');
  });

  it('test_audioRoute_writesWaveformOnSoundCreate', () => {
    // Le champ doit atteindre le `data:` du `sound.create` — pas seulement
    // être lu puis abandonné dans une variable.
    const createBlock = code.slice(code.indexOf('prisma.sound.create'));
    expect(createBlock).toContain('waveform');
  });
});
```
...
**Files:**
- Modify: `packages/shared/prisma/schema.prisma:3073-3088` (modèle `SoundUsage`)
- Modify: `services/gateway/src/services/posts/SoundCaptureService.ts:13-22` (`CaptureTrack`), `:430-440` (`recordUsage`)
- Test: `services/gateway/src/services/posts/__tests__/SoundCaptureService.test.ts`
...
Dans `SoundCaptureService.test.ts`, avec le montage réel du fichier (`buildPrisma`, puis `new SoundCaptureService(prisma, soundsDir, uploadsRoot)`). Le chemin du son **emprunté** (`soundId`) n'écrit pas de `Sound` : il ne fait qu'un `SoundUsage`, donc il faut que `sound.findFirst` rende un son existant, avec sa `durationMs`.

```ts
  const borrowedPrisma = (durationMs: number | null) => buildPrisma({
    sound: {
      findFirst: jest.fn<() => Promise<unknown>>().mockResolvedValue({ id: 's1', durationMs }),
      findMany: jest.fn<() => Promise<unknown[]>>().mockResolvedValue([]),
...
Attendu : ÉCHEC de compilation (`windowAdjusted` inconnu) puis d'assertion.
...
`recordUsage` doit disposer de `soundDurationMs`. Sur le chemin de la **capture** (tâche 4), la valeur vient d'être calculée (`media.duration ?? 0`) : la passer en argument. Sur le chemin du son **emprunté**, la lire dans la même requête que la vérification d'existence du son, en ajoutant `durationMs: true` à son `select` — **ne pas ajouter de requête**.

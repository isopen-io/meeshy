...
- [ ] **Step 1: Test rouge (source)** — le conteneur racine référence `Plan2DView` et ne référence plus la vue mono-piste remplacée ; `Views/Inspector` reste référencé (S4 : l'édition de keyframes n'a pas bougé) ; `ComposerControlsLayer` (possédé par personne d'autre en écriture ici : NE PAS le modifier — l'entrée `openTimeline` est inchangée, assertion négative sur tout diff hors Timeline/**).

...
| Last merged PR | iter-51i #651 ✅ ; iter-52w (ranking type labels) #765 ✅ ; iter-53w (v2 conv list) #766 ✅ ; iter-53wb (page reel deep-link) #764 ✅ ; iter-54w (page story deep-link) `claude/practical-fermat-dcmkus` ⏳ |
| Last Merged Base (commit) | `34585d5` (main HEAD post-#764) — base de l'iter-54w |
...
- **NOUVEAU (53w)** : cluster i18n/a11y v2 repéré par l'explorer, bornés, pour 54w+ : `components/v2/ReplyPreview.tsx` (`CONTENT_TYPE_LABELS` `📷 Photo`/`🎬 Vidéo` FR durs) ; `components/attachments/AttachmentDeleteDialog.tsx` (~5 chaînes dialogue confirm FR) ; `components/auth/PhoneExistsModal.tsx` (~8 chaînes FR + flow SMS) ; `components/v2/PostComposer.tsx` & `AudioPlayer.tsx` (`aria-label` statiques anglais non i18n)
...
| 54w | claude/practical-fermat-dcmkus (i18n page story deep-link) | ⏳ | ⏳ |

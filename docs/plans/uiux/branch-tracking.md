...
| ~~Last completed iteration (web) [56w]~~ | (web only : solde le reliquat borné du cluster 53w — i18n des **deux dialogues modaux FR figés**. (1) **`AttachmentDeleteDialog.tsx`** (aucun hook i18n) : ajout `useI18n('attachments')` + 6 `t()` ; nouveau bloc `attachments.deleteDialog` ×4 locales (`title`/`description`/`serverNote`/`cancel`/`deleting`/`delete`). (2) **`PhoneExistsModal.tsx`** (surface d'entrée critique — flow inscription ; hook `useI18n('auth')` déjà présent, toasts déjà i18n mais JSX FR figé) : 15 chaînes des 3 étapes (`choice`/`verify_code`/`success`) → `register.wizard.phoneExistsModal.*` ×4 locales (boutons isMyAccount/notMyAccount/transferNumber, séparateur `or`, hint SMS, titres+desc dont `enterCodeSentTo {phone}`, a11y sr-only codeInputLabel/codeInputHint, verify/resendCode/`resendIn {seconds}`/cancel, succès). Fallbacks EN en 2e arg pour chaînes simples (anti-flash, leçon 50w) ; 2 clés à paramètre sans fallback string (signature t() exclusive, parité ×4 garantit zéro flash). Parité vérifiée : 15 clés + 6 clés ×4 locales, JSON valide ×8. Docblock FR `PhoneExistsModal` l.58–60 = doc dev, hors périmètre. NE PLUS re-flagger ces 2 dialogues.) |
...
| Last merged PR | iter-55w (micro-surfaces FR) #769 ✅ ; iter-54wb (PostComposer/AudioPlayer) #768 ✅ ; iter-56w (dialogues AttachmentDelete + PhoneExists) #771 ✅ ; iter-57w (ReelPlayer i18n) `claude/practical-fermat-c6vris` ⏳ |
| Last Merged Base (commit) | `652cdf1` (main HEAD post-#771) — base de l'iter-57w |
| Next iteration | **58** — repartir de `main` HEAD post-merge iter-57w |
...
- **NOUVEAU (53w)** : surface **feed** globale non internationalisée (aucun `t()`) — passe i18n dédiée (large). ~~`ReelPlayer` (`components/feed/ReelPlayer.tsx`)~~ → **SOLDÉ en 57w** (12 clés `reel.player.*` ×4 ; `useI18n('reel')` ; fallbacks EN 2e arg ; aria + boutons visibles ; NE PLUS re-flagger). Reste : `ReelsFeedScreen.tsx` (~15 chaînes FR, borné — bon candidat 58w) ; `PostsFeedScreen.tsx` (~30, **large** + incohérence FR/EN à unifier : `'Updating...'`/`'Unable to load feed.'`/`'No posts yet…'` EN durs vs toasts FR).
...
| 56w | claude/practical-fermat-y3ci31-56w (i18n dialogues AttachmentDelete + PhoneExists) | #771 | ✅ |
| 57w | claude/practical-fermat-c6vris (i18n ReelPlayer feed) | ⏳ | ⏳ |

...
| Last completed iteration (web) | **57w** (web only : i18n + a11y du lecteur de reels plein écran `components/feed/ReelPlayer.tsx` (partagé `/feed/reels` + page deep-link `/reel/:id`). Le composant n'avait **aucun hook i18n** : 12 chaînes **FR figées** affichées en TOUTES langues, majoritairement des `aria-label`/`sr-only` → **régression a11y** pour lecteurs d'écran non francophones + rupture Prisme. Migrées vers le namespace **`reel` existant** (créé 53wb) sous un bloc imbriqué `player.*` (13 clés ×4 locales) : `counter {current,total}` (section), `srHeading {name}` (h1 sr-only), `mediaAlt {name}` (fallback alt/aria vidéo+image — la priorité `media.alt` auteur est conservée), `play`/`close`/`previous`/`next`/`mute`/`unmute` (boutons chrome), `like`/`comment`/`share`/`save` (RailButton). Fallbacks EN 2e arg pour les simples (anti-flash, leçon 50w) ; les 3 clés à paramètre sans fallback string (parité ×4, aucun flash). `Meeshy` (nom auteur défaut) = marque, conservé. NE PLUS re-flagger `ReelPlayer.tsx` ni `reel.player.*`. Toutes les surfaces de lecture de reels sont désormais internationalisées.) |
...
| Last merged PR | iter-55w (micro-surfaces FR) #769 ✅ ; iter-54wb (PostComposer/AudioPlayer) #768 ✅ ; iter-56w (dialogues AttachmentDelete + PhoneExists) #771 ✅ ; iter-57w (i18n/a11y ReelPlayer) `claude/practical-fermat-r4vwgd` ⏳ |
| Last Merged Base (commit) | `db1f1fd` (main HEAD post-#771) — base de l'iter-57w |
...
- ~~**NOUVEAU (53w)** : `ReelPlayer` (`components/feed/ReelPlayer.tsx`) non internationalisé (aucun `t()`)~~ → **SOLDÉ en 57w** (13 clés `reel.player.*` ×4 ; bloc imbriqué dans le namespace `reel` existant ; majorité = a11y `aria-label`/`sr-only`). NE PLUS re-flagger `ReelPlayer.tsx`. **Reste (58w+)** : surface **feed** globale `components/feed/PostsFeedScreen.tsx` (29 ko) — audit i18n ciblé, à découper par sous-bloc.
...
| 56w | (i18n dialogues AttachmentDelete + PhoneExists) | #771 | ✅ |
| 57w | claude/practical-fermat-r4vwgd (i18n/a11y ReelPlayer) | ⏳ | ⏳ |

...
| Last completed iteration (web) | **54wb** (web only : solde le cluster i18n/a11y des building blocks design-system **v2** repéré en 53w. (1) **`PostComposer.tsx`** (surface visible — montée dans `PostsFeedScreen`) : `VISIBILITY_OPTIONS[].label` EN durs (`Public`/`Friends`/`Friends except...`/`Only...`/`Private`) → `labelKey` + `t('postComposer.visibility.*')` ; aria-labels `Post content`/`Add photo`/`Add video`/`Change visibility` → `t('postComposer.{contentLabel,addPhoto,addVideo,changeVisibility}')` ; bouton `Publish` → `t('publish')`. (2) **`ReplyPreview.tsx`** : `CONTENT_TYPE_LABELS` FR durs mixés `📷 Photo`/`🎤 Audio`/`🎬 Vidéo` → `CONTENT_TYPE_META {emoji,key}` + `t('replyPreview.{image,audio,video}')` (emoji hors i18n). (3) **`AudioPlayer.tsx`** : `useI18n('common')` ; aria-labels `Pause`/`Play`/`Audio progress` → `t()` ; garde FR `Audio URL non disponible` → `t('audioUnavailable')`. 15 clés `common.{postComposer.*,replyPreview.*,audioProgress,audioUnavailable}` × 4 locales, parité 14/14 ; `tsc` 0 erreur sur les 3 fichiers. **Numérotée 54wb** : collision avec une 54w parallèle (page story #767) — périmètres disjoints, convention 49w/49wb. NE PLUS re-flagger ces 3 fichiers v2 ni ces clés.) |
...
| Last merged PR | iter-53wb (page reel) #764 ✅ ; iter-54w (page story) #767 ✅ ; iter-54wb (v2 composer/reply/audio) #768 ⏳ |
| Last Merged Base (commit) | `bc9eae5` (main HEAD post-#767) — base de l'iter-54wb |
...
- **NOUVEAU (53w)** : fallbacks Suspense FR durs **restants** (story SOLDÉ en 54w) — `app/groups/page.tsx:12` + `app/groups/[identifier]/page.tsx:18` (`Chargement des groupes…`/`…du groupe…`, clés `groups.*` existent, pages `'use client'`) ; `app/auth/verify-phone/page.tsx` + `verify-email/page.tsx` (`LoadingFallback` « Chargement… », pages `'use client'`). Lot i18n borné pour 55+
...
| 54wb | claude/practical-fermat-indb44 (i18n/a11y v2 composer/reply/audio) | #768 | ⏳ |

...
| Last completed iteration (web) | **57w** (web only : i18n complète du lecteur de reels immersif `components/feed/ReelPlayer.tsx` — surface de **contenu** partagée (feed/reels + deep-link reel) qui n'avait **aucun** hook i18n : 13 chaînes FR figées en TOUTES langues, majoritairement `aria-label`/`sr-only` (lues par les lecteurs d'écran dans la mauvaise langue). Hook `useI18n('reel')` ; namespace `reel.json` existant (53wb) étendu d'un **sous-bloc `player`** (12 clés ×4) plutôt qu'un nouveau fichier. Swaps : `position {index,total}` (section), `reelBy {name}` factorisé en variable (h1 sr-only + alt vidéo + alt image), `play`/`close`/`previous`/`next`, `unmute`/`mute` (toggle son), rail `like`/`comment`/`share`/`save`. Fallbacks EN en 2e arg pour les 10 chaînes simples (anti-flash, leçon 50w) ; 2 clés à paramètre sans fallback string (signature t() exclusive, parité ×4 = zéro flash). Parité iOS : `ReelsPlayerView`/`FeedCommentsSheet` localisent toute leur a11y via `String(localized:)` — le web était le seul frontend à laisser ces labels FR bruts. Parité vérifiée : 12 clés ×4, JSON valide ×4. NE PLUS re-flagger `ReelPlayer.tsx`.) |
...
| Last merged PR | iter-55w (micro-surfaces FR) #769 ✅ ; iter-54wb (PostComposer/AudioPlayer) #768 ✅ ; iter-56w (dialogues AttachmentDelete + PhoneExists) #771 ✅ ; iter-57w (ReelPlayer i18n) `claude/practical-fermat-1bs0bb` ⏳ |
| Last Merged Base (commit) | `f8c91d6` (main HEAD post-#771 iter-56w + #675 dependabot) — base de l'iter-57w |
...
- ~~**NOUVEAU (53w)** : `ReelPlayer` (`components/feed/ReelPlayer.tsx`) non internationalisé (aucun `t()`)~~ → **SOLDÉ en 57w** (`useI18n('reel')` + sous-bloc `reel.player` 12 clés ×4 ; 13 chaînes a11y/sr-only/rail migrées ; parité iOS `ReelsPlayerView`). NE PLUS re-flagger `ReelPlayer.tsx`. **Reste pour 58w+** : conteneur **feed** global (`ReelsFeedScreen`/`PostsFeedScreen`) — chaînes restantes éventuelles.
...
| 57w | claude/practical-fermat-1bs0bb (i18n ReelPlayer immersif) | ⏳ | ⏳ |

...
| Last completed iteration (web) | **56w** (web only : poursuit le cluster « micro-surfaces FR restantes » (53w). i18n de `components/attachments/AttachmentDeleteDialog.tsx` — 5 chaînes **FR dures** affichées en TOUTES langues sur une surface **destructive critique** (confirmation de suppression définitive de fichier) : `Confirmer la suppression`/`Êtes-vous sûr…`/`Le fichier sera définitivement supprimé du serveur.`/`Annuler`/`Suppression…`-`Supprimer`. **ZÉRO nouvelle clé** : réutilise la section `attachments.contextMenu.*` existante (`confirmDeleteTitle`, `confirmDeleteDescription`, `confirmDeleteIrreversible`, `cancel`, `deleting`, `delete` — même domaine sémantique que le menu contextuel de pièce jointe, vérifiées à parité ×4 locales en/es/fr/pt). `useI18n('attachments')` — même hook/import que `MessageAttachments.tsx` (unique appelant). Tests : 6 clés `contextMenu.*` ajoutées aux 2 mocks `useI18n` de `MessageAttachments.test.tsx` (37/37 verts), miroir d'`AttachmentGallery.test.tsx`. Fallbacks EN en 2e arg (anti-flash, leçon 50w). Aucun fichier locale touché → aucun risque de divergence de parité. NE PLUS re-flagger `AttachmentDeleteDialog.tsx`.) |
...
| Last merged PR | iter-55w (micro-surfaces FR : fallbacks Suspense + ReplyPreview) #769 ✅ ; iter-54wb (PostComposer/AudioPlayer) #768 ✅ ; iter-56w (i18n AttachmentDeleteDialog) `claude/practical-fermat-42ivgr` ⏳ |
| Last Merged Base (commit) | `5d3fc43` (main HEAD post-#768) — base de l'iter-56w (rebasée) |
...
| 56w | claude/practical-fermat-42ivgr (i18n AttachmentDeleteDialog) | #770 | ⏳ |

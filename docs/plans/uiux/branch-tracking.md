...
| Last merged PR | iter-53w (v2 conv list) #766 ✅ ; iter-53wb (page reel deep-link) #764 ✅ ; iter-54w (page story deep-link) #767 ✅ ; iter-55w (micro-surfaces FR : fallbacks Suspense + ReplyPreview) `claude/practical-fermat-vpzpoy` ⏳ |
| Last Merged Base (commit) | `7928990` (main HEAD post-#767) — base de l'iter-55w |
...
- **NOUVEAU (53w)** : cluster i18n/a11y v2 repéré par l'explorer, bornés : ~~`components/v2/ReplyPreview.tsx` (`CONTENT_TYPE_LABELS` `📷 Photo`/`🎬 Vidéo` FR durs)~~ → **SOLDÉ en 55w** (`v2chat.{photo,audio,video}`, emoji dans le code) ; reste pour 56w+ : `components/attachments/AttachmentDeleteDialog.tsx` (~5 chaînes dialogue confirm FR) ; `components/auth/PhoneExistsModal.tsx` (~8 chaînes FR + flow SMS) ; `components/v2/PostComposer.tsx` & `AudioPlayer.tsx` (`aria-label` statiques anglais non i18n)
...
| 55w | claude/practical-fermat-vpzpoy (i18n fallbacks Suspense + ReplyPreview) | ⏳ | ⏳ |

...
| Last completed iteration (web) | **56w** (web only : solde le carry-over 53w→56w+ « dialogues confirm & flow téléphone ». (1) **`AttachmentDeleteDialog.tsx`** — 6 chaînes FR dures (titre/description/note/annuler/supprimer/suppression…) → câblage `useI18n('attachments')` **réutilisant** le bloc existant `attachments.contextMenu.{confirmDeleteTitle,confirmDeleteDescription,confirmDeleteIrreversible,cancel,delete,deleting}` : **ZÉRO nouvelle clé** (source unique, FR identiques) ; la mention « irréversible » fusionnée dans la note fichier (supprime une redondance UI) ; fallbacks EN (leçon 50w). (2) **`PhoneExistsModal.tsx`** — ~15 chaînes FR dures du chrome visible des 3 étapes (choice/verify_code/success) alors que seuls les toasts étaient i18n → nouveau sous-bloc `auth.register.wizard.phoneExistsModal` (12 clés ×4 : itsMyAccount, notMyAccountContinue, or, transferNumber, smsWillBeSent, smsVerification, enterCodeSentTo{phone}, codeInputLabel, codeInputHint, verify, resendInSeconds{seconds}, creatingAccount) + **réutilisation** `phoneReset.{cancel,resendCode,success}` (FR vérifiés identiques) ; params interpolés via `t(key,params)`. Diffs locale strictement additifs (insertion ciblée après `recoverByPhone`) ; parité 4 locales OK. `register-form-wizard.test.tsx` mocke entièrement PhoneExistsModal → 0 impact. NE PLUS re-flagger ces 2 fichiers ni ces chaînes. JSDoc interne PhoneExistsModal:58-60 = doc dev, conservée. Reste cluster en 57w+ : `ReelPlayer`/feed.) |
...
| Last merged PR | iter-54w #767 ✅ ; iter-55w #769 ✅ ; iter-54wb (PostComposer/AudioPlayer) #768 ✅ ; iter-56w (dialogues confirm & flow téléphone) `claude/practical-fermat-6k4qmh` ⏳ |
| Last Merged Base (commit) | `e1960c0` (main HEAD post-#768/#769 + dependabot #667/#661) — base de l'iter-56w |
| Next iteration | **57** — repartir de `main` HEAD post-merge iter-56w |
...
- **NOUVEAU (53w)** : `ReelPlayer` (`components/feed/ReelPlayer.tsx`) + surface **feed** globale non internationalisés (aucun `t()`) — passe i18n dédiée (large)
...
| 56w | claude/practical-fermat-6k4qmh (i18n dialogues confirm + flow téléphone) | ⏳ | ⏳ |

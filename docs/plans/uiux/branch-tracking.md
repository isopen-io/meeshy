...
| Last completed iteration (web) | **57w** (web only : solde le différé « `ReelPlayer` non internationalisé » (repéré 53w). `components/feed/ReelPlayer.tsx` (lecteur reel immersif plein écran, monté par la page deep-link `/reel/[postId]` ET par `ReelsFeedScreen`) n'avait **aucun hook i18n** — 13 emplacements user-facing en **FR figé** affichés en TOUTES langues (rupture Prisme UI a11y). Ajout `useI18n('reel')` (namespace existant 53wb réutilisé) + bloc `player` ×12 clés ×4 locales : `aria-label` section `position {current}/{total}`, `sr-only` h1 + fallback alt video/img `byAuthor {name}` (×3), boutons `play`/`close`/`previous`/`next`/`unmute`/`mute`, labels rail d'action `like`/`comment`/`share`/`bookmark`. Clés simples avec fallback EN 2e arg (anti-flash, leçon 50w) ; 2 clés paramétrées (`position`/`byAuthor`) sans fallback string (signature `t()` exclusive params XOR fallback), parité ×4 + cibles sr-only/aria ⇒ zéro flash. Parité vérifiée : 12 clés `player.*` identiques en/fr/es/pt, JSON valide ×4. NE PLUS re-flagger `ReelPlayer.tsx`. Reste du cluster feed différé 58w+ : `PostsFeedScreen.tsx` aria-labels + `title="Feed"`, `ReelsFeedScreen.tsx` `title="Reels"`.) |
...
| Last merged PR | iter-55w (micro-surfaces FR) #769 ✅ ; iter-54wb (PostComposer/AudioPlayer) #768 ✅ ; iter-56w (dialogues AttachmentDelete + PhoneExists) #771 ✅ ; iter-57w (ReelPlayer i18n/a11y) ⏳ |
| Last Merged Base (commit) | `bf731cb` (main HEAD post-#672, inclut #771 iter-56w) — base de l'iter-57w |
...
- ~~**NOUVEAU (53w)** : `ReelPlayer` (`components/feed/ReelPlayer.tsx`) non internationalisé~~ → **SOLDÉ en 57w** (`useI18n('reel')` + bloc `reel.player.*` ×12 ×4 ; NE PLUS re-flagger `ReelPlayer.tsx`). **Reste du cluster feed (58w+)** : `PostsFeedScreen.tsx` (aria-labels FR `Type de fil`/`Stories publiques`/`Humeurs`/`Composer une publication`/`Enregistrer un post audio`/`Publications`/`Chargement de plus de publications` + `title="Feed"`), `ReelsFeedScreen.tsx` (`title="Reels"`) — bornés, lot dédié.
...
| 57w | claude/practical-fermat-mrz8qr (i18n/a11y ReelPlayer) | ⏳ | ⏳ |

/**
 * Migration MongoDB — élargit l'index unique de `AttachmentReaction` de
 * (attachmentId, participantId) à (attachmentId, participantId, emoji).
 *
 ...
 * jamais deux fois le même. `AttachmentReactionService.addAttachmentReaction` devient ADDITIF
 ...
 * (2026-07-04-attachment-reaction-single-per-user-unique-index.mongodb.js). Aucune
 ...
 *
 * Exécution :
 *   mongosh "$DATABASE_URL" < 2026-08-18-attachment-reaction-multi-per-user-unique-index.mongodb.js
 */
...
const targetKey = { attachmentId: 1, participantId: 1, emoji: 1 };
...
// Prisma par défaut (`AttachmentReaction_attachmentId_participantId_emoji_key`). Créer la
...
// no-op.
const anyTripleUnique = db.AttachmentReaction.getIndexes().find(
  ix => JSON.stringify(ix.key) === JSON.stringify(targetKey) && ix.unique
);
const existing = db.AttachmentReaction.getIndexes().find(ix => ix.name === indexName);
...
if (anyTripleUnique) {
  ...
  db.AttachmentReaction.createIndex(targetKey, { name: indexName, unique: true });

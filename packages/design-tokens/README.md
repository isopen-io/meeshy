...
Trois fichiers CSS, et **aucune valeur ailleurs**. `apps/web-v3-old/app/globals.css`
...
refusé par `apps/web-v3-old/scripts/check-jetons.mjs` (conception § 3.2,
...
`apps/web-v3-old/app/globals.css` a d'abord importé la table par chemin relatif
...
`apps/web-v3-old/app/theme-script.tsx`, qui lit `localStorage` puis, à défaut,
...
`apps/web-v3-old/scripts/lib/cascade.mjs` résout la table — `@import` dans l'ordre,
...
Le moteur, symétriquement, est **UN** module : `apps/web-v3-old/app/theme-script.tsx`.
...
2. **La classe par défaut est rendue par le SERVEUR**
   (`apps/web-v3-old/app/layout.tsx` → `<html className={THEME_PAR_DEFAUT}>`).

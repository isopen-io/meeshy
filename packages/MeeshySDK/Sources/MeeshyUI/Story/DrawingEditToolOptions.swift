...
struct DrawingEditToolOptions: View {
    ...
    // MARK: - Layers (stroke list)

    private var layersOptions: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if viewModel.drawingStrokes.isEmpty {
                Text("Aucun trait — dessinez pour commencer")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
    ...
    private func strokeChip(_ stroke: StoryDrawingStroke) -> some View {
        ...
        return HStack(spacing: 6) {
            ...
            Image(systemName: Self.symbol(for: stroke.tool))
                .font(.system(size: 11, weight: .semibold))
            if isSel {
                Button {

...
public final class MessageService: MessageServiceProviding, @unchecked Sendable {
    ...
    /// - Parameter languages: optional ISO 639-1 codes (e.g. ["fr", "en"]) to
    ///   filter translations server-side. Reduces payload by ~50% for multi-lingual
    ///   conversations. Pass `nil` to receive all translations (legacy behaviour).

...
public enum RootLegacyAttachmentsEndpoint: MeeshyEndpoint, Sendable {
    case batchAnalysis
    case byAttachmentIdAnalysis(attachmentId: String)

    public var path: String {
        switch self {
        case .batchAnalysis: return "/attachments/batch/analysis"
        case .byAttachmentIdAnalysis(let attachmentId): return "/attachments/\(attachmentId)/analysis"
        }
    }
}

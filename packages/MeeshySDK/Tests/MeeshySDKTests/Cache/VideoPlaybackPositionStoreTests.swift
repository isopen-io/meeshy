...
final class VideoPlaybackPositionsTests: XCTestCase {

    func test_setting_storesPositionForAttachment() {
        let positions = VideoPlaybackPositions()
    ...
    func test_position_unknownAttachment_returnsNil() {
        XCTAssertNil(VideoPlaybackPositions().position(for: "missing"))
    }
...
@MainActor
final class VideoPlaybackPositionStoreTests: XCTestCase {

    private func makeStore() -> (VideoPlaybackPositionStore, UserDefaults) {
        let suite = "test.videoPositions.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!
        let store = VideoPlaybackPositionStore(userDefaults: defaults, key: "k")
    ...
    func test_save_persistsAcrossStoreInstances() {
        let suite = "test.videoPositions.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!
        let first = VideoPlaybackPositionStore(userDefaults: defaults, key: "k")
        first.save(42.0, for: "a1")

        let second = VideoPlaybackPositionStore(userDefaults: defaults, key: "k")
        XCTAssertEqual(second.position(for: "a1"), 42.0)
    }

    func test_save_evictsOldestBeyondCap() {
        let (store, _) = makeStore()
        let cap = VideoPlaybackPositionStore.maxEntries
        for index in 0...cap {

# Cycle 56 bis — la police d'un conteneur, posée à un conteneur qui n'a pas de hiérarchie

> **Numérotation.** Un second couloir a livré le même jour un journal nommé
> `cycle56` (« la question de la leçon 217, posée à la promesse du SERVEUR »,
> PR #3143). Les deux documents sont conservés ; celui-ci prend le suffixe `-bis`
> selon la convention déjà employée aux cycles 46-bis et 54-bis. La collision du
> COMPTEUR de cycles est la même dette que celle du compteur de leçons (§6).
...
## 6. Pistes pour le cycle 57 (ou 58 — cf. la note de numérotation) — repérées, NON livrées
...
6. **`PUT /conversations/:id` n'accepte pas les identifiants que son schéma
   annonce** (§5).
7. **Les compteurs PARTAGÉS collisionnent dès que deux couloirs livrent le même
   jour** — constat des DEUX intégrations manuelles de `main` de ce cycle, qui
   l'ont produit trois fois :
   - `tasks/lessons.md` porte deux « Leçon 215 » (routine *calling* cycle 140 vs.
     *messagerie* cycle 54-bis). Les deux textes sont conservés VERBATIM avec
     leur numéro : renuméroter la leçon d'un couloir déjà fusionné casserait les
     renvois de son propre CHANGELOG.
   - Deux « Leçon 218 » ont été proposées. **Celle qui n'était pas encore sur
     `main` a cédé** et devient la 219 — règle de préséance retenue ici, parce
     qu'elle ne casse aucun renvoi existant : l'auteur non fusionné est le seul
     à pouvoir corriger les siens.
   - Deux journaux `…-cycle56.md` (add/add). Le nom canonique reste à celui de
     `main` ; celui-ci prend `-bis`, convention déjà employée aux cycles 46-bis
     et 54-bis.

   La dette est le compteur lui-même, pas l'un des documents. Trois compteurs
   sont concernés — leçons, cycles, journaux — et aucun n'a d'allocateur : chaque
   couloir numérote depuis sa propre lecture du fichier, donc la collision est
   certaine, pas accidentelle. À trancher pour elle-même (un préfixe par couloir,
   ou un numéro dérivé du couloir plutôt que du dépôt).

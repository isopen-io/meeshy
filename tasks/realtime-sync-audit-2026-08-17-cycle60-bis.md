# Cycle 60 bis — la dérogation ne protégeait de rien : le préfixe avait trois autres portes

> **Note d'intégration.** Une AUTRE exécution de la routine a livré un cycle 60
> pendant celui-ci (`realtime-sync-audit-2026-08-17-cycle60.md`, « le drapeau que
> le web décodait pour le jeter » — la garde monotone de l'aperçu de ligne de
> liste). Les deux sont indépendants et ont fusionné sans conflit de code ; ce
> journal est donc renuméroté *bis*, suivant la convention des cycles 54/56/57.
>
> Les deux se **renforcent**, et il faut le savoir pour relire le §2.3 ci-dessous.
> Le cycle 60 a ajouté une garde monotone à `mergeConversationUpdate` : un
> `conversation:updated` nommant un message plus ANCIEN que celui que la ligne
> décrit est désormais REJETÉ — sauf s'il porte `previewRecalculated: true`, par
> lequel le serveur déclare un recul légitime. Or l'aperçu personnel recalculé
> sur lequel repose la suppression du Site B ci-dessous **recule par
> construction** (le remplaçant d'un message masqué est plus ancien que lui) et
> **porte précisément ce drapeau** (`emitConversationPreviewUpdate` le pose).
>
> Autrement dit : la garde du cycle 60 est ce qui garantit que la diffusion sur
> laquelle ce cycle-ci s'appuie atterrit, au lieu d'être jetée comme un doublon
> tardif. Sans son exemption, retirer l'invalidation du Site B aurait laissé la
> ligne périmée. Les deux cycles se sont croisés sur le même mécanisme par deux
> chemins opposés — l'un ajoutant la garde, l'autre supprimant le filet
> destructeur qui la masquait — et le résultat n'est correct que parce que les
> deux ont atterri.

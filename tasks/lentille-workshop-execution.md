...
4. **Vérifier localement** ce qui est vérifiable sous Linux : `cd packages/shared && npx vitest run` (le paquet a migré sous vitest — `bun test` y rend ~43 faux échecs de minuteurs, R5-10), `cd apps/web && bun run test`, `cd services/gateway && bun run test:coverage` (prérequis bun de `CLAUDE.md` : `npx prisma generate --generator client` + `bun run build` dans `packages/shared`). L'iOS ne compile pas sous Linux — voir §3.
...
| V4 | **livrée, PORTE V2 NON PRONONÇABLE (REV-4 rendue 2026-08-16), chantier web EN PAUSE sur décision produit — iOS d'abord.** 13 tâches livrées (WL-100..108, WF-110..113) sur `feat/lentille-web-v4` (worktree, non poussée), 626 suites / 12 767 tests web + shared verts en local. **REV-4 (Opus, lecture seule)** : 5 blockers, tous « du câblage, pas de la conception » — **B1** la passe de perspective + l'élection ne démarrent JAMAIS en prod (ordre des effets React : le hook lit `scrollContainerRef` AVANT l'effet qui le peuple ; StrictMode de `next dev` masquait le défaut, les tests peuplaient la ref à la main) ; **B2** drapeau ON ⇒ pagination (sentinelle IntersectionObserver) et branches vides perdues — le Mount remplace `renderContent` en bloc (L17 sans jeton web) ; **B3** drapeau ON ⇒ les 6 actions historiques du ⋮ de rang (épingler, sourdine, archiver…) inatteignables (L07 sans jeton web) ; **B4** `check-law-literals.sh` ROUGE sur la branche — 10 littéraux en docstrings `/* */` de 6 fichiers TOUS créés par V4 (la mention antérieure « préexistants » sur cette ligne était FAUSSE — vert sur main ; issue : durcir S-003 au strip des blocs OU reformuler) ; **B5** garde d'ensemble de matrice non déterministe (scan repo synchrone sous testTimeout 5 s — rouge à froid 11 s, vert à chaud). Réserves : R4-1 pendant Lentille de `behaviour-matrix-parity` manquant (10 id L ni classés ni justifiés — le filet qui aurait attrapé B2/B3) ; R4-2 L16 aria-label nombre nu / L08 tags non implémentés (tokens morts) ; R4-3 L02 brouillon tout-destructive (1 ligne) ; R4-6 memo LentilleRow annulé par onClick littéral ; R4-5 **SOLDÉ (cycle 61)** — les vecteurs gelés franchissent enfin la frontière `dist/` via `apps/web/__tests__/lentille/shared-law-dist-parity.test.ts` (46 vecteurs, 3 lois : focus-curve, scroll-activity, river-lanes), là où `packages/shared/__tests__/vectors/` n'importe que la SOURCE. **Découverte au passage, non corrigée car architecturale** : la ligne `'^@meeshy/shared/(.*)$': '.../packages/shared/dist/$1'` d'`apps/web/jest.config.js` est **MORTE** — `next/jest` génère son propre mapper depuis les `paths` du `tsconfig.json`, où `@meeshy/shared/*` pointe vers la SOURCE, et cette génération l'emporte ; toute suite web qui importe `@meeshy/shared/...` charge donc la source, jamais le build, contrairement au commentaire de configuration. Piège aggravant : `require.resolve` rend POURTANT un chemin `dist/` (il consulte l'autre mapper), si bien qu'un témoin bâti sur lui se déclare satisfait alors que l'`import` de la même ligne a chargé la source — la 1re version du guard est tombée dedans et passait, 46 vecteurs verts, en re-testant le monde déjà couvert ; c'est une MUTATION délibérée du build (constante de loi altérée dans `dist/`) qui l'a démasquée. Version livrée : import par CHEMIN RELATIF EXPLICITE vers `dist/`, discrimination prouvée DANS LES DEUX SENS (rouge sur build divergent, verte sur build correct). **À décider par l'équipe** : réparer le mapper mort (⇒ toutes les suites web basculent sur `dist/`, risque de casse large) ou retirer la ligne et assumer « le web teste la source » ; R4-9 pont invisible = miroir R-c (G-124). Arbitrages rendus : contraste « Toi » → amender §WS-4 des DEUX côtés (≥ 4,5:1, le test basculera exprès) ; prévision d'encoche sans `lastReadAt` → MASQUER (« AUTO » nu — une absence ne se convertit jamais en affirmation), levée G-123 ; carte dans wrapper (isolation:isolate, zéro relayout par construction) ACCEPTÉE ; casse encoche + `modes.bubbles` défensif ACCEPTÉS. Validé re-prouvé : Prisme chemin EXACT partagé, OFF bit-à-bit + boundary, R-h LEVÉE, a11y statique, reduce-motion conforme amendement, 1 rAF/surface, tokens sans littéral exécutable. **Reprise (V4bis)** : B4/B5 → B2/B3 → B1+R4-6 → R4-1 → REV-4bis | (worktree) | web local vert ; CI quality serait rouge (B4) |
| **V4bis + V4ter** | ✅ **PORTE V2 PRONONCÉE (2026-08-17, REV-4ter)** — pause web levée sur directive utilisateur, corpus V4 réconcilié avec le tronc vivant (merge `274aaf09`, ~200 commits, 3 arbitrages documentés) puis mergé sur main (`549bbf37`) — le web rejoint le régime « merge régulièrement ». V4bis : B4 (`d0e15b6d` strip automate awk auto-testé), B5 (`588b585f` index en process, 0,96 s à froid), B2 (`b7213ea8` sentinelle extraite `useLoadMoreSentinel`), B3 (`5bbaba37` 6 actions partagées, jamais copiées), B1 (`3433025d` ref par callback — la passe démarre enfin en prod, StrictMode masquait), R4-6 (`ddc20269` memo effectif), R4-1 (`d66720d7` matrice Lentille 17 id armée). **REV-4bis** (Opus) : les 5 REV-4 TENUS mais 3 blockers neufs → V4ter : B1′ aria-label honnête (résolution UNIQUE rendu+aria, 5 témoins de discrimination RED prouvés, `cbbc2c5a`) + R4-3 soldée + R5-2 ; B2′ façade magasin unique (le fil OBÉIT au choix, migration one-shot, **AMENDEMENT S1 : `bulles` entre dans `ReadingModePreferenceSchema`** — TS+Swift 6 switches, 22 vecteurs régénérés en exécutant la loi, pouvoir ON nul, `4b5192a4..92af9a64`) ; B3′ périmètre R15 étendu à la peau `reading/` du tronc (RED prouvé, `ScrollTimePill` → `useScrollActivity`, exclusion Tailwind `-900` datée, `32689bfd`) ; axe-core INSTALLÉ (`jest-axe`, 4 suites jsdom sur composants réels, `eae8d653`). **REV-4ter** (Opus) : verdict PRONONÇABLE avec RESTRICTION — certifie le périmètre WL-100..108/WF-110..113 (bâti, prouvé, absences nommées re-prouvées), PAS la parité 32/32 (web 17/32) ; débloque V5, PAS l'activation V6. Critère de porte AMENDÉ : référentiel re-prouvé **32 id** (17 L + 15 F, jamais 44) ; « chaque id classé : couvert avec preuve OU non couvert avec raison TYPÉE (`absent-structurel`/`source-amont-attendue`/`hors-périmètre-du-lot`) re-prouvée ». Réserves tracées : R5-3 (aria L04 muette + raison L16 sur-déclarante), R5-4 (garde « un seul magasin » contournable — 3ᵉ store déposé passe vert), R5-5 (exclusion Tailwind par LIGNE, occurrence mixte passe), R5-6 (scope web sans identité — **condition** : LWS-3/G-121 avant activation multi-comptes), R5-7 (`nested-interactive` = **condition d'activation V6**), R5-8 (`aria-required-children` grille Rivière, non montée), R5-9 (raisons Focal sous le standard de preuve), R5-10 (doc runner corrigée : shared sous vitest). R4-5 soldée par le tronc (cycle 61, garde `dist/`-parité + mapper Jest mort découvert). Verdict complet : artifact `dc9103d1`. | `6f31ff7d` | web 668 suites / 13 148 verts ; shared 83/2 168 ; snapshots OFF bit-à-bit |
...
| V5 | **LIVRÉE INTÉGRALEMENT (2026-08-17), REV-5 en cours.** Couloir ac : G-120 (`e384834a` Prisma `readingMode`), G-121 (route + broadcast), G-122 (`07971f65` pont ✦ non-N+1, 5 requêtes constantes prouvées), G-123 (`f4efb893` attache payload + `suggestedMode`), G-124 (injection iOS, `cfcbf40a`/`a70b4865`, + tri de 14 témoins `d203951a`). Couloir ad : G-125 (`c295d22d` résumé borné par identifiants, 1 ligne posée sur la génération), G-126 (`03b4eaea` chemin NON ÉCRIVANT prouvé par clôture d'imports + témoins mutés), G-127 (`3c9b9650` intersection exacte, repli C2 intégral, arbitrage E7 : langue ABSENTE jamais fabriquée). Couloirs ae/af : R-130 livrée par le tronc vivant (PR #3170, amendée R2/R3 §7bis/§7ter puis A.5/A.6 PR #3174), R-132 miroir Swift 53 vecteurs (`da67c071`), R-131+R-133 tokens `river` + peau iOS Canvas (`0098326f`, garde de consommation étendue aux 3 sections), R-134 peau web SVG (`7d42bbfc`), R-135+R-136 dégrisage 2 OS + complétude + A.5/A.6 appliqué aux 2 peaux (`cfc18722`). Erratum i18n `riviere.bubble.replyTo` 7 langues (`8c23e101`) ; compiles réparées par le flux (`53bbc6c2` NSLock, `5b749cb2` ISO8601 non-Sendable). **Points de couture pour REV-5 (REV-4ter)** : pont ✦ `NO_LOCAL_CACHE` en prod (`bridgeFor` rend `null` pour tous) ; `suggestedMode` lu par zéro fichier de production ; Rivière sélectionnable au menu mais écran jamais monté au fil (choisir Rivière ⇒ voir Focal) ; « Bulles » ON sans effet ; R-e `agent_grammar` → reprise G-126. | `3c9b9650` · `cfc18722` | **REV-5 RENDUE (2026-08-17) : 2 BLOCKERS, le reste TENU — V6 non ouvrable avant levée.** B1 : le pont ✦ n'atteint jamais un rang WEB — `transformConversationData` jette `bridge`/`lastReadAt` (transformers.service.ts:456-500), `use-socket-cache-sync.ts:847` ignore `bridge` ; iOS, lui, AFFICHE le pont (chemin prouvé maillon par maillon — le provider G-124 reste « câblé, pas branché », état voulu qui rend P7 neutre). B2 : le fan-out socket paie 5 requêtes Prisma PAR destinataire (croissance linéaire prouvée : 11→56 requêtes pour 1→10 destinataires) sur le chemin d'envoi — correctif : plafond honnête ou batch inversé + témoin de compteurs. TENU : non-N+1 G-122/123 (témoin discriminant par croissance, régression délibérée attrapée), C1/C2/C3/E7 (clôture C3 rougit sur import écrivant ; 5 branches de repli C2 vertes ; E7 jusqu'au payload réel), P7 neutre, Rivière conforme (53 vecteurs 2 côtés, §7bis/7ter A-C, S1 `bulles` cohérent). Coutures statuées : (a)=B1 web seul ; (b) `suggestedMode` 0 consommateur → R6-5/Q-141 ; (c) Rivière non montée → R6-3, `riviere_mode` OFF jusqu'à R-137/Q-146 ; (d) « Bulles » ON sans effet → R6-4/Q-142 (LensSwitcher sans drapeau) ; (e) `agent_grammar` orphelin, commentaire de site faux → R6-2/Q-143. Réserves neuves R6-1 (clôture C3 denylist → allowlist, 1 ligne), R6-6 (double lecture curseurs), R6-7 (étage agent mono-langue de fait) ; R5-4 : surface AGRANDIE par G-121/G-124 (4e point d'entrée serveur, prouvé bien visé aujourd'hui). Ordonnancement V6 recommandé : B1 → B2 → Q-140..145 + Q-146 (verrou d'activation). **BLOCKERS LEVÉS le jour même** : B1 (`b3a8803a`) — pont soudé wire→rang (transformeur + type partagé + relais socket nil-efface jumeau du Swift), témoin `bridge-wire-to-row` sur composants réels, encoche prédictive alignée (`lastReadAt` réel, casts défensifs supprimés) ; B2 (`0c5adf65`) — inversion batchée `buildBridgeDataForViewers` (10 requêtes CONSTANTES vs 5+5N mesuré 10→55, droits PAR lecteur prouvés par mutation — 4 assertions discriminantes), doc du fan-out corrigée pour dire le vrai coût, 4 témoins de forme d'appel réécrits (le défaut était leur assertion), aucun comportement perdu. **V6 OUVRABLE** dès l'atterrissage des lots produit en cours (parité fil, fidélité maquettes). |
| V6 + **CLÔTURE PHASE 1** | **LIVRÉE (2026-08-17 soir)** — Q-140 matrice 32 id rejouée 2 OS + rapport (`cadf498a`) ; Q-141 R1→R20 : 15 prouvés / 3 device / 2 non-tenus R13·R18, arbitrage R6-5 BRANCHER (`ebd09bb4`) ; Q-143 perf mesurée — fan-out constant confirmé, passe web opacity+transform seuls, FINDING 150 alloc/frame vs « zéro alloc » du contrat ; R6-2 agent_grammar branché OFF, R6-6 curseurs mutualisés (`fd1b7b74`) ; Q-144 innocuité OFF 3 surfaces (`37c330b8`) ; Q-146 : 4 gardes durcies par contournement PROUVÉ + verrou d'activation Rivière (`95a499a1`) ; L16-iOS : l'aria entend le pont (`5f65e41f`) ; suggestedMode branché serveur-puis-local, vrai point de décision re-prouvé (`784f3c16`) ; Q-142 a11y croisée : rang sans contrôle imbriqué (card action), grille Rivière row/gridcell, « Bulles » ON branché (arbitrage CONTRE le masquage — l'aller simple), contraste étendu, FINDING Q142-a 4,393:1, zéro disableRules axe restant (`81e93f3a`). **Q-145 CLÔTURE** (`0d25fd11`, `tasks/lentille-cloture-phase1.md`) : §9.1 littérale NON-CLOSE — 4 points rouges (R13 jamais câblé ; matrice thread web 6/15 et Script sans témoin ; perf jamais mesurée device + 1+2N alloc ; parité 17/32) ; au référentiel AMENDÉ par REV-4ter : **CLOSE-AVEC-RÉSERVES** — le choix de grille est une décision produit. Dossier d'activation : 6 leviers, 5 paliers, critères de retour arrière. FAIT CRITIQUE : iOS `reading_modes` est DÉJÀ ON pour toute installation via la cascade bêta I-075 — le palier 0 consiste à confirmer ou retirer cette cascade. Dette ordonnée D-1..D-17 (dont R-137 montage Rivière au fil, Android LWS-12). Erratum de revue soldé dans la foulée : la garde de matrice vitest borne sa lecture aux extensions porteuses (B5 porté au jumeau, 2,9 s → 0,6 s, suite au défaut verte). | `0d25fd11` | phase 1 close (réf. amendé) — activation = décision utilisateur |
| **DÉCISIONS D'ACTIVATION + SOLDAGE** | **EXÉCUTÉES (2026-08-18)** — l'utilisateur a arbitré (délégation explicite sur les options recommandées du dossier Q-145). **① Cascade bêta I-075 RETIRÉE** (`36b4864a`) : absence de toute expression ⇒ OFF, opt-in explicite préservé, `isExplicitlySet` ajouté, 4/144 combinaisons changent, item « Focal (bêta) » du long-press conservé ; RÉSIDU consigné : toggle Réglages affiche ON sur install neuve (copie 7 langues ou écart à accepter). **② Grille AMENDÉE fait foi** ⇒ phase 1 **CLOSE-AVEC-RÉSERVES** actée ; **⑥** R13 retiré de la grille (capacité d'affichage conservée) ; **⑦** Script hérite des preuves de Focal (risque densité nommé) ; **⑧** contrat §4.1 amendé « allocation bornée O(rangs visibles) » ; **⑨** `agent_grammar` : décision écrite « pas maintenant » ; **⑪** L03/L05/L09 « iOS seulement » ; **⑫** Android NO-GO daté (`7e7c4495`, originels cités partout, jamais de réécriture silencieuse). **③ Palier 1 GO** (opt-in URL en production, trace profiler à faire pendant la session). **④/D-2 SOLDÉE** (`93f6607a`) : `--muted-foreground` clair 46,1 % → 45 %, focus card 4,393 → **4,567:1**, RED authentique, 182 consommateurs vérifiés à la hausse, sombre intact ; FINDING NEUF **D-18** consigné (`d66046bc`) : heure iOS ≈ 1,67:1 (tint indigo 40 %), aucun témoin de ratio iOS — précondition palier 2. **⑤/D-5 SOLDÉE** (`cf507313`) : `useThreadActiveReadingMode` + 5ᵉ param de `notchText` — LensSwitcher et encoche disent « AUTO · Bulles », additif pur, 0 test existant modifié. **D-1 SOLDÉE** (`9a195795`) : Dynamic Type sur les 8 branches de ligne 2 de la rangée (hauteur fixe 64 pt ⇒ politique lineLimit(1) délibérée prouvée par comptage). **D-12 SOLDÉE** (`86a27edc`) : l'heure de la liste vit — un tick partagé 60 s (cause réelle : memo() jamais re-rendu), RED « 23:59 » → « Hier 23:59 ». **D-13 TRANCHÉE** (`1f585c8b`) : mapper mort retiré, mort prouvée par empoisonnement de dist/ jamais chargé. **D-4/R5-6 SOLDÉE** (`cfe4e277`) : clé scopée `meeshy:reading-mode:<scopeId>:<conversationId>` (u-/a-), migration = SUPPRESSION jamais adoption (le précédent iOS refusait aussi l'adoption), route G-121 branchée (GET au chargement + PUT fire-and-forget + broadcast versionné, précédence par version déjà arbitrée), témoin R5-6 RED 9/12 sur l'ancien code, web complet 698/13493 vert. PALIERS : 2 attend CI macOS vert + D-18 + `MEESHY_FLAG_READING_MODES=1` au schéma TestFlight ; 3 attend l'écrit du résidu point 4 (fait au §9.1-AMENDEMENT) — mécaniquement PRÊT. | `cfe4e277` | 6 chantiers mergés ff-only le jour même ; tronc vert sous flux externe continu |

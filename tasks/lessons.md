...
4. **Un même nom d'événement pour deux faits différents rend TOUS les clients faux, et dans des
   directions opposées.** `conversation:joined` est à la fois « untel devient membre » et
   l'accusé « ton socket est entré dans la room », réémis à chaque ouverture. Le web incrémentait
   sur les deux (compteur gonflé d'un membre par ouverture) ; iOS n'en tirait rien (compteur qui
   ne peut que descendre). **Aucun des deux n'avait tort : le payload ne permettait pas de
   choisir.** Quand deux clients traitent le même événement de façons incompatibles et défendables,
   le défaut est dans le contrat, pas dans les clients.

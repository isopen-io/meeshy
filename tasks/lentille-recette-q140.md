...
| L12 | 2 chemins long-press → `LentillePeekView` + sous-menu « Mode de lecture » | ⚠️ AMENDÉ 2026-08-21 (décision user) : l'aperçu = `ConversationPreviewView` (derniers messages, bannière, avatar, icônes d'en-tête) sur les DEUX chemins ; `LentillePeekView` SUPPRIMÉE ; le sous-menu reste — `LongPressPreviewGuardTests` | ✅ `LentillePeek.tsx`/test (web inchangé) | ≠ iOS (assumé) |

...
| **D-18** | **FINDING iOS (soldage D-2 du 2026-08-18) — le point médian/l'heure de la rangée Lentille iOS mesurent ≈ 1,67:1 en thème clair.** `MeeshyColors.textMuted(light)` = `indigo500` à 40 % d'opacité sur `backgroundSecondary(light)` = `#F8F7FF` (`packages/MeeshySDK/Sources/MeeshyUI/Theme/MeeshyColors.swift:110-116`). Structurellement différent du gris web soldé en D-2 (tint semi-transparent vs échelle de gris) et **aucun témoin de ratio** ne le verrouille (grep `MeeshyTests` : gardes de source seulement). Mesure ponctuelle non verrouillée — à instruire : mesurer proprement, corriger le tint ou consigner l'écart daté, ET poser le témoin manquant. | Q142-a (correctif web, 2026-08-18) | **palier 2** (même critère AA que D-2) | Design + Sonnet (iOS) | **S** |

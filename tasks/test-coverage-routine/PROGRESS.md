...
| gateway | `pnpm --filter gateway test:coverage` | ~73.0 (local) / ~68.5 (CI est.) | ~68.7 (local) / ~64.2 (CI est.) | 2026-06-29 (post gateway-manifest-gap4: gap2☑CaptchaService/TURNCredentialService/auth-types, gap3☑magic-link/mentions/message-read-status/reactions (138+tests), gap4☑invitations/maintenance/user-stats-branch (22+tests), manifest ticked 8 files; 309 suites / 9460 tests / 1 skipped; threshold floor unchanged lines:67/branches:63/statements:67/functions:67 — CI-bun-calibrated gap ~4.5pp, floors safe) |

...
A run resolves a `(feature × app)` cell to the concrete files here and brings each to **100%
line+branch**.

Checkbox meaning:
- `[ ]` — no obvious test today → write tests to 100%.
- `[~]` — a same-named test file exists (heuristic), but it may be shallow → **verify** real
  coverage is 100% line+branch; fill gaps; only then flip to `[x]`.
- `[x]` — verified 100% line+branch **and** reviewer-approved (set by the routine).
...
100%. That's the full surface the routine works through, one slice per 5h run.
...
from the current tree. It does **not** know which files the routine has already driven to 100% —

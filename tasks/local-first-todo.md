...
- [ ] **T3** (S, sdk) — Ne plus réinitialiser `lastFetchedAt` (horloge fraîcheur L2) sur flush de mutation locale (`GRDBCacheStore.flushKeyToL2:530`).
- [ ] **T4** (M, sdk) — Flusher les entrées L1 dirty avant éviction LRU dans `touchKey` (+ purger dirtyKeys fuités). Dépend T3.

...
- **Non fait volontairement** : le jumeau iOS potentiel (`BubbleCallNoticeView.swift` /
  ...
  sur le même correctif serveur) toujours non mergée sur `main` au moment de cette vague — `Test
  gateway` y échoue sur `ReactionService.test.ts`/`AttachmentReactionService.test.ts`
  (multi-réactions, cycle 68, hors périmètre calling, non touché ici) ; laissé à la routine "keen
  hamilton" propriétaire de ce chantier.

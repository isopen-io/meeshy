...
const SPRITE = join(HERE, '../../../packages/icons/sprite.svg');
...
if (missing.length) {
  console.error(`Glyphes absents du sprite : ${missing.join(', ')}`);
  process.exit(1);
}

const entries = USED.map((id) => {
  const { viewBox, body } = symbols.get(id);
  const name = id.replace(/^ph-/, '').replace(/-([a-z])/g, (_, c) => c.toUpperCase());
  return `  ${name}: { viewBox: ${JSON.stringify(viewBox)}, corps: ${JSON.stringify(body)} },`;
}).join('\n');

writeFileSync(
  OUTPUT,
  `/* GENERE par scripts/extract-glyphs.mjs depuis packages/icons/sprite.svg.

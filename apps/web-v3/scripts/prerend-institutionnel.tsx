...
import { PageInstitutionnelle } from '../src/institutionnel/page';
import { PAGE_A_PROPOS } from '../src/institutionnel/about';
import { PAGE_CONTACT } from '../src/institutionnel/contact';
import { PAGE_PARTENAIRES } from '../src/institutionnel/partners';
import { PAGE_CONFIDENTIALITE } from '../src/institutionnel/privacy';
import { PAGE_CONDITIONS } from '../src/institutionnel/terms';
import type { PageDeContenu } from '../src/institutionnel/type';

const ICI = dirname(fileURLToPath(import.meta.url));
const DIST = join(ICI, '../dist');

const PAGES: readonly { readonly route: string; readonly page: PageDeContenu }[] = [
  { route: 'about', page: PAGE_A_PROPOS },
  { route: 'contact', page: PAGE_CONTACT },
  { route: 'partners', page: PAGE_PARTENAIRES },
  { route: 'privacy', page: PAGE_CONFIDENTIALITE },
  { route: 'terms', page: PAGE_CONDITIONS },
];
 ...
 */
const ORIGINE = process.env.MEESHY_ORIGINE_PUBLIQUE ?? 'https://meeshy.me';

/**
 * La feuille DÉDIÉE à ces pages — pas celle de l'application.
 *
 * Vite en produit deux (voir `vite.config.ts`) : `index-*.css` porte toute
 * l'application, `institutionnel-*.css` ne porte que ce que ces cinq pages
 ...
 */
function feuilleProduite(): string {
  const assets = join(DIST, 'assets');
  const css = readdirSync(assets).filter((f) => f.startsWith('institutionnel-') && f.endsWith('.css'));
  if (css.length !== 1) {
    throw new Error(
      `Attendu UNE feuille \`institutionnel-*.css\` dans dist/assets, trouvé ${css.length}. ` +
 ...
 */
const SCRIPT_DE_THEME = `(function(){try{var c=localStorage.getItem('meeshy.schema');var l=c?c==='clair':window.matchMedia('(prefers-color-scheme: light)').matches;document.documentElement.classList.toggle('light',l);document.documentElement.classList.toggle('dark',!l);}catch(e){}})();`;

function document(route: string, page: PageDeContenu, feuille: string): string {
  const corps = render(<PageInstitutionnelle page={page} />);
  const url = `${ORIGINE}/${route}`;
  ...
  const titre = page.titre.includes('Meeshy') ? page.titre : `${page.titre} · Meeshy`;
  return `<!doctype html>
 ...
 */
function verifie(route: string, html: string, page: PageDeContenu): void {
  const corps = /<body>([\s\S]*)<\/body>/.exec(html)?.[1] ?? '';
  const echecs: string[] = [];

  if (corps.length < 500) echecs.push(`corps quasi vide (${corps.length} caractères)`);
  if (!corps.includes(echappe(page.titre))) echecs.push('le titre de la page est absent du corps');
  ...
  for (const section of page.sections) {
    if (!corps.includes(echappe(section.titre))) echecs.push(`section absente : « ${section.titre} »`);
  }
  if (!html.includes(`<link rel="canonical" href="${ORIGINE}/${route}">`)) echecs.push('canonical absent ou faux');
  if (!html.includes('property="og:url"')) echecs.push('og:url absent');
  if (!html.includes('property="og:description"')) echecs.push('og:description absent');
  // ZÉRO script externe : c'est la promesse de ces pages. Le seul `<script>`
  // toléré est le thème, inline.
  if (/<script[^>]+src=/.test(html)) echecs.push('un script EXTERNE est référencé');
  if (/<link[^>]+rel="stylesheet"/.test(html)) echecs.push('une feuille EXTERNE est référencée');

  if (echecs.length > 0) {
    console.error(`\n  /${route} — ${echecs.length} invariant(s) rompu(s) :`);
    for (const e of echecs) console.error(`    · ${e}`);
    process.exit(1);
  }
}

const feuille = feuilleProduite();
let total = 0;
for (const { route, page } of PAGES) {
  const html = document(route, page, feuille);
   ...
   */
  verifie(route, html, page);

  const rep = join(DIST, route);
  mkdirSync(rep, { recursive: true });
  writeFileSync(join(rep, 'index.html'), html);

 ...
 */
const ROUTES_INSTITUTIONNELLES = ['about', 'contact', 'partners', 'privacy', 'terms'];

const FORME_A_BARRE = new RegExp(`^/(${ROUTES_INSTITUTIONNELLES.join('|')})/$`);
...
self.addEventListener('fetch', (evenement) => {
  ...
  const adresse = new URL(evenement.request.url);
  const trouve = FORME_A_BARRE.exec(adresse.pathname);
  ...
  evenement.respondWith(
    Promise.resolve(Response.redirect(`${adresse.origin}/${trouve[1]}${adresse.search}`, 302)),
  );
});

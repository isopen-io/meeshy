...
test('deux gestes, et le lecteur atterrit DANS la conversation créée', async ({ browser }) => {
  ...
  await page.locator('input[name="nom"]').fill('Le potager du quartier');
...
test('la feuille marche ENTIÈRE sans JavaScript', async ({ browser }) => {
  ...
  await page.locator('input[name="nom"]').fill('Sans une ligne de script');
  await page.locator('button[type="submit"]').click();
...
test('un refus garde la saisie et dit le motif', async ({ browser }) => {
  ...
  await page.goto(`${v3.base}/chats?nouvelle`);
  await page.locator('input[name="description"]').fill('Entre voisins');
  await page.locator('input[name="nom"]').fill(' ');
  await page.locator('button[type="submit"]').click();

  await expect(page.locator('[role="alert"]')).toBeVisible();
  await expect(page.locator('input[name="description"]')).toHaveValue('Entre voisins');

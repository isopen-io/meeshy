...
import { clampRiverFocusRank, riverFocusRankAt } from './focus';

/** Port de `apps/web/components/conversations/riviere/__tests__/river-focus.test.ts` (Jest → bun:test). */
...
describe('riverFocusRankAt — hauteur de lecture → rang fractionnaire (§7ter B)', () => {
  ...
  test('un rang sans extent mesuré est ignoré (mesure partielle, ResizeObserver en vol)', () => {
  ...
  test('liste de rangs vide → rang 0 par défaut (span 1, fraction plafonnée à 1), jamais une exception', () => {
...
describe('clampRiverFocusRank — bornes [-0.5, rankCount - 0.5]', () => {
  test('laisse passer une valeur déjà dans les bornes', () => {
    expect(clampRiverFocusRank(1.5, 5)).toBeCloseTo(1.5, 5);
  });

  test('plafonne au-dessus du dernier rang', () => {
    expect(clampRiverFocusRank(100, 5)).toBeCloseTo(4.5, 5);
  });

  test('plafonne au-dessous du premier rang', () => {

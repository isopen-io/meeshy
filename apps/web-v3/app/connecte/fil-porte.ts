...
import { accuseLecture, aAccuser, desepingle, epingle, modifie, peutModifier, peutRetirer, reagis, retire } from '@/lib/api/fil-mutations';
...
/**
 * L'ORDRE DE LECTURE (§ 4 étape 2 des spécifications #5163/#5387, complété
 * par #5385) : `annuler-le-retrait` → `retirer` → `epingler`/`desepingler` →
 * `modifie` → `reaction`+`message` → `reponse`/`message`. Un formulaire ne
 * porte qu'UN de ces genres à la fois — le menu d'une ligne, sa fenêtre
 * servie, le composeur et la
 */

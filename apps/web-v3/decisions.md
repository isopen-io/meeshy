...
## D-32 · Un message épinglé, transféré, modifié, système ou emoji seul se reconnaît dans le fil — sticker, lieu et citation de story restent HORS TYPE, jamais inventés — 2026-09-10 (#5936)

**Le constat.** D-23 (§ « Ce que ça ne couvre pas ») l'écrivait déjà : « les autres états manquants de la bulle (supprimé, système, appel, sticker — `bulle.md` § 6.6, § 9) restent hors périmètre ». `focal-row.tsx`/`bubble.tsx` ne rendaient ni badge (épinglé, transféré, modifié), ni rangée système, ni traitement particulier pour un message emoji seul — un message PORTAIT ces états (`pinnedAt`, `forwardedFromConversationId`, `isEdited`, `messageSource: 'system'`) sans qu'aucune des deux peaux ne les rende visibles.

**La règle.** `badgesOf`/`systemRowOf`/`bodyKindOf` (`src/lib/view/message-badges.ts`) sont les SITES UNIQUES de ces décisions, appelés par les DEUX peaux et par `composeMessageLabel` (D-31, qui gagne enfin un segment « transféré » — le seul des quatre badges qu'il n'avait jamais porté). `MessageBadges`/`EditedMark`/`SystemNotice` (`message-blocks.tsx`) sont les composants de présentation partagés.

- **Épinglé et transféré** se peignent AU-DESSUS du contenu, avant la citation — même position que `FocalRow.contentColumn.badgesSection` / `BubbleStandardLayout.body`. **Modifié** vit dans la colonne MÉTA, à côté de l'heure et de l'accusé — même rang que `BubbleEditedIndicator` sur iOS, jamais dans `MessageBadges` (ils n'ont pas la même place sur le modèle qu'ils miment).
- **Transféré ne nomme JAMAIS une personne** — `forwardedFromConversation.title` servi ⇒ « Transféré depuis {titre} », absent ⇒ « Transféré » seul, jamais un repli sur `identifier` (ce que `apps/web/lib/forward-badge.ts` fait à tort — écart NON repris ici).
- **La rangée système** (`messageSource: 'system'`) ne porte ni avatar ni méta d'auteur, sur les deux peaux — PLATE en Focal/Script (miroir `FocalSystemRows`), capsule centrée en Bulles (miroir `BubbleSystemNoticeView`, `Spacer` des deux côtés — PAS alignée côté expéditeur comme `BubbleDeletedView`). Le texte est le contenu SERVI par le Prisme, jamais recomposé : un résumé d'appel (`messageSource: 'system'` lui aussi) traverse par le MÊME chemin générique — sa mise en forme riche (icône, sens, qualité réseau, `BubbleCallNoticeView.swift`) reste hors périmètre. `systemRowOf` n'intervient QU'au cas `standard` de `protectionOf` : un message système protégé (rare, jamais produit aujourd'hui) reste un tombstone, fail-closed.
- **Un emoji seul s'affiche en grand (40 px)**, sur le texte SERVI (`bodyKindOf`), jamais sur `message.content` brut — un témoin dédié (`states-emoji-translated`) fait échouer tout résolveur qui lirait le contenu original au lieu de la traduction préférée.

**Ce que ça N'ATTRAPE PAS, et pourquoi — issues compagnons à ouvrir** :
1. **Sticker, lieu (`location`) et citation de story (`postReplyTo`)** — la passerelle les HISSE sur le fil (`hoistStickerOnto`/`hoistLocationOnto`, `services/gateway/src/routes/conversations/threads.ts:153`), mais `Message`/`MessageWithTranslations` (`packages/shared/types/conversation.ts`) ne les DÉCLARENT PAS au type. Une capacité gateway non exposée au type ne s'invente pas côté client (`CLAUDE.md` § SDK Purity, réaffirmé par D-31 pour l'anneau de story) : les fabriquer sur une fixture typée `Message` aurait menti sur la forme réelle du domaine partagé. Trois issues à ouvrir — une par capacité, chacune commençant par le type partagé, jamais par le rendu.
2. **Aucun glyphe « transféré »** — `scripts/extract-glyphs.mjs` n'a pas d'équivalent à `arrowshape.turn.up.right.fill` (`BubbleForwardedIndicator.swift:111`). Le badge reste textuel, même famille que « Sans compte » (D-31).
3. **La rangée système générique ne distingue pas un résumé d'appel** d'une arrivée ou d'un autre événement — c'est le contenu SERVI qui porte toute la distinction aujourd'hui, jamais une mise en forme dédiée par `kind`.

**Ce que ça a coûté** (`node scripts/measure-weight.mjs`, avant/après) : `first_paint` 36,86 → 36,87 Ko gzip — inchangé au sens du plafond (`budgets.json`, 40 Ko) : `message-badges.ts` n'entre que dans le chunk du fil, chargé à la demande. `bun run gate` : 1672 témoins `bun test`, `check-thread-states.mjs` § 9 (Focal/clair, Bulles/sombre, `scripts/lib/check-badges.mjs`) tous verts.

**Ce que la revue a corrigé** — deux couleurs de badge sous l'AA, trouvées PAR le gate qu'elles venaient d'introduire, jamais par relecture visuelle : (1) le badge « épinglé » utilisait `var(--accent)`, la teinte DÉRIVÉE par conversation (`ColorGeneration.swift`) qui n'offre aucune garantie de contraste — mesuré 2,24:1 sur `c-states` en clair, remplacé par `--color-ios-ink-2` (déjà AA partout ailleurs) ; (2) « modifié » portait `META_TEXT_OPACITY` (0,55, le traitement discret de l'heure) — mesuré 2,21:1 (clair) / 1,89:1 (sombre), sous l'AA dans les DEUX schémas : retiré, la couleur seule suffit à le distinguer visuellement. **Un jeton PRÉEXISTANT reste sous l'AA et n'est PAS corrigé ici** : `--ios-bubble-meta-mine` (blanc 70 % sur indigo-500, déjà porté par l'heure et l'accusé de CHAQUE bulle envoyée, mesuré 3,02:1 en sombre — iOS porte la MÊME opacité, `BubbleEditedIndicator.swift:19-21`) ; le témoin de contraste de « modifié » porte donc sur un message REÇU (`states-all-badges`, Kwame), jamais sur `states-edited` (message à soi) — réparer le jeton élargirait ce lot à toute la colonne méta d'une bulle envoyée, sur les trois plateformes : issue compagnon à ouvrir.

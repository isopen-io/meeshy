...
final class StoryRepostFlowTests: XCTestCase {
    ...
    func test_flux3_kebabEditerEtRepublier_opensComposerPost_publishes() async throws {
        ...
        XCTAssertNotNil(capturedVisibility,
                        "onPublishRepost receives the audience chosen in the composer")

        // 3.c — Replay the production-side callback contract: the caller
        // (StoryViewerView.swift) forwards captured args to PostService.repost
        // with the documented mapping — including the chosen audience, which
        // decides who will see the resulting post.
        ...
        XCTAssertEqual(mockService.lastRepostVisibility, capturedVisibility,
                       "The audience chosen in the composer reaches the service call — it is " +
                       "what decides who sees the resulting post")
    }

...
// MARK: - rejectPendingCall Source Audit

/// Verifies the behavioral contract of `rejectPendingCall`: it must
/// notify the server (emit `call:end`) and clear all pending-call state so
/// the banner disappears and the pending call cannot be accidentally answered later.
@MainActor
final class CallManagerRejectPendingCallTests: XCTestCase {
    ...
    func test_rejectPendingCall_guardsPendingCallExists() throws {
        ...
        guard let body = functionBody(of: "func rejectPendingCall()", in: source) else {
            XCTFail("rejectPendingCall not found"); return
        }
        XCTAssertTrue(
            body.contains("pendingIncomingCall") && body.contains("guard"),
            "rejectPendingCall must guard on pendingIncomingCall being non-nil — " +
            "calling it when there is no pending call should be a no-op"
        )
    }

    func test_rejectPendingCall_emitsCallEndToServer() throws {
        let source = try callManagerSource()
        guard let body = functionBody(of: "func rejectPendingCall()", in: source) else {
            XCTFail("rejectPendingCall not found"); return
        }
        XCTAssertTrue(
            body.contains("emitCallEnd"),
            "rejectPendingCall must call emitCallEnd so the server tears down the " +
            "pending call and notifies the caller — without this the caller is left ringing indefinitely"
        )
    }
    ...
    func test_rejectPendingCall_clearsPendingIncomingCall() throws {
        ...
        XCTAssertTrue(
            ...
            "rejectPendingCall must nil out pendingIncomingCall — otherwise a second " +
            "tap on 'Reject' would emit a duplicate call:end for an already-rejected call"
        )
    }

    func test_rejectPendingCall_hidesCallWaitingBanner() throws {
        let source = try callManagerSource()
        guard let body = functionBody(of: "func rejectPendingCall()", in: source) else {
            XCTFail("rejectPendingCall not found"); return
        }
        XCTAssertTrue(
            ...
            "rejectPendingCall must set showCallWaitingBanner = false so the banner " +
            "dismisses immediately without waiting for a server-side event"
        )
    }
}

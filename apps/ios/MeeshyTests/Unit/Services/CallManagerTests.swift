...
// MARK: - Call waiting: TURN credentials + early-hangup banner cleanup

@MainActor
final class CallWaitingPendingCallTests: XCTestCase {
    ...
    func test_pendingIncomingCall_carriesIceServers() throws {
        // Without an `iceServers` field, `endCurrentAndAnswerPending()` has no
        // way to hand fresh TURN credentials to the call it promotes — the
        // answered call would silently proceed with `iceServers: nil`.
        let source = try callManagerSource()
        XCTAssertTrue(
            source.contains("var pendingIncomingCall: (callId: String, fromUserId: String, fromUsername: String, isVideo: Bool, iceServers: [IceServer]?)?"),
            "pendingIncomingCall must carry iceServers so a call promoted via " +
            "endCurrentAndAnswerPending() can be configured with valid TURN credentials."
        )
    }

    func test_reportIncomingVoIPCall_busyPath_capturesIceServersOnPendingCall() throws {
        let source = try callManagerSource()
        ...
        XCTAssertTrue(
            body.contains("pendingIncomingCall = (callId: callId, fromUserId: callerUserId, fromUsername: callerName, isVideo: isVideo, iceServers: iceServers)"),
            "reportIncomingVoIPCall's busy path must capture the VoIP push's iceServers " +
            "into pendingIncomingCall, not just callId/fromUserId/fromUsername/isVideo."
        )
    }

    func test_handleIncomingCallNotification_busyPath_capturesIceServersOnPendingCall() throws {
        let source = try callManagerSource()
        ...
        XCTAssertTrue(
            body.contains("pendingIncomingCall = (callId: callId, fromUserId: fromUserId, fromUsername: fromUsername, isVideo: isVideo, iceServers: iceServers)"),
            "handleIncomingCallNotification's busy path must capture iceServers into " +
            "pendingIncomingCall, not just callId/fromUserId/fromUsername/isVideo."
        )
    }

    func test_endCurrentAndAnswerPending_forwardsIceServersToPromotedCall() throws {
        // The whole point of carrying iceServers on pendingIncomingCall: the
        // promoted call must actually be configured with them, not just store
        // them unused.
        let source = try callManagerSource()
        guard let body = functionBody(of: "func endCurrentAndAnswerPending()", in: source) else {
            XCTFail("endCurrentAndAnswerPending() not found in CallManager.swift"); return
        }
        XCTAssertTrue(
            body.contains("iceServers: pending.iceServers"),
            "endCurrentAndAnswerPending() must pass pending.iceServers into " +
            "handleIncomingCallNotification(...) — otherwise the answered call proceeds " +
            "with no TURN credentials."
        )
    }

    func test_callEndedSink_clearsPendingIncomingCall_whenCallerHangsUpBeforeAnswer() throws {
        // If the 2nd (call-waited) caller hangs up/cancels before the user
        // answers or rejects it, `call:ended` arrives for a callId that is
        // NOT `currentCallId` (the active call), so `handleRemoteEnd`'s own
        // `currentCallId == callId` guard never fires for it — the banner
        // would otherwise stay stuck pointing at a call the gateway already
        // tore down.
        let source = try callManagerSource()
        guard let range = source.range(of: "socket.callEnded") else {
            XCTFail("socket.callEnded subscription not found in CallManager.swift"); return
        }
        let sinkBody = String(source[range.lowerBound...].prefix(1200))
        XCTAssertTrue(
            sinkBody.contains("self.pendingIncomingCall?.callId == event.callId"),
            "The socket.callEnded sink must check pendingIncomingCall?.callId against " +
            "the incoming event and clear it — otherwise the call-waiting banner never " +
            "clears when the waiting caller hangs up before being answered."
        )
    }

    func test_callMissedSink_clearsPendingIncomingCall_whenCallerHangsUpBeforeAnswer() throws {
        let source = try callManagerSource()
        guard let range = source.range(of: "socket.callMissed") else {
            XCTFail("socket.callMissed subscription not found in CallManager.swift"); return
        }
        let sinkBody = String(source[range.lowerBound...].prefix(1200))
        XCTAssertTrue(
            sinkBody.contains("self.pendingIncomingCall?.callId == event.callId"),
            "The socket.callMissed sink must also clear a matching pendingIncomingCall, " +
            "mirroring the socket.callEnded sink."
        )
    }
}

// MARK: - `.failed` teardown must report to CallKit

@MainActor
final class EndCallInternalFailedReasonReportsToCallKitTests: XCTestCase {
    ...
    func test_endCallInternal_reportsFailedReasonToCallKit_beforeClearingActiveCallUUID() throws {
        // Most `.failed(...)` teardown call sites (setup/negotiation errors)
        // never told CallKit the call ended — CallKit was left with a stuck
        // ringing card / a stale Recents entry. `endCallInternal` is the
        // single choke point every teardown path funnels through, so the
        // report belongs here rather than patched at each `.failed(...)`
        // call site. Every OTHER reason (.rejected/.remote/.missed/
        // .connectionLost) is already reported at its own call site before
        // invoking endCallInternal, so gating on `case .failed` here cannot
        // double-report.
        let source = try callManagerSource()
        guard let body = functionBody(of: "private func endCallInternal(reason: CallEndReason)", in: source) else {
            XCTFail("endCallInternal(reason:) not found in CallManager.swift"); return
        }
        guard let reportRange = body.range(of: "callProvider.reportCall(with: uuid, endedAt: Date(), reason: .failed)"),
              let clearRange = body.range(of: "activeCallUUID = nil") else {
            XCTFail("Expected a CallKit .failed report before activeCallUUID = nil in endCallInternal"); return
        }
        XCTAssertTrue(
            body.contains("if case .failed = reason, let uuid = activeCallUUID"),
            "endCallInternal must gate the CallKit report on `case .failed = reason` " +
            "so it never double-reports a reason already reported at its own call site."
        )
        XCTAssertLessThan(
            reportRange.lowerBound,
            clearRange.lowerBound,
            "The CallKit .failed report must happen BEFORE activeCallUUID is nilled — " +
            "reportCall needs the still-valid CXCallUpdate UUID."
        )
    }
}

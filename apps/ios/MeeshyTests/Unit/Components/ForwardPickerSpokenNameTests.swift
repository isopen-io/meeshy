...
final class ForwardPickerSpokenNameTests: XCTestCase {
    ...
    /// AFFICHÉ. La rangée n'a qu'une seule entrée de nom (`name:`), alimentée
    /// par `target.title` — jamais une entrée d'accessibilité séparée qui
    /// pourrait diverger de l'affichage.
    func test_pickerFeedsTheDisplayedNameToTheRow() throws {
        ...
        XCTAssertTrue(
            code.contains("name: target.title"),
            "la rangée doit recevoir le nom affiché de la cible"
        )
        XCTAssertFalse(
            code.contains("a11yName"),
            "une entrée de nom séparée pour l'accessibilité peut diverger de l'affichage — le prononcé se compose du même `name` que le rendu"
        )
    }

    /// `target.title` d'une conversation vient de `displayName` — le nom
    /// résolu (renommage local compris), jamais `title`, optionnel et nul pour
    /// les conversations directes.
    func test_conversationTargetsCarryTheDisplayedName() throws {
        let url = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("Meeshy/Features/Main/ViewModels/ForwardPickerViewModel.swift")
        let code = AppSourceGuard.stripComments(try String(contentsOf: url, encoding: .utf8))

        XCTAssertTrue(
            code.contains("title: conversation.displayName"),
            "la cible conversation doit porter le nom affiché résolu de la conversation"
        )
    }
}

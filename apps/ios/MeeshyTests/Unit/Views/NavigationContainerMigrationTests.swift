...
final class NavigationContainerMigrationTests: XCTestCase {
    ...
    // MARK: - Migrated in 218i — the last holdout

    /// The mood composer is presented as a `.sheet` from all three of its call
    /// sites, which on iPad is a form sheet — i.e. *regular* width, exactly where
    /// the double-column default bites. Both of its controls (Fermer, Publier)
    /// are `ToolbarItem`s, so the collapse could displace the dismiss affordance
    /// and the screen's primary action at once.
    ...
    /// 214i pinned the remaining offenders to `{StatusComposerView.swift}` and
    /// noted that the expectation would drop to the empty set once that last file
    /// was migrated. 218i migrated it, so this is no longer a tolerated-debt list
    /// with one entry — it is an absolute invariant: **no** source in the shipping
    /// app targets may declare the deprecated container.
    ///
    /// Deliberately still a set comparison rather than an `isEmpty` check: when it
    /// fails, the diff names the offending file instead of only asserting that one
    /// exists.
    func test_noNavigationViewRemains() throws {
        XCTAssertEqual(
            try filesUsingDeprecatedContainer(), [],
            "A NavigationView container was reintroduced. Use NavigationStack: it is available " +
            "unconditionally at the iOS 16.0 deployment floor, and NavigationView's default " +

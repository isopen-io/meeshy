...
enum SentimentLevel: String, CaseIterable {
    case veryNegative
    case negative
    ...
    var emoji: String {
        switch self {
        case .veryNegative:     return "\u{1F621}"  // angry face
        case .negative:         return "\u{1F620}"  // angry
        case .slightlyNegative: return "\u{1F615}"  // confused
        case .neutral:          return "\u{1F610}"  // neutral
        case .slightlyPositive: return "\u{1F642}"  // slightly smiling
        case .positive:         return "\u{1F60A}"  // smiling with eyes
        case .veryPositive:     return "\u{1F929}"  // star-struck
        }
    }

    static func from(score: Double) -> SentimentLevel {
        switch score {
...
struct DetectedLanguage: Identifiable {
    let id: String  // NLLanguage rawValue
    let code: String
    let flag: String
    let name: String

    static let supported: [DetectedLanguage] = [
    ...
    static func find(code: String) -> DetectedLanguage? {
...
class TextAnalyzer: ObservableObject {
    @Published var sentiment: SentimentLevel = .neutral
    @Published var language: DetectedLanguage? = nil
    @Published var languageOverride: DetectedLanguage? = nil
    ...
    var displayLanguage: DetectedLanguage? {
    ...
    func setLanguageOverride(_ lang: DetectedLanguage?) {
    ...
    private func performAnalysis(text: String) {
        ...
        // Sentiment via keyword heuristic
        let score = computeSentiment(text: cleaned)
        DispatchQueue.main.async {
            self.sentiment = SentimentLevel.from(score: score)
        }

        // Language via NLLanguageRecognizer
        recognizer.reset()
    ...
    private func computeSentiment(text: String) -> Double {
        ...
        for word in words {
            if let val = Self.positiveWords[word] {
                score += val
            } else if let val = Self.negativeWords[word] {
                score += val
            }
        }

        // Normalize: divide by word count, clamp to [-1, 1]
        let normalized = score / Double(words.count) * 2.0
...
struct SmartContextZone: View {
    @ObservedObject var analyzer: TextAnalyzer
    let accentColor: String
    let isCompact: Bool  // true = show only sentiment (multiline mode)
    var showFlag: Bool = true  // only show flag after first newline
    ...
    var body: some View {
        VStack(spacing: isCompact ? 0 : 2) {
            ...
            // Language flag (shown when showFlag is true, even in compact mode)
            ...
            ZStack {
                ...
                ScrollView {
                    LazyVStack(spacing: 4) {
                        // Auto-detect option
                        Button {
                            analyzer.setLanguageOverride(nil)
                        } label: {
                            HStack(spacing: 14) {
                                Text("\u{1F310}")
                                    .font(.system(size: 28))
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    ...
                                    if let detected = analyzer.language {
                                ...
                                if analyzer.languageOverride == nil {
                        ...
                        // Language list
                        ForEach(DetectedLanguage.supported) { lang in
                            Button {
                                analyzer.setLanguageOverride(lang)
                            } label: {
                                HStack(spacing: 14) {
                                    Text(lang.flag)
                                        .font(.system(size: 28))
                                    ...
                                    if analyzer.languageOverride?.id == lang.id {
                                        Image(systemName: "checkmark.circle.fill")
                                            .font(.system(size: 22))
                                            .foregroundColor(Color(hex: "4ECDC4"))
                                    }

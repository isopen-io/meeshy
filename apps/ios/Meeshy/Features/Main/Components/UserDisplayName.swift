...
// MARK: - Display Name Resolution (private core)

/// Priority: displayName > firstName+lastName > username > fallback
private func resolveDisplayName(
    ...
    if let dn = displayName?.trimmingCharacters(in: .whitespaces), !dn.isEmpty {
        return dn
    }
    ...
    if let un = username?.trimmingCharacters(in: .whitespaces), !un.isEmpty {
        return un
    }
    return fallback
}
...
func getUserDisplayName(_ user: MeeshyUser?, fallback: String = "Utilisateur inconnu") -> String {
...
func getUserInitials(_ user: MeeshyUser?, fallback: String = "?") -> String {
    resolveInitials(from: getUserDisplayName(user, fallback: fallback))
}

// MARK: - APIMessageSender

func getUserDisplayName(_ sender: APIMessageSender?, fallback: String = "Utilisateur inconnu") -> String {
...
func getUserInitials(_ sender: APIMessageSender?, fallback: String = "?") -> String {
    resolveInitials(from: getUserDisplayName(sender, fallback: fallback))
}

// MARK: - APIConversationUser

func getUserDisplayName(_ user: APIConversationUser?, fallback: String = "Utilisateur inconnu") -> String {
    guard let user else { return fallback }
    return resolveDisplayName(displayName: user.displayName, username: user.username, fallback: fallback)
}

func getUserInitials(_ user: APIConversationUser?, fallback: String = "?") -> String {
    resolveInitials(from: getUserDisplayName(user, fallback: fallback))
}

// MARK: - APIAuthor

func getUserDisplayName(_ author: APIAuthor?, fallback: String = "Utilisateur inconnu") -> String {
...
func getUserInitials(_ author: APIAuthor?, fallback: String = "?") -> String {

...
private struct MyStoryRow: View {
        ...
        HStack(spacing: 12) {
                ...
                HStack(spacing: 12) {
                    ...
                    metric(icon: "heart.fill", value: story.reactionCount,
                           label: String(localized: "story.mine.metric.reactions.a11y",
                                         defaultValue: "\(story.reactionCount) réactions"))
            ...
            if !isSelecting {
                Image(systemName: "ellipsis")
                    ...
                    .padding(8)
                    // Décoratif : les actions de la ligne sont exposées à
                    // VoiceOver via le `.contextMenu` (rotor Actions).
    ...
    private func metric(icon: String, value: Int, label: String) -> some View {
        ...
        .foregroundColor(.secondary)
        // Icône + nombre nu = illisible en VoiceOver ; on annonce le sens
        // (« 42 vues ») via un libellé combiné dédié.

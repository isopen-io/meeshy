...
/// Renders the SDK's locale-agnostic `RelativeTime` ladder into a localized,
/// compact label: seconds (`À l'instant`) → minutes → hours → days → absolute
/// date. Shared by `FeedPostCard` and the reel overlay so timestamps read
...
enum RelativeTimeFormat {
    static func string(for date: Date, now: Date = Date()) -> String {
        ...
        case .now:
            return String(localized: "time.justNow", defaultValue: "À l'instant", bundle: .main)
        case .minutes(let m):
            return String(localized: "time.minutesShort", defaultValue: "\(m)m", bundle: .main)
        case .hours(let h):
            return String(localized: "time.hoursShort", defaultValue: "\(h)h", bundle: .main)
        case .days(let d):
            return String(localized: "time.daysShort", defaultValue: "\(d)j", bundle: .main)
        case .date(let date):
            return absoluteFormatter(for: date, now: now).string(from: date)
        }
    }

    private static func absoluteFormatter(for date: Date, now: Date) -> DateFormatter {
...
struct ReelPageView: View {
    ...
    private var infoOverlay: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(RelativeTimeFormat.string(for: reel.timestamp))
                .font(.caption2)

...
struct UserStatsView: View {
    ...
    private var statsCards: some View {
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                statCard(value: "\(viewModel.stats?.totalMessages ?? 0)", label: String(localized: "user.stats.messages", defaultValue: "Messages", bundle: .main), color: MeeshyColors.errorHex, icon: "bubble.left.fill")

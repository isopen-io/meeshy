...
struct CommunityLinkDetailView: View {
    ...
    private var headerCard: some View {
        VStack(spacing: 10) {
            ZStack {
                ...
                // Hero glyph frozen: constrained inside a fixed 60×60 circle — a Dynamic-Type
                // scaling glyph would overflow the badge. Decorative → hidden from VoiceOver
                // (the community name below carries the meaning). Doctrine 74i/86i.
        ...
        Button(action: action) {
            VStack(spacing: 6) {
                ZStack {
                    ...
                    // Glyph frozen: constrained inside the fixed 52×52 badge (doctrine 74i/86i).
                    // The button's text label below is the VoiceOver label.
    ...
    private func infoRow(_ label: String, value: String) -> some View {
        HStack {
            ...
            Spacer()
            Text(value).font(MeeshyFont.relative(13, weight: .medium)).foregroundColor(theme.textPrimary)

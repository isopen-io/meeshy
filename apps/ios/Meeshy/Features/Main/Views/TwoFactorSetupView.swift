    ...
    private func backupCodesList(_ codes: [String]) -> some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
            ForEach(codes, id: \.self) { code in
                Text(code)
                    // Code monospace dans une grille 2 colonnes bornée : gardé hors
                    // Dynamic Type pour qu'un code de secours (sécurité-critique) ne soit
                    // jamais tronqué/débordé à grande taille (cf. doctrine layout borné).
                        ...
                        if !codeSubmitted {
                            ...
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                                ForEach(viewModel.recoveryCodes, id: \.self) { code in
                                    Text(code)
                                        // Code monospace en grille 2 colonnes bornée : gardé hors
                                        // Dynamic Type pour qu'un code de secours ne soit jamais
                                        // tronqué à grande taille (cf. doctrine layout borné).
                                        .font(.system(size: 14, weight: .semibold, design: .monospaced))
                                        .foregroundColor(theme.textPrimary)
                                        .frame(maxWidth: .infinity)
                                        .padding(.vertical, 8)

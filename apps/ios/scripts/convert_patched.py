#!/usr/bin/env python3
"""
Convert OpenVoice V2 to CoreML with patched modules.
Avoids IntTensor indexing that causes 'intimplicit' error.
"""
...
SPEC_CHANNELS = 513  
EMBEDDING_DIM = 256
HIDDEN_CHANNELS = 192  # From config
...
def fused_tanh_sigmoid_multiply_fixed(input_a, input_b, n_channels):
    """Fixed version that uses integer directly instead of tensor indexing."""
...
class WNPatched(nn.Module):
    """Patched WN that avoids IntTensor indexing."""
    
    def __init__(self, original_wn, hidden_channels):
        super().__init__()
        self.hidden_channels = hidden_channels
        self.n_layers = original_wn.n_layers
        self.in_layers = original_wn.in_layers
        self.res_skip_layers = original_wn.res_skip_layers
        self.drop = original_wn.drop
        if hasattr(original_wn, 'cond_layer'):
            self.cond_layer = original_wn.cond_layer
        else:
    ...
    def forward(self, x, x_mask, g=None, **kwargs):
        ...
        for i in range(self.n_layers):
            ...
            # Use fixed integer directly instead of IntTensor[0]
            acts = fused_tanh_sigmoid_multiply_fixed(x_in, g_l, self.hidden_channels)
            acts = self.drop(acts)
...
class PosteriorEncoderPatched(nn.Module):
    """Patched posterior encoder."""
    
    def __init__(self, original_enc_q, hidden_channels):
        super().__init__()
        self.pre = original_enc_q.pre
        self.enc = WNPatched(original_enc_q.enc, hidden_channels)
        self.proj = original_enc_q.proj
        self.out_channels = original_enc_q.out_channels
    ...
    def forward(self, x, x_mask, g=None):
        ...
        z = m * x_mask  # No sampling
        return z, m, logs, x_mask


class VoiceConverterPatched(nn.Module):
    """Full patched voice converter."""
    
    def __init__(self, model, hidden_channels):
        super().__init__()
        self.enc_q = PosteriorEncoderPatched(model.enc_q, hidden_channels)
    ...
    def forward(self, spec, g_src, g_tgt):
        ...
        # Use zeros for zero_g=True
        zero_g = torch.zeros_like(g_src)
        
        z, m, logs, y_mask = self.enc_q(spec, mask, g=zero_g)
...
def main():
    print("=" * 60)
    print("OpenVoice V2 -> CoreML (Patched)")
    print("=" * 60)
    ...
    wrapper = VoiceConverterPatched(model, hidden_channels)
    wrapper.train(False)

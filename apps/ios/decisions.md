...
- L'evenement `message:send` a ete etendu pour porter les effets de message — `isBlurred`, `expiresAt` (ephemere), `effectFlags` — ajoutes au schema Zod `SocketMessageSendSchema` et au `messageRequest` du handler `handleMessageSend`, a parite stricte avec la route REST. `MessageProcessor.saveMessage` recompose le bitfield `effectFlags` a l'identique pour les deux transports.
**Garde (reste sur REST)**: messages E2EE (le chiffrement iOS produit un payload de forme REST — `content` base64 + `isEncrypted`/`encryptionMode` — et non la forme socket `encryptedContent`/`encryptionMetadata` du web), view-once (`isViewOnce`/`maxViewOnceCount`, non portes ni par REST ni par `message:send`), et messages avec pieces jointes (voie WS dediee `message:send-with-attachments`).

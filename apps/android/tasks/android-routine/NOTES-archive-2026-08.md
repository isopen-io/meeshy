# Notes archive — 2026-08

Older entries moved out of `NOTES.md` per the archival threshold in
`tasks/android-parity-ios-debt-agent-prompt.md` §Hygiène (routine iteration 10, 2026-08-09).
Same append/oldest-first convention as the live file — the oldest entry in this archive is at
the top, the most recent (pre-archival cutoff) at the bottom. See `NOTES.md` for current,
live entries (most recent lessons).
...
- **Extend a grouping SSOT, don't fork it.** `ConversationSections.of` gained a second
  ...
  **default empty list must reduce to the exact prior behaviour** so every pre-existing test passes
  untouched — here an empty catalogue means no `categoryId` can match, so all non-pinned rows land in
  `ALL` just like before. Verify that by running the OLD tests unchanged (8/8 green) *before* adding the
  new category cases; if an old test needs editing you've changed behaviour, not extended it.
- **Fold orphaned buckets into the catch-all inside the `groupBy` key, not after.** iOS concatenates
  `bySection["__other__"] + filtered.filter { orphaned }` post-hoc, which reorders. A single key function —
  `if (pinned && catId==null) PINNED else if (catId in catalogue) catId else OTHER` — sends uncategorized
  *and* orphaned rows to the same `OTHER` bucket in one pass, preserving encounter order (`groupBy` is
  order-stable in Kotlin) and dropping nothing. Cleaner and more correct than the port's source.
- **A pinned row with a category belongs in its category, not Épingles** (iOS `isPinned && sectionId==nil`
  is the *pinned-bucket* guard, not "pinned always floats"). This is the subtle branch — mutation-proof it
  by dropping the `&& categoryId==null` guard and confirming exactly the one "stays-in-category" test fails.
- **An exhaustive `when(kind)` in the Composable forces edits when you add an enum constant.** Adding
  `CATEGORY` to `ConversationSectionKind` broke the screen's `titleRes()/icon()/containerColor()` `when`s at
  compile time — expected. Give `CATEGORY` real branches (folder glyph, accent) and render the header from
  `section.title ?: stringResource(kind.titleRes())` so category sections show their own name; the
  `titleRes()` arm for `CATEGORY` is a never-shown fallback (documented).
...
2. **`suggestingUsername` returns the SAME instance when nothing should change** (username already non-blank, or a name still missing) — mirrors the web `!prev.username` guard and makes the auto-fill idempotent + cheap; asserted with `isSameInstanceAs`.
3. **A `requireAccount` share link cannot be joined anonymously — encode that as `canSubmit == false` + a `requiresAccount` steer, don't let the form build a request the gateway will reject.** This is the one SOTA-over-web+iOS divergence and the discriminating mutation target (killing `!requireAccount &&` in `canSubmit` fails exactly `requireAccount_blocksAnonymousSubmit_…`). The screen renders a distinct "sign in" arm for it — no dead end.
4. **`toRequest()` is the single wire-builder: trim every field + null-omit each empty optional** (username/email/birthday → `null`, never `""`) + `fr` fallback for a blank language. A malformed body can't leave the type; the VM never hand-assembles an `AnonymousJoinRequest`.
5. **A new `meeshy://join/{id}` deep link needs NO `AndroidManifest` change** — the app already declares a host-less `<data android:scheme="meeshy"/>` autoVerify intent-filter that catches every `meeshy://` host, so registering the route with `navDeepLink { uriPattern = "meeshy://join/{identifier}" }` in `MeeshyApp` is sufficient. (An `https://meeshy.me/join/{id}` App-Link WOULD need a manifest intent-filter — deferred as its own slice.)
6. **`preview()` is a pure read — assert it touches NOTHING.** `AnonymousSessionRepository.preview()` wraps the no-auth `anonymous/link/{id}` GET; its tests assert `store.load()==null` && `tokenStore.sessionToken==null` after both success and failure, so a preview can never strand a half-authenticated guest.
Gate: `:app:assembleDebug` + whole-graph `testDebugUnitTest` BUILD SUCCESSFUL, +30 tests.

## Lesson (2026-07-25, `conversation-settings-form`) — a dirty-diff reducer is the cleanest settings-editor core; complete a `[~]` item by shipping the missing HALF; `by collectAsStateWithLifecycle()` needs the `getValue` import
Shipped the admin conversation-settings editor, closing the `[~]` moderation item. Durable lessons:
1. **Model a settings editor as an immutable dirty-diff reducer, not a bag of `@State`.** `ConversationSettingsForm` holds the four edited values **plus a `baseline`** captured at `from(conversation)`; `isDirty`/`canSave` compare the two, and `toUpdate()` emits a **minimal patch of only the changed fields** (null-omit). This makes a no-op save structurally impossible (`toUpdate()==null`) and never overwrites an unchanged field — a clean SOTA win over iOS's always-PUT-the-full-object editor, and the discriminating mutation target (killing `if(!isDirty)return null` fails exactly the 5 partial-patch/dirty tests). `rebaselined()` re-anchors the baseline to the edited values after a server-accepted save so a second immediate save is inert.
2. **Snap off-menu server values at seed time so the picker always has a match AND the seed is never spuriously dirty.** `SlowModeOptions.nearest()` maps a raw `slowModeSeconds` (a migrated `45`) onto the closest offered `{0,10,30,60,300}`. Crucially `from()` snaps the **baseline too**, so loading a `45`-second conversation shows `30` selected with `isDirty==false` (not a phantom pending change). Tie → smaller (ascending `minByOrNull` returns first min); `>max` → clamp; `null`/`≤0` → off.
3. **A `[~]` (partial) parity item is completed by shipping the missing HALF, not by re-doing the done half.** Slow-mode composer *enforcement* (`chat-slow-mode-cooldown`) and attachment *gating* (`composer-send-gate`) already shipped; the item stayed `[~]` only for the admin *editor*. Read the item's "Remaining for this item:" note — it named exactly the editor — and ship that. One slice flipped it to `[x]`.
4. **`val x by someFlow.collectAsStateWithLifecycle()` requires `import androidx.compose.runtime.getValue`.** Without it the delegate fails with *"State<…> has no method getValue(…), so it cannot serve as a delegate"*, and the error **cascades** into a dozen bogus "Unresolved reference" errors on the unwrapped value's members — fix the one missing import, not the symptoms. (`ChatScreen` had it; a fresh sheet file didn't.)
5. **Bind a header-action sibling ViewModel via a `conversationId` on `UiState` + `hiltViewModel()` inside the sheet + `LaunchedEffect(id){ vm.load(id) }`** — no assisted injection, no nav-arg re-read, no new route. The VM injects only the repo (Hilt-provided); `load(id)` supplies the runtime id and is idempotent. Gate: `./apps/android/meeshy.sh check` (whole graph) BUILD SUCCESSFUL, +37 tests.

## Lesson (2026-07-25, `signup-availability-probe`) — a background field update must not clear a surfaced error; and don't scan-dedup a conflated StateFlow (it's dead code); and the container needs a UTF-8 locale or the Kotlin compiler crashes on non-ASCII test-method filenames
Wired the debounced availability network probe into `RegistrationViewModel`, closing the last orphan seam of the wizard. Four durable lessons:
1. **Split `editFields` (user edit → clears `errorMessage`) from `updateFields` (background verdict → preserves it).** The naive wiring routed the availability verdict through `editFields`, which resets `errorMessage = null`. Once the verdict came from a *background* debounce probe (fired by `advanceUntilIdle` after `register()` failed), it silently wiped the "Username taken" error banner — caught by `register_onRecapValid_failure_surfacesErrorAndNoRealtime` going red. Rule: **only a user-initiated field edit may clear a surfaced error; any background/network-driven field mutation must leave transient UI state (`errorMessage`) intact.**
2. **A conflated `StateFlow` already gives you `removeDuplicates` — don't re-implement dedup.** The first design used `scan` to carry the previously-emitted value into `SignupAvailabilityPolicy.*Intent(current, previous)` so the `Unchanged` arm could suppress re-probes (mirroring iOS `.debounce().removeDuplicates()`). A mutation (`intent(current, acc.first)` → `intent(current, null)`) changed **nothing** — because a StateFlow source only re-emits on change, so `current == previous` at a debounce emission never happens; the `Unchanged` branch was unreachable dead code. Simplified to `debounce → distinctUntilChanged → intent(value, null) → Check?probe:clear`, leaving 2 reachable branches. **Test-drive the mutation before claiming a branch is load-bearing; if killing it changes no test, it's dead code, not coverage.** The `Unchanged` arm stays covered by the policy's own `:core:model` unit tests.
3. **The build container defaults to a `POSIX`/non-UTF-8 locale (`LANG=`, `LC_CTYPE=POSIX`).** The Kotlin daemon then throws `java.nio.file.InvalidPathException: Malformed input or input contains unmappable characters` when writing a `.class` for a test method whose name contains a non-ASCII char (e.g. the em-dash in `ActiveCallRepositoryTest`'s `returns null … — a probe never crashes …`). Fix: `export LANG=C.utf8 LC_ALL=C.utf8` before `./gradlew` (only `C.utf8` is available, not `C.UTF-8`). Without it, `:sdk-core:compileDebugUnitTestKotlin` fails with an "Internal compiler error" that looks unrelated to your diff.
4. **`cd` inside a Bash tool call persists to later calls** (the shell cwd carries over). A `sed` run after `cd feature/auth/…/auth` left cwd there, so the next `./gradlew` failed "No such file". Always `cd /home/user/meeshy/apps/android` at the top of each gradle invocation, or use absolute paths.

## Lesson (2026-07-25, `registration-wizard-viewmodel`) — the app-side ViewModel that finally WIRES a family of orphan cores is a legitimate, high-value slice; keep it a thin caller, defer the network seam, and make one SOTA divergence the discriminating branch
First app-side wiring of the registration flow: `:feature:auth/RegistrationViewModel` binding the ~6 shipped `:core:model/auth` cores into one observable UDF wizard. (1) **When many pure cores have shipped but nothing consumes them, wiring them into a real `@HiltViewModel` is the correct next slice — it's strictly more "real" than another core and creates no orphan output.** The ViewModel holds only raw inputs (`currentStep` + `RegistrationFields` + submit flags); every *decision* is a `get()`/method delegating to a core (`canProceed`→`RegistrationStepGate`, `fill`→`RegistrationProgressBar`, `next`→`RegistrationStepNavigator.advance`). Re-implement nothing — if you're writing an `if (step == …)` in the VM, the core already has it. (2) **Match the sibling ViewModel's proven seams verbatim.** `AuthViewModel.login` already established the pattern: `viewModelScope.launch` → `authRepository.<call>` → `RealtimeSessionCoordinator.onAuthenticatedChanged(true)` on `NetworkResult.Success`, error message on `Failure`. `register()` copies it exactly (RECAP-gate + `isSubmitting` single-flight guard added). The test harness (`FakeAuthApi` + `InMemoryTokenStore` + `SessionRepository` + `mockk` coordinator, `StandardTestDispatcher` + `advanceUntilIdle`) is a straight lift from `AuthViewModelTest` — no new infra. (3) **Defer the async network probe behind a plain setter seam.** The wizard's availability check is a 1 s debounce → `checkAvailability` network call; wiring it now would balloon the slice and couple the VM to an unbuilt `AuthService`. Instead expose `onUsernameAvailability(Boolean?)`/`onEmailAvailability`/`onPhoneAvailability` — synchronous setters a *future* slice drives. The VM is fully testable today; the seam is a clean future join, not a dead-end. (4) **Make the one SOTA-over-iOS divergence a pinned, discriminating branch.** iOS keeps a stale `usernameAvailable: Bool?` after the field is edited (the gate can briefly pass on a verdict for a since-changed value); Android clears it to `null` on every `onUsernameChange`/`onEmailChange`/`onPhoneChange`. That single behaviour is the mutation target: dropping `usernameAvailable=null` from `onUsernameChange` fails **exactly** `editingUsername_invalidatesStaleAvailability` (23 run / 1 failed / no collateral). (5) **`:feature:auth` gets the `:core:model` cores for free via `:sdk-core`'s `api(project(":core:model"))`** — no build.gradle edit needed; `import me.meeshy.sdk.model.auth.*` just resolves. (6) **Mutation-restore caveat (again):** the VM was an untracked new file during the mutation check, so I applied it with `sed` and restored with `Edit` (string-exact) — `git checkout` can't revert an untracked file. Gate: `:feature:auth:testDebugUnitTest` 23/23 new + `:app:assembleDebug` (whole graph) BUILD SUCCESSFUL.

## Lesson (2026-07-24, `auth-token-refresh-authenticator`) — wire the pure 401 policy into a THIN OkHttp `Authenticator`; two Kotlin/OkHttp gotchas cost a build each
Wiring the `TokenRefreshPolicy.decideOn401` core (shipped `auth-token-refresh-policy-core`) into a real `okhttp3.Authenticator`. (1) **A KDoc containing a code span with `/*` opens a NESTED block comment and Kotlin fails with "Unclosed comment".** I wrote `` `/auth/*` `` inside a `/** … */` — the `/*` inside started a nested comment that never closed, killing `kspDebugKotlin` with a cryptic line-past-EOF error. Kotlin block comments nest; never put a literal `/*` (or a glob like `/auth/*`) in a doc comment — use `…` (`/auth/…`) or escape it. (2) **OkHttp `Request.Builder().method("POST", null)` throws `IllegalArgumentException`** — POST/PUT/PATCH *require* a non-null body. A test helper building a POST request for the "preserve method on replay" case must pass a real body: `"".toRequestBody()` (import `okhttp3.RequestBody.Companion.toRequestBody`). GET/HEAD are the ones that must be null. (3) **The Authenticator stays framework glue, all *decisions* stay in the pure `:core:model` policy** — `authenticate` reads `endpoint`+`hasRefreshedOn401` and `when`s over `decideOn401`'s sealed result; the only new logic here is the token-store side effects (rewrite bearer / `clear()`) and `endpointOf` (strip `apiPathPrefix` so real `/api/v1/…` routes match the policy's `/auth/…` literals). No decision is re-implemented → SSOT held. (4) **`Response.priorResponse != null` is the idiomatic OkHttp retry-once loop guard** — no hand-rolled counter. It's non-null on the *second* 401 because OkHttp chains the retried response's prior. Tested with a constructed `Response.Builder().priorResponse(prior)` — no MockWebServer needed; constructed `Response`s + a fake `TokenRefresher` cover every arm. (5) **Break the create-time cycle with `lateinit var api`**: the Authenticator needs `api.auth.refresh`, but `api` doesn't exist when the OkHttpClient is built. A `TokenRefresher` closure capturing a `lateinit var api` is safe because it's only invoked at 401 time (long after assignment); `/auth/refresh` being policy-ineligible guarantees the refresh call can't re-enter the Authenticator (no recursion). `runBlocking` inside `authenticate` is legal — Authenticators run on OkHttp's background dispatcher thread. (6) **SOTA-over-iOS: a *blank* refreshed token → teardown** (`newJwt.isNullOrBlank()`), not just null — iOS trusts any non-nil string. (7) **Mutation (RED proof):** forcing `hasRefreshed = false` fails **exactly** `eligibleEndpoint_secondAttempt_priorResponsePresent_tearsDownWithoutRefreshing` (18 run / 1 failed / no collateral). Applied via `sed` on a `cp`-saved pristine copy then `cp` back — the untracked-new-file `git checkout` caveat still applies (both new files were uncommitted during the mutation). Gate: `:core:network:testDebugUnitTest` 11/11 new + `gradle assembleDebug testDebugUnitTest` all-modules BUILD SUCCESSFUL.

## Lesson (2026-07-22, `anonymous-session-permissions-core`) — server-advertised capabilities are UNTRUSTED: put the client-side forcing in ONE factory that every consumer reuses; guard the transform's nullables that iOS force-unwraps
Porting iOS anonymous (shared-link) guest permissions (`ParticipantModels.swift` `defaultUser`/`defaultAnonymous` + `AnonymousSessionContext.swift`'s `toSessionContext`). (1) **The whole security value is that a guest's `canSendVideos/Audios/Locations/Links` are forced `false` on the CLIENT regardless of the join response.** A compromised/buggy gateway sending `canSendVideos:true` must still not open the video button. Encode that forcing in exactly one place — `ParticipantPermissions.anonymous(messages, files, images)` — and have BOTH `defaultAnonymous` and `toSessionContext` build through it. Never re-list the four false flags at each call site (that's how one site silently drifts to trusting the server). (2) **Check what's already ported before adding models.** `ParticipantPermissions` (all 7 flags, defaulting true), `AnonymousParticipant` (only 3 negotiable flags, defaulting false), `AnonymousJoinResponse` and `JoinedConversation` already existed in `:core:model` — the slice added only the two companion constants + the `anonymous()` factory + `AnonymousSessionContext` + the extension transform. Zero duplication. (3) **iOS force-unwrap → Kotlin nullable-guard is a real behavioural divergence worth a test, not a silent one.** iOS `toSessionContext` reads `participant.id`/`conversation.id` on non-optional fields; Android's response models make them nullable (the wire genuinely can omit them), so `toSessionContext()` returns `AnonymousSessionContext?` — `null` for missing participant/conversation OR a blank session token (a token that can't authenticate any later `X-Session-Token` request is a dead session). Pinned by `toSessionContext_missingParticipant/Conversation_returnsNull` + `blank`/`empty` token. Document it as an intentional SOTA-over-iOS (graceful vs crash). (4) **Identifier-mapping tests must use DISTINCT literals** (`session-abc`/`participant-xyz`/`conversation-123`/`link-777`) so a field swap (e.g. `linkId`↔`conversationId`) is caught — equal placeholders would pass a swapped mapping. (5) **Mutation (RED proof):** flipping `anonymous()`'s `canSendVideos = false` → `true` fails **exactly 4** tests (the two `anonymous()` hardening cases, `defaultAnonymous`, and `toSessionContext_hardensGuestCapabilities`) — "no collateral" here means "exactly the SSOT's consumers", which is the point: one forcing, four dependents. Applied via `perl -0pi` anchored on the two preceding lines (the file was already committed, so `git checkout --` cleanly restored it — the untracked-file caveat from prior slices did NOT apply because I committed the checkpoint first). Gate: `:core:model:testDebugUnitTest` 12/12 new (whole module green) + `assembleDebug testDebugUnitTest` all-modules BUILD SUCCESSFUL.

## Lesson (2026-07-22, `auth-token-refresh-policy-core`) — a same-named core may already exist: EXTEND it, don't duplicate; lift a stateful `URLSession` 401 loop into sealed pure decisions
Porting the reactive side of iOS transparent session refresh (`APIClient.requestWithHeaders` / `mapUnauthorized`). (1) **Grep for the target object name across `--include=*.kt` BEFORE creating a new file.** `TokenRefreshPolicy` already existed (shipped by `auth-jwt-expiry-core` with only the proactive `shouldRefresh` = `refreshSession(force:)` guard) — and its *doc comment in `JwtExpiryTest`* already forward-referenced it. The right move was to **extend the existing object** in `JwtExpiry.kt` with the endpoint-eligibility gate + the 401 branch, not spin up a second `TokenRefreshPolicy`. A duplicate would have collided and split the SSOT. (2) **A stateful `URLSession` retry loop is a decision tree hiding inside imperative control flow — lift each branch into a pure function returning a sealed decision.** iOS inlines `isRefreshOrAuth`, the proactive `if let token, shouldAttemptRefresh && isTokenExpired`, `mapUnauthorized`, the `hasRefreshedOn401` gate, and the post-refresh status cascade all inside one ~170-line method, so *none* of it is testable without real HTTP. Android: `isRefreshEligible` (bool), `shouldRefreshBeforeSend` (bool, reuses `JwtExpiry.isExpired` — no expiry re-impl), `mapUnauthorized`→`UnauthorizedMapping`, `decideOn401`→`Unauthorized401Decision{InvalidCredentials, RefreshAndRetry, Teardown}`, `classifyRetryStatus`→`RetryOutcome{Success, Teardown, ServerError}`. The future OkHttp `Authenticator` becomes a thin caller; every arm is JVM-testable. (3) **The credential-vs-session split is the whole point of `mapUnauthorized`: a 401 on `/auth/login*` must NEVER tear down a session** (there is none — it's a wrong password), and `decideOn401` must keep returning `InvalidCredentials` even when `hasRefreshedOn401 == true`. Pinned by `on401_loginEvenAfterRefreshFlag_stillInvalidCredentials`. (4) **`/auth/refresh` is eligible-excluded AND session-mapped**, so a 401 there → `Teardown` (the refresh token itself is dead) — a distinct arm from a regular endpoint's first-time `RefreshAndRetry`. (5) **SOTA-over-iOS: a *blank* server message falls back to the default** (iOS only `??` nil-coalesces), so an empty gateway body never surfaces as an empty error dialog. Documented as an intentional divergence, tested both null→default and blank→default. (6) **Mutation (RED proof):** dropping `!hasRefreshedOn401` from `decideOn401` fails **exactly** `on401_regularEndpointAfterAlreadyRefreshing_tearsDown` (29 run / 1 failed / no collateral) — the refresh-once gate is a real isolated behaviour. Applied via `sed` on a `cp`-saved pristine copy (untracked-file `git checkout` caveat still holds). Gate: `:core:model:testDebugUnitTest` 29/29 new (whole module green) + `:app:assembleDebug` BUILD SUCCESSFUL.

## Lesson (2026-07-22, `auth-language-step-selection-core`) — port a SwiftUI picker's scattered filter/label/select/preview into one pure SSOT that REUSES the language catalogue; faithful over "clever" (untrimmed `isEmpty`, French default for English)
Porting iOS `StepLanguageView` (`OnboardingStepViews.swift`) — the Step 6 system/regional content-language picker with a live translation preview. (1) **A SwiftUI picker spreads four separable decisions across its `body` + a `@State editingTarget`: the search filter, the summary-card label, the slot-aware selection write, and the preview mapping. Lift all four into one framework-free `object` + a `(system, regional)` state data class + a `LanguageSlot` enum.** The composable becomes a thin caller (`select(slot, code, state)`, `filter(query)`, `translationPreview(system)`), and every branch is JVM-testable — no Robolectric needed for any of the logic. (2) **The picker list is NOT a new list — it's the existing catalogue SSOT.** iOS `LanguageSelector.defaultLanguages = LanguageData.allLanguagesCommonFirst`; Android already has `LanguageData.allLanguagesCommonFirst`, so `pickerLanguages` is a `get()` delegating to it (no hand-copied 76-language list, zero drift). Same for `summaryLabel`/`selectedLanguageName` → resolve through `LanguageData.info(code)`. Reusing the catalogue is the whole SSOT point; re-declaring the list would have been the anti-pattern. (3) **Faithful port beats "improving" the filter.** iOS `filteredLanguages` guards on `searchText.isEmpty` (UNtrimmed) then `.lowercased().contains` on `name`/`id`. Tempting to trim (so a whitespace-only query = no filter, arguably nicer UX), but that silently diverges Android from iOS behaviour — parity is the routine's job. Ported verbatim: `if (query.isEmpty()) return languages`, filter on `nativeName`/`code`. Documented the choice. (4) **`translationPreview` maps 11 explicit languages + a French `default:` for everything else — INCLUDING English.** iOS `translatedExample`'s `default:` returns the French sentence; English is not one of the 11 arms, so `translationPreview("en")` correctly returns French. Pinned this explicitly (`translationPreview_englishSystem_fallsBackToFrenchDefault`) so a future "add en" edit can't silently change it without a red. (5) **The two summary/name resolvers differ ONLY in the resolved-branch shape** (`"<flag> <nativeName>"` vs bare `nativeName`) and share the raw-code fallback — keep them separate functions, not one with a flag param; the composable needs both shapes and separate functions keep each branch trivially coverable. (6) **Mutation (RED proof):** `summaryLabel`'s `?: return code` → `?: return ""` fails **exactly** `summaryLabel_unknownCode_fallsBackToRawCode` (the blank-code test still passes since it expects `""`), 32 run / 1 failed / no collateral — proving the raw-code fallback is a real, isolated behaviour. **`git checkout` can't revert an untracked new file mid-mutation** (repeat of the progress-bar lesson) — I used `sed` to apply the mutation and an `Edit` to restore, both string-exact. Gate: `:core:model:testDebugUnitTest` 32/32 new (whole module green) + `:app:assembleDebug` BUILD SUCCESSFUL.

## Lesson (2026-07-22, `registration-step-navigation-core`) — port a ViewModel that folds an ordinal move + a per-field gate + a side-effect into ONE pure SSOT: pass the gate in as a boolean, return the side-effect as a decision, resolve bounds through `fromIndex`
Porting iOS `RegistrationViewModel`'s four navigation methods (`nextStep`/`previousStep`/`skipCurrentStep`/private `nextStepForced`). (1) **Each iOS method recomputes `allSteps.firstIndex(of: currentStep)` inline and open-codes the `idx < count-1` / `idx > 0` bound before mutating `currentStep`.** Lift all four into one framework-free `object` whose functions **return the target step (or null for an inert move)** rather than mutating — the ViewModel becomes a thin caller (`navigator.advance(...)?.let { currentStep = it }`). (2) **Resolve `next`/`previous` through the enum's own `fromIndex(index ± 1)`** (shipped by the progress-bar slice) so the "null at the last/first step" bound is a *single* source of truth, not `idx < count-1` duplicated per method as iOS does — `fromIndex` already returns null out of range, so `next(RECAP)` and `previous(PSEUDO)` fall out for free. (3) **The `canProceed` gate is per-step field validation that reads live wizard state — it stays app-side.** Don't drag username-availability / email / password-match logic into `:core:model`; instead `advance(current, canProceed: Boolean)` takes the already-computed gate as a parameter (`if (canProceed) next(current) else null`). This keeps the core pure *and* pins the gate as a discriminating branch: dropping the guard fails exactly the two `canProceed = false` tests. (4) **A method with a side-effect (`skipCurrentStep` sets `skipPhone = true; phoneNumber = ""`) ports as a returned *decision*, not a mutation.** `skip(current)` returns `SkipOutcome(target = next(current), clearPhone = current == PHONE)` — the core surfaces *whether* to clear the phone; the app-side ViewModel performs the field write. The `clearPhone = current == PHONE` predicate is the discriminating branch (forcing it to `false` fails exactly `skip_onThePhoneStep`, no collateral). (5) **`advance` returns null in two distinct ways — blocked (`!canProceed`) and at-the-last-step (`next` is null even when proceeding)** — cover both arms (`advance_whenBlocked` + `advance_atTheLastStep_isNullEvenWhenCanProceed`) so the gate and the bound are each pinned. Gate: `:core:model:testDebugUnitTest` 18/18 new (whole module green) + `:app:assembleDebug` BUILD SUCCESSFUL.

## Lesson (2026-07-22, `registration-progress-bar-core`) — port a SwiftUI progress-bar's scattered `rawValue` comparisons into one pure partition; leave enum display metadata to the UI; `git checkout` can't revert an untracked file mid-mutation-check
Porting iOS `InteractiveProgressBar` (`OnboardingAnimations.swift`) + the `onStepTapped` closure in `OnboardingFlowView`. (1) **A SwiftUI view spreads its ordering logic across `stepColor(for:)`, `.disabled(step.rawValue > current)`, and a tap closure — lift all three into one pure `object` over an ordinal enum.** `fill(step,current)` returns a `StepFill{COMPLETED,CURRENT,UPCOMING}` enum (iOS's three `stepColor` arms), `canJumpTo` = `step.index <= current.index` (the tap gate / `.disabled` inverse), `jumpTarget(tapped,current)` returns the target step **or null** so the "jump back only, never forward" UX rule is one named decision the composable calls (`jumpTarget(...)?.let(onNavigate)`). The three functions form two *different* partitions (fill splits COMPLETED vs CURRENT; canJumpTo groups them) so neither is a tautology of the other. (2) **Do NOT port an iOS enum's display metadata into `:core:model`.** iOS `RegistrationStep` carries `funHeader`/`funSubtitle`/`iconName`/`accentColor`; those are i18n copy + design-system colour and belong to the UI layer. Port only the ordinal identity (`index` + `ordered`/`total`/`fromIndex`) so the core stays framework-free — the composable maps step → strings/colour app-side. (3) **Two mutation checks pin the two boundaries cleanly:** `fill`'s `<`→`<=` makes the current step read COMPLETED → fails **exactly** the 3 current-step fill tests; `canJumpTo`'s `<=`→`<` makes the current step unreachable → fails **exactly** the 4 current-step reachability tests (incl. `jumpTarget_forTheCurrentStep`). Both isolated, no collateral. (4) **`git checkout <file>` fails to revert a mutation on a *brand-new untracked* source file** (`pathspec did not match any file(s) known to git`) — the file isn't in the index yet. During the mutation loop, `cp` the pristine source to the scratchpad first and restore from that copy (or `git add` it before mutating), not `git checkout`. Gate: `:core:model:testDebugUnitTest` 22/22 + `:app:assembleDebug` BUILD SUCCESSFUL.

## Lesson (2026-07-21, `auth-jwt-expiry-core`) — port an iOS *inline* JWT decoder as a pure `:core:model` object; read `exp` as a **number only**; base64url via `java.util.Base64`; prove the decoder is real by encoding tokens, not mocking
Porting iOS `AuthManager.isTokenExpired(_:now:)` — a `nonisolated static` inline JWT-payload decoder — plus the `refreshSession(force:)` guard. Android had **no** token-expiry notion: `AuthInterceptor` attaches the JWT blindly. (1) **An "inline decoder on a singleton" ports cleanly to a pure `object` because iOS already made it `nonisolated static` for exactly this testability reason** — lift it verbatim into `:core:model/auth/JwtExpiry.kt`, zero framework/Keychain coupling. The one true dependency is JSON parsing (`kotlinx.serialization.json`, already a `:core:model` `api` dep) + base64 (`java.util.Base64`, JDK, `minSdk 26` so URL decoder is available). (2) **`java.util.Base64.getUrlDecoder()` is the faithful analogue of iOS's manual `-→+ / _→/` + pad-to-4 + standard-decode** — the URL decoder maps `-_` itself and tolerates missing padding, so you don't re-implement the normalisation; wrap in `runCatching{}.getOrNull()` so a non-base64 segment returns null → expired (iOS's "base64 decode failed → true"). (3) **Read `exp` as a JSON number ONLY.** iOS `json["exp"] as? TimeInterval` rejects a stringified `"exp":"1700…"`; the kotlinx analogue is `(elem as? JsonPrimitive)?.takeIf { !it.isString }?.doubleOrNull` — because `JsonPrimitive.doubleOrNull` alone would happily parse `"1700"` (a *string* primitive) via `content.toDoubleOrNull()`, silently diverging from iOS. The `if (exp.isString) return null` guard is the discriminating branch; a mutation dropping it fails exactly the string-`exp` test. (4) **The boundary is strict `<` (`exp - margin < now`) — the threshold instant is still valid.** iOS: `Date(exp).addingTimeInterval(-30) < now`. Pin it with three tests around threshold=exp−margin (below/at/above); `<`→`<=` fails exactly the two "==threshold" tests (default-margin and zero-margin), no collateral. Surface the 30s margin as a **parameter** (iOS hard-codes it inline) — faithful default + more flexible. (5) **Prove a decoder is behavioural by building tokens for real**, not mocking: `Base64.getUrlEncoder().withoutPadding().encodeToString(payloadJson.toByteArray())` in the test helper, so every test round-trips through the production `getUrlDecoder()`. Malformed-branch coverage = null/blank, segment-count ≠ 3 (1/2/4 parts), un-decodable base64, non-JSON payload, JSON-array (not-object) payload, absent `exp`, stringified `exp`. (6) **`TokenRefreshPolicy.shouldRefresh(force, token, now) = force || isExpired`** is `refreshSession(force:)`'s guard verbatim (iOS skips the round-trip only when `!force && !isTokenExpired`); keep it a separate tiny `object` so the interceptor/Authenticator wiring (next slice, app-side) has a named single decision to call. Gate: whole `:core:model:testDebugUnitTest` green (24/24 new) + `:app:assembleDebug` BUILD SUCCESSFUL.

## Lesson (2026-07-21, `profile-presence-window-reconcile`) — a "leave main green" repair: reconcile the *test* to the SSOT, never the *resolver*; a comment can lie, the SSOT can't
Clearing the previous run's flagged deterministic red: `:feature:profile` `ProfileHeaderBuilderTest` had two tests asserting **AWAY** for a disconnected user with a **10-min-old** `lastActiveAt`. (1) **When a test disagrees with a resolver, find the single source of truth before deciding which is wrong.** The presence SSOT is `packages/shared/utils/user-presence.ts` (rule "1/3/5 — 2026-07-20": ≤60 s ONLINE, ≤3 min AWAY, ≤5 min IDLE, >5 min OFFLINE; the anti-stale guard is **5 min**, there is no 30-min window). It is mirrored **verbatim** in `:core:model` `Presence.kt` `UserPresence.state` (constants `AWAY_WINDOW_MS=180_000`, `IDLE_WINDOW_MS=300_000`) and **locked** by `PresenceTest` ("windows are 60s-3min-5min", green on main). `ProfileHeaderBuilder.build` *delegates* to `UserPresence.state`, so **production was already correct** — 10 min → OFFLINE. The tests were wrong, justified by a comment fabricating a *"30-min anti-stale guard"* that exists nowhere. **A code comment is not a spec; the SSOT + its lock test are.** (2) **The fix is a test reconcile, not a resolver edit — and NOT a weakening.** Deliberately left `Presence.kt` untouched (editing it to make the bad test pass would diverge Android from the web/iOS mirrors — the exact SSOT violation the routine forbids). Rewrote the two wrong tests to the spec and **strengthened** coverage: the builder's presence projection now exercises all three disconnected-decay arms via the public `build` API — AWAY (2-min), **IDLE (4-min, an arm the builder path never covered before)**, OFFLINE (10-min) — plus a new IDLE last-seen-carry test. Correcting a wrong expectation to the spec + adding arms is strengthening; deleting the assertion would have been weakening. Net +3 (27→30). (3) **RED→GREEN proof for a repair = restore `git show origin/main:<testfile>` and run.** `:feature:profile:testDebugUnitTest` on the restored file → 137 tests, exactly 2 failed (the two named), no collateral; reconciled → 30/30, module green. (4) **`gradle --tests "…"` from the repo root fails ("does not contain a Gradle build") — a prior `cd /home/user/meeshy` in a compound command had moved the Bash cwd.** `cd apps/android` explicitly before every `gradle` call; don't assume cwd persisted from an earlier successful run. (5) **The `:sdk-core` DataStore timeout flake is real and intermittent** — `ThemeStoreTest.dataStore_hydratesAlreadyPersistedChoiceOnConstruction` (and its `InterfaceLanguageStore`/`PrivacyPreferences` siblings) can fail once in the full `testDebugUnitTest` run yet pass 3/3 under `--rerun-tasks --tests` in isolation. It's a `TimeoutCancellationException` (5 s Turbine/`withTimeout`) under parallel classloading, **not** a regression; distinguish it from a deterministic red by re-running the single test in isolation before blaming your diff.

## Lesson (2026-07-21, `auth-saved-account-picker-core`) — port a mutating `@Published` singleton to pure immutable list→list transforms; RUN the full-repo suite because main is red on a *deterministic* profile test, not just the DataStore flake
Porting iOS `AuthManager`'s saved-account logic (a `@Published var savedAccounts: [SavedAccount]` mutated in place by `upsert`/`remove` + sorted on load). (1) **A mutating collection on a stateful singleton ports to a pure `object` of `List<T> → List<T>` functions, one per iOS mutation.** `upsertSavedAccount` (replace-in-place-by-id-else-insert-at-0) → `upsert(accounts, account)` returning a new list via `indexOfFirst` + `mapIndexed` (early-return `listOf(account) + accounts` for the not-found arm); `removeFromSavedAccounts` (`removeAll`) → `remove` via `filterNot`; `loadSavedAccounts`' sort → `sorted`. The app-side store keeps the mutable `StateFlow` + persistence and calls these; the core stays framework-free and every branch is JVM-testable. Add explicit `doesNotMutateInput` tests — a pure transform that accidentally used a mutable receiver would pass the happy path but fail these. (2) **The D4 stable-sort tie-break is the discriminating behaviour, and iOS already has a test pinning it** (`SavedAccountsSortStabilityTests`): `sortedWith(compareByDescending { lastActiveAt }.thenBy { id })`. Dropping `.thenBy { it.id }` fails exactly the identical-timestamp + mixed-timestamp + idempotent tests (no collateral) — because a timestamp-only comparator leaves same-timestamp accounts in input order, which is not stable across persisted-JSON round-trips. Port the iOS test's three scenarios verbatim plus empty/single. (3) **`shortName = displayName ?? username` — harden the null-only fallback to also catch blank**, so `"   "` → username not an empty row. Faithful-plus-hardening, consistent with the OTP ASCII-digit and custom-host-trim decisions; document it as intentional. (4) **⚠ Do NOT scope the gate to only your module and assume the rest of main is green.** This run's full-repo `testDebugUnitTest` surfaced a **deterministic** `:feature:profile` `ProfileHeaderBuilderTest` failure (2 tests) that reproduces on clean `origin/main` — the test expects `AWAY` for a disconnected user idle 10 min, but `:core:model` `Presence.getPresenceStatus` returns `OFFLINE` past its 3-min `AWAY_WINDOW_MS`. That is a real "main is red" broken window (distinct from the known `:sdk-core` DataStore flake, which is a timeout that passes in isolation). **Always run the full suite, and when a red is deterministic + reproduces on main, flag it as a dedicated repair slice** (`profile-presence-window-reconcile`) rather than silently scoping around it — noting it is how the next run knows to fix it before shipping more features.

## Lesson (2026-07-21, `auth-server-environment-selector`) — port an iOS stateful config singleton by splitting its *pure derivations* from its *persistence*; use `java.net.URI` for the URL parsing; and the `custom` origin is deliberately empty
Porting iOS `MeeshyConfig` (a `UserDefaults`-backed `@unchecked Sendable` singleton) to Android. (1) **A stateful config singleton is two things — persistence + pure string derivations — and only the derivations belong in `:core:model`.** iOS folds `selectedEnvironment`/`customHost` (UserDefaults) together with `apiBaseURL`/`serverOrigin`/`webOrigin` (pure computed props). The clean split: a pure `ServerEnvironment` enum + `ServerEnvironmentResolver` object own every *derivation* (JVM-testable, zero framework), and the app-side store (DataStore) owns the mutable "selected env + custom host" + the `MeeshyConfig`-equivalent apply/restore actuator. Don't drag `UserDefaults`/DataStore into the pure core. (2) **`java.net.URI` is the faithful analogue of iOS `URL(string:)` for host/port extraction** — `uri.host`/`uri.port` (−1 when absent) mirror Swift's `url.host`/`url.port` optional. Wrap the construction in `runCatching { URI(s) }.getOrNull()` and fall through to the verbatim input on parse failure or null scheme/host — that reproduces iOS's `guard let … else { return apiBaseURL }` exactly, and makes `"not a url"` (space → `URISyntaxException`) and `"gate.meeshy.me/api/v1"` (relative, `scheme == null`) both return verbatim. A malformed input is the discriminating branch for the parse-guard. (3) **The `custom` case's `origin` is deliberately `""`** (iOS returns `""` for `.custom`) — its real origin is computed at resolve time from the operator-supplied host, not stored on the enum. Model `apiBaseUrl(env, customHost)` with a `when(env) { CUSTOM -> normalizeCustomHost(customHost); else -> env.origin }` so the empty-origin case never leaks into a URL. (4) **`normalizeCustomHost`'s `startsWith("http")` guard is iOS-verbatim** (`host.hasPrefix("http")`) — it matches both `http://` and `https://` (and technically `httpfoo`, but that's iOS's contract; the server rejects a bad host). Trim first — iOS's `LoginView.applyCustomHost` trims `.whitespaces` before calling `applyEnvironment`, so trimming in the normaliser is faithful, not an unforced change. (5) **`webOrigin` strips only the *leading* `gate.`** via `removePrefix("gate.")` (not a global replace): `gate.staging.meeshy.me` → `staging.meeshy.me`, keeping the interior. The discriminating mutation is dropping the whole strip (→ `host`), which fails exactly the two gate-host webOrigin tests + the production composition chain, no collateral on the localhost/loopback/non-gate/malformed arms.

## Lesson (2026-07-21, `auth-signup-availability-local-gate`) — model a Combine `.debounce().removeDuplicates().sink{guard}` chain as a pure `(current, previous) → Intent` where **dedup is checked first**; keep the timing operator app-side
Porting the three iOS registration live-availability chains (`$username/$email/$phoneNumber .debounce(1s).removeDuplicates().sink { guard localValid else { clear }; check }`). (1) **The pure core is the sink body, not the operator.** The 1 s delay and `removeDuplicates` are reactive-operator concerns — leave them app-side (`Flow.debounce(1_000).distinctUntilChanged()`), and model only the *decision* the sink makes as a pure `intent(current, previous): AvailabilityIntent` (`Unchanged`/`Clear`/`Check(query)`). This keeps the whole branchy decision JVM-testable with zero coroutine/framework machinery. (2) **Order of the guards is behaviour — `removeDuplicates` runs BEFORE the `guard localValid`, so dedup must be the first `when` arm.** A still-valid value equal to the last emission must return `Unchanged` (never re-probe), which a validity-first ordering would break by returning `Check`. Pin it with a dedicated `dedupWinsEvenWhenValueIsValid` test (valid input, `previous == current` → `Unchanged`) — it's the discriminating case a naive "check validity then dedup" port fails. Model `previous` as `String?` so the first emission (`previous == null`) is never mistaken for a duplicate. (3) **The proceed gate reuses the same local predicate — do NOT duplicate it.** iOS `canProceed.pseudo` = `isUsernameValidLocally(username) && usernameAvailable == true`; the availability *debounce* uses the same `isUsernameValidLocally`. One `SignupFieldValidation` object owns the predicate; both the intent policy and the step gate call it (SSOT — a stale `availableTrue` can't advance a now-too-short handle because the gate re-checks local validity). Discriminating mutation: the step gate `usernameAvailable == true` → `!= false` fails **exactly** the null-availability test (43 run, 1 failed) — the `Boolean?` three-state gate (null = "not yet checked" ≠ false = "taken") is behavioural. (4) **iOS `isEmailValidLocally` is deliberately loose** (`contains("@") && contains(".")` — not a real email regex); port it verbatim, the server does the authoritative check. iOS username charset is Unicode `CharacterSet.alphanumerics ∪ {_,-}`; Kotlin `Char.isLetterOrDigit()` is the faithful analogue (minor divergence on bare combining marks Mn, negligible for a handle). Username length is measured **after trim**, so pad-to-20-with-spaces-around-2-real-chars is valid — test it.

## Lesson (2026-07-21, `auth-password-requirements`) — the gradle *wrapper* download is blocked; use the system Gradle; and the iOS *view-level* predicate ≠ the *canProceed* gate — port both, they differ
Porting iOS registration Step 5's two remaining pure cores (the requirements checklist + the confirm gate). (1) **`./gradlew` fails in this container** — the wrapper tries to fetch `gradle-8.11.1-bin.zip` from `services.gradle.org` → redirects to `github.com/gradle/gradle-distributions/...` → **403 through the agent proxy**. `meeshy.sh {build,test,check}` all shell out to `./gradlew`, so they can't run here. **Use the pre-installed system Gradle** instead: `/opt/gradle/bin/gradle` (8.14.3) with `export ANDROID_HOME=$HOME/android-sdk; export LANG=C.UTF-8`. The routine's `meeshy.sh check` = `gradle assembleDebug testDebugUnitTest`; run that verbatim. (2) **A SwiftUI View has *two* different password predicates and they are NOT the same — read both.** `StepPasswordView.passwordsMatch` = `!confirmPassword.isEmpty && password == confirmPassword` (drives the inline green/red match card), but `RegistrationViewModel.canProceed` for `.password` = `password.count >= 8 && password == confirmPassword` (drives the Continue button). They diverge on "identical but < 8 chars": the card would say "match!" yet the step must not advance. Modelled as two fields (`match` verdict vs `canProceed` gate) on one `PasswordEntryState`, and pinned with the `matchingButTooShort` test. The confirm field itself only renders once `password.count >= 8`, so `match` is forced to `UNDETERMINED` while the field is hidden (a lone `password == confirm` on empties would wrongly read MATCHED). (3) **Keep the checklist core distinct from the score meter.** `PasswordStrength` (0..5, two length gates + special-char band) already existed; the `requirementsCard` is a *different* shape — four discrete rule rows with no special-char row and a single length gate. Don't fold them; iOS renders them from separate logic and they answer different questions ("how strong" vs "which rules pass"). Discriminating mutation: dropping `showConfirmField &&` from `canProceed` fails exactly the both-empty + too-short tests, no collateral.

## Lesson (2026-07-20, `prisme-device-locale-priority`) — extend the widely-consumed resolver via an interface default getter (zero call-site churn); mirror iOS's *asymmetric* normalisation; the discriminating mutation is the re-validation, not the reduction
Extending `LanguageResolver` (consumed by ~40 sites across `:core:model`/`:sdk-ui`/every feature) to add the Prisme `deviceLocale` 4th priority. (1) **Add a new optional input to a broadly-implemented interface via a default getter, not a constructor param.** `interface ContentLanguagePreferences { … val deviceLocale: String? get() = null }` lets `MeeshyUser`, every `EmptyContentPreferences` object, and every test `Prefs` compile untouched; only the one implementer that has the value (`MeeshyUser`) overrides it. The 4th-priority arm then flows into `resolveUserLanguage` *and* `preferredTranslation` (which iterates `preferredContentLanguages`) with **no** change to any call site — the whole feature is two files of resolver logic + one decoded field. (2) **The TS SSOT and the iOS mirror deliberately disagree on in-app normalisation — follow the one that matches the real data shapes.** TS `resolveUserLanguage` normalises in-app codes too; iOS `preferredContentLanguages` keeps in-app codes **verbatim** and normalises **only** the device locale. Android follows iOS: an existing test pinned `Prefs("EN","en","fr") → ["EN","fr"]` (casing preserved), and the only BCP-47 shape that actually reaches resolution is the OS locale (settings pickers write clean 2-letter codes) — so normalising *only* deviceLocale is both correct and keeps every pre-existing test green. Rewriting that assertion to `["en","fr"]` would have been an unforced behaviour change. **Scan the existing test of the function you're extending before touching its normalisation.** (3) **Port `normalizeLanguageCode` with the explicit ISO 639-2/3 table, never `slice(0,2)`** — `"swe"`→`"sv"` not Swahili `"sw"`, `"fil"` rejected not Finnish `"fi"`. The SOTA branch (surpassing a naive port) is **re-validating the reduced target against the catalogue** so a table entry whose 639-1 target isn't shipped (`"orm"`→`"om"`) returns `null`; the discriminating mutation is dropping that `&& reduced in supportedCodeSet` (fails exactly the unsupported-target test, no collateral) — NOT mutating the reduction itself (which would fail several). Derive `supportedCodeSet` from the existing `LanguageData.allLanguages` (SSOT), don't hand-list it. Note the pre-existing `LanguageData.aliases` (`fil`→`tl`) is a *separate* lenient lookup for display `info()`, and correctly diverges from the Prisme normaliser which rejects `fil`.

## Lesson (2026-07-20, `call-transcript-buffer`) — port a stateful iOS accumulator to one immutable reducer; test the *two distinct orderings* (insertion vs display) separately, and reuse the existing resolver instead of a parallel projection
Porting iOS `CallTranscriptionService.appendSegment` (a mutable `allSegments`/`segments`/`persistedSegments` service) to a pure `LiveTranscript` data class. (1) **Two orderings hide in one accumulator and each needs its own test.** iOS caps by *insertion* order (`suffix(limit)` over `allSegments`) but *displays* in *capture-time* order (`sorted { capturedAt }`). A "keep-latest-by-time" cap would pass a naive retention test — so add the discriminating test that appends the *earliest-timestamped* segment *last* with a small `retentionLimit`: an insertion-order cap evicts a *different* segment than a time-order cap, proving which one is implemented. (2) **The finality gate is the RED-proof lever.** `filterNot { speakerId == && !isFinal }` — drop the `!isFinal` and a new segment evicts the speaker's finalized lines too; that mutation fails exactly the finals-must-survive tests (incl. any ordering test that stacks multiple finals for one speaker) with no collateral on retention/projection/purity. Behavioural, not tautological. (3) **Reuse the resolver SSOT, don't re-project.** The buffer exposes `captionLines(mode)` by mapping its segments through the already-landed `CallCaptionResolver` (+ a `toCaptionSegment()` bridge) rather than re-deriving the Prisme translated/original fallback — one place owns "which text renders under which mode". (4) **Kotlin `sortedBy` is a stable sort** — equal `capturedAtMs` keeps insertion order, so an explicit stable-order test documents the tie-break the overlay relies on for consecutive same-instant utterances. Guard `takeLast` against a non-positive `retentionLimit` with `coerceAtLeast(0)` so the reducer stays total (Kotlin `takeLast(negative)` throws).

## Lesson (2026-07-20, `call-reliability-policy`) — a big iOS `nonisolated enum` of static decisions ports cleanly to one pure Kotlin `object`; pin production constants with default-param tests, not tautologies
Three takeaways porting iOS `CallReliabilityPolicy` (a namespace of ~8 static reliability decisions). (1) **A Swift `nonisolated enum Foo { static func … }` used purely as a stateless namespace maps 1:1 to a Kotlin `object Foo { fun … }`** — no state, no instances, and the nested `enum HalfOpenOutcome`/`ConnectingOutcome`/… lift to top-level `enum class`es in the same file. Keep the verdicts as distinct enums (not booleans) so the `when` on them stays exhaustive-without-`else` at the call site (the future actuator). (2) **To prove a policy's *default parameter* equals the iOS constant without a tautological `assertThat(CONST).isEqualTo(5)`, write a behavioural test that calls the function *omitting* the tuning arg at the exact boundary** — e.g. `evaluateHalfOpen(inbound = 5, …)` (no `requiredInboundPackets`) returning `Healthy` pins `RTP_GATE_REQUIRED_PACKETS = 5` through observed behaviour; `secondsInConnecting = 12.0` (no `restartAfterSeconds`) → `RestartIce` pins `CONNECTING_RESTART_SECONDS`. These go RED if the constant drifts from iOS, are non-tautological, and double as the boundary tests. (3) **Drop a "for-parity" constant the slice doesn't consume.** First draft added `MAX_RECONNECT_ATTEMPTS = 3` (iOS has it) but no function in this pure slice reads it — that is dead code / an untested public symbol. Removed it; the app-side reconnect-cap actuator will add it when it actually caps `reconnectAttempt`. Keep each slice's added surface fully exercised. Discriminating mutation for a two-arm gate: neutralise `outbound > 0` → `true` (fails exactly the `outbound == 0` mic-off test); the grace-window early-return means an in-grace test still passes, so the mutation is cleanly isolated to one test.

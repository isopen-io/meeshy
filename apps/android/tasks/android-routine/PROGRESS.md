...
Ordered by value:
0. ~~**Snap-to-guide + out-of-bounds warning**~~ ✅ shipped as `story-canvas-snap-guides` — pure
   `StorySnapResolver` (per-axis nearest-guide snap + safe-zone verdict) reused through the existing
   element-drag path, with an accent guide-line overlay + warning border. See run log.
1. ~~**Canvas toolbar/FAB**~~ ✅ shipped as `story-composer-band` — the two-FAB

...
class ConversationRepository @Inject constructor(
    ...
    suspend fun updateUnreadCount(conversationId: String, unreadCount: Int) {
        val existing = conversationDao.allIds()
        if (!existing.contains(conversationId)) return
        val result = apiCall { conversationApi.getById(conversationId) }
        if (result is NetworkResult.Success) {
            val updated = result.data.copy()
            val now = SystemCacheClock.nowMillis()
            val row = ConversationEntity(
                id = updated.id,
                payload = MeeshyApi.json.encodeToString(updated),
                updatedAt = isoToEpochMillis(
                    updated.updatedAt
                        ?: updated.lastMessage?.createdAt
                        ?: updated.createdAt,
                ),
                cachedAt = now,
            )
            conversationDao.upsertAll(listOf(row))
        }
    }

    suspend fun refreshOne(conversationId: String) {
        when (val result = apiCall { conversationApi.getById(conversationId) }) {

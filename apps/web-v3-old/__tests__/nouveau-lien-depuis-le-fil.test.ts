...
const FIL: EtatDuFil['fil'] = {
  ...
  messages: [],
  plusAncien: null,
  // Un groupe privé, modéré par le lecteur — le cas NOMINAL où la passerelle
  // admet la création (`mayMintShareLink`) : la puce est correctement rendue.
  // Les défauts (`direct`, rang trop bas) ont leur propre bloc plus bas
  // (correction de revue, #5034).
  type: 'group',
  rang: 'moderator',
};
...
/**
 * LA PUCE NE PROPOSE JAMAIS UN GESTE QUE LA PASSERELLE REFUSE (défaut de
 * revue, #5034) — `mayMintShareLink`
 * (`services/gateway/src/routes/links/utils/share-link-mint.ts:66-73`) :
 * `direct` JAMAIS (quel que soit le rang), `public` TOUJOURS, sinon au moins
 * MODÉRATEUR. Avant ce correctif, `versLeLien` ne regardait QUE `porte.genre
 * === 'membre'` — la puce se rendait donc sur un tête-à-tête et pour un
 * membre ordinaire d'un groupe privé, deux cas où la passerelle répond 403.
 */
describe('la puce « Lien » n’offre jamais un geste que la passerelle refuse (#5034)', () => {
  it('ne se rend PAS sur un DIRECT (tête-à-tête) — refusé quel que soit le rang', () => {
    const doc = documentDuFil(ETAT({ fil: { ...FIL, type: 'direct', rang: 'creator' } }));
    expect(doc).not.toContain('class="partager"');
  });

  it('ne se rend PAS pour un MEMBRE ordinaire d’un groupe privé — rang insuffisant', () => {
    const doc = documentDuFil(ETAT({ fil: { ...FIL, type: 'group', rang: 'member' } }));
    expect(doc).not.toContain('class="partager"');
  });

  it('ne se rend PAS quand la passerelle n’a résolu aucun rang', () => {
    const doc = documentDuFil(ETAT({ fil: { ...FIL, type: 'group', rang: null } }));
    expect(doc).not.toContain('class="partager"');
  });

  it('se rend pour un MODÉRATEUR ou plus d’un groupe privé', () => {
    for (const rang of ['moderator', 'admin', 'creator']) {
      const doc = documentDuFil(ETAT({ fil: { ...FIL, type: 'group', rang } }));
      expect(doc).toContain('class="partager"');
    }
  });

  it('se rend TOUJOURS sur une conversation `public`, quel que soit le rang (ou son absence)', () => {
    const doc = documentDuFil(ETAT({ fil: { ...FIL, type: 'public', rang: null } }));
    expect(doc).toContain('class="partager"');
  });

  it('ne se rend pas quand le type n’est pas servi (fail-closed)', () => {
    const doc = documentDuFil(ETAT({ fil: { ...FIL, type: undefined } }));
    expect(doc).not.toContain('class="partager"');
  });
});
...
describe('la porte du fil — créer un lien de partage (§ 12.10.5, #5034)', () => {
  ...
  /**
   * LES DEUX MOTIFS ANGLAIS DE `mayMintShareLink` SONT TRADUITS (défaut de
   * revue, #5034) — `traduisLeMotifDuLien` (`lib/contenu/liens.ts`). Ligne de
   * défense après le gate de `peutCreerUnLien` : un rang rétrogradé entre le
   * chargement du fil et la soumission atteint quand même la passerelle.
   */
  it.each([
    ['Cannot create share links for direct conversations', 'Impossible de créer un lien de partage pour une conversation privée à deux.'],
    ['You do not have the necessary rights to perform this operation', 'Vous n’avez pas les droits nécessaires pour créer un lien sur cette conversation.'],
  ])('traduit le motif « %s »', async (motifAnglais, motifFrancais) => {
    bouchon(() => new Response(JSON.stringify({ success: false, error: { message: motifAnglais } }), { status: 403 }));

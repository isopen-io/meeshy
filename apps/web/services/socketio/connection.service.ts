...
export class ConnectionService {
  ...
  setupConnectionListeners(onAuthenticated?: (user: User) => void, onDisconnected?: (reason: string) => void, onError?: (error: any) => void, onSessionRevoked?: () => void): void {
    ...
    socket.on(SERVER_EVENTS.AUTH_TOKEN_EXPIRED, () => {
      ...
      //
      // The store's `sessionToken` is passed through (#4405, étape 2) so this
      // path arms the server's sliding session window exactly like the REST
      // path (`api.service.ts.refreshAuthToken`) and like iOS already does.

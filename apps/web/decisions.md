...
~~**La carte du débounce se PURGE** (`pruneDebounce`, `perform-send.ts`)~~ — **Plus AUCUN dédoublonnage par CONTENU (#7985, directive porteur 2026-09-25)** : le débounce de 600 ms sur `(conversation, texte, réponse, pièces)` bloquait 😂 😂 😂 tapés en série ; il est retiré avec sa carte, comme son miroir iOS. La SEULE déduplication qui reste est celle du MESSAGE, par `clientMessageId` (rejeu, écho socket, passerelle idempotente). Le double clic sur « Envoyer » est tenu par le composeur, qui vide son brouillon SYNCHRONIQUEMENT à l'envoi (`draftNow`, `composer.tsx`) : un second clic servi avant le rendu suivant ne trouve plus rien à envoyer.
...
**La CI construit la coque ANDROID, iOS reste local (2026-09-25, #6217).** Le job `shell-android` de `.github/workflows/ci.yml` lance le MÊME pilote que la recette (`node scripts/build-shells.mjs --target android`) avec `MEESHY_SHELL_BUILD_NUMBER=${{ github.run_number }}` — la source explicite ci-dessus, qui rend le fail-closed sur checkout superficiel inoffensif — et l'audit de l'APK construit (`auditBuiltApkVersion`) y rougit donc à chaque divergence entre l'artefact, `package.json` et le numéro de run. Il ne tourne que si la poussée touche ce que le pilote LIT (domaine `shell_android` de `scripts/ci-affected.mjs` : `android/`, `capacitor.config.ts`, `package.json`, `vite.config.ts`, le pilote et ses deux bibliothèques), et publie l'APK de recette (7 jours) pour un `adb install -r` sans machine outillée. **L'asymétrie est assumée** : la coque iOS exige un runner macOS et la préférence de machine #5657, non résolue ; sa preuve d'intégration se rejoue donc encore à chaque recette locale. `bun run gate` n'appelle pas ce job et n'a pas changé.
...
1. *Pellicule du MESSAGE, pas de la conversation.* iOS montre « Média 4 sur 7 » (toute la conversation, mesuré sur Ref-Native) ; le web ne pagine que les pièces DU message ouvert (`items = visual`, le tableau déjà partitionné par `Attachments`). Une pellicule conversation-entière suppose un magasin global (positions de TOUS les messages médias du fil) et trois actions supplémentaires (Enregistrer, Réagir/Répondre/Créer avec ce média) — hors de ce tour. **Issue de suivi #6303**, référence `ConversationView+MediaGallery.swift` / `+Actions.swift`. *(Amendé par D-130, 2026-09-26 : ouverte depuis l'écran « Médias, liens et documents », la visionneuse feuillette l'index visuel de TOUTE la conversation, paginé ; ouverte depuis le fil, elle garde la pellicule du message — voir D-130 pour la raison.)*
...
**LA COQUE CAPACITOR N'A PAS DE MISE À JOUR EN VOL, et n'en aura pas par ce chemin.** Elle n'émet aucun service worker (`check-shell-dist.mjs`), embarque ses actifs (`webDir: 'dist'`, aucun `server.url`) et reçoit une version neuve par son magasin d'applications. Le contrôleur ne s'y inscrit pas (`__SHELL__`), la bannière ne peut donc pas y apparaître, et son cache de requêtes persisté se purge sur le `buster` — donc à chaque version publiée, `package.json` étant la source de la version des coques (`scripts/sync-shell-version.mjs`). **Depuis #6937, elle ANNONCE une version publiée sur le magasin** : `GET /api/v1/app/shell-version?platform=android|ios` (`services/gateway/src/routes/app.ts`) sert `SHELL_LATEST_VERSION` et la fiche du magasin (`getAppStoreUrl`, la même résolution que le `storeUrl` du 426). `lib/app-update/shell-update.ts` la lit au démarrage et à chaque retour au premier plan, et pose `appUpdateStore.store` ; la MÊME bannière s'affiche, avec les mêmes libellés sauf l'indication (`appUpdate.storeHint`), et son action est un LIEN vers la fiche, que la coque confie au système. Une variable vide, une version illisible ou pas plus récente que `__APP_VERSION__`, une fiche non `https:` ou une panne réseau ne fabriquent rien. « Attendre » tient pour CETTE version : une publication plus récente rouvre la bannière. **Qui pose `SHELL_LATEST_VERSION`** : la mise en production d'une coque sur un magasin — tant qu'aucune ne l'est (D-30 (d)), la variable reste vide et la bannière muette.
...
**Addendum #7116 (2026-09-24) — les trois surfaces AUTEUR, et ce qu'elles n'imitent PAS d'iOS.** « Vues », « Partager » et « Enregistrer » ont désormais leur effet ; la loi n'a pas bougé (`showsViews`/`showsExport = isOwnStory`), le GESTIONNAIRE (garde 2) porte ce que le web sait faire.

- **Vues** — `GET /api/v1/posts/:postId/interactions` (`services/gateway/src/routes/posts/interactions.ts:669-701`, auteur seul, 403 sinon), la route que le lecteur iOS lit (`StoryInteractionService.swift:125-147`). La feuille suit la loi d'hôte PARTAGÉE (`useCommentsSheetHost`, Q5 : focus rendu, fermeture au changement de story) et met la lecture en pause ; son en-tête lit le compte AUTORITATIF (`viewCount`, note C4 d'iOS) et `story:viewed` le tient à jour en direct (`publication-views-realtime.ts`). Sans `impressionCount` : la passerelle ne le sert pas sur le corpus des stories — l'en-tête dit « N vues », jamais « · M impressions » inventé.
- **Partager** — **écart assumé** : iOS bake un MP4 et propose la langue à graver (`StoryExportShareViewModel.swift`) ; le web ne peut rien baker DANS le geste (D-48 : `navigator.share` n'ouvre que pendant l'activation) et partage le LIEN canonique (`publicationShareUrl`), par le site UNIQUE `lib/view/publication-share.ts` que `usePostGesture` emploie aussi — le premier jet en avait écrit une jumelle. La story est en pause sous la feuille du système, comme sur iOS.
- **Enregistrer** — **écart assumé** : le web télécharge le MÉDIA de la story par la route d'export FILIGRANÉE (`GET /posts/:postId/media/:mediaId/export`, `media-export.ts:112-190`, miroir serveur de `MeeshyExportWatermark`) ; les calques d'une scène (textes, stickers) ne sont PAS gravés, contrairement au bake iOS. L'anneau suit `StorySaveProgressMapper` (téléchargement 0…0,9, livraison ensuite, ton inerte + balayage) ; le flux d'export n'a PAS de `Content-Length` (`createReadStream`), donc l'état NOMINAL de l'anneau est INDÉTERMINÉ (`aria-busy`, balayage) — jamais un « 0 % » figé.
- **La paire Partager/Enregistrer apparaissait et disparaissait ENSEMBLE — CORRIGÉ à la revue suivante (2026-09-24, défaut 2).** Le premier jet faisait dépendre `share` de la même porte que `save` (`hasFileDeliveryDoor`), alors que `share` ne LIT jamais cette porte : il partage un LIEN (`sharePublicationLink` → `portailDuNavigateur()`), où la coque Android a déjà `MeeshySharePlugin` (#7710) même sans `DownloadListener` ni `navigator.share`. Sur cette coque, « Partager » disparaissait donc pour une raison qui ne le concernait pas — un contrôle qui AURAIT eu un effet, cité par la garde d'un AUTRE contrôle. `exportMedia` (la présence d'un média EXPORTABLE, `storyDownloadableMedia`) reste la SEULE garde commune aux deux — c'est la loi pure (`showsExport`) — et la porte de livraison de FICHIER ne garde plus que `save`. Sur la coque Android aujourd'hui : Vues + Partager, jamais Enregistrer — **issue compagnon #7788** (`MeeshySharePlugin.saveFile`, même famille que #7710), aucun Java écrit dans ce lot. Une story texte n'offre toujours que « Vues ».
- **L'activation du geste peut expirer PENDANT le téléchargement, dans une coque sans ancre de repli (défaut 1, 2026-09-24) — corrigé pour ce que le code peut prouver sans appareil.** `navigator.share` y refuse par `NotAllowedError` ; le premier jet de la deuxième revue rendait alors `unavailable` → « Échec de l'enregistrement », un verdict DÉFINITIF que rien ne prouve : le tap SUIVANT repart d'une activation fraîche. `deliver-file.ts` porte désormais une quatrième issue, `expired`, annoncée « Réessayez — appuyez de nouveau sur Enregistrer. » (`story.save.retry`) — le job se libère, comme tout autre dénouement. **Non mesuré sur un appareil réel** (aucun simulateur `54438823-…` accessible depuis ce poste Linux) : si l'activation d'un WKWebView Capacitor s'avère plus courte que le téléchargement le plus rapide possible, retaper échouerait aussi indéfiniment, et la vraie réparation serait alors un état « prêt — toucher pour enregistrer » (la portion réseau finie AVANT d'armer le partage) ou un pont natif `saveFile`. À vérifier au prochain tour avec accès au simulateur.
- **Échap appartient à la couche ouverte** (extension de D-91) : le premier jet fermait le lecteur ENTIER quand on fermait la feuille « Vues » au clavier (mesuré au navigateur). `useStoryKeyboardShortcuts` cède Échap dès qu'une couche est ouverte.
- **Poids** — `story_reader` 10,95 Ko gzip (mesure du 2026-09-24, plafond 11) : le téléchargement et la livraison sont chargés à la demande (`import()` au premier « Enregistrer », D-54), la feuille aussi (`publication_viewers_sheet`, nommée HORS du motif `story-…` du lecteur). **`download-simple` (miroir de `square.and.arrow.down.fill`) EST ENTRÉ, corrigé à la revue suivante (défaut 3, 2026-09-24)** : la mesure « 11,08 Ko > 11 avec `download-simple` » citée par la revue ne se reproduit pas — `GLYPHS` est une table PARTAGÉE (chunk `glyph-*.js`, `glyph.tsx` l'important pour TOUS les écrans) dont `story_reader` ne reçoit qu'une RÉFÉRENCE, jamais une copie ; `node scripts/measure-weight.mjs` rend le même ordre de grandeur avant et après (10,93 → 10,95 Ko, l'écart tenant à la disparition de l'ancien commentaire de marge, pas au glyphe). « Enregistrer » porte donc `GLYPHS.downloadSimple`, jamais l'arbitrage de #7121 — qui mesurait un ajout d'une AUTRE forme (une clé absente du socle, payant sa propre table) et ne s'appliquait pas à celui-ci.
...
**Le compteur de commentaires d'une carte du fil cesse d'être inerte** : il conduit à `/post/$post#commentaires`. `repostCount`, LUI, EST RESTÉ une STATISTIQUE jusqu'à ce lot — voir **D-125**, qui le lève avec un effet ET une confirmation, et qui périme ce paragraphe.
...
**Addendum 2026-09-24 (#6484) — un TROISIÈME hôte, et la loi d'hôte devient un module.** Le lecteur des Réels monte la MÊME feuille depuis son rail (`ReelActionRail` d'iOS : aimer · commenter · enregistrer · repartager). La feuille a été renommée pour ce qu'elle est — `components/publication-comments-sheet.tsx` (`PublicationCommentsSheet`) ; l'attribut `data-story-comments-sheet` garde son nom, lu par trois gates. Ce que chaque hôte DOIT tenir vit désormais à UN endroit, `lib/view/use-comments-sheet-host.ts` : mémoriser le focus AVANT l'ouverture et le rendre APRÈS la levée de l'inertie (la maxime de D-90 ci-dessous, « celui qui détruit le focus est le seul à pouvoir le rendre »), fermer au changement de sujet — et un seul appel `lazy()` (`publication-comments-sheet-lazy.tsx`), partagé, pour qu'aucun lecteur ne paie deux fois la liste préchargée du fil. Reste propre à chaque hôte : la PAUSE (la story se fige ; un réel continue de jouer sous la feuille, comme sous la `.sheet` d'iOS) et l'inertie de ce que la feuille recouvre.

La revue-correction y a trouvé la **quatrième entrée** du défaut que D-90 et D-91 ferment : la feuille PREND le focus à son montage, et sur la feuille NUE — hors de tout champ — une flèche faisait défiler le pager `inert` recouvert (un `scrollTo` programmé ignore l'inertie, qui ne retient que ce que l'utilisateur touche) ; sur la story, une flèche l'avançait et Espace relançait sa lecture. Dans les deux cas la feuille se fermait avec le brouillon. Feuille ouverte, les raccourcis d'écran se taisent — `screenGestureYields({ layerOpen })`, la loi du doigt, appliquée au clavier par les deux lecteurs.

**`repostCount` restait une statistique sur les CARTES** (Flux, détail, profil), le seul rail des Réels le rendant actionnable — **périmé par D-125** (2026-09-25, #6278), qui étend le même repartage SIMPLE, optimiste et append-only (`lib/api/publication-repost.ts`, miroir de `ReelsViewModel.repost`) aux cartes, DERRIÈRE une confirmation. Le témoin d'ABSENCE de rôle bouton sur la carte est retourné par celui de D-125.
...
**Ce que ça ne tranchait PAS, et qui est tranché depuis** : `D-125` (2026-09-25, #6278) lève `repostCount` sur le rail du Flux — un effet ET une confirmation, ensemble, comme cette décision l'exigeait.
...
## D-114 — Les effets de message S'EXÉCUTENT : la règle 32 est levée pour eux, sous une exception bornée (2026-09-23, #7596)

**Décision porteur 2026-09-23** : « inutile de comptabiliser les effets dans les messages, juste les effectuer/exécuter et permettre l'interaction quand nécessaire ». C'est l'exception que la charte attendait : `EffectsIndicator` (#6175) rendait à la place un badge statique « baguette + nombre », précisément parce que la règle 32 (un seul `@keyframes`, n'animer que `opacity`/`scale`) interdisait secousses, particules et comète. Le badge et son compteur sont retirés ; le libellé accessible n'énumère plus les effets non plus (une décoration n'est pas une information).

**Ce que l'exception autorise, et rien de plus :**

1. **Où** : uniquement dans l'hôte des effets (`components/message-effects-host.tsx`), monté par `ThreadModes` autour de CHAQUE peau — Focal, Script, Bulles, Rivière et tout mode futur (`routes/thread-modes-effects.test.tsx`, table exhaustive au type). Aucune peau ne câble un effet elle-même.
2. **Comment** : la Web Animations API (`element.animate`), jamais un `@keyframes` de plus dans les feuilles — les images clés sont des DONNÉES pures (`lib/effects-playback.ts`), miroir des durées et courbes de `MessageEffectModifiers.swift`. Propriétés animées : `transform` (secousse, zoom, explosion, waouh, pulsation), `opacity` (halo, fondus, éclat), `filter` sur la seule apparition « waouh » (0,5 s, une fois), `stroke-dashoffset` de la comète. Le halo persistant ne repeint pas : son ombre est posée UNE fois sur un calque dont seule l'opacité respire. Particules (confettis, feux d'artifice, scintillement) : un `<canvas>` par effet, `requestAnimationFrame`, densité plafonnée à 2, scintillement à 10 Hz comme iOS.
3. **Quand** : à l'ENTRÉE de la bulle à l'écran (`IntersectionObserver`, un par message À EFFETS seulement — un message sans effet ne monte rien). Refaire défiler la bulle rejoue l'apparition (horloge d'affichage d'iOS, aucune mémoire de lecture). **À la sortie, tout s'arrête** : animations annulées, boucles coupées, calques retirés — aucune particule ne tourne hors champ, ce qui tient la liste virtualisée à 60 i/s. Rien ne passe par l'état React : ni re-rendu, ni changement de hauteur mesuré par le virtualiseur (tout est en `transform` ou en calque `position: absolute`).
4. **`prefers-reduced-motion`** : aucun mouvement. Les apparitions sont remplacées par un simple éclat d'opacité (0,45 s) ; halo et aurore restent POSÉS et fixes ; pulsation et scintillement (mouvement pur) disparaissent — `reduceMotionSafeMask` d'iOS. La règle générale de `@layer base` (qui écrase les durées CSS) ne touche pas la Web Animations API : c'est le plan, pas la feuille, qui décide.

**Écart assumé** : iOS ne rejoue pas un effet au toucher (il le rejoue à la réapparition) ; le web fait de même, aucun geste « rejouer » n'est ajouté. La recette côte à côte (#7596) juge le rendu ; la fluidité se mesure au défilement sur appareil, pas en unitaire.

## D-115 — Une publication part avec l'audience CHOISIE, jamais un défaut recopié côté client (2026-09-24, #7683)

**Le fait.** `lib/api/stories-publish.ts` ne portait AUCUN champ `visibility` : le corps de `POST /posts` ne disait jamais qui devait voir la publication, et la passerelle posait seule son défaut (`FRIENDS` pour une story, `PUBLIC` sinon, `core.ts:421`). Un auteur qui croyait s'adresser à ses contacts publiait donc un post PUBLIC sans avoir pu le dire — un défaut de SÉCURITÉ (dimension 1), première tranche livrée de la ligne « audience » du registre #7463.

**LE STUDIO N'ENVOIE `visibility` QUE LORSQUE L'AUTEUR L'A CHOISIE.** Le défaut reste une règle SERVEUR : recopier `FRIENDS`/`PUBLIC` en littéral côté client serait exactement la faute qui avait laissé les stories web à `FRIENDS` pendant que les posts naissaient publics (`packages/shared/types/post.ts:15-27`). La pastille du pied du studio (`routes/story-compose-audience.tsx`, miroir de `MeeshyComposerHost+Socle.swift:196-229`) dit toujours ce qui PARTIRA — l'audience choisie, ou sinon le défaut de la passerelle pour le format en cours (`defaultAudienceOf`, `publication-audience.ts`) — mais seule la valeur CHOISIE voyage dans le corps.

**LES SIX AUDIENCES, DANS L'ORDRE iOS** (`PostVisibility.swift:48-50`) : `PUBLIC, COMMUNITY, FRIENDS, EXCEPT, ONLY, PRIVATE`. `ONLY` et `EXCEPT` demandent un sélecteur de personnes que le web n'a pas encore (#7463, seconde tranche) : ils restent AU MENU de la feuille, grisés AVEC leur raison (`audienceAvailability`, loi 4 + `ComposerFormatAvailability.swift:16-60`) — jamais absents, jamais choisissables. La loi D-100 de republication (`repostVisibilityInheritsAudienceList`, `@meeshy/shared/utils/repost-audience`) est importée pour l'offre (`offeredAudiences`), pas réécrite — le studio n'a pas encore de flux de republication, mais le jour où il en a un, aucune réécriture n'est nécessaire ici.

**LA MÉMOIRE EST UNE CLÉ DISTINCTE DU BROUILLON, ET SURVIT À SA PURGE** (`meeshy.studio.audience.<viewerId>`, `StudioDraftStore.rememberAudience`/`lastAudience`) — miroir de `StoryVisibilityPreferenceStore.swift`, à UNE différence assumée : iOS tient une clé par FORMAT (post/statut) plus ce store pour story/réel ; le web a UN studio pour les trois formes, donc UNE mémoire. `ONLY`/`EXCEPT` ne s'y écrivent jamais (`isRememberableAudience`) — leur portée est une liste que la mémoire ne peut pas reproduire, la proposer à nouveau élargirait l'audience en silence. La graine à l'ouverture suit deux rangs : le brouillon persisté (rang 1, un choix déjà fait pour CETTE publication) puis la mémoire (rang 2) — relue UNE SEULE FOIS, à la construction, jamais à une bascule de format (`ComposerMoodSurface.swift:662-703` : une bascule qui relit la mémoire élargirait en silence l'audience que l'auteur vient de resserrer).

**LA FEUILLE ET L'ÉDITEUR SE CHARGENT À LA DEMANDE — LE PLAFOND DU STUDIO NE MONTE PAS** (revue-correction #7683). La première forme liait la feuille d'audience au chunk du studio et dupliquait deux glyphes du socle : 16,04 Ko pour un plafond de 15, que le lot avait porté à 18. Un plafond qui monte à chaque lot n'est plus un gate. La feuille (`routes/story-compose-audience-sheet.tsx`) et l'éditeur d'objet (`routes/story-compose-editor.tsx`, fermé par défaut) sont désormais `lazy()` — la discipline de `LanguageSheet`/`EffectsSheet` du composeur du fil : ce qu'on n'ouvre pas ne se télécharge pas. Mesuré : 14,16 Ko, plafond 15 inchangé. Les deux modes NOMINATIFS ont leur propre jeu de glyphes (`glyphs-story-audience-people.ts`), lié à la feuille seule : la pastille ne peut pas les peindre.

**LE TYPE DIT CE QUE LE STUDIO SAIT CHOISIR.** `StudioDraft.visibility` est `ChoosableAudience | null` (`PostVisibility` moins `EXCEPT`/`ONLY`), et non `PostVisibility` : un brouillon ne peut pas tenir un mode nominatif sans sa liste — la passerelle le refuserait (`types.ts:319-323`, 400). Un snapshot qui en porte un (version antérieure, donnée altérée) se relit SANS audience ; le rang 2 (la mémoire) sert alors. Le jour du sélecteur de personnes, ce champ deviendra un type SOMME qui porte la liste AVEC le mode — jamais un `PostVisibility` nu accompagné d'un tableau voisin.

**RIEN CHOISI, LA FEUILLE DIT LA MÊME CHOSE QUE LA PASTILLE** : la rangée du défaut du format est la rangée courante (`aria-current`, coche), sous-titrée « · par défaut ». La pastille ne peint que la valeur (« Contacts · par défaut » ne tient pas à côté de la capsule Publier) et porte la mention dans sa DESCRIPTION accessible ; sous 360 px, son mot s'efface et l'icône reste, comme iOS aux paliers d'accessibilité (`socleShowsLabels`, #4057).

**LE MESSAGE DU PIED A SA PROPRE LIGNE.** Partagée avec la pastille et la capsule Publier, la ligne d'aide, de refus et d'échec ne gardait que quelques pixels à 320 px : la première forme la TRONQUAIT, et l'échec d'une publication ou la raison d'un réel refusé se réduisaient à une lettre. La rangée d'actions reprend celle du socle iOS (`MeeshyComposerHost+Socle.swift:43-51` : audience, espace, Publier), le message passe au-dessus, pleine largeur.

**CE QUI RESTE HORS TRANCHE** (#7463) : le sélecteur de personnes pour `ONLY`/`EXCEPT` (demande `GET /api/v1/directory/…`, pas branché ici), les mentions et les hashtags de la feuille iOS (le studio web n'a ni l'un ni l'autre). Des lignes de suivi du même registre, jamais une dette silencieuse.

**LA CONTRADICTION ENTRE `packages/shared/types/post.ts` ET LA PASSERELLE QUI TOURNE reste ENTIÈRE** : le commentaire du paquet partagé dit « une publication naît PUBLIQUE — posts, réels et stories confondus » tandis que `core.ts:421` pose `FRIENDS` pour une story. Ce lot se conforme à la passerelle TELLE QU'ELLE EST (`defaultAudienceOf` épingle `core.ts:421`, jamais le commentaire) — la contradiction est l'issue compagnon #7695, pas un patch de ce lot.

## D-116 — Une publication porte des PAGES ; l'agencement se choisit au moment de publier, pour un post de plusieurs pages seulement (2026-09-24, #7684)

**Le fait.** Le studio unique (#7497) ne tenait qu'UN fond, UN calque, UN son : un post de plusieurs médias ne pouvait pas partir du web, et la disposition que le fil sait déjà lire (`resolveMosaicLayout`, D-70) n'avait aucun auteur web. iOS publie une slide par PAGE et choisit la disposition sur la flèche Publier (`ComposerPublishMenu.swift`, `ComposerMosaicChoice.swift`).

**UNE PAGE EST UNE SCÈNE DU DOCUMENT.** `StudioDraft = { pages, currentPage, visibility, language? }` ; `StudioPage` (`lib/stories/studio-page.ts`) porte ce que le brouillon portait seul : textes, fond, calque, son. Toutes les pages partent dans UN `POST /posts` (`composeStoryCanvasPages`) — une scène par page publiable, sous l'`id` de sa page. Une page SANS matière ne produit aucune scène, et ne COMPTE pas (`studioPublishablePageCount`) : ni pour le sous-menu de disposition, ni pour le refus d'une story. Compter `pages.length` offrait une disposition qu'une page vide rendait sans effet — un contrôle inerte (loi 4). Le plafond est celui de la passerelle : dix scènes (`CanvasV3Schema`), dix médias (`MAX_POST_MEDIA`) ; remplacer un média d'une porte déjà occupée ne compte pas contre lui.

**LES IDENTIFIANTS DE TEXTE SONT UNIQUES DANS LE DOCUMENT, PAS DANS LA PAGE** — `translationSetPath` (`storyEffectsV3.ts:635-643`) grave la traduction sur le PREMIER objet d'un `id`, toutes scènes confondues ; `background`/`overlay`/`sound` restent uniques par scène (aucun lecteur ne les cherche à travers les scènes).

**LA DISPOSITION EST UN MENU DANS LE MENU, POUR POST SEUL.** Là où elle voyage (`layoutIsServed` : Post, deux pages publiables ou plus), la ligne « Publication » du menu `[Publier … | ▾]` ne publie plus : elle DÉPLIE ses cinq agencements, dans l'ordre `ComposerMosaicChoice.ordered` (gardé contre le Swift par `check-curve`, PARTIE 12), et chacun publie. Le navigateur n'a pas de menu imbriqué : les lignes se déplient en place, dans le même `role="menu"` et le même parcours clavier (`ArrowRight` déplie, `ArrowLeft` replie). La partie principale de la capsule publie le GESTE en cours (`PublishChoice` : format ET disposition) — aucune disposition tant que l'auteur n'en a choisi aucune, le repli du lecteur s'applique. Une première forme posait un second chevron à côté de la ligne Post, hors du parcours clavier, et une ligne Post qui publiait sans disposition : deux gestes pour une ligne, et ce n'est pas la structure d'iOS.

**UNE STORY DE PLUSIEURS PAGES NE SE REFUSE PLUS — ELLE PART EN AUTANT DE STORIES (#7707, amende ce paragraphe).** Le canal `.scene` d'iOS (`ComposerPublishChannel.swift:79-104`, « UN POST PAR SLIDE ») est désormais écrit côté web. Le canal de chaque format est une DONNÉE, `PUBLICATION_CHANNEL` (`lib/stories/publication-kind.ts`, `Record<PublicationKind, 'scene' | 'document'>`, exhaustif comme le `switch` Swift) ; `studioPublishPlan` (`lib/stories/studio-publish.ts`, l'API UNIQUE du calcul d'un envoi — `studioPublishPayload` a disparu avec son dernier consommateur) n'en tire que le GROUPEMENT des pages : `scene` ⇒ une publication par page publiable (SA scène, SES `mediaIds`, SA légende, `originalLanguage` seulement si CETTE page porte du texte), `document` ⇒ une publication portant toutes les pages, inchangé. `runStudioPublish` (`studio-publish-flow.ts`) les envoie en SÉQUENCE, dans l'ordre des pages, arrêtée à la PREMIÈRE panne — miroir `StoryViewModel+PublicationUpload.swift:99-125`. **Une page partie ne repart jamais** (miroir `publishedPostIds`, `+Publication.swift:240-245`) : elle quitte le brouillon DÈS que sa story est commise — le rail la perd sous les yeux de l'auteur, la capsule dit « Publication k/N… », et le magasin est écrit sur-le-champ, pas seulement par l'effet de persistance, parce qu'un studio quitté pendant l'envoi ne rend plus rien. Quitter le studio ARRÊTE la séquence ENTRE deux requêtes, jamais au milieu d'une (une création coupée en vol peut avoir été commise sans que l'écran le sache) ; la story déjà en vol part, et l'auteur n'est pas ramené de force. Une panne partielle dit « k sur N publiées — les autres restent ici. » + la cause, et le retry ne renvoie que ce qui reste ; une panne à la première requête garde le message d'avant. Le refus `story-with-several-pages` est SUPPRIMÉ (type, catalogue des sept langues, témoins). Le RÉEL, lui, part toujours en un document (canal `.document` d'iOS) ; deux images sur deux pages le qualifient (`qualifiesAsReel`, aplatie sur toutes les pages).

**LE RAIL DES SCÈNES VIT DANS LA BARRE HAUTE** (`ComposerTopBar.swift` : ✕ · rail · ⋯) et prend la place du titre dès la deuxième page — le titre reste au lecteur d'écran ; la scène ne change pas de hauteur quand le rail paraît. La tuile montre la VIGNETTE du fond, jamais un moteur de scène (dix moteurs de 25 px re-rendraient à chaque frappe), mémoïsée sur ses primitives. Cibles web ≥ 44 : le visuel reste 9:16 (24,75 × 44), la cible fait 44 × 44 ; la corbeille a SA cible de 44 × 44 posée À CÔTÉ de la tuile courante, jamais par-dessus — une première forme la posait sur la tuile, et un tap sur la scène courante la supprimait. Seule sa pastille mord le coin, comme la croix d'iOS. Une page dont un média a échoué le dit sur sa tuile (`data-page-phase="failed"`), sinon Publier resterait inerte pour une raison que l'écran ne montre pas. Retirer une page ABANDONNE ses montées en vol et révoque ses `blob:`.
...
**HORS TRANCHE, CONSIGNÉ** : la lecture des scènes suivantes d'un RÉEL côté web (#7708 — le lecteur ne rend que la scène 0) ; réordonner et dupliquer une page (`moveSlide`, `duplicateSlide`) ; la tuile qui montre la scène composée plutôt que le fond ; l'annulation d'un envoi de story EN VOL (miroir `cancelUpload` iOS, qui supprime les slides déjà commises — le web n'a pas de file d'envoi de fond, #7707).
...
**LA PILE DES PLUS ACTIFS VIT DANS L'EN-TÊTE DÉPLIÉ** (`components/active-members-stack.tsx`, sélection `lib/view/top-active-members.ts`, miroir `topActiveMembersList`). Replié, l'en-tête porte déjà à 320 px le retour, le chip de mode, l'appel, la recherche et l'avatar (208 px fixes plus le chip) : 74 px de plus y écraseraient le chip que `check-reading-mode.mjs` mesure à cette largeur. Déplié, c'est l'état d'identité de l'en-tête ; le titre y tronque, son nom entier restant dans les détails qu'il ouvre. Ses gestes sont ceux de toute identité.

**TOUCHER UNE IDENTITÉ : LA STORY NON VUE, SINON LE PROFIL — PARTOUT** (décision porteur du 2026-09-24). Une seule règle, `identityTarget` (`lib/view/identity-target.ts`), sans variante : l'avatar et le nom du fil, de la carte d'une publication, de l'en-tête et de la pile des plus actifs ouvrent la story quand elle n'a pas été vue, et le profil sinon. C'est déjà la règle d'iOS (`MeeshyAvatar.swift:361-365`, `.unread` ⇒ story, sinon `onSenderTap`) ; jumelle iOS #7831. Une story DÉJÀ VUE reste atteignable par « Voir la story » de l'appui long. Elle remplace le défaut de #7241 (« tout anneau ouvre sa story ») ; témoins : `identity-target.test.ts`, `components/identity-seen-story.test.tsx` (fil, rangée plate, publication, avatar, nom).

**HORS TRANCHE, CONSIGNÉ** : médias (#7834), statistiques (#7835), options (#7836) et « Quitter » (#7837).

## D-120 — La pastille « moi » du rail de stories ouvre TOUJOURS le listing « Mes stories », jamais ma story directement — amendement de D-83 (2026-09-24, #6149)

**CE QUE D-83 LAISSAIT OUVERT.** D-83 (2026-09-17) donnait à ma cellule deux portes — le (+) et l'humeur — et laissait l'AVATAR ouvrir directement ma story quand `hasActiveStory` était vrai, exactement comme la tuile d'un autre auteur. C'était l'écart avec iOS que #6149 corrige : `ConversationListView.swift:1394-1397` et `StoryTrayActions.swift:71-75` sont sans ambiguïté — le tap sur MON avatar ouvre TOUJOURS le listing « Mes stories », jamais le lecteur. Créer une story reste le rôle du (+) ; ouvrir directement une story reste celui d'une tuile D'AUTRUI.

**L'ANNEAU ET LA PORTE SONT DEUX QUESTIONS DISTINCTES**, et D-83 les confondait dans un seul champ. `StoryRailSelfEntry` porte désormais `hasActiveStory` (l'anneau accentué — au moins une de mes stories est ACTIVE) ET `hasAnyStory` (la porte du listing — j'ai au moins une story, active ou dans sa fenêtre d'archive). Le corpus du plateau (`?scope=stories`, `PostFeedService.ts:283-312`) garde mes stories EXPIRÉES pendant `AUTHOR_ARCHIVE_WINDOW_MS` : sans cette distinction, une story trop vieille pour l'anneau aurait aussi fermé le listing — miroir exact de `StoryTrayActionResolver.avatarTap(hasMyStory:hasAnyStory:)`, qui ouvre `.manageStories` même quand SEULES des stories expirées existent (« aucun chemin ne menait plus vers ses stories passées »).

**UNE ADRESSE, PAS UNE FEUILLE** (même raisonnement que `/status/new`, D-83) : `/stories/mine` plutôt qu'une sheet iOS, parce que le retour matériel Android et le partage d'adresse le demandent. Le listing lit le MÊME cache que le rail (`useStoryTray`, `STORY_TRAY_QUERY_KEY`) — zéro requête à l'ouverture — et projette mes stories actives par une loi pure, `myActiveStories()` (`lib/view/my-stories.ts`), miroir de `MyStoriesView.swift:104-113`.

**SUPPRIMER EST LE MÊME GESTE QUE LE MENU « ⋯ » D'UNE CARTE DU FLUX** (`deletePostAction`, `publication-actions.ts`) plutôt qu'un second geste écrit à côté : une story est une publication comme une autre pour `DELETE /posts/:postId`. `story-caches.ts` (site UNIQUE, `removeStoryFromCaches`) retire la ligne des deux corpus de stories (`STORY_TRAY_QUERY_KEY`, `STORY_FEED_QUERY_KEY`) et de sa fiche unitaire, sans toucher aux caisses du Flux ; un refus relit `STORIES_QUERY_PREFIX` en plus du Flux. Écart assumé avec iOS, qui attend le serveur avant de purger (`StoryViewModel.swift:470-481`) : optimiste, comme le reste du Flux (Instant App Principles § Optimistic Updates).

**CE QUI N'EST PAS PORTÉ, HORS TRANCHE** : les onglets File / Brouillons / Archive (seul « Publiées » existe encore côté web), la sélection multiple, et six des neuf entrées du menu « … » d'iOS (Modifier, Visibilité, Partager, Enregistrer, Transférer, Republier) — consignées dans l'issue compagnon #7856. Conséquence assumée tant que l'Archive manque : un auteur dont TOUTES les stories ont expiré atteint le listing (la porte `hasAnyStory`) et y lit l'état vide d'iOS (« Vos stories publiées apparaîtront ici tant qu'elles sont actives. ») avec sa porte vers le studio — jamais un écran blanc. Partager/Enregistrer par rangée en particulier : `use-story-owner-rail.ts` est un hook PAR STORY COURANTE couplé au lecteur (pause/resume, `storySaveStore`), pas une loi par rangée de listing ; le réutiliser exigerait un hôte par carte, le dupliquer créerait une jumelle.

**REVUE-CORRECTION (2026-09-24).** Cinq points que la première forme n'avait pas, et que les écrans suivants copieront : (1) la bande épinglée route MON anneau par la même règle (`StoryTrayView.swift:887-915`, `selfAvatarCell`) — elle ouvrait encore ma story ; (2) l'annonce du lien dit la destination, « Gérer mes stories » (`avatarAccessibilityLabel`), jamais « Votre story » ; (3) la confirmation se FERME au geste — une modale grisée jusqu'à la réponse réseau n'était pas optimiste — et `deletePost` réapplique son retrait à la confirmation, si bien qu'aucune sérialisation des suppressions n'est nécessaire ; (4) hors ligne, « Supprimer » s'éteint et un bandeau le dit AVANT le geste : le retour en arrière d'un refus est une relecture, qu'aucun réseau ne servirait ; (5) la confirmation est `components/confirm-dialog.tsx`, la SEULE du dépôt pour un écran neuf (les trois copies antérieures y migrent par #7858) — elle écrit le geste destructif en rouge sur la carte (l'alerte d'iOS), le blanc sur `--color-danger` sombre ne tenant que 3,2:1 — et l'issue de la suppression passe par `LiveAnnouncement`, visible, plutôt que par une région `sr-only` qui rendait un refus MUET à l'œil. La carte est une SURFACE qui contient vignette, bande et date (`MyStoryCard.swift:75-100`), et l'en-tête porte le (+) de la barre iOS.

## D-119 — Taper `@` montre d'abord ses contacts, depuis le cache, dans tous les champs qui mentionnent (2026-09-24, #7846)

**LA RÈGLE DU PORTEUR, À LA LETTRE.** `@` seul : les CONTACTS puis les PARTICIPANTS du contexte, immédiatement, sans réseau. Une lettre : filtre ET tri locaux (pseudo qui commence, puis mot du nom qui commence, puis contient — `rankMentionCandidates`). Deux lettres : la recherche distante part, débouncée (300 ms), une frappe plus récente annule la précédente. Lettres suivantes : l'affinage local est instantané et la réponse précédente reste servie, filtrée, pendant la recherche suivante — rien ne clignote. Ordre contacts → participants → autres, sans doublon (`composeMentionList`, `lib/view/mention-query.ts`).

**AUCUN CACHE NEUF.** Les contacts SONT le panier `accepted` des demandes d'amitié (`friend-requests.ts`), déjà persisté, déjà lu par « Nouvelle conversation », déjà invalidé par le temps réel ; sa projection (`decodePerson`) ne porte que ce que la rangée affiche. `useMentionContacts` (`lib/view/mention-contacts.ts`) n'allume son observateur qu'à la frappe de `@` : frais, rien ne part ; périmé, il est servi puis réchauffé ; vide, il se charge. La résolution de « l'autre partie » est celle de `friendsOf`, jamais réécrite.

**UN SEUL MÉCANISME** : `useMentionField` (`lib/view/use-mention-field.ts`) — curseur, focus, clavier de la liste, insertion `@username ` — et `MentionFieldPanel` (`components/mention-suggestions.tsx`). Le champ ne fournit que son texte et son CONTEXTE (`MentionSource`) : celui que le fil publie (`publishMentionSource`, #7826), ou celui que rend `useMentionSource` — une publication existante interroge `contextType=post`, une publication pas encore écrite passe par l'annuaire (`/directory/people`).

**LES SITES, RELEVÉS CONTRE CE QUE LA PASSERELLE LIT.** Branchés : le composeur du fil (messages, `contextType=conversation`) ; le composeur de commentaire et le champ de modification d'un commentaire (`resolveMentionedUsers` + `createCommentMentionNotificationsBatch`, `contextType=post`) ; le texte de la scène du studio — story, post, réel (`collectMentionableText` lit `content` et `storyEffects.textObjects[].text`, annuaire) ; la note d'une humeur (le `content` du post MOOD, annuaire). Non branchés, parce que la passerelle n'y lit aucune mention : la bio du profil, la légende d'un média (`PostMedia.caption`, un contenu distinct), les champs de communauté et de groupe. `LegendePlan` (#6956) n'est monté par aucun écran.

**LA BORNE PASSE DE 8 À 24 RANGÉES** — la liste est bornée en hauteur et défile ; les contacts passant en tête, huit rangées auraient caché les participants d'un fil derrière le carnet d'adresses.

## D-121 — Modifier le texte d'une publication est le geste optimiste du menu « ⋯ », un seul vol à la fois (2026-09-24, #7534)

**Le menu « ⋯ » d'une carte du fil n'offrait que Supprimer et Épingler (#7533) ; iOS y ajoute Modifier (`EditPostSheet.swift`) — réservé à l'auteur connecté, comme les deux gestes voisins.** `postMenuEntries` continue de trancher côté client ce qui s'AFFICHE ; la passerelle reste l'autorité sur ce qui PASSE (`PUT /api/v1/posts/:id`, 403 ⇒ `'failed'`, jamais un rejeu offert).

**LA LOI EST PURE, ET SE LIT SUR LE TEXTE ROGNÉ.** `publicationEditState()` (`src/lib/feed/publication-edit.ts`), miroir `EditPostSheet.swift:199-247` : `changed` compare les deux textes ROGNÉS (un espace de tête ou de fin n'arme pas « Publier », même garde que `performCommentEdit`) ; `isValid` borne le texte rogné à `POST_CONTENT_MAX_LENGTH` (5000, `UpdatePostSchema.content`) ; `remaining` compte le texte BRUT, jamais négatif. `submittable` FUSIONNE `changed` et `isValid` — contrairement à iOS qui garde les deux gardes séparées — parce que le web n'a qu'UN bouton à désactiver et les deux raisons répondent à la même question posée à l'écran.

**UN SEUL VOL D'ÉDITION À LA FOIS, PAR PUBLICATION** (`editPost`, `src/lib/api/publication-actions.ts`, miroir `isHeartInFlight`) : `EditPostOutcome` ajoute `'busy'` à `PostActionOutcome` — distinct de `'done'` (le défaut majeur que corrige cette garde : un second appel pendant que le premier `PUT` vole encore rendait `'done'` SANS RIEN ENVOYER, et l'hôte annonçait « Publication modifiée » pour un texte parti nulle part) et de `'failed'` (ce n'est pas un refus SERVI — rejouer une fois le premier revenu peut réussir).

**OPTIMISTE, AVEC RETOUR EN ARRIÈRE LIMITÉ AUX CHAMPS TOUCHÉS.** `withTextOf` pose le nouveau `content`/`translations` sur la carte SANS toucher au reste (médias, langue, type, lieu, audience — hors tranche, #7866) ; `mergeServedPost` et `findLiveCardPost` sont le site UNIQUE de fusion dans le registre des caisses, réutilisés (pas dupliqués) depuis #7533. Un refus restaure l'instantané précédent, sans défaire les gestes concurrents survenus entre-temps (mêmes principes qu'`deletePost`, D-120).

**LA FEUILLE EST `<dialog>` MODALE, CHARGÉE À LA DEMANDE** (`components/publication-edit-sheet.tsx`) : retour matériel Android, `lang` sur le champ (Prisme), cibles de 44 px, `role="alert"` sur l'échec avec un bouton Réessayer, fermer pendant l'envoi est sûr (le vol continue, la carte se met à jour à son retour). Pied plutôt que barre du haut (convention web des feuilles du dépôt, écart assumé avec l'en-tête iOS) : Annuler / Publier au bas de la feuille, jamais dans l'en-tête.

**HORS TRANCHE, CONSIGNÉ** : médias, langue, type, lieu, audience (#7866) ; file hors ligne pour une correction commencée sans réseau — fermer la feuille perd la modification (#7867) ; garde d'inventaire du menu sur ses hôtes (#7868).

## D-122 — La coque Android déclare `ACCESS_NETWORK_STATE`, comme le web le consomme déjà (2026-09-24, #7844)

**Le web savait déjà répondre au réseau qui coupe** (`src/lib/net/online.ts`, `useOnline` sur `useSyncExternalStore` + les événements `online`/`offline`, consommé par `src/lib/api/socket.ts`, `receipts.ts`, `media-absent.ts`) — mais la WebView Android d'une coque Capacitor ne lève ces événements que si le manifeste déclare `android.permission.ACCESS_NETWORK_STATE`, et `apps/web/android/app/src/main/AndroidManifest.xml` ne la portait pas. L'app native gelée (`apps/android/…/AndroidManifest.xml:6`) la déclare déjà — c'est ELLE qui sait détecter la coupure sans dépendre d'un timeout de requête, jamais un comportement à réinventer côté coque.

**LE GATE EST LA RÉFÉRENCE UNIQUE DES PERMISSIONS DE LA COQUE** (`scripts/check-android-manifest.mjs`, `REQUIRED_PERMISSIONS`) : `INTERNET`, `RECORD_AUDIO`, `MODIFY_AUDIO_SETTINGS` (déjà posées, #5668) et désormais `ACCESS_NETWORK_STATE`. Il est FAIL-CLOSED — vert sur AUCUNE des formes où la permission n'est pas réellement accordée : absente, doublée, commentée, mauvaise casse (le manifeste XML est sensible à la casse), `tools:node="remove"` (la fusion du manifeste la retire), `android:maxSdkVersion` (l'octroi s'arrête à ce niveau d'API). La relecture du gate (#7869, `310f5a9d18` — trois points d'étape l'avaient précédée sous la référence inexistante « #7844-residual ») a montré qu'un premier jet pouvait rester vert sur quatre formes fausses (casse, `tools:node="remove"`, `android:maxSdkVersion`, `uses-permission-sdk-23`) et, pire, MUET sous un lien symbolique posé à la place du fichier réel — `isEntryPoint` (`scripts/lib/entry-point.mjs`) ferme cette garde d'entrée pour ce gate ; neuf autres scripts de `scripts/` portent encore la forme brute et migrent par #7870.

**HORS TRANCHE, CONSIGNÉ** (#7844) : la recette manuelle sur l'AVD `Meeshy_Poc_Web-v31` (mode avion → pastille hors-ligne, retour réseau → reconnexion du socket sans relancer l'app) n'est pas rejouable depuis un conteneur Linux sans SDK Android — à consigner en commentaire de l'issue lors d'une recette faite depuis un poste qui héberge l'émulateur.

## D-123 — Une scène se PARCOURT au doigt : l'horloge du moteur accepte un `seek`, la barre du réel à scène et le segment actif de la story deviennent un slider — amendement de D-86 (2026-09-25, #7879)

**D-86 disait « un réel composé se rejoue comme sa scène » ; la demande porteur du 2026-09-25 ajoute qu'il se PARCOURT.** Toucher la barre (réel) ou le segment courant (story) l'AGRANDIT (3 → 10 px) et montre une poignée de 16 px ; glisser pointe le temps et la scène se redessine À CHAQUE mouvement ; relâcher reprend DEPUIS le temps pointé. Même geste qu'iOS (`ReelScrubBar`), même mécanique que la piste de la visionneuse (`media-transport.tsx`), dont la loi pure — `seekFraction`, `keyboardSeekTarget` (qui gagne un `step` optionnel), `formatMediaTime` — est sortie dans `lib/view/seek-track.ts` et ré-exportée : l'importer depuis `media-transport.ts` créait un chunk partagé `media-transport-*` STATIQUE, que `measure-weight` refuse (`media_transport.dynamic_only`).

**L'HORLOGE EST LE SEUL CANAL** (`useSceneClock`, ajouts D-11) : `seek(t)` borne à `[0, durée]`, redessine les abonnés SUR-LE-CHAMP (pause comprise — aucune trame n'est attendue), émet `onTime`, réarme la fin d'une scène qui ne boucle pas et relance sa boucle d'images ; `subscribeSeek` n'est entendu qu'aux seeks, jamais aux trames ; `now()` sert le pas clavier. `ScenePlayer.onClock` remet la poignée UNE fois. Aucun `setState` par mouvement : le rempli, la poignée et `aria-valuenow` s'écrivent par ref (`SceneScrubPainter`) ; l'état React ne change qu'à l'appui et au relâcher.

**TOUS LES MÉDIAS SUIVENT LA TIMELINE DE LA SCÈNE — UNE loi, en lecture ET au seek** (retour porteur du 2026-09-25 sur la PR #7886). La première forme du lot ne recalait les médias qu'au seek, sur `t` brut : elle ignorait la FENÊTRE d'apparition d'un média posé et sa COUPE, et en lecture les `<video>`/`<audio>` faisaient `play()` au montage, sur leur propre horloge. Désormais `mediaTimeAt` (`lib/canvas/media-seek.ts`) est la loi unique — `temps média = f(temps scène, fenêtre, coupe, boucle, durée)`, miroir de `StoryMediaLayer.trimmedSeekTarget` (min(coupe.début + max(0, t − entrée), coupe.fin)) et de `StoryBackgroundLayer.loopedScrubTarget` (t modulo la durée d'un fond qui boucle, #7892 côté iOS). Ses trois timelines : `backgroundMediaTimeline` (fond, boucle), `objectMediaTimeline` (vidéo et son posés : `timing` / `startTime`, `sourceStart`/`sourceEnd`, `loop`), `trackMediaTimeline` (piste : `startOffsetMs`, `bounds`, `loop`). Un SITE UNIQUE l'applique aux éléments, `useSceneMediaSync` (`components/scene-media-seek.ts`) : au seek, pose précise (50 ms) ; en lecture, quand l'horloge MÈNE (`isDriving`), chaque trame vérifie la fenêtre (entrer ⇒ lire, sortir ⇒ pause) et recale une dérive de plus de 300 ms (repliée pour un média qui boucle, `mediaDrift`) ; à la reprise, tout repart du temps de la scène. Le lecteur de story remet la durée de la diapositive au moteur (`durationSeconds` → `fallbackDurationSeconds`) : l'horloge y mène même sans objet temporisé. Quand elle ne mène pas, le média joue librement et ne se recale qu'aux seeks (la piste garde son départ différé en temps de LECTURE). La pause pendant le glissé vient de `playing`, que l'hôte coupe — le glissé est un état DISTINCT de la pause du tap : relâcher ne défait jamais une pause choisie.

**Mesuré au navigateur** (réel `reel-scene-loop`, fond vidéo de 3 s, piste déclarée de 7 s ⇒ scène de 7 s) : pointer 2 s ⇒ vidéo 2,02 s ; pointer 5 s ⇒ vidéo 2,00 s et la MÊME image (empreinte identique) ; pointer 6,5 s ⇒ 0,50 s, même image qu'à 0,5 s ; relâcher reprend depuis le temps pointé.

**LE GESTE GAGNE** sur le tap de pause et le pager des réels (`touch-action: none`, capture du pointeur, propagation arrêtée) et sur les bandes de tap, l'appui long et le clavier du lecteur de story (`data-claims-gesture`, D-91). La zone de frappe fait 44 px ; les commandes peintes APRÈS elle (rail et identité du réel, identité et croix de la story) la recouvrent et gardent leur geste. Le conteneur des segments de story passe de `progressbar` à `group` : les enfants d'une `progressbar` sont présentationnels, le slider y aurait disparu des lecteurs d'écran. Valeur en POURCENT (`aria-valuenow`, ce que `check-story-scene.mjs` lit déjà), temps dit par `aria-valuetext` — clés `media.video.position*`, déjà dans les sept catalogues : même mot que la visionneuse, et aucun ajout à `catalog-fr.ts`, hors budget (1209 lignes).

**Story : le glissé et l'horloge portent l'IDENTITÉ de leur story** (`routes/use-story-scrub.ts`), jamais une remise à zéro en effet parent — la course que `readyStoryId` évite déjà. Le minuteur (`elapsedRef` + `startTsRef`) repart du temps pointé ; une story sans scène ne règle que sa progression, donc le temps qui lui reste.

**HORS TRANCHE, CONSIGNÉ** (#7879) : le chemin v1 d'une story VIDÉO (`StoryMediaLayer`, `<video autoPlay loop>`) ne suit pas le temps pointé — il ne suivait déjà ni la pause ni le minuteur ; la piste se lit de gauche à droite en RTL, comme celle de la visionneuse.

## D-124 — Un lien Meeshy ouvert par Android entre dans la coque : App Links de `meeshy.me`, schéma `meeshy://`, et un pont qui retient le lien du lancement à froid (2026-09-25, #5819)

**D-27 réparait la moitié « une WebView pointée sur `/c/<id>` rend le fil » ; l'entrée système manquait** : aucun `<intent-filter>` VIEW dans le manifeste, et Capacitor ne route aucun intent — sa WebView démarre toujours sur `/`. La coque déclare désormais les App Links de `meeshy.me` et `www.meeshy.me` (`autoVerify`) et le schéma court `meeshy://` (miroir de `CFBundleURLSchemes` iOS). `MeeshyLinksPlugin.java` relaie chaque intent VIEW au web par l'événement `appUrlOpen`, **retenu** (`notifyListeners(…, true)`) : `BridgeActivity.load()` rejoue l'intent de lancement avant que la page n'ait chargé, et c'est la rétention qui empêche le lien d'un lancement à froid de se perdre. Côté web, `src/lib/links/shell-deep-links.ts` s'abonne après la première peinture, seulement sous `__SHELL__`, par `window.Capacitor.addListener` (aucun `@capacitor/app` : même lecture brute que `native-shell.ts`), et traduit l'adresse par la loi de `internal-link.ts` (#7849).

**LE MANIFESTE NE RÉCLAME QUE CE QUE L'APP SERT.** Un lien vérifié ne repasse plus par le navigateur : réclamer un chemin sans écran ouvrirait l'app sur rien. Les chemins suivent l'`apple-app-site-association`, amputés de ceux que la table des routes ne sert pas (`/join/`, `/conversation/`, `/users/`, `/share`, `/p/`, `/s/`) ; `src/lib/links/app-links.test.ts` lit le manifeste livré et exige que chaque chemin réclamé passe `isAppPath` (sorti de `rich-text.tsx` vers `src/routes/app-paths.ts`, partagé) — mutation vérifiée : `/join/` ajouté fait rougir le témoin.

**HORS TRANCHE, CONSIGNÉ** (#5819) : la vérification réelle du domaine exige l'empreinte SHA-256 de la clé de signature release (ou Play App Signing) dans `public/.well-known/assetlinks.json`, qui porte encore `REPLACE_WITH_RELEASE_SHA256` — sans elle, Android ne vérifie pas le domaine et propose le choix app / navigateur, le schéma `meeshy://` fonctionnant dans tous les cas. L'entrée `me.meeshy.app.debug` du même fichier vise un paquet qui n'existe pas dans cette coque (`applicationId "me.meeshy.app"` en debug aussi). La recette `adb shell am start -a android.intent.action.VIEW -d …` et la moitié iOS (Universal Links de la coque) restent à faire ; aucun SDK Android dans le conteneur de ce lot, le Java n'y est pas compilé.

## D-125 — `repostCount` devient un bouton sur TOUTES les cartes, derrière une confirmation : le geste est irréversible, l'envoi ne part jamais du seul tap — amendement de D-89 et D-100, périme les paragraphes qui y disaient `repostCount` inerte (2026-09-25, #6278)

**Décision** — sur toute carte qui monte `usePostGesture` (Flux, détail d'une publication, signets, hashtag, profil, Réels), `repostCount` EST un bouton dès qu'un hôte sait repartager, exactement comme « Commenter » et « Enregistrer » (loi 4 : un contrôle existe s'il a un effet). Mais `onRepost` ne l'ENVOIE plus : il OUVRE une demande de confirmation (`pendingRepostId`), et seul `confirmRepost()` — jamais `onRepost` seul — appelle `repostAction`. `cancelRepost()` referme sans aucun effet, ni réseau, ni cache, ni annonce. Les trois vivent dans le SEUL hôte (`lib/view/use-post-gesture.ts`), et un unique composant de présentation, `components/post-repost-confirm.tsx` (`PostRepostConfirm`, sur `ConfirmDialog`), est monté par les six écrans qui déstructurent `onRepost` — jamais une confirmation recopiée par appelant.

**Pourquoi une confirmation, alors que la carte a déjà rendu le bouton actionnable.** Le repartage est APPEND-ONLY : `markReposted` (`lib/feed/interactions.ts`) n'a pas de symétrique, et l'interface n'offre nulle part de « retirer mon repost ». Un doigt qui glisse sur la rangée d'actions publiait donc, sans le vouloir et sans recours, une republication PUBLIQUE. iOS ne l'envoie jamais depuis le seul tap non plus : `FeedPostCard.swift:1010-1053` pose `showRepostOptions = true` au tap, et c'est l'alerte Repartager / Citer / Annuler qui appelle `onRepost?(post.id)` — le geste de la carte OUVRE un choix, il ne l'exécute pas.

**Ce que la v3.1 REPREND, et ce qu'elle ADAPTE.** Elle reprend l'ouverture d'un choix avant l'envoi, les deux libellés qui restent DISPONIBLES (« Repartager », « Annuler » — `common.cancel`, déjà partagé par toute confirmation du dépôt) et l'absence de rôle destructeur (`tone: 'default'`, comme le bouton `.cancel` d'iOS n'est pas non plus `.destructive` : reposter n'efface rien). Elle ADAPTE la présentation : iOS ouvre une ALERTE À TROIS BOUTONS (Repartager / Citer / Annuler) parce que « Citer » y a un effet ; la v3.1 n'a pas encore le composeur prérempli (`repostOfId`, #7463) — poser un troisième bouton qui ne ferait rien violerait la MÊME loi 4 que celle qui vient de rendre `repostCount` cliquable. Elle ouvre donc `ConfirmDialog` (la modale `<dialog>` déjà partagée par « Mes stories », « Débloquer », « Déconnexion » — jamais une quatrième boîte divergente) réduite à ses deux choix RÉELLEMENT disponibles. Le jour où « Citer » existe, c'est CETTE modale qui apprend un troisième bouton — jamais un second contrôle à côté.

**Ce que ça ne change PAS.** Le contrat de `FeedPostCard` (et de `ReelPage`) reste : `onRepost(postId)` part au clic, immédiatement, sans état interne côté carte (`feed-post-card-gestures.test.tsx`, inchangé) — la carte ignore toujours tout de la confirmation, qui est un problème d'HÔTE, pas de présentation de la rangée. `performRepost` (optimiste, refus ⇒ rollback, 409/410 ⇒ optimiste gardé) ne bouge pas davantage : la confirmation se pose EN AMONT du même appel, jamais une seconde mécanique réseau.
...
## D-130 — Chaque conversation a son écran « Médias, liens et documents », lu sur l'index SERVEUR ; la visionneuse ouverte depuis lui feuillette TOUTE la conversation — amendement de D-54 (2026-09-26, #8103, #6303, #7834)

**Contexte.** Le web n'avait aucun écran média (la feuille de détails portait l'en-tête, le partage et les membres, #7829). iOS a l'onglet Médias de `ConversationInfoSheet.swift`, tiré des messages EN MÉMOIRE — vide depuis la liste des conversations. La passerelle sert désormais l'index d'une conversation (`GET /conversations/:id/messages?view=media&kinds=…&q=…&before=…`, #8095 puis #8098) : sept genres, recherche, pagination par curseur, vue unique exclue aux deux niveaux.

**Décision.**
- **L'index vient de la passerelle, jamais du fil chargé** (`lib/api/conversation-media-hub.ts`). La page du cache a la forme de `MessagesPage` : le garde-fou du fil (`nextMessagesCursor`, dont le refus d'un `before` inconnu que la passerelle RESSERT) s'applique sans jumelle. L'ordre reste celui SERVI (récent d'abord) : l'extension vers le passé ajoute à la FIN, et la page ouverte en visionneuse ne bouge pas.
- **Une clé de cache par (conversation, genre, recherche)** : un segment déjà vu se peint sans squelette et se revalide en fond ; un segment jamais vu montre son squelette, jamais les éléments du voisin (`placeholderData` ne garde que le MÊME genre, pour qu'une recherche en cours laisse les résultats du terme précédent). La recherche attend 300 ms de frappe posée, un terme de moins de deux caractères ne part pas (400 serveur), et la requête du terme abandonné est ANNULÉE (TanStack annule une requête sans observateur dont la fonction a lu le `signal` — témoin `conversation-media-hub.test.ts`).
- **Le serveur rend des MESSAGES, l'écran des ÉLÉMENTS** (`lib/view/media-hub.ts`) : chaque segment ne garde que ce qui est le sien, selon la MÊME partition que la clause serveur (visuel `image/*`·`video/*`, audio `audio/*`, contact `text/vcard`·`text/x-vcard`, document tout le reste ; lien = adresses `http(s)` du contenu via `segmentText` ; conversation = adresses `meeshy.me/(chat|join|c|conversation)/…` ; lieu = `placeOf`). Une partition différente ferait servir un message dont le segment n'afficherait rien.
- **Aucun lecteur réécrit** (`components/conversation-media-hub-items.tsx`) : un vocal se joue par `Attachments` → `VoiceAttachment` (reprise, rapport d'écoute), un document s'ouvre par la rangée du fil (`<a target="_blank">`, téléchargé à l'ouverture, ouverture rapportée), une carte de visite par la carte du fil (#8101, `Attachments` → `ContactCard`), une adresse de conversation par sa carte (#8099, `ConversationLinkCards` — Rejoindre, Ouvrir), un lieu par `LocationCard`, un autre lien Meeshy navigue DANS l'app (`internalPathOf`), un lien extérieur s'ouvre dans un nouvel onglet.
- **Les octets ne partent qu'à l'ouverture** : la grille ne charge que la VIGNETTE servie, en `loading="lazy"`, sur le fond ThumbHash — jamais le fichier d'une vidéo, jamais une pièce floutée (substitut `eyeSlash`). La pellicule de la visionneuse charge aussi ses vignettes paresseusement.
- **« Aller au message » n'est posé que pour un message du fil CHARGÉ** (`canJumpTo`) : le saut du fil (`useThreadJump`) n'atteint pas un message plus ancien que la fenêtre, et l'ouverture sur un message précis est #7420. Un bouton qui ne mènerait nulle part serait un contrôle qui ment.
- **La visionneuse conversation-entière** (`components/media-hub-viewer-host.tsx`, D-54 amendé) : `MediaViewer` reçoit l'index visuel chargé, l'auteur et la date de CHAQUE page (`carrierAt`), l'auteur de chaque page pour le rapport d'ouverture (`isMineAt`), et prie l'hôte d'étendre la liste à trois pages du bout (`onNearEnd`) ; la fenêtre de rendu ±1 est inchangée. La légende n'est pas remise : ce serait une seconde descente du Prisme. Ouverte depuis une feuille (`<dialog>` en `showModal()`), la couche se pose DANS ce dialogue (`container`) — la couche supérieure du navigateur recouvrirait et rendrait inerte tout ce qui est hors d'elle.
- **Depuis le FIL, la pellicule reste celle du message.** L'y passer à la conversation entière ne se ferait pas sans casser l'existant : le gate DOM de D-54 (`scripts/lib/check-media-grid.mjs`) et les témoins de `media-viewer.test.tsx` mesurent la pellicule D'UN message (« media-13 : 4 vignettes »), et la seule liste que le fil connaît est sa fenêtre CHARGÉE — une pellicule qui changerait de longueur en défilant. Le chemin conversation-entière est l'écran des médias ; #6303 garde aussi les quatre actions (Enregistrer, Réagir, Répondre, Créer avec ce média), qui ne sont pas dans ce lot.
- **La feuille de détails** porte la section « Médias » (#7834) : les quatre dernières vignettes (même clé de cache que la grille — ouvrir l'écran ensuite ne recharge rien) et l'entrée « Médias, liens et documents ». L'écran est un chunk À LA DEMANDE (`budgets.json › conversation_media_hub`).
- **La recherche d'une feuille passe à `onInput`** (`sheet.tsx`) : c'est la convention des champs texte du dépôt, et la seule forme qu'un témoin happy-dom rappelle (`test-support/react-events-probe.test.tsx`) — la recherche de toute feuille était jusque-là sans témoin.

**Ce qui reste.** Le miroir iOS (l'onglet Médias devient cet écran, #8103 côté iOS) ; les quatre actions de la visionneuse et la pellicule conversation-entière depuis le fil (#6303) ; l'ouverture du fil sur un message ancien (#7420).

## D-131 — Pendant un appel, l'application reste utilisable : pastille, bulle déplaçable, image dans l'image, périphériques retenus par appareil (2026-09-26, #8046)

**Le constat.** La couche d'appel (#6382) peignait l'écran plein ou une pastille ; iOS replie en plus l'appel en BULLE déplaçable (`CallBubbleView.swift`) et le fait flotter hors de l'app (`PiPCallController.swift`). Le web n'avait ni l'un ni l'autre, ni choix de caméra, de micro ou de sortie.

**Ce qui est tranché.**
- **Trois affichages, un appel** : `CallDisplay` = `full | pill | bubble`. La pastille se replie en bulle par un bouton (« Réduire en bulle ») OU un glissement horizontal franc (≥ 64 px, le swipe d'iOS). La bulle porte la vidéo du pair (sinon la mienne, sinon le portrait), se déplace au doigt et à la souris, se CLIPSE au bord le plus proche, se déplace au clavier (flèches — l'`accessibilityAdjustableAction` d'iOS), et garde sa place sur l'appareil (`meeshy.call.bubble.v1`). Un toucher rend l'écran d'appel ; sa barre garde micro, image dans l'image et raccrocher (cibles de 44). Elle ne recouvre jamais l'en-tête (64 px) ni le champ d'écriture (96 px) : on écrit dessous. Pas de pincement en quatre paliers comme iOS : une taille vidéo, une taille portrait.
- **Image dans l'image** : Document Picture-in-Picture d'abord (une vraie fenêtre, qui reçoit un PORTAIL de l'arbre de la couche — un seul magasin, la fenêtre ne peut pas diverger de l'écran), `requestPictureInPicture` d'une `<video>` cachée sinon (montée tant qu'une vidéo peut flotter : la PiP doit partir DANS le geste). La bascule automatique quand l'onglet se masque passe par l'action Media Session `enterpictureinpicture` — le seul chemin sans geste qu'un navigateur accepte. La coque Android (WebView) n'a ni l'un ni l'autre : aucun bouton n'y promet la PiP.
- **Périphériques** : une feuille ouverte de l'en-tête de l'écran d'appel. Un choix est EXIGÉ au sélecteur (`deviceId: exact` — un appareil occupé échoue au lieu d'ouvrir son voisin) et DEMANDÉ au démarrage des appels suivants (`ideal` — un casque débranché ne fait pas échouer l'appel). La préférence (`meeshy.call.devices.v1`) n'est écrite qu'une fois l'appareil ouvert et remis au moteur (`engine.replaceInput`). La sortie passe par `setSinkId` sur chaque `<audio>` de pair (un magasin, pour qu'un pair arrivé après le choix sorte au même endroit) ; dans la coque, par les routes natives du lot 7 (`shellAudioRoutes` / `setShellAudioRoute`, D6).
- **La bulle et la fenêtre PiP sont des chunks FRÈRES de l'écran d'appel**, chargés par `call-layer.tsx`, jamais par `call-overlay` : un chunk chargé par un autre à la demande en importerait statiquement les modules communs (`budgets.json` › `dynamic_only`). L'écran d'appel reste à 5,4 Ko.
...
## D-132 — On partage son écran pendant un appel : l'écran REMPLACE la piste vidéo émise, il s'affiche ENTIER chez le pair, et le bouton n'existe que là où `getDisplayMedia` existe (2026-09-26, #8063)

**Le constat.** La passerelle relaie le partage (`call:toggle-screen` → `call:media-toggled`, `mediaType: 'screen'`, PR #8126) et iOS le porte (ReplayKit) ; le web n'avait ni le geste ni la réception — `decodeMediaToggled` rendait `null` hors audio et vidéo.

**Ce qui est tranché.**
- **Une piste vidéo, jamais deux.** Le moteur (`engine.toggleScreen`) ouvre `getDisplayMedia` (vidéo seule, `contentHint: 'detail'`), REMPLACE la piste vidéo émise sur chaque lien — `replaceTrack`, ou une renégociation quand la ligne vidéo ne faisait que recevoir (appel vocal : `peer-link.setVideoTrack` passe la ligne en `sendrecv`) — et annonce `call:toggle-screen`. La caméra, si elle tournait, est RELÂCHÉE le temps du partage (voyant éteint) et rouverte à l'arrêt ; sinon l'arrêt rend un appel vocal. Même contrat qu'iOS : le pair lit l'écran sur la piste vidéo du membre, sans seconde ligne.
- **L'arrêt a deux portes** : le bouton, et « Arrêter le partage » du navigateur, qui termine la piste (`onended`). Les deux passent par le même `stopScreen`. La caméra ne se rallume pas PAR-DESSUS un partage (bouton désactivé) : on arrête d'abord.
- **Chez le pair, l'écran partagé prend la scène** (`callLayout` → `'screen'`, en direct comme en groupe), ENTIER (`object-fit: contain` — rogner un écran en cache le texte), sous une bannière « X partage son écran » ; la fin rend l'état de sa caméra. L'image dans l'image montre l'écran partagé d'abord. Celui qui partage lit la pastille « Vous partagez votre écran », et sa vignette n'est plus en miroir.
- **Le bouton se DÉTECTE, il ne se configure pas** : `canShareScreen(navigator.mediaDevices)`. La WebView de la coque Android et Safari iOS n'ont pas `getDisplayMedia` : aucun bouton n'y promet le partage (loi « un contrôle existe s'il a un effet »).
- **Le gate a un pair qui décroche** (`fixtures-call-peer.ts`) : une vraie `RTCPeerConnection` dans la page, armée par `meeshy.fixtures.callPeer`, chargée à la demande par le bouchon de socket et élaguée des builds `gateway`. `check-calls-screen.mjs` y mesure que l'écran ARRIVE (images décodées par le pair), ce qu'aucune doublure de `bun test` ne peut dire.

**Ce qui n'est pas fait — ÉMETTRE depuis la coque Android.** La WebView n'a pas `getDisplayMedia`, et la faire émettre un écran exigerait un chemin natif : `MediaProjection` (consentement `createScreenCaptureIntent`), un service au premier plan de type `mediaProjection` (Android 14 : démarré APRÈS le consentement, permission `FOREGROUND_SERVICE_MEDIA_PROJECTION`, déclaration Play Console), puis faire entrer les images dans le WebRTC de la WebView — soit une seconde pile WebRTC native (`org.webrtc`) qui dupliquerait signalisation et maillage, soit des images copiées par le pont vers un `canvas.captureStream()` (quelques images par seconde, coût batterie et latence à mesurer sur appareil). Aucun des deux ne se compile ni ne se vérifie sans SDK Android : ce lot ne déclare donc NI la permission NI le type de service (une déclaration sans consommateur est morte, et sensible côté Play). RECEVOIR un écran partagé fonctionne dans la coque (c'est une piste vidéo comme une autre) ; seul l'émettre y manque, et son bouton y est absent.

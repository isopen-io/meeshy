...
**UNE STORY DE PLUSIEURS PAGES SE REFUSE, AVEC SA RAISON.** iOS la publie en un post PAR slide (canal `.scene`) ; le web n'a pas ce canal, et son lecteur de story ne rend que la scène 0 — publier le document entier perdrait les pages suivantes EN SILENCE. Tant que le canal N-posts n'est pas écrit, `studioPublishRefusal` rend `story-with-several-pages`, dit au pied et au menu. Le RÉEL, lui, part en un document (canal `.document` d'iOS) ; deux images sur deux pages le qualifient (`qualifiesAsReel`, aplatie sur toutes les pages).

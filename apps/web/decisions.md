...
- **Aucun lecteur réécrit** (`components/conversation-media-hub-items.tsx`) : un vocal se joue par `Attachments` → `VoiceAttachment` (reprise, rapport d'écoute), un document ou une carte de contact s'ouvre par la rangée du fil (`<a target="_blank">`, téléchargé à l'ouverture, ouverture rapportée), un lieu par `LocationCard`, un lien Meeshy navigue DANS l'app (`internalPathOf`). La carte de conversation (#8099) et la carte de visite (#8101) ne sont pas encore dans `dev` : rendu simple, à remplacer par leurs composants quand ils y seront.

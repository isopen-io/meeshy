...
- **La paire Partager/Enregistrer apparaît et disparaît ENSEMBLE** (`StoryExportRailButtons`) : elle n'est remise que si la story porte un média EXPORTABLE (`storyDownloadableMedia` — la carte `EXTENSION_BY_MIME` de la passerelle) ET que l'hôte a une PORTE de livraison (`file-delivery-host.ts`). Mesuré dans le dépôt : ni `@capacitor/android` 8.5.1 (aucun `DownloadListener`) ni `@capacitor/ios` 8.5.1 (aucun `WKDownloadDelegate`) ne branchent le téléchargement de leur WebView, et la coque Android n'a pas `navigator.share` — la paire y est donc ABSENTE plutôt qu'inerte. Une story texte n'offre que « Vues ».
...
- **Poids** — `story_reader` 10,93 Ko gzip -9 (mesure du 2026-09-24, plafond 11) : le téléchargement et la livraison sont chargés à la demande (`import()` au premier « Enregistrer », D-54), la feuille aussi (`publication_viewers_sheet`, nommée HORS du motif `story-…` du lecteur). **La marge restante est de 0,07 Ko** : le glyphe `download-simple` (miroir de `square.and.arrow.down.fill`) n'est PAS entré — « Enregistrer » garde `GLYPHS.archive`, et toute surface de plus sur ce lecteur passe par l'arbitrage de #7121.

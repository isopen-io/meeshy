...
**UNE STORY DE PLUSIEURS PAGES NE SE REFUSE PLUS — ELLE PART EN AUTANT DE STORIES (#7707, amende ce paragraphe).** Le canal `.scene` d'iOS (`ComposerPublishChannel.swift:79-104`, « UN POST PAR SLIDE ») est désormais écrit côté web : `studioPublishPlan` (`lib/stories/studio-publish.ts`) décide du NOMBRE de publications par DONNÉE (`choice.kind`) — STORY rend un `StudioPublication` par page publiable (SA scène, SES `mediaIds`, SA légende), POST et RÉEL en rendent UN portant toutes les pages, inchangé. `runStudioPublish` (`studio-publish-flow.ts`) les envoie en SÉQUENCE, dans l'ordre des pages, arrêté à la PREMIÈRE panne — miroir `StoryViewModel+PublicationUpload.swift:99-125` et `publishedPostIds` (`+Publication.swift:240-245`) : un échec partiel laisse les pages déjà PARTIES parties (`withoutPages` les retire du brouillon), dit « k sur N publiées » + la cause, et un retry ne renvoie JAMAIS ce qui est déjà parti. Le refus `story-with-several-pages` est SUPPRIMÉ (type, catalogue des sept langues, témoins). Le RÉEL, lui, part toujours en un document (canal `.document` d'iOS) ; deux images sur deux pages le qualifient (`qualifiesAsReel`, aplatie sur toutes les pages).

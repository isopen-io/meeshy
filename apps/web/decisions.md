...
**LES MÉDIAS COURENT SUR LEUR PROPRE HORLOGE** et ne s'y recalent qu'aux seeks (`useMediaSeek`, seek TOLÉRANT à 50 ms) : fond vidéo (boucle), média posé (`payload.loop`), son posé, et la piste de fond (`trackSeekPlan` : le départ différé `startOffsetMs` se recompte depuis le temps pointé, la position tombe dans la fenêtre `bounds`). La pause pendant le glissé vient de `playing`, que l'hôte coupe — le glissé est un état DISTINCT de la pause du tap : relâcher ne défait jamais une pause choisie.

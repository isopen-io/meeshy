...
**CE QUI N'EST PAS PORTÉ, HORS TRANCHE** : les onglets File / Brouillons / Archive (seul « Publiées » existe encore côté web), la sélection multiple, et cinq des huit entrées du menu « … » d'iOS (Modifier, Visibilité, Partager, Enregistrer, Transférer, Republier) — chacune une issue compagnon de #6149. Partager/Enregistrer par rangée en particulier : `use-story-owner-rail.ts` est un hook PAR STORY COURANTE couplé au lecteur (pause/resume, `storySaveStore`), pas une loi par rangée de listing ; le réutiliser exigerait un hôte par carte, le dupliquer créerait une jumelle.

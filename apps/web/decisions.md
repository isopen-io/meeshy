...
**`repostCount` reste une statistique sur les CARTES** (Flux, détail, profil) : seul le rail des Réels le rend actionnable, par un repartage SIMPLE, optimiste et append-only (`lib/api/publication-repost.ts`, miroir de `ReelsViewModel.repost`) — iOS n'y ouvre pas de composeur prérempli non plus. Le témoin d'ABSENCE de rôle bouton sur la carte tient toujours.

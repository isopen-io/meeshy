/**
 * Tests for the reel deep-link page's enriched share flow (Task 4, point 2).
 */
...
describe('ReelPage — enriched share', () => {
  ...
  beforeEach(() => {
    ...
    mockSharePost.mockResolvedValue({ shared: true, shareCount: 1, shortUrl: 'https://meeshy.me/l/reelxyz', token: 'reelxyz' });
  ...
  it('requests a tracking link and copies it to the clipboard (no navigator.share)', async () => {
    ...
    await waitFor(() => expect(clipboardWriteText).toHaveBeenCalledWith('https://meeshy.me/l/reelxyz'));
  });
  ...
  it('hands the tracked link to navigator.share when available', async () => {
    ...
    await waitFor(() =>
      expect(mockNavigatorShare).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://meeshy.me/l/reelxyz' })),
    );

...
// Mock i18n : t(key, fallback) renvoie le fallback anglais (comportement réel
// hors traductions chargées) → assertions sur les libellés anglais.
...
describe('ConfigModal', () => {
  ...
  describe('Rendu initial', () => {
    it('affiche le modal quand isOpen est true', () => {
      ...
      expect(screen.getByText(/Settings and Configuration/)).toBeInTheDocument();
    });

    it('n\'affiche pas le modal quand isOpen est false', () => {
      render(<ConfigModal {...defaultProps} isOpen={false} />);

      expect(screen.queryByText(/Settings and Configuration/)).not.toBeInTheDocument();
    });

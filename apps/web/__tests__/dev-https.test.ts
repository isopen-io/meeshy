...
describe('le HTTPS de dév de apps/web (#4479)', () => {
  ...
  it('replie sur le certificat auto-signé de Next quand le partagé manque', () => {
    expect(plan([]).args).toEqual(['dev', '-p', '3100', '--experimental-https']);
  ...
  it('dit, dans le repli, que meeshy.local:3100 ne sera PAS servi — la bannière du Makefile l’annonce', () => {
    expect(plan([]).notice).toBe(SELF_SIGNED_FALLBACK);
    expect(plan([]).notice).toContain('meeshy.local:3100');
  ...
  it('laisse passer les arguments de Next tels quels — le port reste déclaré dans package.json', () => {
    expect(planDevHttps({
      nextArgs: ['-p', '4100', '--turbo'],
      certificate: CERT,
      key: KEY,
      exists: () => true,
    }).args.slice(0, 4)).toEqual(['dev', '-p', '4100', '--turbo']);
  });

  it("vise l'emplacement PARTAGÉ que make setup-certs alimente, hors des deux zones — le MÊME que la zone v3", () => {

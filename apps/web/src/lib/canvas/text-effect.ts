/**
 * **L'axe EFFET d'un texte de story — la table UNIQUE, côté web-v2** (#4870,
 * portée dans le chantier par #6943).
 *
 * **Pourquoi ce fichier existe ICI et pas en `@meeshy/shared`** : il est la
 * COPIE EXACTE de `apps/web/lib/story-text-effect.ts`, le legacy que le
 * chantier remplace (`apps/web` ne reçoit plus que des correctifs
 * d'incident). Le mutualiser demanderait de toucher le legacy gelé ; la table
 * disparaîtra avec lui. Tant que les deux coexistent, **toute évolution
 * touche les QUATRE miroirs** — iOS `StoryTextEffect.swift`, Android
 * `StoryTextEffect.kt`, `apps/web` et celui-ci.
 *
 * **C'est cette table qui rend l'arbitrage « aucune police web » tenable**
 * (#6943) : depuis #4850 un `textStyle` ne change QUE la police — ce qui
 * BRILLE ou porte une OMBRE vit ici, en `em`, donc en CSS pur et à zéro
 * octet téléchargé. Le poids de première peinture est déjà au-dessus de son
 * plafond (#6940) ; les dix-huit familles typographiques attendront leur
 * budget, les vingt-cinq effets n'en demandent aucun.
 *

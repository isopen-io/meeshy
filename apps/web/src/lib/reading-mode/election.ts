 ...
 * de la loupe, autour de son centre, écrêtée à la marge verticale de sa carte
 * et à la gouttière de la rangée. DEPUIS #8147 la loi vit dans
 * `@meeshy/shared/utils/focal-metrics` — partagée avec iOS, et avec le
 * dépliage d'un message long ; `metrics.test.ts` tient les cotes dérivées de
 * Swift (`check-curve.mjs`) égales à celles de la loi partagée.
 */
export { focalLoupeScale } from '@meeshy/shared/utils/focal-metrics';

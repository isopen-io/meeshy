 ...
 * translucide réécrivait son flou. Ce composant est le pendant —
 * un seul endroit, pour que la matière soit la même partout.
 *
 * **La matière elle-même vit dans `styles/glass.css`** (#6124) : ce composant
 * en était déclaré le site unique, mais une règle en prose ne tenait que là où
 * quelqu'un s'en souvenait — neuf surfaces la réécrivaient. Les densités y sont
 * arbitrées sur une mesure de contraste flou désactivé, et
 * `scripts/lib/glass-site.mjs` interdit de la réécrire ailleurs.
 *
 ...
 * LISIBLE : le fond de repli est donc opaque à 80 ou 92 %, jamais transparent.
 * Une surface qui compte sur le flou pour son contraste devient illisible dès
 * que le flou n'arrive pas — et rien ne le signale.
 */
...
export function GlassBack({ children, label = 'Retour' }: { children: React.ReactNode; label?: string }) {
  return (
    <span
      className="glass grid size-11 shrink-0 place-items-center rounded-full"
      aria-hidden="true"
      style={{

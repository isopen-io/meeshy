...
export function ConfigModal({ isOpen, onClose, currentUser, onUserUpdate }: ConfigModalProps) {
  ...
  const tabs: TabConfig[] = [
    {
      id: 'user',
      label: t('configModal.userProfile', 'User profile'),
      icon: <User className="h-4 w-4" />,
      component: <UserSettings user={currentUser} onUserUpdate={onUserUpdate} />
    },
    {
      id: 'language',
      label: t('configModal.languageTranslation', 'Languages & Translation'),
      icon: <Globe className="h-4 w-4" />,
      component: <LanguageSettings user={currentUser} onUserUpdate={onUserUpdate} />
    },
    {
      id: 'theme',
      label: t('configModal.appearance', 'Appearance'),
      icon: <Palette className="h-4 w-4" />,
      component: <ThemeSettings />
    },
    {
      id: 'stats',
      label: t('tabs.stats', 'Statistics'),
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        <DialogHeader className={`px-4 py-3 border-b sm:px-6 sm:py-4 ${styles.configModalHeader}`}>
          <DialogTitle className="text-lg sm:text-xl font-semibold">{t('configModal.title', 'Settings and Configuration')}</DialogTitle>
        </DialogHeader>
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="lg:hidden p-3 sm:p-4 border-b bg-background flex-shrink-0">
              ...
              <select
                ...
                aria-label={t('configModal.sectionAriaLabel', 'Settings section')}
              >
                {tabs.map((tab) => (

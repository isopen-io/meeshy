        ...
        },
        {
          onSuccess: () => {
            ...
            const desc = mediaCount > 0
              ? t('toast.storyVisibleMedia', { count: mediaCount })
              : t('toast.storyVisible', 'Your story is visible to your friends.');
  ...
  const handlePublish = useCallback(
    (data: { content: string; type: 'POST' | 'STORY' | 'STATUS'; visibility: string }) => {
      createPostMutation.mutate(
        ...
        {
          onSuccess: () => showToast(t('toast.published', 'Published!'), 'success', t('toast.postShared', 'Your post has been shared.')),
  ...
  const handleAudioPublish = useCallback(
    async (data: { audioFile: File; transcription: MobileTranscription | null; content?: string }) => {
      try {
        ...
        showToast(t('toast.uploadError', 'Upload error'), 'error', t('toast.audioUploadError', "Couldn't upload the audio."));
      }
    },
    [createPostMutation, showToast, t],
  );
    ...
    <DashboardLayout title={t('title', 'Feed')} className="!max-w-none !px-0">
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div aria-live="polite" className="sr-only">
          {cacheState === 'stale' && feedQuery.isFetching ? t('updatingSr', 'Updating feed…') : ''}
        </div>
        ...
        {feedQuery.isError && (
          <div className="text-center py-12" role="alert">
            <p className="text-[var(--gp-text-muted)] mb-4">{t('errorLoad', 'Unable to load feed.')}</p>

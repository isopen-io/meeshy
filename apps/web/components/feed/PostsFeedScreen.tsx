...
function formatRelativeTime(date: string | Date, t: TranslateFn): string {
  ...
  if (minutes < 1) return t('relativeTime.now', 'Just now');
...
export function PostsFeedScreen() {
        ...
        },
        {
          onSuccess: () => {
            ...
            const desc = mediaCount > 0
              ? tStory('publishedBodyWithMedia', { count: mediaCount })
              : tStory('publishedBody', 'Your story is visible to your friends.');
            showToast(tStory('published', 'Story published!'), 'success', desc);
          },
          onError: () => showToast(t('toast.error', 'Error'), 'error', tStory('publishError', "Couldn't publish the story.")),
        },
      );
    },
    [createStoryMutation, userLanguage, showToast, t, tStory],
  );
  ...
  const handleStoryDelete = useCallback(
    (storyId: string) => {
      deleteStoryMutation.mutate(storyId, {
        onSuccess: () => showToast(tStory('deleted', 'Story deleted'), 'success'),
        onError: () => showToast(t('toast.error', 'Error'), 'error', tStory('deleteError', "Couldn't delete the story")),
      });
    },
    [deleteStoryMutation, showToast, t, tStory],
  );
  ...
  const handleStoryReply = useCallback(
    (_id: string, text: string) => showToast(tStory('replySent', 'Reply sent'), 'success', text),
    [showToast, tStory],
  );
  ...
  const handlePublish = useCallback(
    (data: { content: string; type: 'POST' | 'STORY' | 'STATUS'; visibility: string }) => {
      createPostMutation.mutate(
        ...
        {
          onSuccess: () => showToast(t('toast.published', 'Published!'), 'success', t('toast.publishedBody', 'Your post has been shared.')),
          onError: () => showToast(t('toast.error', 'Error'), 'error', t('toast.publishError', "Couldn't publish the post.")),
        },
      );
    },
    ...
    (data: { content: string; visibility: string }) => {
      ...
      updatePostMutation.mutate(
        ...
        },
        {
          onSuccess: () => {
            setEditingPost(null);
            showToast(t('toast.postUpdated', 'Post updated'), 'success');
          },
      ...
      try {
        ...
        createPostMutation.mutate(
          ...
          },
          {
            ...
            onError: () => showToast(t('toast.error', 'Error'), 'error', t('toast.audioPublishError', "Couldn't publish.")),
          },
        );
      } catch {
        showToast(t('toast.uploadError', 'Upload error'), 'error', t('toast.uploadErrorBody', "Couldn't upload the audio."));
      }
    },
    [createPostMutation, showToast, t],
  );
  ...
  const handleStatusPress = useCallback(
    () => showToast(t('toast.statusSelected', 'Mood selected'), 'info'),
    [showToast, t],
  );
  ...
  return (
    <DashboardLayout title={t('title', 'Feed')} className="!max-w-none !px-0">
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 py-6">
        <h1 className="sr-only">{t('srHeading', 'Feed — posts, reels and stories')}</h1>
        <FeedTabs active="posts" />

        {/* Story Tray */}
        <section aria-label={t('sections.storiesPublic', 'Public stories')}>
          <h2 className="sr-only">{t('sections.stories', 'Stories')}</h2>
          <StoryTray
            stories={storyItems}
        ...
        <section aria-label={t('sections.compose', 'Compose a post')}>
          ...
          <div className="flex gap-3 items-start mb-6">
            ...
            <button
              ...
              aria-label={t('recordAudio', 'Record an audio post')}
            >

    ...
    <Card className="border-yellow-200 dark:border-yellow-800">
      <CardHeader className="bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20">
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>{t('admin.ranking.filters.title')}</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-6">
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.ranking.filters.entityType')}
              </label>
              <Select value={entityType} onValueChange={onEntityTypeChange}>
                <SelectTrigger className="border-yellow-300 focus:ring-yellow-500">
                  <SelectValue placeholder={t('admin.ranking.filters.selectType')} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="users">
                    <div className="flex items-center space-x-2">
                      <Users className="h-4 w-4" />
                      <span>{t('admin.ranking.filters.entityUsers')}</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="conversations">
                    <div className="flex items-center space-x-2">
                      <MessageSquare className="h-4 w-4" />
                      <span>{t('admin.ranking.filters.entityConversations')}</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="messages">
                    <div className="flex items-center space-x-2">
                      <FileText className="h-4 w-4" />
                      <span>{t('admin.ranking.filters.entityMessages')}</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="links">
                    <div className="flex items-center space-x-2">
                      <LinkIcon className="h-4 w-4" />
                      <span>{t('admin.ranking.filters.entityLinks')}</span>
                    </div>
                  </SelectItem>
                </SelectContent>
              </Select>
                  ...
                  <div className="max-h-[320px] overflow-y-auto">
                    {criteriaList.length > 0 ? (
                      ...
                      <div className="p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                        {t('admin.ranking.filters.noCriteriaFound')}
                      </div>
                    )}
                  </div>
                </SelectContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.ranking.filters.period')}
              </label>
              <Select value={period} onValueChange={onPeriodChange}>
                <SelectTrigger className="border-yellow-300 focus:ring-yellow-500">
                  <SelectValue placeholder={t('admin.ranking.filters.selectPeriod')} />
                </SelectTrigger>
                <SelectContent>
                  {PERIODS.map((p) => (
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.ranking.filters.resultCount')}
              </label>

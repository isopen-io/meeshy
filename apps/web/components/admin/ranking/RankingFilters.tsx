...
export function RankingFilters({
  ...
  const periods = React.useMemo(() =>
    PERIOD_VALUES.map(v => ({ value: v, label: t(`ranking.period${v === 'all' ? 'All' : v}`) })),
    [t]
  );
  const criteriaList = React.useMemo(() => {
    const criteria = RANKING_CRITERIA[entityType].map(c => ({
      ...c,
      label: t(criterionLabelKey(c.value))
    }));
    if (criteriaSearch) {
    ...
    <Card className="border-yellow-200 dark:border-yellow-800">
      <CardHeader className="bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20">
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>{t('ranking.filterTitle')}</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-6">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('ranking.resultCountLabel')}
              </label>
              <Select value={limit.toString()} onValueChange={(value) => onLimitChange(parseInt(value))}>
                <SelectTrigger className="border-yellow-300 focus:ring-yellow-500">
                  <SelectValue placeholder={t('ranking.resultCountLabel')} />
                </SelectTrigger>
                <SelectContent>

...
const PERIOD_VALUES = ['1d', '7d', '30d', '90d', '180d', '365d', 'all'] as const;
...
export function RankingFilters({
  ...
  const periods = React.useMemo(() =>
    PERIOD_VALUES.map(v => ({ value: v, label: t(`admin.ranking.period${v === 'all' ? 'All' : v}`) })),
    [t]
  );
    ...
    <Card className="border-yellow-200 dark:border-yellow-800">
      <CardHeader className="bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20">
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>{t('admin.ranking.filterTitle')}</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-6">
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.ranking.entityTypeLabel')}
              </label>
                ...
                <SelectContent>
                  <SelectItem value="users">
                    <div className="flex items-center space-x-2">
                      ...
                      <span>{t('admin.ranking.entityUsers')}</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="conversations">
                    <div className="flex items-center space-x-2">
                      <MessageSquare className="h-4 w-4" />
                      <span>{t('admin.ranking.entityConversations')}</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="messages">
                    <div className="flex items-center space-x-2">
                      <FileText className="h-4 w-4" />
                      <span>{t('admin.ranking.entityMessages')}</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="links">
                    <div className="flex items-center space-x-2">
                      <LinkIcon className="h-4 w-4" />
                      <span>{t('admin.ranking.entityLinks')}</span>
                    </div>
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.ranking.criterionLabel')}
              </label>
                  ...
                  <div className="max-h-[320px] overflow-y-auto">
                    {criteriaList.length > 0 ? (
                      ...
                      <div className="p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                        {t('admin.ranking.noCriteriaFound')}
                      </div>
                    )}
                  </div>
                </SelectContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.ranking.resultCountLabel')}
              </label>

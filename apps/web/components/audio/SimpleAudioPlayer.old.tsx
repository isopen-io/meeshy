...
import { Play, Pause, AlertTriangle, Gauge, Download, Mic2, Baby, Skull, Music, Sliders, Globe, FileText, ChevronDown, Loader2, Languages } from 'lucide-react';
...
import type { AudioTranslationReadyEventData, TranslatedAudioData } from '@meeshy/shared/types/socketio-events';
import { apiService } from '@/services/api.service';
import MediaManager from '@/utils/media-manager';
import { meeshySocketIOService } from '@/services/meeshy-socketio.service';

// Données de traduction audio
...
interface SimpleAudioPlayerProps {
  ...
  messageId?: string; // ID du message pour recevoir les traductions en temps réel
...
export const SimpleAudioPlayer: React.FC<SimpleAudioPlayerProps> = ({
  ...
  // États pour les traductions audio
  ...
  // États pour la transcription seule (audio → texte)
  const [isTranscribing, setIsTranscribing] = useState(false);
  const [transcriptionError, setTranscriptionError] = useState<string | null>(null);

  // États pour la traduction (audio → texte traduit + audio traduit)
  ...
  useEffect(() => {
    ...
    const unsubscribe = meeshySocketIOService.onAudioTranslation((data: AudioTranslationReadyEventData) => {
      // Vérifier que cette traduction concerne cet attachement
      if (data.attachmentId !== attachment.id) return;

      console.log('🎵 [SimpleAudioPlayer] Traduction audio reçue:', {
      ...
      // Mettre à jour les audios traduits
  ...
  // Quand la transcription arrive via WebSocket, arrêter les états de chargement
  useEffect(() => {
    if (transcription) {
      if (isTranscribing) {
  ...
  // Quand les audios traduits arrivent via WebSocket, arrêter l'état de traduction
  useEffect(() => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // TRANSCRIPTION SEULE (audio → texte)
  // ═══════════════════════════════════════════════════════════════════════════

  /**
   * Demander uniquement la transcription de l'audio (texte)
   * Route: POST /api/v1/attachments/:id/transcribe
   */
  const requestTranscription = useCallback(async (options?: {
    useLocalTranscription?: boolean;
  }) => {
    // TODO: Transcription locale (not implemented pour le moment)
    if (options?.useLocalTranscription) {
      setTranscriptionError('Transcription locale non implémentée');
      console.warn('[SimpleAudioPlayer] Local transcription not implemented yet');
      return;
    }

    if (isTranscribing) return;

    // Si transcription déjà disponible, ne pas redemander
    if (transcription) {
      console.log('[SimpleAudioPlayer] Transcription already available');
      return;
    }
    ...
    try {
      ...
      console.log('📝 [SimpleAudioPlayer] Transcription request sent:', { attachmentId });

      // Le résultat arrivera via WebSocket (AUDIO_TRANSLATION_READY avec transcription)
      // Timeout de sécurité après 60s
      setTimeout(() => {
      ...
      console.error('❌ [SimpleAudioPlayer] Transcription request failed:', error);
      setIsTranscribing(false);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // TRADUCTION AUDIO (audio → texte traduit + audio traduit)
  // ═══════════════════════════════════════════════════════════════════════════

  /**
   * Demander la traduction de l'audio (transcription + traduction texte + génération audio)
   * Route: POST /api/v1/attachments/:attachmentId/translate
   */
  const requestTranslation = useCallback(async (options?: {
    ...
    try {
      ...
      // Appeler l'API de traduction d'attachement
      // La transcription + audios traduits seront reçus via WebSocket (AUDIO_TRANSLATION_READY)
      const response = await apiService.post<{ success: boolean; data?: any; error?: string }>(
      ...
      console.log('🌐 [SimpleAudioPlayer] Translation request sent:', {
        attachmentId,
        targetLanguages: options?.targetLanguages || ['en', 'fr']
      });

      // Le résultat arrivera via WebSocket (useEffect ci-dessus)
      // Timeout de sécurité après 120s (traduction plus longue)
      setTimeout(() => {
      ...
      console.error('❌ [SimpleAudioPlayer] Translation request failed:', error);
      setIsTranslating(false);
  ...
  // URL audio actuelle (originale ou traduite)
  ...
  }, [selectedLanguage, translatedAudios, attachmentFileUrl]);
  ...
  // Type pour les configurations d'effets
  ...
  // Recharge automatiquement quand on change de langue
  useEffect(() => {
    let isMounted = true;
    ...
    const loadAudio = async () => {
      ...
      try {
        ...
        // NOTE: buildApiUrl dans config.ts gère déjà tous les cas de préfixes:
        // - /api/vX → utilise tel quel (pas de duplication)
        // - /api/ → ajoute la version
        // - autre → ajoute /api/v1
        // Donc PAS BESOIN d'enlever le préfixe ici
  ...
  }, [attachmentId, currentAudioUrl, selectedLanguage]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          ...
          <div className="flex items-center justify-center gap-2">
                  ...
                  <div className="relative h-16 flex items-center justify-center">
                    ...
                    <input
                      ...
                      onTouchMove={(e) => {
                        ...
                        handlePlaybackRateChange({ target } as React.ChangeEvent<HTMLInputElement>);
                      }}
            ...
            {/* Bouton Transcription (audio → texte uniquement) */}
            {isTranscribing ? (
              ...
              // État: transcription disponible - clic pour afficher/masquer
              <button
              ...
              // État: pas de transcription - bouton pour déclencher
              <button
            ...
            {/* Bouton Traduction (audio → texte traduit + audio traduit) */}
            {isTranslating ? (
              ...
              // État: traductions disponibles - déjà géré par le bouton Langue ci-dessus
              // On n'affiche pas de bouton supplémentaire car le Globe est déjà là
              null
            ) : (
              // État: pas de traduction - bouton pour déclencher
              <button
      ...
      {translationError && translatedAudios.length === 0 && (
        <div className="mt-1 flex items-center gap-1 text-xs text-red-500">
          <AlertTriangle className="w-3 h-3" />
      ...
      {selectedLanguage !== 'original' && (
        <div className="mt-1 flex items-center gap-1 text-xs text-green-600 dark:text-green-400">
          ...
          {translatedAudios.find(a => a.language === selectedLanguage)?.voiceCloned && (
            <span className="text-purple-500" title="Voix clonée">🎭</span>
          )}
        </div>
      )}

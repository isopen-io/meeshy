...
export function RegisterFormWizard({
  ...
  // Username check effect - validation immédiate à la frappe
  useEffect(() => {
  ...
  // Email validation effect - validation immédiate à la frappe
  useEffect(() => {
    if (!formData.email) {
      setEmailValidationStatus('idle');
      setEmailErrorMessage('');
      // Réinitialiser existingAccount si c'était basé sur l'email
      setExistingAccount(prev => prev?.type === 'email' ? null : prev);
      return;
    }
  ...
  // Phone validation effect - validation immédiate à la frappe
  useEffect(() => {
    if (!formData.phoneNumber) {
      setPhoneValidationStatus('idle');
      setPhoneErrorMessage('');
      // Réinitialiser existingAccount si c'était basé sur le téléphone
      setExistingAccount(prev => prev?.type === 'phone' ? null : prev);
      return;
    }
  ...
  // Common input classes with glass effect - backdrop-blur only on desktop for performance

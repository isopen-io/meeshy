 ...
 *
 * ### Aucune revendication d'origine n'y figure, et c'est mesuré
 *
 * `Post.originalLanguage` décrit la langue de `content` comme une
 * REVENDICATION — de l'auteur (F5, `storyEffects` sans texte) ou d'une
 * transcription serveur. Le seul signal qu'un reconnaisseur vocal pourrait
 * offrir ici, `recognition.lang`, est réglé depuis la préférence de
 * l'auteur : une hypothèse SERVIE au reconnaisseur, jamais une mesure de ce
 ...
 *
 * `detectedLanguage` ci-dessous EST cette langue mesurée (#5349) : une
 * détection on-device (tinyld) sur le texte réellement tapé, jamais une
 * préférence d'interface. Elle entre par un champ DISTINCT plutôt que de
 * réutiliser `originalLanguage`, dont le sens reste « revendication » —
 * `PostTranslationService` (gateway) la préfère à sa propre détection par
 * regex, sans jamais laisser `originalLanguage` perdre son rang.
 */
export interface ComposerDocumentPayload {
  ...
  /**
   * Langue MESURÉE sur `content` (tinyld, `detectMeasuredLanguage`), jamais
   * une préférence d'interface. Absente quand la mesure n'est pas fiable
   * (texte trop court, confiance trop faible) — jamais devinée pour combler
   * l'absence. Distincte d'`originalLanguage`, voir la note d'en-tête.
   */

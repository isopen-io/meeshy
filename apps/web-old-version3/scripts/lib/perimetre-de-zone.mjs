...
// `scripts/check-v3-pipeline.mjs` interdit à tout fichier de `apps/web-old-version3/` d'atteindre le disque
// hors de son paquet par un chemin RELATIF (l'étage builder du Dockerfile ne copie que
// `apps/web-old-version3/`, donc un `../../scripts/lib/…` manquerait dans l'image). `eslint.config.mjs`
...
// cette config, et l'étage builder du Dockerfile ne copie que `apps/web-old-version3/` (`.dockerignore`

  ...
  it('garde « à établir » ce qui n\'a pas pu être joint — jamais un zéro', () => {
    const baseline = composeBaseline({
      ...
      mesures: [
        mesureIndisponible({
          url: 'https://meeshy.me/',
          commande: 'node apps/web-old-version3/scripts/baseline.mjs',
          raison: 'proxy 403',
        }),
      ],
    });
...
const ligneMesuree = (url: string): LigneDeBaseline =>
  composeMesure({
    url,
    commande: `node apps/web-old-version3/scripts/baseline.mjs ${url}`,
    http: 200,
    dureeMs: 640,
    requetesEmises: 3,
    requetesTerminees: 3,
...
describe('la ligne de base est BRANCHÉE', () => {
  ...
  it('a un hôte qui sait la prendre : un workflow qui lance baseline.mjs', () => {
    ...
    expect(workflow).toContain('apps/web-old-version3/scripts/baseline.mjs');

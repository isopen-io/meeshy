...
| `thread.media-grid.{light,dark}.png` | **ajoutées le 2026-09-13 (#6169), sur « Meeshy Ref-Native » (`3E761BC1-…`), PAS sur le simulateur de la liste ci-dessus** — un message semé sur staging à QUATRE photos (« Quatre captures du chantier web-v2 — grille 2x2 (#6169) »), en **Focal** (chip « AUTO Focal ») : boîte de 300 × 240, **quatre cases ARRONDIES SÉPARÉMENT**, l'écart de 2 laissant voir le FOND du fil (jamais noir — `FocalAttachmentBlock.swift:203-213`) · texte du message SOUS la grille · affordance de réaction · sur les cases 2 à 4 le **badge de téléchargement** (`DownloadBadgeView`, anneau + « 170 ko ») que le web n'a pas (#6304). Les images elles-mêmes sont des captures de web-v2 : c'est le CADRE qui est natif. Vérifications : `Meeshy.app` (jamais `App.app`) ; `describe-all` 17 nœuds sur le fil ; comptes de staging à l'écran ; `beta_features_enabled = 1`. **Sans `*.a11y.txt`** — l'arbre n'a pas été sauvegardé au moment de la prise | tap sur la conversation, défilement jusqu'au message |
| `thread.media-viewer.dark.png` | la visionneuse ouverte sur la 1ʳᵉ tuile du même message : Fermer à gauche, compteur « 4 / 7 » (la pellicule iOS est CONVERSATION-ENTIÈRE — #6303), Enregistrer à droite · pied « Demo · 13 sept. à 10:13 », `780 × 1 688 · 136.6 KB` · colonne Réagir / Répondre / Créer avec ce média. `describe-all` 14 nœuds. **Sans `*.a11y.txt`** | tap sur la 1ʳᵉ tuile |

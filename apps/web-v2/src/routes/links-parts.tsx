...
import type { MyShareLink, ShareLinkInactiveReason, ShareLinksSummary } from '@/lib/api/links';
import { translate } from '@/lib/i18n-catalog';
import type { InterfaceLanguage } from '@/lib/interface-language';
import { canReactivate, displayNameOf } from '@/lib/links/view';
...
/** Miroir `ShareLinkDetailView.headerCard` — plus la CAUSE d'un lien inactif, que la passerelle sert (#3740). */
export function ShareLinkHero({ language, link, url }: { readonly language: InterfaceLanguage; readonly link: MyShareLink; readonly url: string }) {
  return (
    <div data-share-link-hero className="grid justify-items-center gap-3 rounded-hero px-5 py-5 text-center" style={CARD_STYLE}>
      <StatusDisc active={link.isActive} size={60} />
      <p data-share-link-title className="break-words font-bold" style={{ color: INK, fontSize: 'var(--text-lg)' }}>
        {displayNameOf(link)}
      </p>
      <span className="flex flex-wrap items-center justify-center gap-2">
        <span
          data-share-link-status
          className={`rounded-chip px-2.5 py-1 text-chip font-semibold ${link.isActive ? SECTION_BRAND_INK : ''}`}
          style={{ backgroundColor: tinted(link.isActive ? SHARE_TINT : NEUTRAL_TINT, 14), ...(link.isActive ? {} : { color: INK_2 }) }}
        >
          {statusLabel(language, link.isActive)}
        </span>
        {link.conversationTitle === null ? null : (
          <span data-share-link-conversation className="text-caption" style={{ color: INK_2 }}>
            {link.conversationTitle}
          </span>
        )}
      </span>
      <p data-share-link-url className="break-all font-mono text-chip" style={{ color: INK_2 }}>
        {url}
      </p>
      {link.inactiveReason === null ? null : (
        <p data-share-link-reason className="text-caption" style={{ color: INK_2 }}>
...
function ActionButton({
  ...
  return (
    <button
      type="button"
      data-share-link-action={action}
      onClick={onPress}
      className="grid flex-1 justify-items-center gap-1.5 rounded-card py-1 focus-visible:outline-2 focus-visible:outline-offset-2"
      style={{ minHeight: 44, outlineColor: BRAND }}
    >
      <span aria-hidden="true" className="grid size-12 place-items-center rounded-[12px]" style={{ color: tint, backgroundColor: tinted(tint, 15) }}>
        {glyph}
      </span>
      <span className="text-chip font-medium" style={{ color: INK_2 }}>
        {label}
...
/**
 * Miroir `ShareLinkDetailView.actionsBar` — Copier, Partager, Désactiver ou
 * Activer. « Supprimer » n'est pas dessiné : `DELETE /links/:linkId` ne fait
 * que DÉSACTIVER (`admin.ts`), et iOS l'annonce « irréversible » — un contrôle
 * qui ment deux fois (#6411, D-63). « Activer » n'existe que s'il rend le lien
 * utilisable (`canReactivate`).
 */
export function ShareLinkActions({
  ...
  return (
    <div data-share-link-actions className="flex items-start gap-3">
      <ActionButton
        action="copy"
        label={translate(language, copied ? 'links.detail.copied' : 'links.detail.copy')}
        glyph={copied ? <Glyph name="check" size={22} /> : <LinksGlyph name="copy" size={22} />}
        tint={copied ? 'var(--color-success)' : SHARE_TINT}
        onPress={onCopy}
      />
      <ActionButton action="share" label={translate(language, 'links.detail.share')} glyph={<LinksGlyph name="export" size={22} />} tint={SHARE_TINT} onPress={onShare} />
      {link.isActive ? (
        <ActionButton action="disable" label={translate(language, 'links.detail.disable')} glyph={<LinksGlyph name="pauseCircle" size={22} />} tint="var(--color-warning)" onPress={onToggle} />
      ) : canReactivate(link) ? (
        <ActionButton action="activate" label={translate(language, 'links.detail.activate')} glyph={<LinksGlyph name="playCircle" size={22} />} tint="var(--color-success)" onPress={onToggle} />
      ) : null}
    </div>
  );
}

function UsageCard({ stat, glyph, value, valueLabel, label }: { readonly stat: string; readonly glyph: ReactNode; readonly value: string; readonly valueLabel: string | null; readonly label: string }) {
  return (
    <div data-share-link-stat={stat} className="flex items-center gap-3 rounded-card px-3.5 py-3" style={CARD_STYLE}>
      <span aria-hidden="true" style={{ color: SHARE_TINT }}>
        {glyph}
      </span>
      <span className="grid">
        <strong className="font-bold" style={{ color: INK, fontSize: 'var(--text-lg)' }} {...(valueLabel === null ? {} : { 'aria-hidden': true })}>
          {value}
        </strong>
        {valueLabel === null ? null : <span className="sr-only">{valueLabel}</span>}
        <span className="text-caption" style={{ color: INK_2 }}>
          {label}
        </span>
      </span>
...
/** Miroir `ShareLinkDetailView.statsSection` — utilisations et maximum (∞ sans limite). */
export function ShareLinkUsage({ language, link }: { readonly language: InterfaceLanguage; readonly link: MyShareLink }) {
  const format = new Intl.NumberFormat(language);
  return (
    <section aria-labelledby="share-link-stats" className="grid gap-2">
      <h2 id="share-link-stats" className={SECTION_TITLE_CLASS} style={{ color: INK_2 }}>
        {translate(language, 'links.detail.stats')}
      </h2>
      <div className="grid grid-cols-2 gap-3">
        <UsageCard stat="uses" glyph={<LinksGlyph name="userPlus" size={22} />} value={format.format(link.currentUses)} valueLabel={null} label={translate(language, 'links.detail.stats.uses')} />
        <UsageCard
          stat="max"
          glyph={<LinksGlyph name="infinity" size={22} />}
          value={link.maxUses === null ? '∞' : format.format(link.maxUses)}
          valueLabel={link.maxUses === null ? translate(language, 'links.unlimited') : null}
          label={translate(language, 'links.detail.stats.max')}
        />
      </div>
    </section>
  );
...
/** Miroir `ShareLinkDetailView.infoSection` — identifiant, création, expiration. */
export function ShareLinkInformation({ language, link }: { readonly language: InterfaceLanguage; readonly link: MyShareLink }) {
  const date = new Intl.DateTimeFormat(language, { dateStyle: 'medium', timeStyle: 'short' });
  const rows = [
    { row: 'identifier', label: translate(language, 'links.detail.identifier'), value: link.identifier ?? link.linkId, mono: true },
    { row: 'created', label: translate(language, 'links.detail.createdAt'), value: date.format(new Date(link.createdAt)), mono: false },
    ...(link.expiresAt === null ? [] : [{ row: 'expires', label: translate(language, 'links.detail.expiresAt'), value: date.format(new Date(link.expiresAt)), mono: false }]),
  ];
  return (
    <section aria-labelledby="share-link-info" className="grid gap-2">
      <h2 id="share-link-info" className={SECTION_TITLE_CLASS} style={{ color: INK_2 }}>
        {translate(language, 'links.detail.info')}
      </h2>
      <dl className="grid overflow-hidden rounded-card" style={CARD_STYLE}>
        {rows.map((row, index) => (
          <div
            key={row.row}
            data-share-link-info={row.row}
            className="flex items-center gap-3 px-3.5 py-3"
            style={index === 0 ? undefined : { borderTop: '1px solid color-mix(in srgb, var(--color-ios-ink-3) 18%, transparent)' }}
          >
            <dt className="shrink-0 text-body" style={{ color: INK_2 }}>
              {row.label}
            </dt>
            <dd className={`min-w-0 flex-1 truncate text-end text-body font-medium ${row.mono ? 'font-mono' : ''}`} style={{ color: INK }}>
              {row.value}
            </dd>
          </div>
        ))}

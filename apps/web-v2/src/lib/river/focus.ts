/**
 * `focus.ts` — hauteur de lecture → RANG FRACTIONNAIRE, ARITHMÉTIQUE PURE
 * (R-134, §7ter B).
 *
 * PORTÉ tel quel depuis `apps/web/components/conversations/riviere/
 * river-focus.ts` (#5696, étape 2) — seule édition : `interface` → `type`.
 *
 ...
 *
 * La peau (travail 2) appelle cette fonction avec les EXTENTS MESURÉS
 * (jamais une hauteur de rang supposée) et la MÊME bande de focus que le
 * reste de la Lentille (`FOCUS_BAND_OFFSET`, `@meeshy/shared/utils/
 * focus-curve`) — jamais une seconde loi de défilement. Le résultat alimente
 * `resolveRiverLaneHeaders({ geometry, focusRank })` — cette fonction-ci ne
 * connaît RIEN de la loi Rivière elle-même (aucun import de `river-lanes`).
 */

export type RiverRowExtent = {
  readonly top: number;

 ...
 *
 *   1. `scripts/prerender-institutional.tsx` — qui ÉCRIT les documents ;
 ...
 *   3. `scripts/check-institutional.mjs` — qui VÉRIFIE, sur un navigateur
 ...
 * fonctionnait.
 */
export const INSTITUTIONAL_PATHS = INSTITUTIONAL_ROUTES.flatMap((r) => [`/${r}`, `/${r}/`]);
 ...
 */
export const INSTITUTIONAL_PATTERN = new RegExp(`^/(?:${INSTITUTIONAL_ROUTES.join('|')})/?$`);

...
**Ce que ça coûte** — `story_reader` passe de 6,29 à **9,67** Ko gzip (plafond porté à 11, mesuré, source dans `budgets.json`). Le FIL de commentaires n'y entre pas : il est chargé à la demande (D-54). *(Chiffre REMESURÉ deux fois le 2026-09-19 : 9,32 annoncé par le lot, 9,36 à la remesure sur l'arbre du rail inerte, 9,67 après la revue-correction qui y fait entrer D-91 et la région vivante du lecteur. Sans conséquence sur le verdict, et c'est justement pourquoi il fallait le corriger — un chiffre servi qui n'est pas le chiffre mesuré ne rougit nulle part, et la remesure suivante se comparerait à une référence inventée.)*
...
**Ce que ça coûte** — `story_reader` passe de 9,36 à **9,67** Ko gzip (mesuré, plafond 11 inchangé). Le lecteur y gagne aussi sa région `role="status"` (voir ci-dessous).

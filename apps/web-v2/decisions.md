...
**Ce que ce lot NE fait PAS**, assumé et suivi par issues : la bulle d'humeur ancrée (`StatusBubbleOverlay`, largeur `min(250, conteneur − 48)`, bascule au-dessus de l'ancre sous 45 % de hauteur) ; le composeur d'humeur complet d'iOS (`ComposerMoodSurface.swift`, 1 108 lignes) — ici douze emojis et un mot de 80 caractères ; un emoji LIBRE hors de la grille ; la pastille d'humeur d'un auteur qui n'a QU'un statut et pas de story (l'écart de `withMoods`, entier pour les autres) ; l'avatar RÉEL dans ma cellule (initiales, comme le reste du rail) ; le retrait d'une humeur posée.

...
**Ce que la revue-correction RAPPORTE sans le corriger : les bandes d'un fond AJUSTÉ ne sont pas habillées sur la carte de fil.** `POST_SCENE_DECORATED` est la PREMIÈRE fixture de fil en `videoFitMode: 'fit'`, et elle montre le défaut : `BackgroundLayer` peint `object-contain` sans aucun sol, donc les bandes laissent voir l'aplat de CARTE. iOS sert le remplissage PAR DÉFAUT (`MeeshyScenePlayer.servesLetterboxFill = true`, et `FeedSceneAutoplay.swift:159` n'en passe aucune) ; le web ne le pose que dans l'hôte de story (`story-scene-layer.tsx`, `data-scene-letterbox`) — donc jamais dans le fil. Conséquence mesurée : un texte BLANC posé sur ces bandes était invisible en schéma CLAIR et parfaitement lisible en sombre. La fixture porte désormais sa pastille (`textBg`, le seul chemin du moteur qu'aucune fixture n'exerçait) pour ne plus dépendre du schéma, et un invariant l'exige ; l'habillage lui-même est un lot d'HÔTE — il faut trancher le sol d'une scène sans `thumbHash` (`letterboxIsServed` exige une source), ce qu'une revue ne décide pas seule.

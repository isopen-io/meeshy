...
**Ce que ça coûte** — `story_reader` passe de 6,29 à **9,63** Ko gzip (plafond porté à 11, mesuré, source dans `budgets.json`). Le FIL de commentaires n'y entre pas : il est chargé à la demande (D-54). *(Chiffre REMESURÉ deux fois le 2026-09-19 : 9,32 annoncé par le lot, 9,36 à la remesure sur l'arbre du rail inerte, 9,63 après la revue-correction qui y fait entrer D-91 et la région vivante du lecteur. Sans conséquence sur le verdict, et c'est justement pourquoi il fallait le corriger — un chiffre servi qui n'est pas le chiffre mesuré ne rougit nulle part, et la remesure suivante se comparerait à une référence inventée.)*
...
**Pourquoi une loi de POSSESSION et non une liste de touches.** Une liste (« ne pas prendre Espace quand un champ a le focus ») aurait dû grandir à chaque champ, à chaque touche, à chaque écran — c'est-à-dire jamais. La question juste se pose dans l'autre sens : *le nœud qui a le focus revendique-t-il des touches ?* Elle se répond une fois, par un sélecteur, et elle couvre les trois symptômes ci-dessus comme ceux qu'un écran futur apportera.

**Ce que ça coûte** — `story_reader` passe de 9,36 à **9,63** Ko gzip (mesuré, plafond 11 inchangé). Le lecteur y gagne aussi sa région `role="status"` (voir ci-dessous).

**Le témoin vit dans le GATE NAVIGATEUR**, et les trois symptômes s'y mesurent SÉPARÉMENT (`check-story-scene.mjs`, point 7) : un `if` autour des deux derniers les aurait fait DISPARAÎTRE du décompte quand le premier tombe, et une absence de témoin n'est pas un témoin vert. Falsifié : la garde retirée, 12 invariants rougissent (3 symptômes × 4 configurations), le total restant à 126.
...
**Reste ouvert, et nommé** : `routes/reels.tsx:211` porte le même écouteur de `window` sans cette cession. Il ne montre aucun symptôme aujourd'hui (aucune zone de saisie sur cet écran, et son `preventDefault` ne couvre que les flèches), mais c'est la jumelle qui attend son composeur — issue de suivi.

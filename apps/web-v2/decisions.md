...
**Ce que ça coûte** — `story_reader` passe de 6,29 à **9,56** Ko gzip (plafond porté à 11, mesuré, source dans `budgets.json`). Le FIL de commentaires n'y entre pas : il est chargé à la demande (D-54). *(Chiffre REMESURÉ deux fois le 2026-09-19 : 9,32 annoncé par le lot, 9,36 à la remesure sur l'arbre du rail inerte, 9,56 après la revue-correction qui y fait entrer D-91, la région vivante du lecteur et le retour du focus. Sans conséquence sur le verdict, et c'est justement pourquoi il fallait le corriger — un chiffre servi qui n'est pas le chiffre mesuré ne rougit nulle part, et la remesure suivante se comparerait à une référence inventée.)*
